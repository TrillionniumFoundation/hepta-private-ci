...
range returns an error; there is no implicit saturation or projection. Unknown profile IDs reject.
...
Digest equality alone says that source and target named the same normalization;
it does not explain that normalization. Consumers requiring semantic resolution
use `rescale_signal_registered`, which requires both schemas' normalization
digest to resolve to the same immutable `Normalization` definition in an
...
implies byte or digest equivalence. Receipts carry
`NonAuthorizingPosture::DENY_ALL`, a type that cannot represent authority.
They do not certify normalization provenance outside the supplied registry,
statistical accuracy or production profile approval. Client/wire compatibility,
named product composition and production admission remain separate integration
gates.
