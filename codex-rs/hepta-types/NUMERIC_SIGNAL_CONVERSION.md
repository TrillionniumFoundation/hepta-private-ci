...
`rescale_signal_registered` is the registry-enforced boundary. Before
conversion it requires the shared normalization digest to resolve to a
`RegistryKindV1::Normalization` definition in the caller-supplied immutable
`ContractRegistryV1`. Missing, zero or mismatched normalization fails closed.
...
implies byte or digest equivalence. Receipts always have sealed
`AuthorityPosture::DENY_ALL`; they do not certify who approved a normalization
definition, statistical accuracy or production profile approval. Client/wire
compatibility and production admission remain separate integration gates.
