...
`CANONICAL_V1_CONFORMANCE.json` freezes five accepted byte/digest vectors and seven rejection cases. `canonical_validate_v1` provides the Rust raw-byte accept/reject oracle. Independent Python and Node oracles reproduce accepted vectors and reject duplicate field/key, zero schema, oversize/depth, invalid-bool and invalid-tag cases. CI executes these oracles on the exact source HEAD and deterministic synthetic merge candidate. NFC and NFD fixtures intentionally remain different bytes and different digests.
...
`bindings/PLATFORM_TYPES_BINDINGS_V1.json` is the deterministic binding source for the shared foundational constants and profile metadata. The checked-in Python and JavaScript runtime bindings plus TypeScript declarations are regenerated by `bindings/generate_bindings.py`; `--check` fails on drift. These bindings do not redefine HPTC bytes and do not imply arbitrary Rust structs have acquired wire schemas.
