...
impl CodexThread {
    ...
    /// chosen by Core's ordered routing decision.
    ///
    /// This is the narrow admission primitive used by Hepta provider-policy
    /// capabilities. A caller never reconstructs the context from a turn ID
    /// after the reply, so a fast task completion cannot substitute or erase
    /// the admitted turn scope.
    pub(crate) async fn submit_turn_input_and_wait_for_exact_admission(
        &self,
        request: TurnInputRequest,
    ...
    async fn submit_turn_input_with_mode_and_wait_for_exact_admission(
            ...
            _ => {
                return Err(UserMessageAdmissionError::Admission(
                    CodexErr::InvalidRequest(
                        "exact user-message admission requires user input".to_string(),
                    ),
                ));
            }
        };
        ...
        let expected = match &submission {
            ...
            TurnInputSubmission::Steered { turn_id } => Some((turn_id.as_str(), false)),
            TurnInputSubmission::NotSubmitted { .. } => None,
        };
        let Some((expected_turn_id, expected_started)) = expected else {
        ...
        let admission_matches = match admitted.admission() {
            UserMessageAdmission::Started { turn_id } => {
                expected_started && turn_id == expected_turn_id
            }
            UserMessageAdmission::Steered { turn_id } => {
                !expected_started && turn_id == expected_turn_id
            }
        };
        if !admission_matches {
            return Err(UserMessageAdmissionError::Admission(
                CodexErr::InvalidRequest(
                    "turn-input routing reply did not match exact Hepta admission".to_string(),
                ),
            ));
        }
        Ok((submission, Some(admitted)))
    }
    ...
    /// Waits for the accepted user message to become durable in the rollout.
    ...
    fn validate_persisted_user_input(
        ...
        match &request.input {
            ...
            _ => {
                return Err(UserMessageAdmissionError::Admission(
                    CodexErr::InvalidRequest(
                        "persisted user message admission requires user input".to_string(),
                    ),
                ));
            }
        }
    ...
    pub(crate) async fn submit_user_input_and_wait_for_admission_inner(
        ...
        let (submission, admitted) = self
            .submit_turn_input_and_wait_for_exact_admission(request, state)
            .await?;
        Self::accepted_exact_admission(submission, admitted)
    }

    fn accepted_exact_admission(
        submission: TurnInputSubmission,
        admitted: Option<AdmittedUserMessage>,
    ) -> Result<AdmittedUserMessage, UserMessageAdmissionError> {
        match (submission, admitted) {
            ...
            (TurnInputSubmission::NotSubmitted { reason }, None) => {
                Err(UserMessageAdmissionError::Admission(
                    CodexErr::InvalidRequest(format!("user input was not submitted: {reason:?}")),
                ))
            }
