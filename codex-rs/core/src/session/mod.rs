...
impl SessionIo {
    ...
    pub(crate) async fn submit_turn_input(
        ...
        let id = new_submission_id();
        self.submit_turn_input_with_id(id, request, mode).await
    }

    /// Submits turn input under a caller-reserved submission ID.
    ///
    /// Exact-admission callers reserve the ID before enqueueing so the session
    /// can complete their pending admission at the same Start/Steer decision
    /// point that owns the accepted [`TurnContext`].
