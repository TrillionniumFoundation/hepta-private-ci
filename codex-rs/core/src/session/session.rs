...
type PendingStartTransitionCompletion = (
    Arc<()>,
    Arc<StartTransitionCompletion>,
    StartTransitionCleanupSlot,
);

type PendingTaskTerminalizationCompletion = (
    Arc<()>,
    Arc<StartTransitionCompletion>,
...
pub(crate) struct Session {
    ...
    pub(crate) pending_start_transition_completions:
        std::sync::Mutex<Vec<PendingStartTransitionCompletion>>,
    ...
    pub(crate) pending_task_terminalization_completions:
        std::sync::Mutex<Vec<PendingTaskTerminalizationCompletion>>,
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let hook_mcp_session = Arc::new(OnceLock::new());
            ...
            let (hooks, async_hook_results) = Hooks::new(
                ...
                Arc::new(CoreHookMcpExecutor {
                    ...
                    thread_id,
                    session: Arc::clone(&hook_mcp_session),
                }),
            )?;
            ...
            hook_mcp_session
                .set(Arc::downgrade(&sess))
                .map_err(|_| anyhow::anyhow!("hook MCP session binding initialized twice"))?;
