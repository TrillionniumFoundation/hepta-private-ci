//! Read regressions ported from #912 to the existing #774 ledger API.
//! Resident record lookup is not archive lookup or an assertion of causal validity.
...
fn first_middle_and_last_identity_use_the_correct_immutable_record() {
    ...
    for index in [0, 128, 256] {
        let record = ledger.record(&id(&format!("record-{index}")));
        assert_eq!(record, ledger.records().get(index));
    }
    assert!(ledger.record(&id("missing")).is_none());
}
...
fn replay_rebuilds_positions_and_preserves_revoked_history() {
    ...
    assert!(restored.record(&id("record-0")).is_some());
...
fn bounded_pages_are_borrowed_and_cover_history_once() {
    ...
    let first = ledger.records_after(None, 7);
...
#[test]
fn explicit_page_limit_is_respected_without_changing_the_existing_api() {
    let ledger = populated(4097);
    assert_eq!(ledger.records_after(None, 4096).len(), 4096);
    let cursor = Some(must(LogicalSequence::new(4096)));
    assert_eq!(ledger.records_after(cursor, 4096).len(), 1);
}
