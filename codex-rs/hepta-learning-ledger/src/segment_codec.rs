...
pub(crate) fn replay(
    ...
    while cursor < length {
        ...
        if core.retained_records_since(prior.sequence) >= limits.records as u64
...
/// Read one exact historical record from an immutable sealed segment without
/// replaying semantic state for every prior segment. The caller supplies the
/// archive catalog's predecessor anchor; this function validates every frame
/// and the final seal in the selected bounded segment before returning data.
...
pub(crate) fn validate_anchor(
    ...
    if let LedgerRecovery::Acknowledged(anchor) = recovery {
        ...
        let chain_digest = core
            .chain_digest_at(anchor.sequence)
            .ok_or(DurableLedgerError::AcknowledgedHistoryMissing)?;
        if chain_digest != anchor.chain_digest {
