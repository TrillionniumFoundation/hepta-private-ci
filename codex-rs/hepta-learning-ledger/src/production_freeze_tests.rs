...
#[test]
#[ignore = "target-host freeze growth observation; not a deployment latency qualification"]
fn borrowed_freeze_growth_emits_history_and_latency_without_snapshot_clone() {
    let fixture = Fixture::new();
    let directory = fixture.directory();
    ...
    let mut writer =
        LedgerWriter::from_durable(ledger, witness, activated_trust(), &directory, &directory)
            .unwrap();
    let mut head = append_episode(&mut writer);
    let mut count = 2;
    for target in [64_usize, 256, 1024, 4096] {
        for index in count..target {
            let mut request = decision();
            request.record_id = id(&format!("freeze-record-{index}"));
            request.episode_id = id(&format!("freeze-episode-{index}"));
            let evidence = sign(
                writer.verifier(),
                "generator",
                LearningEvidenceRoleV1::Generator,
        ...
        let expected = writer.dataset_freeze_signing_payload(&plan()).unwrap();
        let evidence = sign(
            writer.verifier(),
            "evaluator",
            LearningEvidenceRoleV1::Evaluator,
        ...
        for _ in 0..32 {
            ...
            assert_eq!(receipt.snapshot.source_record_digests.len(), target);
            assert_eq!(receipt.snapshot.pending_outcomes as usize, target - 2);
        }
        samples.sort_unstable();
        println!(
            "HEPTA_FREEZE_GROWTH_V1 records={target} p50_ns={} p95_ns={} p99_ns={} ledger_bytes={} witness_bytes={}",
            samples[15],
            samples[29],
            samples[30],
