...
use super::NduAdaptiveAdmissionError;
use super::NduAdaptiveAdmissionInputV1;
use super::admit_adaptive_ndu_profile_v1;
...
fn policy() -> NduConvergencePolicyV1 {
    NduConvergencePolicyV1 {
        policy_id: id("convergence-policy"),
...
fn evidence(spectral: FixedQ32) -> NduConvergenceEvidenceV1 {
    NduConvergenceEvidenceV1 {
        ...
        evaluator_identity: id("learning-eval"),
...
#[test]
fn accepted_independent_certificate_is_consumed_at_admission_boundary() {
    let certificate =
        issue_ndu_convergence_certificate_v1(evidence(ratio(90, 100)), &policy())
            .expect("accepted certificate");
    assert_eq!(certificate.decision(), NduConvergenceDecisionV1::Accepted);
    let covariance = covariance();
    let admitted = admit_adaptive_ndu_profile_v1(NduAdaptiveAdmissionInputV1 {
        convergence_certificate: &certificate,
        convergence_expectation: expectation(),
        covariance_profile: &covariance,
...
#[test]
fn rejected_independent_certificate_cannot_enter_ndu_admission() {
    let certificate =
    ...
    let covariance = covariance();
    assert!(matches!(
        admit_adaptive_ndu_profile_v1(NduAdaptiveAdmissionInputV1 {
            convergence_certificate: &certificate,
            convergence_expectation: expectation(),
            covariance_profile: &covariance,
            objective_digest: digest("objective"),
        ...
        Err(NduAdaptiveAdmissionError::Convergence(_))
    ));
}
