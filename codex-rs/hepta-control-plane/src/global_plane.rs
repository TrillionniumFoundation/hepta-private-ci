...
pub fn admit_fleet_allocation_owner_v1(
    ...
    if owner.summary.owner_id != fleet_owner_id
        || owner.summary.revision != expected_revision
        || owner.summary.source_frontier_digest != source_frontier_digest
        || owner.summary.readiness != OwnerReadinessV1::Ready
        || owner.admission_digest.is_zero()
    {
        return Err(GlobalPlaneError::InvalidOwnerBinding);
    }
