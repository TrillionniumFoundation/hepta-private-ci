...
fn candidate(
    ...
    PlanCandidateV1 {
        ...
        effect_binding_digest: (name != "abstain").then(|| digest("effect-binding")),
        resource_costs: vec![
            PlannerAxisValueV1 {
...
fn authenticated_multi_owner_fleet_plan_runs_real_ndu_and_emits_deny_all_request() {
    ...
    let raw_fleet = fleet_owner(objective, generation, configuration);
    let (fleet_signed, fleet_issuer) = sign_owner(&raw_fleet);
    let mut fleet_replay = ReplayWindow::new(16);
    let authenticated_fleet = authenticate_owner_summary_v1(
        raw_fleet,
        &fleet_signed,
        &fleet_issuer,
        &mut fleet_replay,
    ...
    .expect("signed fleet owner admission");
