...
use crate::EffectBindingV1;
use crate::GrantRequestV1;
use crate::canonical_effect_binding_digest_v1;
...
fn request() -> GrantRequestV1 {
    let effect_binding_digest =
        canonical_effect_binding_digest_v1(&effect_binding()).expect("effect binding");
    GrantRequestV1 {
...
fn independent_authority_claims_exact_planner_binding() {
    ...
    let binding =
        final_use_binding_for_grant_request_v1(&request, &effect_binding).expect("binding");
    let signed = signed_grant(&signing, binding.clone());

    let token =
        claim_final_use_for_grant_request_v1(&authority, &signed, &request, &effect_binding)
...
fn payload_drift_is_rejected_by_independent_authority_binding() {
    ...
    let effect_binding = effect_binding();
    let binding =
        final_use_binding_for_grant_request_v1(&original, &effect_binding).expect("binding");
    let signed = signed_grant(&signing, binding);

    let mut changed = original;
    changed.final_payload_digest = digest("changed-payload");
    assert_eq!(
        claim_final_use_for_grant_request_v1(&authority, &signed, &changed, &effect_binding,)
...
#[test]
fn effect_identity_drift_is_rejected_before_authority_claim() {
    let temporary = tempfile::tempdir().expect("tempdir");
    let signing = SigningKey::from_bytes(&[34; 32]);
    let authority = authority(&temporary.path().join("authority"), &signing);
    let request = request();
    let original = effect_binding();
    let binding =
        final_use_binding_for_grant_request_v1(&request, &original).expect("original binding");
    let signed = signed_grant(&signing, binding);

    let mut changed = original;
    changed.destination_id = id("provider:other");
    assert_eq!(
        claim_final_use_for_grant_request_v1(&authority, &signed, &request, &changed)
            .expect_err("destination drift must fail before authority claim"),
        AuthorityBridgeError::EffectBindingMismatch
    );
}
