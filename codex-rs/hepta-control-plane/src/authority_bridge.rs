...
pub fn grant_request_digest_v1(request: &GrantRequestV1) -> Result<Digest32, AuthorityBridgeError> {
    if request.plan_digest.is_zero()
        ...
        || request.effect_binding_digest.is_zero()
...
/// kernel.authority. The structured identity is supplied at the final boundary,
/// but its canonical digest was sealed into the candidate before planning.
...
pub fn final_use_binding_for_grant_request_v1(
    ...
    let expected_effect_digest = canonical_effect_binding_digest_v1(effect_binding)
        .map_err(|_| AuthorityBridgeError::InvalidRequest)?;
    if expected_effect_digest != request.effect_binding_digest {
        return Err(AuthorityBridgeError::EffectBindingMismatch);
    }
    let request_digest = grant_request_digest_v1(request)?;
    Ok(FinalUseBinding {
        subject_id: effect_binding.subject_id.as_str().to_string(),
        destination_id: effect_binding.destination_id.as_str().to_string(),
        request_sha256: request_digest.into_array(),
        scope_sha256: effect_binding.scope_digest.into_array(),
...
pub fn claim_final_use_for_grant_request_v1(
    ...
    let binding = final_use_binding_for_grant_request_v1(request, effect_binding)?;
