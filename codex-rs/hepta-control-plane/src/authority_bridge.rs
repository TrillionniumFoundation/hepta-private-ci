...
/// adapter. The host-supplied monotonic time must still precede the sealed
/// planner expiry. control.runtime never constructs or signs FinalUseGrant values.
pub fn claim_final_use_for_grant_request_v1(
    ...
    let expected_binding = current_final_use_binding(
        request,
        now_micros,
        subject_id,
        destination_id,
    ...
    claim_final_use(authority, signed_grant, &expected_binding).map_err(Into::into)
}

/// Preferred irreversible-dispatch helper. The caller crosses only its short,
/// local durable-intent/worker boundary inside the planner-expiry check and
/// second live revocation fence after durable nonce claim. Remote execution and
/// terminal reconciliation stay
/// outside this closure. Control still does not mint or sign authority.
pub fn with_authorized_grant_request_v1<T>(
    ...
    let binding = current_final_use_binding(
        request,
        now_micros,
        subject_id,
        destination_id,
    ...
    let token = claim_final_use(authority, signed_grant, &binding)?;
    dispatch_final_use(authority, token, &binding, dispatch_boundary).map_err(Into::into)
}

fn current_final_use_binding(
    request: &GrantRequestV1,
    now_micros: u64,
    subject_id: &StableId,
    destination_id: &StableId,
    ...
    if now_micros >= request.expires_at_micros {
        return Err(AuthorityBridgeError::PlannerRequestExpired);
    }
