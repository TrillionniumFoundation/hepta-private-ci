...
impl PlannerJournalStoreV1 {
    ...
    pub fn persist(&mut self, journal: &PlannerJournalV1) -> Result<(), PlannerStoreError> {
        self.persist_inner(journal, false)
    }
    ...
    fn persist_inner(
        ...
        if let Err(error) = write_state(&self.root, &bytes, fail_after_replace) {
            if error == PlannerStoreError::Unavailable {
                self.failed = true;
                return Err(PlannerStoreError::RecoveryRequired);
            }
            return Err(error);
    ...
    #[cfg(test)]
    pub(super) fn persist_with_post_replace_failure_for_test(
        &mut self,
        journal: &PlannerJournalV1,
    ) -> Result<(), PlannerStoreError> {
        self.persist_inner(journal, true)
    }
