...
impl PlannerJournalStoreV1 {
    ...
    pub fn persist(&mut self, journal: &PlannerJournalV1) -> Result<(), PlannerStoreError> {
        ...
        if let Err(error) = write_state(&self.root, &bytes) {
            if error == PlannerStoreError::Indeterminate {
                self.poisoned = true;
            }
            return Err(error);
        }
...
fn write_state(directory: &File, bytes: &[u8]) -> Result<(), PlannerStoreError> {
    ...
    #[cfg(test)]
    if take_directory_sync_failure_after_rename() {
        return Err(PlannerStoreError::Indeterminate);
    }
    directory
        .sync_all()
...
#[cfg(test)]
std::thread_local! {
    static FAIL_DIRECTORY_SYNC_AFTER_RENAME: std::cell::Cell<bool> =
        const { std::cell::Cell::new(false) };
}

#[cfg(test)]
pub(super) fn inject_directory_sync_failure_after_rename() {
    FAIL_DIRECTORY_SYNC_AFTER_RENAME.with(|flag| flag.set(true));
}

#[cfg(test)]
fn take_directory_sync_failure_after_rename() -> bool {
    FAIL_DIRECTORY_SYNC_AFTER_RENAME.with(|flag| flag.replace(false))
}
