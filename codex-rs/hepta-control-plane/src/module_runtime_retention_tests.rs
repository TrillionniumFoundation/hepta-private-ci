...
fn abi(name: &str, epoch: u64, predecessor: Option<&RuntimeModuleAbiV1>) -> RuntimeModuleAbiV1 {
    RuntimeModuleAbiV1 {
        ...
        candidate_artifact_digest: Digest32::of_bytes(
            format!("candidate-{name}-{epoch}").as_bytes(),
        ),
...
fn thousand_upgrade_rollback_cycles_keep_payloads_bounded_and_epochs_fenced() {
    ...
    for _ in 0..1_000 {
        ...
        assert_eq!(
            snapshot.active[0].implementation_digest,
            previous.implementation_digest
        );
        assert!(
            registry.records.len() <= 4,
            "payload history grew without bound"
        );
...
fn full_pending_queue_does_not_starve_rollback_or_expand_selected_capacity() {
    ...
    for index in 0..MAX_PENDING_RUNTIME_MODULES {
        let name = format!("module-{index}");
        let active = registry
            .record(&id(&name), generation(2))
            .expect("active")
            .abi
            .clone();
        registry
            .register_candidate(abi(&name, 3, Some(&active)))
            .expect("pending candidate");
    }
    assert_eq!(
        registry.pending_candidate_count(),
        MAX_PENDING_RUNTIME_MODULES
    );
    assert_eq!(
    ...
    assert_eq!(
        registry.active_generation(&id("module-0")),
        Some(generation(4))
    );
}
...
fn selected_capacity_counts_draining_and_quarantined_writers() {
    ...
    registry
        ...
        .expect("pending is a separate budget");
    registry
    ...
    registry
        .begin_retire(&id("module-0"), generation(1))
        .unwrap();
    assert_eq!(
        registry.promote_after_handoff(&candidate.module_id, candidate.generation, witness()),
        Err(RuntimeModuleRegistryError::Bounds)
    ...
    registry
        .finish_retire(&id("module-0"), generation(1))
        .unwrap();
    registry
...
fn rejected_candidates_release_work_budget_but_not_their_epoch_fence() {
    ...
    for epoch in 1..=1_000 {
        ...
        registry
            .quarantine(&candidate.module_id, candidate.generation)
            .unwrap();
...
fn compaction_cannot_reopen_bootstrap_for_a_retired_identity() {
    ...
    registry
        .activate_bootstrap(&first.module_id, first.generation)
        .unwrap();
    registry
        .begin_retire(&first.module_id, first.generation)
        .unwrap();
    registry
        .finish_retire(&first.module_id, first.generation)
        .unwrap();
    ...
    registry
        .enter_shadow(&replacement.module_id, replacement.generation)
        .unwrap();
    registry
        .enter_canary(&replacement.module_id, replacement.generation)
        .unwrap();
    registry
        .promote_after_handoff(&replacement.module_id, replacement.generation, witness())
        .unwrap();
}
...
fn pending_candidate_pins_its_own_predecessor_payload() {
    ...
    registry
        .quarantine(&second.module_id, second.generation)
        .unwrap();
    registry
        .register_candidate(abi("unrelated.module", 1, None))
        .unwrap();
    assert!(
        registry
            .record(&first.module_id, first.generation)
