...
use super::inject_directory_sync_failure_after_rename;
...
fn post_rename_directory_sync_failure_poison_requires_reopen() {
    ...
    inject_directory_sync_failure_after_rename();
    assert_eq!(
        store
            .persist(&journal)
            .expect_err("post-rename sync ambiguity must fail closed"),
    ...
    assert_eq!(
        store
            .persist(&journal)
            .expect_err("poisoned writer must not continue"),
        PlannerStoreError::Indeterminate
    );
    drop(store);

    let (_reopened_store, reopened) =
        PlannerJournalStoreV1::open(&root, &[]).expect("reconcile by reopening");
    assert_eq!(reopened.entries(), journal.entries());
    assert_eq!(reopened.selected_plan_digest(), Some(digest("decision")));
}
