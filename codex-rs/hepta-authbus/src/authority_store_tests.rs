...
#[tokio::test]
async fn policy_state_survives_reopen_and_identity_cannot_change() {
    let root = TempDir::new().expect("temp dir");
    let path = root.path().join("authbus.sqlite");
    let store = AuthBusAuthorityStore::open(&path)
        .await
    ...
    let decision = store
        ...
        .await
        .expect("authorize after reopen");
    assert!(decision.allowed());

    let mut changed_identity = policy(PolicyEffect::Deny);
    changed_identity.principal = id("principal:other");
    assert!(matches!(
        store
            .replace_policy(
                changed_identity,
