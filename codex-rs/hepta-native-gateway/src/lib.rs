        ...
        tokio::select! {
            accepted = listener.accept() => {
                ...
                tokio::spawn(async move {
                    if let Err(error) =
                        serve_connection_with_auth(stream, runtime, auth_token).await
                    {
...
#[cfg(test)]
async fn serve_connection(stream: TcpStream, runtime: Arc<HeptaRuntime>) -> Result<()> {
    serve_connection_with_auth(stream, runtime, None).await
}

async fn serve_connection_with_auth(
    ...
    let response = route_request_with_auth(&request, &runtime, auth_token.as_deref())?;
...
fn route_request(request: &[u8], runtime: &HeptaRuntime) -> Result<Vec<u8>> {
    route_request_with_auth(request, runtime, None)
}
...
mod tests {
    ...
    fn keyring_bearer_mode_rejects_unauthenticated_clients() -> Result<()> {
        ...
        let denied = route_request_with_auth(
            b"GET /healthz HTTP/1.1\r\nHost: localhost\r\n\r\n",
            &runtime,
            Some(&token),
        )?;
        ...
        let allowed = route_request_with_auth(request.as_bytes(), &runtime, Some(&token))?;
