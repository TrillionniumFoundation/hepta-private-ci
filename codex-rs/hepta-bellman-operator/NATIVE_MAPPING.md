...
| authenticate frozen training dataset owner | `VerifiedOperatorDatasetV2::from_authenticated_receipt` | `src/dataset_binding.rs` | implemented |
...
`VerifiedOperatorDatasetV2::from_authenticated_receipt` first verifies the full `DatasetSnapshotReceiptV3` digest preimage, then verifies a domain-separated `DatasetOwner` Ed25519 attestation with the host-owned `LearningEvidenceVerifierV1` and requires the verified principal to equal the receipt's embedded producer exactly. The structural-only `from_receipt` constructor remains owner-test/compatibility only and is rejected from repository product source by the Lane-E closed-world gate. `build_targets_bound_v2`, the bound tabular fit and the bound world-model fit require the training rows' evidence digests to equal the frozen source-record set exactly; target construction additionally binds snapshot ID, objective and ledger head, and emits the V3 dataset digest rather than a detached legacy dataset hash; callers cannot substitute a detached dataset digest, omit one frozen row or add a foreign row. A future profile that intentionally transforms one source record into multiple training rows needs a separately typed projection receipt rather than weakening this invariant.

The unbound `build_targets`, `fit_tabular_operator`, `fit_tabular_operator_strict_v2`, `fit_transition_model`, and structural-only `VerifiedOperatorDatasetV2::from_receipt` surfaces are compatibility/owner-test kernels only. `scripts/hepta-lane-e-closure.py` rejects any non-test repository product source that calls them directly; product composition must first authenticate the receipt's exact producer with `VerifiedOperatorDatasetV2::from_authenticated_receipt` and then use the corresponding `*_bound_v2` path. The crate is `publish = false`, so this closed-world rule is the enforced integration boundary rather than advisory prose.
