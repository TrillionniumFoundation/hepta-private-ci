...
The unbound `build_targets`, `fit_tabular_operator`, `fit_tabular_operator_strict_v2` and `fit_transition_model` surfaces are compatibility/owner-test kernels only. `scripts/hepta-lane-e-closure.py` rejects any non-test repository product source that calls them directly; product composition must first construct `VerifiedOperatorDatasetV2` and then use the corresponding `*_bound_v2` path. The crate is `publish = false`, so this closed-world rule is the enforced integration boundary rather than advisory prose.
