...
pub enum GlobalControlHostError {
    ...
    OperationAlreadyCommitted(Digest32),
    Evidence(String),
...
impl GlobalControlHostV1 {
    ...
    ///
    /// A repeated operation identity returns the exact durable receipt digest
    /// before consuming another AuthBus sequence. Failure after replay
    /// consumption but before a durable commit remains fail-closed and cannot
    /// infer an external effect.
    pub async fn plan(
        &mut self,
        fleet_ledger: &LeaseLedger,
        request: GlobalControlHostRequestV1,
    ) -> Result<GlobalControlPlanV1, GlobalControlHostError> {
        let operation_identity = plan_operation_identity(&request.planning_request.plan_id);
        if let Some(receipt_digest) = self.journal.operation_result_digest(operation_identity) {
            return Err(GlobalControlHostError::OperationAlreadyCommitted(
                receipt_digest,
            ));
        }
        ...
        for signed_owner in request.signed_owner_summaries {
            ...
            let durable_receipt = self
                .evidence
                .admit_authbus_message(issuer, &signed_owner.message, &owner_id, scope, payload)
                .await
        ...
        next.record_operation_result(operation_identity, &plan.evaluation.plan)?;
    ...
    /// Apply a trusted current revocation head to the same authority owner used
    /// by the final dispatch boundary. Provider output and planner receipts can
    /// never call this path.
    pub fn apply_final_use_revocations(
        &self,
    ...
    /// while the planner request is current and after durable nonce claim plus
    /// the final live revocation/time fence.
    ...
    /// Recover the terminal decision identity after response loss without
    /// replaying owner admission or recomputing against changed fleet state.
    #[must_use]
    pub fn committed_plan_receipt(&self, plan_id: &StableId) -> Option<Digest32> {
        self.journal
            .operation_result_digest(plan_operation_identity(plan_id))
    }
...
fn plan_operation_identity(plan_id: &StableId) -> Digest32 {
    let raw = plan_id.as_str().as_bytes();
    let mut bytes = b"hepta.supervisor.global-control-operation.v1\0".to_vec();
