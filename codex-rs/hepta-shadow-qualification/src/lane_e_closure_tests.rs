...
fn lane_e_causal_candidate_chain_is_digest_bound_and_deny_all() {
    ...
    let shared_scope = digest("lane-e-evaluation-trust-scope");
    let mut evaluation_generator = generator.clone();
    evaluation_generator.scope_digest = shared_scope;
    evaluation_generator.signing_key_digest =
        Digest32::of_bytes(&generator_key.verifying_key().to_bytes());
    let mut evaluation_evaluator = evaluator.clone();
    evaluation_evaluator.scope_digest = shared_scope;
    evaluation_evaluator.signing_key_digest =
    ...
    let verifier = match LearningEvidenceVerifierV1::new(LearningEvidenceTrustV1 {
        ...
        signers: vec![
            TrustedLearningSignerV1 {
                principal: evaluation_generator.clone(),
            ...
            TrustedLearningSignerV1 {
                principal: evaluation_evaluator.clone(),
        ...
        Err(error) => panic!("evaluation trust failed: {error}"),
    };

    let evaluation_bundle = IndependentEvaluationBundleV1 {
        evaluation_id: id("evaluation-1"),
        candidate_id: artifact_id.clone(),
        baseline_id: id("baseline-1"),
        claim_scope: EvaluationClaimScopeV1::Qualification,
        generator: evaluation_generator.clone(),
        evaluator: evaluation_evaluator.clone(),
        frozen_plan,
        holdout_use: durable_holdout.receipt().clone(),
        objective_digest,
        dataset_digest: dataset.dataset_digest,
        estimand_digest,
    ...
    let evaluation_evidence = SignedEvaluationEvidenceV1 {
        ...
        evaluator_bundle: signed_evidence(
    ...
    let evaluation = match decide_with_signed_durable_evidence_v3(
        evaluation_bundle,
        roles,
        &durable_holdout,
        &evaluation_evidence,
        ...
        Err(error) => panic!("production independent evaluation failed: {error}"),
    };
    assert_eq!(
