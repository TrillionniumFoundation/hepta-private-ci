...
fn worker() {
    ...
    let loaded = load_pinned_candidate(
        File::open(&request.snapshot)
            .unwrap_or_else(|error| panic!("read-only registry: {error:?}")),
    ...
    match request.expected {
        ...
        Expected::Value(expected) => {
            let bytes =
                loaded.unwrap_or_else(|error| panic!("existing artifact owner pin: {error:?}"));
            let model = LoadedTabularOperatorV1::from_pinned_payload(
                ...
                &TabularPayloadPinV1 {
                    ...
                    artifact_digest: request
                        .artifact_digest
                        .parse()
                        .unwrap_or_else(|error| panic!("artifact digest: {error:?}")),
                    objective_digest: digest("fixed-external-task"),
...
fn run(request: &Request) -> u32 {
    let raw =
        serde_json::to_string(request).unwrap_or_else(|error| panic!("fixture request: {error:?}"));
    ...
    let output = child
        .wait_with_output()
        .unwrap_or_else(|error| panic!("process terminality: {error:?}"));
...
fn existing_artifact_owner_new_process_predictions_and_revoked_rollback() {
    let directory =
        tempfile::tempdir().unwrap_or_else(|error| panic!("private fixture namespace: {error:?}"));
    ...
    for (generation, target) in [(1, 1), (2, 7)] {
        ...
        let bytes = encode_tabular_payload_v1(&model)
            .unwrap_or_else(|error| panic!("encode model: {error:?}"));
        let request = Request {
        ...
        write_candidate_payload(
            CreateOnlyArtifactFile::create(&request.payload)
                .unwrap_or_else(|error| panic!("create-only payload: {error:?}")),
            &registry,
    ...
    let current = write_registry_snapshot(
        CreateOnlyArtifactFile::create(&revoked_snapshot)
            .unwrap_or_else(|error| panic!("new registry: {error:?}")),
        &registry,
        receipt.binding,
    )
