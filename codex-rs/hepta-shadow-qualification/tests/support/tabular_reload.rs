...
fn fixture<T, E: Debug>(result: Result<T, E>, context: &str) -> T {
    match result {
        Ok(value) => value,
...
fn id(value: &str) -> StableId {
    fixture(StableId::new(value), "fixture identity")
}
...
fn fit(generation: u64, value: i64) -> TabularOperatorArtifactV1 {
    fixture(
        fit_tabular_operator_strict_v2(TabularOperatorPlanV1 {
            ...
            generation: fixture(Generation::new(generation), "generation"),
...
impl Request {
    fn manifest(&self) -> ArtifactManifest {
        ArtifactManifest {
            ...
            generation: fixture(Generation::new(self.generation), "generation"),
            predecessor_id: (self.generation == 2).then(|| id("tabular-1")),
            content_digest: fixture(self.payload_digest.parse(), "payload digest"),
            objective_digest: digest("fixed-external-task"),
...
fn worker() {
    ...
    let request: Request = fixture(serde_json::from_str(&raw), "host fixture request");
    let loaded = load_pinned_candidate(
        fixture(File::open(&request.snapshot), "read-only registry"),
        fixture(File::open(&request.payload), "read-only payload"),
        PinnedCandidateSpec {
            registry_receipt: RegistrySnapshotReceipt {
                binding: fixture(request.binding.parse(), "binding"),
                head_digest: fixture(request.head.parse(), "head"),
                file_digest: fixture(request.snapshot_digest.parse(), "snapshot digest"),
    ...
    match request.expected {
        ...
        Expected::Value(expected) => {
            let bytes = fixture(loaded, "existing artifact owner pin");
            let model = fixture(
            ...
            assert_eq!(
                fixture(model.predict(&id("state"), &id("read")), "prediction")
                    .value
                    .raw(),
                expected
            );
...
fn run(request: &Request) -> u32 {
    let raw = fixture(serde_json::to_string(request), "fixture request");
    let executable = fixture(std::env::current_exe(), "test executable");
    let child = fixture(
    ...
    let pid = child.id();
    let output = fixture(child.wait_with_output(), "process terminality");
    assert!(output.status.success(), "{output:?}");
    let stdout = fixture(String::from_utf8(output.stdout), "utf8");
...
fn existing_artifact_owner_new_process_predictions_and_revoked_rollback() {
    let directory = fixture(tempfile::tempdir(), "private fixture namespace");
    let snapshot = directory.path().join("registry");
    let mut registry = ArtifactRegistry::new();
    let mut requests = Vec::new();
    for (generation, target) in [(1, 1), (2, 7)] {
        let model = fit(generation, target);
        let bytes = fixture(encode_tabular_payload_v1(&model), "encode model");
        let request = Request {
