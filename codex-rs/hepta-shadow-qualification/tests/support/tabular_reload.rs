...
fn id(value: &str) -> StableId {
    checked(StableId::new(value), "fixture identity")
}
...
impl Request {
    fn manifest(&self) -> ArtifactManifest {
        ArtifactManifest {
            ...
            generation: checked(Generation::new(self.generation), "generation"),
            predecessor_id: (self.generation == 2).then(|| id("tabular-1")),
            content_digest: checked(self.payload_digest.parse(), "payload digest"),
            objective_digest: digest("fixed-external-task"),
...
fn worker() {
    ...
    let request: Request = checked(serde_json::from_str(&raw), "host fixture request");
    let loaded = load_pinned_candidate(
        checked(File::open(&request.snapshot), "read-only registry"),
        checked(File::open(&request.payload), "read-only payload"),
        PinnedCandidateSpec {
    ...
    match request.expected {
        ...
        Expected::Value(expected) => {
            let bytes = checked(loaded, "existing artifact owner pin");
            let model = LoadedTabularOperatorV1::from_pinned_payload(
                ...
                &TabularPayloadPinV1 {
                    ...
                    sensor_core_digest: digest("fixed-grid"),
                    training_profile_digest: digest("strict-tabular"),
                    generation: checked(Generation::new(request.generation), "generation"),
                },
            );
            let model = checked(model, "loaded model binding");
            assert_eq!(
                checked(model.predict(&id("state"), &id("read")), "prediction")
                    .value
                    .raw(),
                expected
            );
...
fn run(request: &Request) -> u32 {
    let raw = checked(serde_json::to_string(request), "fixture request");
    let child = Command::new(checked(std::env::current_exe(), "test executable"))
        .args([
            "--exact",
        ...
        .spawn();
    let child = checked(child, "new host process");
    let pid = child.id();
    let output = checked(child.wait_with_output(), "process terminality");
    assert!(output.status.success(), "{output:?}");
    let stdout = checked(String::from_utf8(output.stdout), "utf8");
...
fn existing_artifact_owner_new_process_predictions_and_revoked_rollback() {
    let directory = checked(tempfile::tempdir(), "private fixture namespace");
    let snapshot = directory.path().join("registry");
    let mut registry = ArtifactRegistry::new();
    let mut requests = Vec::new();
    for (generation, target) in [(1, 1), (2, 7)] {
        let model = fit(generation, target);
        let bytes = checked(encode_tabular_payload_v1(&model), "encode model");
        let request = Request {
