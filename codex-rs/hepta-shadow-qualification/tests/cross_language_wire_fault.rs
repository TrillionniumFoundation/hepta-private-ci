...
fn run_python(frame: &[u8]) -> std::process::Output {
    ...
    let Some(mut stdin) = child.stdin.take() else {
        panic!("python stdin must be piped");
    };
    must(stdin.write_all(encode_hex(frame).as_bytes()));
...
fn rust_python_wire_roundtrip_and_payload_fault_reject() {
    ...
    let Some(last_byte) = tampered.last_mut() else {
        panic!("encoded frame must not be empty");
    };
    *last_byte ^= 0x01;
