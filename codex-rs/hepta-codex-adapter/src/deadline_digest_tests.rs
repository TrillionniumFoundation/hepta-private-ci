...
fn deadline_is_bound_and_exclusive() {
    assert_ne!(request_digest(&intent(2_000)), request_digest(&intent(3_000)));
    assert_eq!(adapt_request(2_000, intent(2_000)), Err(Error::DeadlineExpired));
}
#[test]
fn exact_request_retry_is_stable() {
    assert_eq!(adapt_request(1, intent(2_000)).unwrap(), adapt_request(1, intent(2_000)).unwrap());
}
#[test]
