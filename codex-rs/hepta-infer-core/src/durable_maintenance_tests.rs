...
fn release(control: &mut DurableInferenceControl, id: &str) {
    ...
    control
        .stop_native_before_dispatch(id, "fixture completed".to_owned())
        .unwrap();
}
...
fn post_compaction_peer_updates_reuse_verified_archive() {
    ...
    for i in 0..32 {
        release(&mut first, &format!("seed-{i}"));
    }
    ...
    for i in 0..64 {
        ...
        second
            .stop_native_before_dispatch(&id, "peer finished".to_owned())
            .unwrap();
    }
    assert_eq!(
        first.maintenance_stats().archive_verified_bytes,
        first_bytes
    );
    assert_eq!(
        second.maintenance_stats().archive_verified_bytes,
        second_bytes
    );
...
fn simultaneous_post_compaction_writers_preserve_every_retired_identity() {
    ...
    for writer in 0..2 {
        ...
        handles.push(std::thread::spawn(move || {
            ...
            for i in 0..128 {
                ...
                // Two real writers can hold one reservation each at once.
                // A one-slot profile would correctly reject the second writer.
                retry_writer(|| control.reserve_native(request(&id), 2));
    ...
    for writer in 0..2 {
        for i in 0..128 {
            assert_eq!(
                reopened
                    .reserve_native(request(&format!("writer-{writer}-{i}")), 1)
                    .unwrap()
                    .state,
...
fn cumulative_released_inventory_is_incremental_and_survives_reopen() {
    ...
    for generation in 0..16 {
        for i in 0..16 {
            release(&mut control, &format!("g{generation}-{i}"));
        }
        ...
        for _ in 0..3 {
            assert_eq!(
                control
                    .archive_released_native()
                    .unwrap()
                    .released_archive_bytes,
    ...
    assert_eq!(control.maintenance_stats().inventory_entries, 0);
    assert_eq!(
        control.reserve_native(request("g0-0"), 1).unwrap().state,
        NativeReservationState::Released
    );
...
fn invalid_inventory_rebuilds_once_without_reviving_commands() {
    ...
    for i in 0..8 {
        release(&mut control, &format!("r{i}"));
    }
    let total = control
        .archive_released_native()
        .unwrap()
    ...
    control.archive_released_native().unwrap();
    assert_eq!(
        control.maintenance_stats().inventory_entries,
        stats.inventory_entries
    );
    assert_eq!(
        control.reserve_native(request("r0"), 1).unwrap().state,
        NativeReservationState::Released
    );
}
...
fn released_publisher_reclaims_partial_temporary_but_never_overwrites_conflict() {
    ...
    assert_eq!(
        maintenance::publish_released(&target, b"conflicting"),
        Err(Error::Conflict)
    );
...
fn released_archive_process_exit_child() {
    ...
    let mut control =
        DurableInferenceControl::open(Path::new(&directory).join("inference.journal"), 16).unwrap();
    for i in 0..8 {
        release(&mut control, &format!("child-{i}"));
    }
    control.archive_released_native().unwrap();
...
fn released_archive_process_exit_at_each_publication_boundary_is_retryable() {
    ...
    for phase in [
        ...
        assert_eq!(
            receipt.archived,
            if phase == "hot-compacted" { 0 } else { 8 },
            "{phase}"
        );
        assert_eq!(receipt.remaining_native, 0);
        let actual: u64 = fs::read_dir(released_archive_dir(&fixture.path()))
            .unwrap()
            .map(|e| e.unwrap())
        ...
        for i in 0..8 {
            assert_eq!(
                reopened
                    .reserve_native(request(&format!("child-{i}")), 1)
                    .unwrap()
                    .state,
                NativeReservationState::Released
            );
...
fn io_delta(
    ...
    after
        ...
        .map(|(key, value)| {
            (
                key.clone(),
                value.saturating_sub(*before.get(key).unwrap_or(&0)),
            )
        })
        .collect()
}
...
fn post_compaction_multi_generation_curve() {
    for history in [64, 1024, 32_768] {
        ...
        for generation in 0..history / 32 {
            for i in 0..32 {
                release(&mut first, &format!("h{generation}-{i}"));
            }
            first.archive_released_native().unwrap();
        }
        let mut second = DurableInferenceControl::open(fixture.path(), 256).unwrap();
        let verified_before = first.maintenance_stats().archive_verified_bytes
            + second.maintenance_stats().archive_verified_bytes;
        ...
        for i in 0..128 {
            ...
            second
                .stop_native_before_dispatch(&id, "curve complete".to_owned())
                .unwrap();
            lock_ns.push(second.maintenance_stats().lock_held_ns - before);
            update_us.push(started.elapsed().as_micros() as u64);
        }
        let update_io = process_io();
        let update_journal_bytes =
            fs::metadata(fixture.path()).unwrap().len() - before_journal_bytes;
        let verified_after = first.maintenance_stats().archive_verified_bytes
            + second.maintenance_stats().archive_verified_bytes;
        assert_eq!(
            verified_after, verified_before,
            "peer updates reread an unchanged archive"
        );
        ...
        assert_eq!(
            first.maintenance_stats().inventory_entries,
            entries_before,
            "ordinary maintenance scanned cumulative history"
        );
        drop(first);
        drop(second);
        let started = Instant::now();
        ...
        assert_eq!(
            reopened.reserve_native(request("h0-0"), 1).unwrap().state,
            NativeReservationState::Released
        );
        let recovery_io = process_io();
        let resident = fs::read_to_string("/proc/self/status")
            .unwrap_or_default()
            .lines()
