    ...
    match record.state {
        ...
        NativeReservationState::Released => {
            let local_stop = record.pre_dispatch_stop.is_some()
                && record.reconciled_no_admission.is_none()
                && record.dispatch.is_none()
                && record.observation.is_none();
            let no_admission = record.pre_dispatch_stop.is_none()
            ...
            if !local_stop && !no_admission && !terminal {
...
fn held_tokens(record: &NativeRunRecord) -> Result<u64, Error> {
    if record.pre_dispatch_stop.is_some() || record.reconciled_no_admission.is_some() {
        return Ok(0);
    }
    Ok(record
        .observation
