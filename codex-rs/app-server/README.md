...
- `turn/recover` — explicitly resume the single interrupted regular model turn identified by `threadId` and `turnId` when recovery authority exists and the thread is idle. The request accepts no settings or input overrides. Core consumes the candidate once, verifies the exact provider-request fingerprint, annotated-history boundary, turn context, lineage, generation, and thread-owned environment selection, then reopens the same logical turn. Any mismatch fails closed before a new provider send. Cold and paginated recovery require the durable rollout binding; ephemeral recovery authority is same-process only and does not survive restart. The feature is disabled for ordinary Codex sessions unless `hepta_turn_recovery` is enabled. It does not guarantee provider physical-sampling or output-arrival exactly-once; durable/cold threads gate model-visible persistence and product effects behind durable local revocation, while ephemeral threads use an in-process revocation.
...
Completed and failed turns automatically start the next queued submission. Interrupted turns leave the queue paused, including after `thread/resume`. Start the queue head with `thread/queue/start`, or select a queued submission by passing `queuedSubmissionId`. Verification of an existing same-client-ID, same-payload binding precedes the idle check, so a queued row already bound to exactly one durable turn reconciles to that turn without another Core admission even if the loaded thread is active. Otherwise an idle thread starts a new turn, while an active thread returns an invalid-request error and leaves the row unchanged. The queued submission's client message ID remains stable, and its queue entry is removed only after Core admission or a verified durable reconciliation. An ordinary `turn/start` does not consume queued submissions.
