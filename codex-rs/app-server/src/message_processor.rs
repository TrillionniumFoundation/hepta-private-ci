...
impl MessageProcessor {
            ...
            let manager = ThreadManager::new(
                ...
                thread_extensions(
                    ...
                    ThreadExtensionDependencies {
                        ...
                        hepta_cognitive_production_mutation: hepta_cognitive_production_mutation
                            .clone(),
                        hepta_local_turn_lifecycle_enabled,
                        hepta_local_development_policy,
                        hepta_qualification_turn_writer_enabled,
                        hepta_qualification_turn_writer: hepta_qualification_turn_writer.clone(),
                        hepta_prompt_runtime_host: hepta_prompt_runtime_host.clone(),
                    },
                ),
    ...
    fn initialized_client_request_future(
        ...
        boxed_request_match!(codex_request, {
            ...
            ClientRequest::ThreadQueueAdd { params, .. } => processor
                .thread_queue_processor
                .add(params)
                .await
                .map(|response| Some(response.into())),
            ...
            ClientRequest::ThreadQueueList { params, .. } => processor
                ...
                .await
                .map(|response| Some(response.into())),
            ClientRequest::ThreadQueueUpdate { params, .. } => processor
                .thread_queue_processor
                .update(params)
                .await
                .map(|response| Some(response.into())),
            ClientRequest::ThreadQueueDelete { params, .. } => processor
                .thread_queue_processor
                .delete(params)
                .await
                .map(|response| Some(response.into())),
            ClientRequest::ThreadQueueReorder { params, .. } => processor
                .thread_queue_processor
                .reorder(params)
                .await
                .map(|response| Some(response.into())),
            ClientRequest::ThreadQueueStart { params, .. } => processor
                .thread_queue_processor
            ...
            ClientRequest::ProjectList { params, .. } => {
                processor.project_processor.project_list(params).await
            },
            ...
            ClientRequest::ProjectImport { params, .. } => {
                processor.project_processor.project_import(params).await
            },
            ClientRequest::ProjectUpdate { params, .. } => {
                processor.project_processor.project_update(params).await
            },
            ClientRequest::ProjectMove { params, .. } => {
                processor.project_processor.project_move(params).await
            },
            ClientRequest::ProjectDelete { params, .. } => {
                processor.project_processor.project_delete(params).await
            },
            ...
            ClientRequest::GetAccountTokenUsage { params, .. } => {
                processor.account_processor.get_account_token_usage(params).await
            },
