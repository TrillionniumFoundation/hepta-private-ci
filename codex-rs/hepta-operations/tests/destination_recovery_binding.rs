...
async fn growing_history_keeps_early_identity_and_domain_count_after_restart() {
    ...
    for limit in [1, 16, 64, 256] {
        ...
        assert_eq!(
            store.observe(&first.identity).await.expect("observe early request"),
            Some(first.clone())
        );
        assert!(matches!(
            store.begin_apply(&first.identity).await.expect("early replay"),
...
fn run_crash_worker(path: &Path, mode: &str, expected: i32) {
    let mut child = Command::new(std::env::current_exe().expect("test executable"))
        .args(["--ignored", "--exact", "destination_crash_worker", "--nocapture"])
        .env(ROOT_ENV, path)
        .env(MODE_ENV, mode)
        .stdin(Stdio::null())
        .spawn()
