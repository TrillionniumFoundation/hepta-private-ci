...
impl ArtifactStorageAdminV1 {
    ...
    /// Resolve one storage leaf under the enrolled root. Every component must
    /// be a normal relative component. The parent must already exist and its
    /// canonical path must remain below the enrolled root.
...
mod tests {
    ...
    fn art_09_storage_admin_removes_only_zero_length_orphan() {
        ...
        assert_eq!(
            admin.cleanup_zero_length_orphan(orphan).expect("cleanup orphan"),
