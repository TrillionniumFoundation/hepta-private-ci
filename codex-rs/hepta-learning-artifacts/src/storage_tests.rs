...
fn current_head_witness_round_trips_only_with_current_requirement() {
    ...
    assert_eq!(
        ...
        .fixture_error("expected storage rejection"),
...
fn current_head_witness_rejects_stale_or_tampered_receipts() {
    ...
    assert_eq!(
        read_registry_head_witness(file.open(), tampered, &head_requirement(20))
            .fixture_error("expected storage rejection"),
        ArtifactStorageError::Corrupt
    );
    assert_eq!(
        read_registry_head_witness(file.open(), receipt, &head_requirement(101))
            .fixture_error("expected storage rejection"),
        ArtifactStorageError::HeadWitnessMismatch
    );
}
...
fn existing_nonempty_file_is_never_reopened_for_writing() {
    ...
    assert_eq!(
        file.create().fixture_error("expected storage rejection"),
...
fn existing_empty_and_truncated_files_are_never_adopted() {
    ...
    assert_eq!(
        empty.create().fixture_error("expected storage rejection"),
        ArtifactStorageError::AlreadyExists
    );
...
fn existing_symlinks_are_never_followed_for_creation() {
    ...
    assert_eq!(
        linked.create().fixture_error("expected storage rejection"),
        ArtifactStorageError::AlreadyExists
    );
...
#[test]
fn sensor_core_kind_survives_durable_registry_roundtrip() {
    let mut registry = ArtifactRegistry::new();
    registry
        .append(ArtifactEvent::Register {
            event_id: id("register-sensor-core"),
            manifest: ArtifactManifest {
                artifact_id: id("sensor-core"),
                kind: ArtifactKind::SensorCore,
                generation: Generation::new(1).fixture("test fixture"),
                predecessor_id: None,
                content_digest: Digest32::of_bytes(b"sensor-core-bytes"),
                objective_digest: Digest32::of_bytes(b"objective"),
                support_digest: Digest32::of_bytes(b"support"),
                producer_id: id("operator"),
                compatibility_digest: Digest32::of_bytes(b"compatibility"),
                encoded_size_bytes: 17,
            },
        })
        .fixture("test fixture");
    ...
    let receipt =
        write_registry_snapshot(file.create().fixture("test fixture"), &registry, binding())
            .fixture("test fixture");
    let reopened = read_registry_snapshot(file.open(), receipt).fixture("test fixture");
    assert_eq!(
        reopened
            .manifest(&id("sensor-core"))
            .map(|manifest| manifest.kind),
        Some(ArtifactKind::SensorCore)
    );
}
