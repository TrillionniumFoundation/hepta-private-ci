...
impl PrivateStateDirectory {
    ...
    pub fn open_existing_directory(path: &Path) -> io::Result<Self> {
        validate_local_absolute_path(path)?;
        let current_user_sid = current_user_sid()?;
        let system_sid = local_system_sid()?;
        let handle = open_directory(path)?;
    ...
    pub fn entry_exists(&self, name: &str) -> io::Result<bool> {
        ...
        match self.open_existing(name) {
            Ok(_) => Ok(true),
            Err(error) if matches!(error.raw_os_error(), Some(code) if code as u32 == ERROR_FILE_NOT_FOUND || code as u32 == ERROR_PATH_NOT_FOUND) => {
                Ok(false)
            }
            Err(error) => Err(error),
        }
...
fn verify_regular(handle: HANDLE, current: *mut c_void, system: *mut c_void) -> io::Result<()> {
    let info = file_info(handle)?;
    ...
    verify_owner_dacl(handle, current, system, "entry")
}
...
fn verify_directory(handle: HANDLE, current: *mut c_void, system: *mut c_void) -> io::Result<()> {
    ...
    verify_owner_dacl(handle, current, system, "directory")
}

fn verify_owner_dacl(
    handle: HANDLE,
    current: *mut c_void,
    system: *mut c_void,
    object: &str,
    ...
    let result = (|| {
        if owner.is_null() || unsafe { EqualSid(owner, current) } == 0 || dacl.is_null() {
            return Err(io::Error::new(
                io::ErrorKind::PermissionDenied,
                format!("private state {object} owner/DACL does not match the current principal"),
            ));
        }
        ...
        let dacl_end = dacl_start
            ...
            .ok_or_else(|| {
                io::Error::new(
                    ...
                    "private state DACL size overflow",
                )
            })?;
        ...
        for index in 0..acl_info.AceCount {
            ...
            let ace_end = ace_start.checked_add(ace_size).ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    "private state ACE size overflow",
                )
            })?;
            ...
            let sid_start = ace_start.checked_add(sid_offset).ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    "private state SID offset overflow",
                )
            })?;
            ...
            let sid_end = sid_start.checked_add(sid_len).ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    "private state SID size overflow",
                )
            })?;
...
fn current_user_sid() -> io::Result<Vec<u8>> {
    ...
    let result = (|| {
        ...
        if sid_start < buffer_start || sid_start >= buffer_end || unsafe { IsValidSid(sid) } == 0 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
...
fn validate_local_absolute_path(path: &Path) -> io::Result<()> {
    ...
    if !local {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "private state directory must use a local drive path",
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn private_state_directory_can_cross_runtime_worker_boundaries() {
        assert_send_sync::<PrivateStateDirectory>();
    }

    #[test]
    fn private_root_must_be_an_absolute_local_drive_path() {
        assert!(validate_local_absolute_path(Path::new(r"C:\hepta\state")).is_ok());
        assert!(validate_local_absolute_path(Path::new(r"\\?\C:\hepta\state")).is_ok());
        assert!(validate_local_absolute_path(Path::new(r"relative\state")).is_err());
        assert!(validate_local_absolute_path(Path::new(r"\\server\share\state")).is_err());
        assert!(validate_local_absolute_path(Path::new(r"\\?\UNC\server\share\state")).is_err());
    }

    #[test]
    fn private_entry_names_do_not_escape_the_verified_root() {
        assert!(validate_name("authority.json").is_ok());
        for name in [
            "",
            ".",
            "..",
