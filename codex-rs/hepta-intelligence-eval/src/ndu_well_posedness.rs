...
pub fn decide_ndu_well_posedness_v1(
    ...
    let producer_verified = verifier.verify(
        LearningEvidenceRoleV1::Generator,
        producer,
        &producer_payload,
        now,
    )?;
    let evaluator_verified = verifier.verify(
    ...
    let conditional_mean_pass =
        evidence.conditional_mean.standardized_absolute_mean_q32 < CONDITIONAL_MEAN_LIMIT_Q32_RAW;
...
#[must_use]
pub fn ndu_well_posedness_producer_signing_payload_v1(
    evidence: &NduWellPosednessEvidenceV1,
) -> Vec<u8> {
...
#[must_use]
pub fn ndu_well_posedness_evaluator_signing_payload_v1(
    evidence: &NduWellPosednessEvidenceV1,
) -> Vec<u8> {
