...
impl ThreadLifecycleContributor<Config> for GuardianV2Extension {
    fn on_thread_start<'a>(
        ...
        Box::pin(async move {
            ...
            // Luna currently owns its websocket/retry loop and therefore cannot produce the
            // Hepta dispatch witness and exactly-once provider terminal required for every
            // physical model send. Do not let a user-enabled upstream experiment bypass an
            // active Hepta governance boundary. Guardian V2 can be re-enabled here once its
            // sampler is routed through the shared provider-policy lifecycle.
            if input.config.features.enabled(Feature::HeptaGovernance) {
                self.event_sink.emit_warning(ExtensionWarning {
                    thread_id,
                    turn_id: None,
                    message: "Guardian V2 is disabled while Hepta governance is active".to_string(),
                });
                return;
            }
