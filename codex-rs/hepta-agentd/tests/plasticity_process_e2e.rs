...
use codex_hepta_learning_artifacts::{
    ArtifactEvent, ArtifactKind, ArtifactManifest, ArtifactRegistry, CreateOnlyArtifactFile,
    write_registry_snapshot,
};
use codex_hepta_learning_ledger::{
    AuthenticatedPrincipalV1, CandidateSetCompleteness, DatasetFreezeRequestV1, DurableLedger,
    EpisodeDecision, LedgerEvent, freeze_dataset_receipt_v3,
};
...
use codex_hepta_agentd::{
    PlasticityDynamicSignalBindingV1, plasticity_modulator_broadcast_digest_v1,
    plasticity_modulator_digest_v1,
};
    ...
    let trust_signers = keys
        ...
        .map(|(index, key)| {
            let principal = AuthenticatedPrincipalV1 {
                ...
                credential_chain_digest: digest(&format!(
                    "plasticity-process-credential-{index}"
                )),
