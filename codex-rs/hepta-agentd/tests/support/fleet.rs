...
impl FleetHarness {
    ...
    #[allow(dead_code)]
    pub(crate) fn start_with_authbus_trust_and_checkpoint(
        &mut self,
        agent: &AgentFixture,
        trust_file: &Path,
        replay_checkpoint_file: &Path,
    ) -> Result<()> {
        let command = AgentCommand::new(
            agentd_binary()?,
            vec![
                "--authbus-trust-file".into(),
                trust_file.as_os_str().to_owned(),
                "--authbus-replay-checkpoint-file".into(),
                replay_checkpoint_file.as_os_str().to_owned(),
            ],
        )?;
        self.supervisor
            .start(&agent.agent_id, command, Instant::now())?;
