...
//! grant, and `FinalUseAuthority::with_verified_use` performs the final live
//! authority check that linearizes synchronous effect entry. The authority
//! mutex is released before the private Browser pipe is driven; a revocation
//! committed after entry cannot retroactively cancel that entered effect.
