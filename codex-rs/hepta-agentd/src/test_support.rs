...
impl CognitiveTestHost {
    pub async fn start(
        ...
        Self::start_with_dispatch_paths(
            root,
            agent_id,
            model,
            provider_base_url,
    ...
    /// A qualification fixture with an explicitly resolved Codex product binary.
    pub async fn start_with_executable(
        ...
        if !codex_executable.is_absolute() || !codex_executable.is_file() {
            return Err("qualification requires an absolute existing Codex executable".into());
        }
        let mut dispatch_paths = Arg0DispatchPaths::default();
        dispatch_paths.codex_exe = Some(codex_executable);
        Self::start_with_dispatch_paths(root, agent_id, model, provider_base_url, dispatch_paths)
            .await
    }

    /// Explicit first-party executable paths for a real cross-crate product fixture.
    /// The caller resolves Cargo/Bazel runfiles; no ambient PATH fallback is allowed.
    pub async fn start_with_dispatch_paths(
        root: PathBuf,
        agent_id: AgentId,
        model: &str,
        provider_base_url: &str,
        dispatch_paths: Arg0DispatchPaths,
    ) -> TestResult<Self> {
