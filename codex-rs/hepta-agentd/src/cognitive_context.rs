...
static PLANNER_CLOCK_ORIGIN: OnceLock<Instant> = OnceLock::new();
...
pub(crate) async fn read(
    ...
    let encoded_context = serde_json::to_vec(&response)
        .map_err(|error| CognitiveStoreError::Invalid(error.to_string()))?;
    // Planner timestamps are process-generation-local monotonic time. Wall
    // clock remains confined to store APIs that explicitly use Unix seconds.
...
fn monotonic_micros() -> Result<u64, CognitiveStoreError> {
    let elapsed = PLANNER_CLOCK_ORIGIN.get_or_init(Instant::now).elapsed();
