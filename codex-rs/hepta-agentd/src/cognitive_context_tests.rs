...
#[tokio::test]
async fn production_composition_fails_closed_when_owner_frontier_advances_before_consume() {
    let temp = tempfile::tempdir().unwrap();
    let fleet = temp.path().join("fleet");
    std::fs::create_dir_all(&fleet).unwrap();
    ...
    let citation = store
        .append_source(
            ...
            &SourceDraft {
                scope: scope.clone(),
                kind: LedgerSourceKind::ExplicitMemoryDirective,
                event_key: "adversarial-base".to_string(),
                content: b"verified lemon orchard".to_vec(),
                observed_at_unix_seconds: 100,
            },
        )
    ...
    store
        .remember_memory(
            &access,
            &MemoryDraft {
                stable_key: "adversarial-memory".to_string(),
                revision: MemoryRevisionDraft {
                    scope: scope.clone(),
                    content: "verified lemon orchard".to_string(),
                    verification: MemoryVerification::Verified,
    ...
    let result = read_with_revalidation_hook(
        &store,
        &owner,
        /*body_generation*/ 1,
        /*authority_epoch*/ 2,
            ...
            store
                .append_source(
                    ...
                    &SourceDraft {
                        scope,
                        kind: LedgerSourceKind::ExplicitMemoryDirective,
                        event_key: "adversarial-advance".to_string(),
                        content: b"new evidence before consume".to_vec(),
