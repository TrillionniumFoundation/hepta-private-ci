...
use super::read;
use crate::cognitive_authority::test_cognitive_read_authority;
...
async fn context_reads_real_owner_content_and_removes_committed_tombstones() {
    ...
    let memory = store
        ...
        .await
        .unwrap();
    let authority = test_cognitive_read_authority();
    let context = read(&store, &owner, 1, "lemon", 4, Some(&authority), None)
        .await
        .unwrap();
    ...
    store
        ...
        .await
        .unwrap();
    let withdrawn = read(&store, &owner, 1, "lemon", 4, Some(&authority), None)
        .await
        .unwrap();
    ...
    assert!(
        read(&store, &other, 1, "lemon", 4, Some(&authority), None)
            .await
            .is_err()
    );
}
