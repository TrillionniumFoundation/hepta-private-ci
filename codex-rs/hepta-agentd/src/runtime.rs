...
pub async fn run(config: AgentdConfig, arg0_paths: Arg0DispatchPaths) -> Result<(), AgentdError> {
    ...
    let cognitive_read_authority = config.cognitive_read_authority();
    ...
    if let Some(authority) = cognitive_read_authority {
        state
            .cognitive_read_authority
            .set(authority)
            .map_err(|_| {
                AgentdError::Invalid("cognitive read authority already attached".to_string())
            })?;
    }
