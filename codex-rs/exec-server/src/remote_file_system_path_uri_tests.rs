...
#[tokio::test]
async fn authorized_reads_use_cached_capability_and_preserve_wire_authority() {
...
async fn record_authorized_reads(
    ...
    let server = tokio::spawn(async move {
        ...
        let Some((capability, first_reply)) = exchanges.next() else {
            panic!("authorized-read recording requires one capability exchange");
        };
        let request = match read_jsonrpc_websocket(&mut websocket).await {
        ...
        if capability == Some(true) {
            for (_, reply) in std::iter::once((None, first_reply)).chain(exchanges) {
                let request = match read_jsonrpc_websocket(&mut websocket).await {
