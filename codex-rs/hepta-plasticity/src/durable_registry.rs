...
impl DurableProposalRegistry {
    ...
    /// Enroll a production store only if the locked file is physically empty.
    /// The empty-file check happens after exclusive lock acquisition so a
    /// cooperating writer cannot race a pre-lock metadata check.
