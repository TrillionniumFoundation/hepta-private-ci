...
#[test]
fn local_process_driver_run_is_bounded_by_the_request_deadline() {
...
"#,
    ...
    let observed = worker
        .run(
            ...
            &fixture.manifest.model_id,
            WorkerRequest {
                request_id: "request.local.deadline".to_string(),
                reservation_id: "reservation.local.deadline".to_string(),
                model_digest: fixture.manifest.model_digest.clone(),
                input,
                payload_digest: payload_digest.clone(),
                maximum_tokens: 16,
...
#[test]
fn ordinary_success_after_cancel_intent_cannot_be_reauthorized() {
    let mut fixture = Fixture::new();
    fs::write(
        &fixture.runtime,
        r#"#!/usr/bin/env python3
...
PROTOCOL = "hepta.local-model-driver.v1"
handle = "local.handle.cancel-race"
...
"#,
    ...
    let mut worker = InferenceWorker::new(
        100,
        "worker.local.cancel-race".to_string(),
        9,
        verified,
        fixture.driver(),
    )
    ...
    let input = "cancel-race".to_string();
    let payload_digest = sha256(input.as_bytes());
    let cancellation = CancellationToken::new();
    let trigger = cancellation.clone();
    let task = std::thread::spawn(move || {
    ...
    let observed = worker
        .run_cancellable(
            ...
            &fixture.manifest.model_id,
            WorkerRequest {
                request_id: "request.local.cancel-race".to_string(),
                reservation_id: "reservation.local.cancel-race".to_string(),
                model_digest: fixture.manifest.model_digest.clone(),
                input,
                payload_digest: payload_digest.clone(),
                maximum_tokens: 16,
        ...
        .expect("fail-closed cancellation race");
    task.join().unwrap();
