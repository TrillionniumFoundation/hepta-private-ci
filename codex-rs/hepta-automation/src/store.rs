...
impl AutomationStore {
    ...
    pub async fn claim_due(
        ...
        let reclaim = sqlx::query(
            "SELECT r.task_id, r.occurrence, r.scheduled_for_ms, r.client_user_message_id,
                    o.schedule_revision, o.occurrence_id
             FROM automation_runs r
             JOIN automation_tasks t ON t.task_id = r.task_id
             JOIN automation_occurrences o
        ...
        let (task_id, occurrence, schedule_revision, occurrence_id, scheduled_for_ms, client_id) =
            if let Some(row) = reclaim {
                ...
                let row = sqlx::query(
                    "SELECT t.task_id, t.next_occurrence, t.next_run_at_ms,
                     ...
                     JOIN automation_schedule_revisions r
                       ON r.task_id = t.task_id
                      AND r.revision = (
                          SELECT MAX(latest.revision)
                          FROM automation_schedule_revisions latest
                ...
                let occurrence_id = AutomationOccurrenceId::for_schedule(
                    task_id,
                    schedule_revision,
                    scheduled_for_ms,
                )?;
                ...
                sqlx::query(
                    "INSERT INTO automation_occurrences (
                         ...
                         scheduled_for_ms, client_user_message_id, state, created_at_ms, updated_at_ms
                     ) VALUES (?, ?, ?, ?, ?, ?, ?, 'materialized', ?, ?)",
                )
                ...
                // Overlap policy is currently fail-closed 'forbid'.  Remove the
                // scheduler projection while this occurrence is non-terminal;
                // terminalization alone installs the next canonical slot.
    ...
    pub async fn record_dispatch_uncertain(
        ...
        let occurrence = sqlx::query(
            "UPDATE automation_occurrences
             SET state = 'dispatch_uncertain',
                 updated_at_ms = CASE WHEN updated_at_ms < ? THEN ? ELSE updated_at_ms END
             WHERE occurrence_id = ? AND owner_agent_id = ? AND task_id = ? AND occurrence = ?
    ...
    pub async fn abort_dispatch_before_admission(
        ...
        let restored = sqlx::query(
            "UPDATE automation_occurrences
             SET state = CASE
                    WHEN taskflow_run_id IS NULL THEN 'materialized'
                    ELSE 'taskflow_bound'
                 END
             WHERE occurrence_id = ? AND owner_agent_id = ? AND task_id = ? AND occurrence = ?
               AND state = 'dispatch_uncertain'",
        )
        .bind(lease.occurrence_id.as_str())
        .bind(self.owner_agent_id.as_str())
        .bind(lease.task.task_id.to_string())
    ...
    /// lookup returns a durable queue receipt. Queue admission is deliberately
    /// non-terminal: this records only the durable dispatch receipt and never
    /// advances the recurring schedule.
    pub async fn reconcile_dispatch(
        ...
        .await
        .map_err(unavailable)?;
        if outcome_update.rows_affected() != 1 {
            return Err(AutomationError::Conflict);
        }
        let occurrence_update = sqlx::query(
            "UPDATE automation_occurrences
             SET state = 'queue_admitted', queued_submission_id = ?,
                 updated_at_ms = CASE WHEN updated_at_ms < ? THEN ? ELSE updated_at_ms END
    ...
    pub async fn mark_submitted(
        ...
        // Queue acceptance is only a durable dispatch receipt. The task's
        // next_run_at_ms remains NULL until occurrence terminalization.
        transaction.commit().await.map_err(unavailable)?;
