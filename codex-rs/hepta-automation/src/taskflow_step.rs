...
//! owns one append-only, per-step intent/receipt chain.  A prepared row is an
//! outbox item; claim, observation and reconciliation append receipts to that
//! same chain.  No method here invokes a provider, wakes a scheduler, or grants
//! production authority.
//!
//! The table is part of the default automation schema.  Storage availability
//! is deliberately separate from provider enrollment or final-use authority.
//! Every read and mutation verifies the owner, run history, definition binding,
//! event hash chain and exact generation/fence tuple.
...
/// The durable outbox is available in the default build.  Availability does
/// not grant any effect or caller authority.
pub const TASKFLOW_STEP_OUTBOX_DEFAULT_BUILD: bool = true;
