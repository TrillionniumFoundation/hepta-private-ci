...
-- The compressed context bytes are a separate content-addressed lifecycle.
-- Checkpoint metadata is immutable, while payload bytes may be revoked and
-- garbage-collected without rewriting historical checkpoint/proof evidence.
CREATE TABLE cognitive_qualified_compact_payloads (
    ...
    payload_bytes BLOB NOT NULL CHECK (
        length(payload_bytes) BETWEEN 1 AND 67108864
    ),
...
CREATE TRIGGER cognitive_qualified_compact_payloads_no_update
BEFORE UPDATE ON cognitive_qualified_compact_payloads BEGIN
    SELECT RAISE(ABORT, 'qualified compact payloads are content-addressed and immutable');
END;
...
CREATE TABLE cognitive_qualified_compact_payload_revocations (
    ...
    tombstone_frontier INTEGER NOT NULL CHECK (tombstone_frontier >= 0),
    authority_epoch INTEGER NOT NULL CHECK (authority_epoch > 0),
    revocation_digest TEXT NOT NULL CHECK (
...
CREATE TRIGGER cognitive_qualified_compact_payload_revocations_no_update
BEFORE UPDATE ON cognitive_qualified_compact_payload_revocations BEGIN
    SELECT RAISE(ABORT, 'qualified compact payload revocations are immutable');
END;

CREATE TRIGGER cognitive_qualified_compact_payload_revocations_no_delete
BEFORE DELETE ON cognitive_qualified_compact_payload_revocations BEGIN
    SELECT RAISE(ABORT, 'qualified compact payload revocations are immutable');
END;

CREATE TRIGGER cognitive_qualified_compact_payloads_delete_requires_revocation
BEFORE DELETE ON cognitive_qualified_compact_payloads
WHEN NOT EXISTS (
    SELECT 1
    FROM cognitive_qualified_compact_payload_revocations AS revocation
    WHERE revocation.owner_agent_id = OLD.owner_agent_id
      AND revocation.scope_id = OLD.scope_id
      AND revocation.purpose_id = OLD.purpose_id
      AND revocation.payload_digest = OLD.payload_digest
) BEGIN
    SELECT RAISE(ABORT, 'qualified compact payload delete requires revocation');
END;
