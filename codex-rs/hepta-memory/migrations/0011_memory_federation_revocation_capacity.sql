-- Expand only the bounded federation revision ceiling. This keeps one terminal
-- revision available to revoke legacy capabilities that already reached 1024.
-- Event history stays immutable; rows are copied byte-for-byte into the new
-- constrained tables before the old tables are removed.
CREATE TABLE memory_federation_events_next (
    capability_id TEXT NOT NULL,
    revision INTEGER NOT NULL CHECK (revision > 0 AND revision <= 1025),
...
CREATE TABLE memory_federation_heads_next (
    capability_id TEXT PRIMARY KEY,
    revision INTEGER NOT NULL CHECK (revision > 0 AND revision <= 1025),
    FOREIGN KEY (capability_id, revision)
        REFERENCES memory_federation_events_next(capability_id, revision) ON DELETE RESTRICT
) STRICT;

INSERT INTO memory_federation_events_next (
    capability_id,
    revision,
    generation,
    owner_agent_id,
...
SELECT
    ...
    expires_at_unix_seconds,
    recorded_at_unix_seconds
FROM memory_federation_events;

INSERT INTO memory_federation_heads_next (capability_id, revision)
SELECT capability_id, revision FROM memory_federation_heads;

DROP TRIGGER memory_federation_events_no_update;
DROP TRIGGER memory_federation_events_no_delete;
DROP INDEX memory_federation_consumer_heads;
DROP TABLE memory_federation_heads;
DROP TABLE memory_federation_events;

ALTER TABLE memory_federation_events_next RENAME TO memory_federation_events;
ALTER TABLE memory_federation_heads_next RENAME TO memory_federation_heads;
