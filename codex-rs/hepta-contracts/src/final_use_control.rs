...
impl FinalUseRevocationConvergenceVerifier {
    ...
    pub fn verify(
        ...
        for signed in acknowledgements {
            ...
            let signature = Signature::from_slice(&signed.signature)
                .map_err(|_| FinalUseControlError::InvalidSignature)?;
            let key_id = verify_key_ring(keys, signed.ack.authority_epoch, &input, &signature)?;
...
impl FinalUseRevocationFeedVerifier {
    ...
    /// Verify a fresh bootstrap/current-view head without claiming it has been
    /// installed. Only the durable authority owner can acknowledge installation.
    pub fn authenticated_head(
        &self,
        signed: &SignedFinalUseRevocationUpdate,
        now_unix_ms: u64,
    ) -> Result<FinalUseRevocations, FinalUseControlError> {
        self.verify_signed(signed, now_unix_ms)?;
        Ok(signed.update.head.clone())
    }
...
mod tests {
    ...
    fn convergence_report_requires_every_enrolled_node_ack() {
        ...
        let full = verifier
            .verify(&feed_verifier, &signed_update, &[signed_a, signed_b], 2_100)
            .unwrap();
