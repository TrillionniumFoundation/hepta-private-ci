...
fn coefficient_profile_binds_covariance_conversion_units_and_dimensions() {
    ...
    let admitted =
        admit_ndu_coefficient_profile(specification(&covariance, &conversion), &covariance, &conversion)
            .expect("profile");
    ...
    assert_eq!(
        admit_ndu_coefficient_profile(wrong, &covariance, &conversion)
...
fn expired_coefficient_profile_rejects_projection() {
    ...
    let admitted =
        admit_ndu_coefficient_profile(spec, &covariance, &conversion).expect("profile");
    let estimate = ZEstimateV1 {
        z: vec![vec![0.0, 0.0]],
        covariance_profile_digest: covariance.digest(),
        condition_estimate: 1.0,
