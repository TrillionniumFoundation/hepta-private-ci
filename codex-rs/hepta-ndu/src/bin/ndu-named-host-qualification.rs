...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let backup;
    {
        ...
        if reopened.selected_projection_digest(objective, subject)?.is_some() {
    ...
    }
    {
        ...
        if restored.selected_projection_digest(objective, subject)?.is_some() {
    ...
    if !hot_target_pass {
        return Err(format!(
            "named-host latency target failed: p95={p95}us p99={p99}us"
        )
        .into());
    }
    Ok(())
