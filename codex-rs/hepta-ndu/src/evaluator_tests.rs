...
fn profile() -> UtilityProfile {
    UtilityProfile {
        ...
        axis_registry_digest: Digest32::of_bytes(b"hepta.ndu.axis-registry.test.v1"),
        dimensions: vec![
...
fn utility_profile_digest_binds_axis_registry_semantics() {
    ...
    changed.axis_registry_digest = Digest32::of_bytes(b"hepta.ndu.axis-registry.changed.v1");
...
fn empty_axis_registry_digest_fails_closed() {
    ...
    invalid.axis_registry_digest = Digest32::ZERO;
    let error = must_err(canonical_utility_profile_digest(&invalid));
    assert_eq!(error.code(), "NDU-E004");
}
