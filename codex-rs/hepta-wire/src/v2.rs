...
/// HPTA V2 binds schema, producer, generation, lengths and payload into one digest.
///
/// The digest is integrity evidence only. It is not a MAC, signature or authority
/// token and must not be treated as authentication without an authenticated
/// transport or an independently verified signature.
...
impl WireEnvelopeV2 {
    pub fn new(
        ...
        let frame_digest = frame_digest(&schema, &producer, generation, &payload);
        Ok(Self {
            schema,
            producer,
            generation,
    ...
    pub fn decode(encoded: &[u8]) -> Result<Self, WireV2Error> {
        ...
        let observed = frame_digest(&schema, &producer, generation, payload);
...
fn frame_digest(
    ...
    bytes.extend_from_slice(&WIRE_VERSION_V2.to_be_bytes());
...
impl fmt::Display for WireV2Error {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::IdentityLength => {
                formatter.write_str("wire v2 identity length is outside bounds")
            }
            ...
            Self::PayloadLength => {
                formatter.write_str("wire v2 payload length is outside bounds")
            }
