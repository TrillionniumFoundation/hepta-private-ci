//! Generation-bound HNMF local engram expansion with policy-admitted safety semantics.
//!
//! This deterministic core owns no store, encoder, clock, model invocation or
//! effect authority. Zero activation is never support, zero-weight synapses are
//! structurally retained but semantically inert, and confidence is weighted by
//! final activation rather than by active-node count.
...
const ENGRAM_RESOURCE_DOMAIN: &[u8] = b"hepta.engram-resource-receipt.v2";
...
impl EngramDynamicsPolicyV1 {
    pub fn product_default() -> Result<Self, EngramErrorV1> {
        let value = Self {
            policy_id: StableId::new("policy:hnmf-engram-v2")
...
pub fn recall_with_engram(
    ...
    dynamics_policy.validate()?;
    let union =
        build_candidate_union(cue, retrieval_policy, candidates).map_err(EngramErrorV1::Recall)?;
    let admitted = crate::generation_bound::policy_admitted_union(&union, retrieval_policy)
        .map_err(EngramErrorV1::Recall)?;
    let engram = settle_engram(cue, &admitted, engram_snapshot, dynamics_policy)?;
    ...
    let contradiction = !engram.contradictions.is_empty()
        || crate::generation_bound::contradiction_population_count(&admitted.entries) > 0;
