...
/// Validated plugin identities projected for one published MCP runtime.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct SelectedPluginSnapshot {
    pub plugins: Vec<SelectedPluginIdentity>,
}

/// The configured identity of a plugin resolved from one selected capability root.
...
impl McpBinding {
    ...
    pub fn empty(config: Arc<McpConfig>) -> Self {
        Self::new(
            ...
            Arc::new(SelectedPluginSnapshot::default()),
            Vec::new(),
            HashMap::new(),
        )
    }
