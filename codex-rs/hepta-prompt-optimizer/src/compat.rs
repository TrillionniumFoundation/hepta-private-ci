...
//!
//! These APIs accept caller-supplied scores and graph facts. They are not the
//! registered canonical prompt policy, do not authenticate owner evidence, and
//! must not be used by new product callers.
...
fn digest_receipt(
    ...
    bytes.extend_from_slice(b"hepta.prompt-optimizer.compat.portfolio.v1");
...
pub(crate) fn canonical_factor_pair(
    left: &StableId,
    right: &StableId,
) -> (StableId, StableId) {
    if left <= right {
...
const fn disposition_code(value: CandidateDisposition) -> u8 {
    match value {
