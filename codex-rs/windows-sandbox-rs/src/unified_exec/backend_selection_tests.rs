...
fn request<'a>(
    ...
    WindowsSandboxSessionRequest {
        ...
        workspace_roots,
        codex_home,
        command: vec!["cmd.exe".to_string(), "/c".to_string(), "exit 0".to_string()],
        cwd,
        env_map: HashMap::new(),
...
fn deny_write_override_is_forced_through_elevated_broker() {
    ...
    let workspace_roots = workspace_roots_for(&cwd);
    let permission_profile = PermissionProfile::read_only();
    let deny_write_paths = vec![
        AbsolutePathBuf::from_absolute_path(&protected).expect("absolute protected root"),
    ];
    let mut request = request(
        &permission_profile,
        &workspace_roots,
