...
/// monotonic deadline and preserve snapshot/candidate-set bindings.
...
/// Monotonic clock used for total and per-stage budget accounting.
...
/// Cancellation fence checked before and after every owner boundary. A
/// production adapter with blocking I/O must additionally implement owner-side
/// interruption; the synchronous facade cannot preempt code while it is inside
/// the adapter call.
