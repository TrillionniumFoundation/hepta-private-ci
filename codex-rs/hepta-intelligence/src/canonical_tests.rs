...
impl CanonicalOwnerPortsV1 for Ports {
    fn validate_objective(
        ...
        self.receipt(input, "objective.compiler", CanonicalPortDecisionV1::Continue)
    }
...
fn owner_generation_change_between_call_and_publication_fails_closed() {
    ...
    assert_eq!(error, CanonicalIntelligenceError::StaleOwner(id("utility.ndu")));
    assert_eq!(
        ports.calls,
        vec![
            CanonicalStageV1::ObjectiveValidated,
...
fn key_rotation_and_wrong_owner_receipt_fail_closed() {
    ...
    let mut ports = Ports::new();
    assert_eq!(
        prepare_intelligence_run(request.clone(), &mut ports, &mut oracle)
