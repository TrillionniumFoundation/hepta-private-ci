...
fn request() -> CapabilitySnapshotRequestV2 {
    ...
    CapabilitySnapshotRequestV2 {
        ...
        requirements: vec![
            CapabilityRequirementV2 { capability_id: id("context"), owner_id: id("context.compiler"), contract_digest: contract, necessity: CapabilityNecessityV2::Required },
            CapabilityRequirementV2 { capability_id: id("neural-hint"), owner_id: id("neuron.runtime"), contract_digest: contract, necessity: CapabilityNecessityV2::Optional },
        ],
        bindings: vec![CapabilityBindingV2 { capability_id: id("context"), owner_id: id("context.compiler"), contract_digest: contract, implementation_digest: Digest32::of_bytes(b"implementation"), generation: Generation::new(/*value*/ 1).expect("generation") }],
    }
}
...
fn absent_optional_capability_composes_without_neural_or_model_artifact() {
    ...
    let plan = snapshot.compose_plan(id("plan"), Digest32::of_bytes(b"context"), vec![]).expect("existing composer");
    assert_eq!(plan.authority, AuthorityPosture::DENY_ALL);
    assert_eq!(plan.decision, crate::PlanDecision::Abstained(crate::AbstentionReason::NoEligibleCandidate));
...
fn required_missing_and_fabricated_zero_artifact_reject() {
    ...
    assert_eq!(CapabilitySnapshotV2::admit(missing), Err(CapabilitySnapshotErrorV2::MissingRequiredCapability(id("context"))));
    let mut empty = request();
    empty.bindings[0].implementation_digest = Digest32::ZERO;
    assert_eq!(CapabilitySnapshotV2::admit(empty), Err(CapabilitySnapshotErrorV2::BindingMismatch(id("context"))));
}
...
fn declaration_reordering_is_stable_but_replacement_changes_actual_plan() {
    ...
    assert_eq!(CapabilitySnapshotV2::admit(reordered).expect("admitted"), first);
    ...
    assert_ne!(first.compose_plan(id("plan"), Digest32::of_bytes(b"context"), vec![]).expect("plan"), second.compose_plan(id("plan"), Digest32::of_bytes(b"context"), vec![]).expect("plan"));
}
...
fn optional_add_remove_is_explicit_and_does_not_weaken_requirements() {
    ...
    assert_eq!(CapabilitySnapshotV2::admit(selected), Err(CapabilitySnapshotErrorV2::MissingRequiredCapability(id("neural-hint"))));
}
...
fn duplicate_unknown_and_wrong_owner_are_not_admitted() {
    ...
    assert_eq!(CapabilitySnapshotV2::admit(duplicate), Err(CapabilitySnapshotErrorV2::DuplicateBinding(id("context"))));
    let mut unknown = request();
    unknown.bindings[0].capability_id = id("unregistered");
    assert_eq!(CapabilitySnapshotV2::admit(unknown), Err(CapabilitySnapshotErrorV2::UnknownCapability(id("unregistered"))));
    let mut wrong = request();
    wrong.bindings[0].owner_id = id("other.owner");
    assert_eq!(CapabilitySnapshotV2::admit(wrong), Err(CapabilitySnapshotErrorV2::BindingMismatch(id("context"))));
}
...
fn optional_absence_keeps_existing_selection_and_hard_veto_semantics() {
    ...
    let plan = snapshot.compose_plan(id("plan"), Digest32::of_bytes(b"context"), vec![allowed, vetoed]).expect("existing composer");
...
fn snapshot_binds_core_freshness_and_bounded_unique_requirements() {
    ...
    assert_ne!(CapabilitySnapshotV2::admit(changed).expect("admitted").digest(), first.digest());
    let mut changed = request();
    ...
    assert_eq!(CapabilitySnapshotV2::admit(repeated), Err(CapabilitySnapshotErrorV2::DuplicateRequirement(id("context"))));
    let mut oversized = request();
    oversized.requirements = vec![oversized.requirements[0].clone(); 129];
    assert_eq!(CapabilitySnapshotV2::admit(oversized), Err(CapabilitySnapshotErrorV2::CapacityExceeded));
}
