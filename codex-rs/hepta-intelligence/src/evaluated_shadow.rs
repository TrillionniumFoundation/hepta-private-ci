...
use codex_hepta_intelligence_eval::DurableHoldoutUseV1;
...
use codex_hepta_intelligence_eval::decide_with_signed_durable_evidence_v3;
use codex_hepta_intelligence_eval::decide_with_signed_evidence_v2;
use codex_hepta_intelligence_eval::durable_evaluation_signing_payload_v3;
...
/// Production evaluated-shadow request. External qualification must carry the
/// type-level durable holdout proof required by learning.eval's production
/// contract. V1 remains a compatibility surface only.
pub struct EvaluatedShadowRequestV2<'a> {
    /// The snapshot authority epoch belongs to the supplied verifier's trust domain.
    pub run: LaneFRunRequestV1,
    pub evaluation: IndependentEvaluationBundleV1,
    pub metric_roles: Vec<MetricRoleContractV2>,
    pub durable_holdout: &'a DurableHoldoutUseV1,
    pub evaluation_evidence: &'a SignedEvaluationEvidenceV1,
    /// Exact opaque candidate bytes, not a claim that these constitute a model.
    pub candidate_bytes: &'a [u8],
    /// The same evaluator signs `evaluated_candidate_signing_payload_v2`.
    pub candidate_evidence: &'a SignedLearningEvidenceV1,
    pub dataset: &'a DatasetSnapshotReceiptV3,
    pub intuition: CalibratedDecisionRequestV1,
    pub episode_id: StableId,
...
/// Production candidate-byte signing payload. It binds the strict durable
/// learning.eval request in addition to candidate bytes and generation.
pub fn evaluated_candidate_signing_payload_v2(
    ...
    let evaluated = durable_evaluation_signing_payload_v3(evaluation, roles, durable_holdout)
        .map_err(EvaluatedShadowError::Evaluation)?;
    let mut bytes = b"hepta.intelligence.evaluated-candidate.v2-durable\0".to_vec();
    bytes.extend_from_slice(Digest32::of_bytes(&evaluated).as_array());
    bytes.extend_from_slice(Digest32::of_bytes(candidate_bytes).as_array());
    bytes.extend_from_slice(&(candidate_bytes.len() as u64).to_be_bytes());
    bytes.extend_from_slice(&generation.to_be_bytes());
...
/// Production evaluated-shadow composition. It is identical in effect authority
/// to V1, but evaluation admission and the candidate signature are both bound to
/// the durable final-holdout proof before any host port is invoked.
pub fn run_evaluated_shadow_v2<P: LaneFShadowPortsV1>(
    request: EvaluatedShadowRequestV2<'_>,
    verifier: &LearningEvidenceVerifierV1,
    ledger: &mut dyn DurableLearningJournal,
    ports: &mut P,
    now: u64,
    ...
    if request.dataset.snapshot.dataset_digest != bundle.dataset_digest
        || request.dataset.snapshot.objective_digest != bundle.objective_digest
        || bundle.snapshot_ids.as_slice() != [request.dataset.snapshot.snapshot_id.clone()]
        || request.durable_holdout.receipt() != &bundle.holdout_use
    {
        return Err(E::Binding("dataset or durable holdout"));
    }
    let candidate_payload = evaluated_candidate_signing_payload_v2(
        bundle,
        &request.metric_roles,
        request.durable_holdout,
        request.candidate_bytes,
        request.run.snapshot.learning_artifact_generation,
    )?;
    let candidate = verifier
    ...
    let policy_id = bundle.candidate_id.clone();
    let evaluation = decide_with_signed_durable_evidence_v3(
        request.evaluation,
        request.metric_roles,
        request.durable_holdout,
        request.evaluation_evidence,
    ...
    let intuition = decide_calibrated_v2(request.intuition.clone()).map_err(E::Intuition)?;
    let mut admission = b"hepta.intelligence.evaluated-shadow.v2-durable\0".to_vec();
    for digest in [
        evaluation.decision.evidence_digest,
        evaluation.authentication_digest,
        request.durable_holdout.proof_digest(),
        Digest32::of_bytes(&request.candidate_evidence.signing_bytes()),
        Digest32::of_bytes(&candidate_payload),
        snapshot_digest,
        request.run.request_digest,
