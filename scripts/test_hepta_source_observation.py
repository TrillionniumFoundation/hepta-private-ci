...
class SourceObservationTests(unittest.TestCase):
    ...
    def install_map(self, mid, root, identity, policy):
        row = {
            ...
            "module": mid, "laneId": "fixture-lane",
            "sourceBase": identity, "observedAtHead": identity,
            "sourceIdentityPolicy": policy, "observedSourcePaths": [root],
            "declaredRoots": [root], "resolvedRoots": [root],
            "sourceRootPresent": True, "productionImplementation": False,
            "operations": [{"operation": "read", "nativeSymbol": "read",
    ...
    def verify_maps(self, modules):
        with mock.patch.object(subject, "load", return_value={"modules": modules}), \
             mock.patch.object(subject, "lane_by_module", return_value={
                 module["id"]: "fixture-lane" for module in modules
    ...
    def test_all_migrated_maps_with_different_anchors_are_accepted(self):
        ...
        self.commit()
        modules = [
            self.install_map("first", "owner", older, "candidate_or_exact_observation_v1"),
            self.install_map("second", "other", self.identity(), "candidate_or_exact_observation_v1"),
        ]
        self.commit()
        self.verify_maps(modules)

    def test_shared_legacy_identity_is_not_current_evidence(self):
        modules = [self.install_map(mid, root, self.identity(), "legacy_shared_batch")
