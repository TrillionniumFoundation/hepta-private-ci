...
def canonical_path(root: Path, value: Any, label: str, *, require_file: bool | None) -> Path:
    ...
    expected_kind = (
        ...
        if require_file is False
        else path.is_file() or path.is_dir()
    )
    need(expected_kind, f"{label}: missing path {value}")
    return path
...
def verify(root: Path = ROOT) -> int:
    ...
    for entry in entries:
        ...
        for owner_root in resolved_roots:
            canonical_path(root, owner_root, f"{module}: resolved source", require_file=None)
        maps[module] = row
...
def self_test() -> int:
    with tempfile.TemporaryDirectory() as directory:
        ...
        need(canonical_path(root, "owned", "fixture-root", require_file=None) == owned, "canonical directory fixture")
        need(canonical_path(root, "owned/source.rs", "fixture-file", require_file=None) == source, "canonical file fixture")
        for bad in (
