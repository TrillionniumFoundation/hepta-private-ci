...
class WorkspaceInheritanceTests(unittest.TestCase):
    ...
    def test_inherited_edition_uses_the_workspace_edition(self):
        self.workspace('[workspace.package]\nedition = "2024"\n[workspace.dependencies]\nhelper = "1"\n')
        self.package("edition.workspace = true", '[dependencies]\nhelper = { workspace = true, default-features = false }\n')
    ...
    def test_pre_2024_warning_is_not_upgraded_to_an_error(self):
        ...
        for edition in ('edition = "2015"', 'edition = "2018"', 'edition = "2021"', ""):
            with self.subTest(edition=edition):
                self.package(edition, '[dependencies]\nhelper = { workspace = true, default-features = false }\n')
    ...
    def test_workspace_optional_dependency_is_rejected_even_when_unused(self):
        ...
        self.assertTrue(any("workspace.dependencies.helper" in error and "optional" in error for error in self.errors()))
    ...
    def test_missing_workspace_lints_is_reported(self):
        ...
        self.assertTrue(any("workspace.lints is missing" in error for error in self.errors()))

    def test_empty_or_populated_workspace_lints_can_be_inherited(self):
        for lints in ("[workspace.lints]\n", '[workspace.lints.rust]\nunsafe_code = "forbid"\n'):
    ...
    def test_member_lints_cannot_override_inherited_lints(self):
        ...
        self.package(extra='[lints]\nworkspace = true\n[lints.rust]\nunsafe_code = "allow"\n')
        self.assertTrue(any("cannot override workspace.lints" in error for error in self.errors()))
    ...
    def test_bad_inheritance_in_unreachable_fixture_is_not_a_global_gate(self):
        self.write("app/tests/fixture/Cargo.toml", '[package]\nname = "fixture"\nedition.workspace = true\n[lints]\nworkspace = true\n')
        self.assertEqual(self.errors(), [])

    def test_failure_is_read_only_and_does_not_execute_build_scripts(self):
        self.package(extra='[dependencies]\nhelper = { workspace = true }\n[lints]\nworkspace = true\n')
        self.write("app/build.rs", 'compile_error!("the structural preflight must not execute me");\n')
        def inventory():
            return {
        ...
        command = subprocess.run(
            [sys.executable, str(Path(__file__).with_name("hepta_workspace.py")),
