...
class NativeFeedbackPolicyTests(unittest.TestCase):
    def test_native_feedback_precedes_document_gate(self):
        ...
        self.assertLess(text.index("      - name: Compile and test actual imported packages\n"),
                        text.index("      - name: Retain original document and source-owner gates\n"))

    def test_manifest_and_format_checks_precede_native_setup(self):
        text = WORKFLOW.read_text()
        names = ["Verify executable candidate identity",
                 "Check workspace structure before native setup",
                 "Parse actual Cargo workspace before native setup",
                 "Formatting without source mutation", "Prepare native owner-test prerequisites"]
        positions = [text.index("      - name: " + name + "\n") for name in names]
        self.assertEqual(positions, sorted(positions))
        self.assertIn("scripts/hepta_workspace.py", shell(names[1]))
        self.assertIn("cargo metadata --locked --format-version 1 --no-deps", shell(names[2]))
        self.assertIn('cargo fmt "${args[@]}" -- --check', shell(names[3]))

    def test_failure_independence_requires_successful_identity_or_setup(self):
        for name in ("Retain original document and source-owner gates",
                     "UI owner tests independently of document outcome", "Clean tracked source"):
            with self.subTest(name=name):
                self.assertIn("if: ${{ !cancelled() && steps.candidate.outcome == 'success' }}", step(name))
        self.assertIn("if: ${{ !cancelled() && steps.native_ready.outcome == 'success' }}",
                      step("Strict Clippy independently of test outcome"))
        self.assertIn("id: candidate", step("Verify executable candidate identity"))
        self.assertIn("id: native_ready", step("Resolve verified sandboxed V8 artifacts for Cargo"))
        # Setup and native execution keep Actions' default success() guard.
        for name in ("Verify executable candidate identity", "Prepare native owner-test prerequisites",
    ...
    def test_no_failure_suppression_or_privileged_event(self):
        ...
        self.assertNotIn("cargo check", text)  # Do not compile everything twice just for ordering.
    ...
    def test_build_inputs_cannot_miss_the_outer_path_filter(self):
        ...
        for path in (".cargo/**", "codex-rs/.cargo/**", "rust-toolchain",
                     "rust-toolchain.toml", "codex-rs/rust-toolchain",
                     "codex-rs/rust-toolchain.toml", "scripts/hepta_workspace.py",
...
class NativeFeedbackExecutionTests(unittest.TestCase):
    def setUp(self):
        ...
        shutil.copyfile(ROOT / "scripts/hepta_ci_exec.py", self.repo / "scripts/hepta_ci_exec.py")
...
'''
        ...
        self.env = {**os.environ, "PATH": str(self.bin) + os.pathsep + os.environ["PATH"],
                    ...
                    "GIT_CONFIG_GLOBAL": os.devnull, "RUNNER_TEMP": str(self.root / "records"),
                    "TOOL_CALLS": str(self.root / "calls.jsonl"), "PACKAGES": "owner-a owner-b",
                    "HEPTA_CI_LANE": "source-head", "MERGE_SHA": ""}
        self.git("init", "--quiet")
    ...
    def invoke(self, name, **extra):
        cwd = self.repo / "codex-rs" if "working-directory: codex-rs\n" in step(name) else self.repo
        return subprocess.run(["bash", "-c", shell(name)], cwd=cwd,
                              env={**self.env, **extra}, text=True,
                              capture_output=True, timeout=20)

    def record(self, name):
        return json.loads((Path(self.env["RUNNER_TEMP"]) / "hepta-command-records" / name).read_text())
    ...
    def test_wrong_candidate_and_dirty_source_reject_before_dispatch(self):
        self.assertNotEqual(self.invoke("Verify executable candidate identity", TESTED_SHA="f" * 40).returncode, 0)
        (self.repo / "codex-rs/code.rs").write_text("edited after checkout\n")
        self.assertNotEqual(self.invoke("Verify executable candidate identity").returncode, 0)
        self.assertEqual(self.calls(), [])

    def test_merge_lane_cannot_fall_back_to_source_head(self):
        result = self.invoke("Verify executable candidate identity", HEPTA_CI_LANE="base-merge", MERGE_SHA="")
    ...
    def test_metadata_failure_is_retained_and_not_called_a_test_pass(self):
        result = self.invoke("Parse actual Cargo workspace before native setup", FAIL_METADATA="47")
        self.assertEqual(result.returncode, 47, result.stderr)
        record = self.record("workspace-metadata.json")
        self.assertEqual((record["status"], record["command_exit_code"]), ("failed", 47))
        self.assertEqual(record["observed_passed_tests"], 0)
        self.assertEqual(self.calls(), [["cargo", "metadata", "--locked", "--format-version", "1", "--no-deps"]])
    ...
    def test_format_failure_is_not_suppressed(self):
        ...
        self.assertEqual(self.calls(), [["cargo", "fmt", "--package", "owner-a", "--package", "owner-b", "--", "--check"]])

    def test_native_failure_and_strict_clippy_have_independent_records(self):
        failed = self.invoke("Compile and test actual imported packages", FAIL_JUST="19")
        ...
        self.assertEqual(self.calls(), [
            ...
            ["cargo", "clippy", "--locked", "-p", "owner-a", "-p", "owner-b", "--all-targets", "--", "-D", "warnings"],
        ])

    def test_clippy_failure_remains_a_failed_gate(self):
        result = self.invoke("Strict Clippy independently of test outcome", FAIL_CLIPPY="37")
    ...
    def test_document_failure_does_not_consume_ui_feedback(self):
        failed = self.invoke("Retain original document and source-owner gates", DOC_RC="31")
    ...
    def test_actual_record_log_is_in_upload_set(self):
        result = self.invoke("Compile and test actual imported packages", FAIL_JUST="19")
        ...
        pattern = re.search(r"path: .*?/hepta-command-records/([^\n]+)", step("Retain real command records"))[1]
    ...
    def merge_fixture(self, *, wrong_tree=False):
        ...
        merge = self.git("commit-tree", tree, "-p", base, "-p", source, "-m", "synthetic fixture")
        self.git("checkout", "--quiet", "--detach", merge)
        self.env.update(SOURCE_SHA=source, BASE_SHA=base, TESTED_SHA=merge,
                        MERGE_SHA=merge, HEPTA_CI_LANE="base-merge")

    def test_prospective_merge_record_binds_real_tree_and_both_parents(self):
        self.merge_fixture()
        self.assertEqual(self.invoke("Verify executable candidate identity").returncode, 0)
        result = self.invoke("Parse actual Cargo workspace before native setup")
        self.assertEqual(result.returncode, 0, result.stderr)
        record = self.record("workspace-metadata.json")
        self.assertEqual(record["before"]["parents"], [self.env["BASE_SHA"], self.env["SOURCE_SHA"]])
        self.assertEqual(record["recomputed_merge_tree"], self.git("rev-parse", "HEAD^{tree}"))
