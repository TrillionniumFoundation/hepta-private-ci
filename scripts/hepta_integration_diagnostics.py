...
def parse_merge(output: bytes) -> tuple[str, list[dict[str, object]]]:
    ...
    for field in fields[1:]:
        ...
        if mode not in {"100644", "100755", "120000", "160000"} or stage not in {"1", "2", "3"}:
            raise ValueError("invalid merge stage entry")
        stages.append({"path": path.decode("utf-8", "strict"), "mode": mode,
                       "object": object_id(oid), "stage": int(stage)})
    return tree, stages
...
def diagnose(root: Path, source: str, target: str) -> dict:
    ...
    git(root, "diff", "--cached", "--no-ext-diff", "--no-textconv", "--exit-code", "HEAD", "--")
    ...
    return {
        ...
        "target_tree": object_id(git(root, "rev-parse", target + "^{tree}").stdout.decode().strip()),
        "merge_tree": tree, "mergeable": merged.returncode == 0,
        "conflicts": sorted({stage["path"] for stage in stages}),
        "stages": stages, "entries": entries,
        "qualification": False,
    }
...
def retain(root: Path, output: Path, report: dict, archives: bool = False) -> None:
    ...
    (output / "integration.json").write_text(json.dumps(report, indent=2, ensure_ascii=True) + "\n", encoding="utf-8")
    ...
    if archives:
        for label in ("source", "target", "base"):
            ...
            with path.open("xb") as stream:
                result = subprocess.run(
                    ["git", "--no-replace-objects", "archive", "--format=tar.gz", report[label]],
                    cwd=root, stdout=stream, stderr=subprocess.PIPE, timeout=120,
                )
