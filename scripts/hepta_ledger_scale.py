...
def validate(value: object, count: int) -> dict:
    if (
        ...
        or value.get("schema") != "hepta.ledger-core-scale.v1"
    ):
...
def main() -> int:
    ...
    parser.add_argument(
        "--source-sha",
        required=True,
        help="caller-provided build source identity (not an attestation)",
    )
    ...
    try:
        ...
        if (
            ...
            or any(not 1 <= x <= 900000 for x in sizes)
        ):
            raise ValueError("sizes must be unique increasing integers in 1..900000")
        if not 1 <= args.repeats <= 20 or not re.fullmatch(
            "[0-9a-f]{40}", args.source_sha
        ):
        ...
        for count in sizes:
            for repeat in range(args.repeats):
                ...
                rows.append(
                    {
                        "repeat": repeat + 1,
                        **validate(json.loads(process.stdout), count),
                    }
                )
        ...
        summary = [
            {
                ...
                **{
                    f"median_{key}": statistics.median(
                        row[key] for row in rows if row["records"] == count
                    )
                    for key in METRICS
                },
            }
        ...
        result = {
            "schema": "hepta.ledger-core-scale-run.v1",
            ...
            "limitations": [
                "Core only; no durable disk, checkpoint, write amplification, or concurrent agents.",
