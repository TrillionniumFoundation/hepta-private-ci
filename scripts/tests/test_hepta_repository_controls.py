...
class RepositoryControlTests(unittest.TestCase):
        ...
        for value in (
            ...
            with self.subTest(value=value):
                ...
                with self.assertRaisesRegex(
                    controls.ControlError, "review conversations"
                ):
                    self.validate()
    ...
    def test_new_pending_invalidates_old_success(self):
        latest = deepcopy(self.checks[1])
        latest.update(id=201, status="queued", conclusion=None)
    ...
    def test_untrusted_success_cannot_replace_trusted_failure(self):
        ...
        forged = deepcopy(self.checks[1])
        forged.update(id=999, conclusion="success")
        forged["app"]["id"] = APP + 1
    ...
    def test_missing_extra_required_gate_fails(self):
        self.protection["required_status_checks"]["checks"].append(
            {"context": "extra-required", "app_id": 9999}
        )
        with self.assertRaises(controls.ControlError):
            self.validate()
    ...
    def test_policy_drift_during_observation_fails(self):
        after = deepcopy(self.protection)
        after["enforce_admins"]["enabled"] = False
        with patch.object(
            controls,
            "api",
            side_effect=[
    ...
    def test_main_drift_during_observation_fails(self):
        after = deepcopy(self.branch)
        after["commit"]["sha"] = "3" * 40
        with patch.object(
            controls,
            "api",
            side_effect=[
    ...
    def test_api_denial_cannot_be_replaced_with_source_policy(self):
        with patch.object(
            controls, "api", side_effect=subprocess.CalledProcessError(1, ["gh", "api"])
        ):
    ...
    def test_pagination_reads_beyond_first_page(self):
        ...
        with patch.object(
            controls,
            "api",
            side_effect=[{"check_runs": first}, {"check_runs": [{"id": 101}]}],
        ) as api:
            result = controls.collect_checks(
                REPO,
                SHA,
            )
