    ...
    for value in declared:
        ...
        if not binding_path.exists():
            ...
            if binding_path.is_symlink() or not binding_path.is_file():
                raise ValueError(f"invalid alias binding: {value}")
            binding = json.loads(
                binding_path.read_text(encoding="utf-8"), object_pairs_hook=_pairs
            )
            allowed = {
                "schema_version",
                "module",
                ...
                "implementation_root",
                "binding_mode",
                "duplicate_cargo_package_created",
                "model_authority",
                "provider_authority",
                "interpretation",
            ...
            if target in declared or not _path(root, target).is_dir():
                raise ValueError(
                    f"alias target must be an existing distinct directory: {value}"
                )
            nested_binding = _path(root, target) / "BINDING.json"
            ...
            for source in evidence:
                if not _path(root, source).is_file():
                    raise ValueError(
                        f"source evidence must be an existing file: {source}"
                    )
