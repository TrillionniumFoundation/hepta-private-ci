...
class OpaqueInputSelectionTests(unittest.TestCase):
    def setUp(self):
        self.owners = {f"codex-rs/{name}": name for name in
                       ("producer", "reader", "host", "test_host", "test_downstream", "other")}
        self.edges = frozenset({("reader", "host", False),
                                ("reader", "test_host", True),
                                ("test_host", "test_downstream", False)})
        self.plain = ci.Graph(self.owners, self.edges)
        self.opaque = ci.Graph(self.owners, self.edges,
                               opaque_input_consumers=frozenset({"reader"}))

    def test_owned_json_also_selects_opaque_reader_and_dependents(self):
        result = ci.select_packages(["codex-rs/producer/data/policy.json"],
                                    self.opaque, self.opaque)
        self.assertEqual(result["packages"], ["host", "producer", "reader", "test_host"])
    ...
    def test_old_and_new_opaque_consumers_are_both_preserved(self):
        for before, after in ((self.opaque, self.plain), (self.plain, self.opaque)):
            with self.subTest(before=before.opaque_input_consumers):
                result = ci.select_packages(["codex-rs/producer/src/fragment.rs"], before, after)
    ...
    def test_empty_diff_does_not_select_opaque_consumers(self):
        self.assertEqual(ci.select_packages([], self.opaque, self.opaque)["packages"], [])

    def test_known_graph_stays_local(self):
        result = ci.select_packages(["codex-rs/producer/data/policy.json"], self.plain, self.plain)
        self.assertEqual(result["packages"], ["producer"])
        self.assertFalse(result["full_workspace"])

    def test_unknown_input_still_forces_full_fallback(self):
        result = ci.select_packages(["external/unclaimed.bin"], self.opaque, self.opaque)
    ...
    def test_removed_opaque_package_is_not_a_cargo_target(self):
        after = ci.Graph({p: name for p, name in self.owners.items() if name != "reader"},
                         self.edges)
        result = ci.select_packages(["codex-rs/producer/data/policy.json"], self.opaque, after)
        self.assertIn("host", result["packages"])
...
class ExactTreeOpaqueInputTests(unittest.TestCase):
    ...
    def test_computed_include_in_another_package_is_not_missed(self):
        self.write("codex-rs/reader/src/lib.rs",
    ...
    def test_literal_include_is_still_precise(self):
        ...
        self.assertEqual(self.change_input(self.commit())["packages"], ["host", "producer", "reader"])
    ...
    def test_declared_build_script_reads_are_opaque_without_execution(self):
        ...
        self.write("codex-rs/reader/generate.rs",
    ...
    def test_automatic_build_script_is_also_an_opaque_consumer(self):
        self.write("codex-rs/reader/build.rs",
                   'fn main() { let _ = std::fs::read("../producer/data/policy.json"); }\n')
        self.assertEqual(self.change_input(self.commit())["packages"], ["host", "producer", "reader"])
...
class PreflightDiagnosticTests(unittest.TestCase):
    def run_preflight(self, fail_suite):
        workflow = (Path(__file__).resolve().parents[1] / ".github/workflows/"
                    "hepta-repository-integrity.yml").read_text(encoding="utf-8")
        step = workflow.split("- name: Exercise preflight and source-identity regressions", 1)[1]
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            env = dict(os.environ, PATH=str(root) + os.pathsep + os.environ.get("PATH", ""),
                       CALL_LOG=str(log), FAIL_SUITE=fail_suite)
    ...
    def assert_all_suites(self, calls):
        self.assertEqual([line.split()[-1] for line in calls], [
            "test_hepta_workspace.py", "test_hepta_implementation_identity.py",
            "test_hepta_ci_dependency_scaling.py", "test_hepta_ci_nested_inputs.py",
    ...
    def test_early_middle_and_last_failures_are_not_hidden(self):
        for suite in ("test_hepta_workspace.py", "test_hepta_ci_dependency_scaling.py",
                      "test_hepta_ci_opaque_inputs.py"):
