...
The verifier reads the Rust declarations directly and compares them with the
...
def rust_constant(path: str, name: str) -> int:
    body = read(path)
    pattern = re.compile(
        rf"(?m)^\s*(?:pub\s+)?const\s+{re.escape(name)}\s*:\s*"
...
def documented_limits() -> dict[str, int]:
    ...
    if len(blocks) != 1:
        raise SystemExit(f"CONTRACT_LIMITS.md: expected one text limit block, found {len(blocks)}")
    parsed: dict[str, int] = {}
...
def require_inactive_mapping() -> None:
    ...
    if not activations or any(value is not False for value in activations):
        raise SystemExit(f"{path}: activation must remain explicitly false until exact-head proof")
...
def main() -> None:
    compiled = {
        ...
        "MAX_COGNITIVE_CONTEXT_BYTES": rust_constant(
            "codex-rs/hepta-agent-protocol/src/lib.rs", "MAX_COGNITIVE_CONTEXT_BYTES"
        ),
