...
def main() -> int:
    ...
    parser.add_argument("--host-instances", type=int, choices=(8, 256), default=256)
    ...
    checks: list[tuple[str, list[str], dict[str, str], tuple[str, ...]]] = [
        ...
        ("production-authority-package", [*tests, "--features", "production-authority", "--success-output", "immediate"], {}, ()),
        ("implementation-maps", ["python3", "scripts/hepta-implementation-maps.py", "verify", "--expected-sha", source], {}, ()),
        ("format", ["cargo", "fmt", "--manifest-path", manifest, "-p", PACKAGE, "--", "--check"], {}, ()),
        ("strict-clippy", ["cargo", "clippy", "--manifest-path", manifest, "--locked", "-p", PACKAGE, "--all-targets", "--features", "production-authority", "--", "-D", "warnings"], {}, ()),
        ("real-agentd-build", ["cargo", "build", "--manifest-path", manifest, "--locked", "-p", "codex-hepta-agentd", "--bin", "codex-hepta-agentd"], {}, ()),
        ("real-agentd-signed-lifecycle", [*tests, "--features", "production-authority", "--test", "production_release_product", "--success-output", "immediate", "external_signer_caller_and_supervisor_recover_without_replay"], {"HEPTA_SUPERVISOR_QUAL_AGENTD": str(target / "debug/codex-hepta-agentd")}, ("real-agentd-build",)),
        ("host-fixture-build", ["cargo", "build", "--manifest-path", manifest, "--locked", "-p", PACKAGE, "--features", "production-authority", "--bins", "--example", "supervisor_host_qualification"], {}, ()),
        ("io-fault-interposer-build", ["cc", "-shared", "-fPIC", "-Wall", "-Wextra", "-Werror", "-O2", "codex-rs/hepta-supervisor/tests/support/io_fault.c", "-ldl", "-o", str(output / "io-fault.so")], {}, ()),
        (f"native-process-host-{args.host_instances}", [str(target / "debug/examples/supervisor_host_qualification"), str(target / "debug/hepta-supervisord"), str(output / "physical-host.json"), str(args.host_instances)], {"HEPTA_SUPERVISOR_QUAL_PRELOAD": str(output / "io-fault.so")}, ("host-fixture-build", "io-fault-interposer-build")),
    ]
    report: dict = {
        "schema_version": 1, "source_commit": source,
        "source_tree": git("rev-parse", "HEAD^{tree}"),
        "parents": git("show", "-s", "--format=%P", "HEAD").split(),
        "deployment_qualified": False, "independent_acceptance": False,
    ...
    for name, command, overrides, dependencies in checks:
        item = {"name": name, "command": command, "status": "running", "exit_code": None}
        ...
        try:
            with (output / f"{name}.log").open("xb") as log:
                process = subprocess.Popen(command, cwd=ROOT / "codex-rs" if command[0] == "cargo" else ROOT, env=env | overrides,
    ...
    report["source_still_clean"] = git("rev-parse", "HEAD") == source and not git("status", "--porcelain")
