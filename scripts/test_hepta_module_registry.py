...
class ModuleRegistryTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            modules = {
                "modules": [
                    {
                        "id": "demo.one",
                        "rootBindings": [{"path": "codex-rs/hepta-one"}],
                    },
                    {
                        "id": "demo.two",
                        "rootBindings": [{"path": "codex-rs/hepta-two"}],
                    },
                ]
            }
            ...
            self.assertEqual(
                report["unclaimedCargoCrates"][0]["package"], "codex-hepta-three"
            )
    ...
    def test_duplicate_root_binding_is_reported(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(report["status"], "drift")
            self.assertEqual(
                report["ambiguousRootBindings"],
                [{"path": "codex-rs/hepta-one", "modules": ["demo.one", "demo.other"]}],
            )
    ...
    def test_explicit_cargo_registry_rejects_empty_stale_and_unknown_bindings(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            cases = [
                (
                    [],
                    "unclaimedCargoCrates",
                    [{"package": "codex-hepta-one", "path": "codex-rs/hepta-one"}],
                ),
                (
                    [{"packagePath": "codex-rs/hepta-one", "module": "demo.missing"}],
                    "unknownModules",
                    ["demo.missing"],
                ),
                (
