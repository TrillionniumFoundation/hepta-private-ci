...
SPEC = importlib.util.spec_from_file_location("hepta_workflow_owner", ROOT / "scripts/hepta-docs.py")
...
class SharedWorkflowOwnerTests(unittest.TestCase):
    def setUp(self):
        self.packages = json.loads((ROOT / "docs/delivery/WORK_PACKAGES.json").read_text())["packages"]
        self.paths = json.loads((ROOT / "docs/delivery/PATH_OWNERSHIP.json").read_text())
        self.graphs = []
    ...
    def test_one_integrating_writer_with_both_domain_coowners(self):
        ...
        self.assertTrue({"automation.taskflow", "channel.matrix"} <= set(writers[0]["coOwnerModules"]))
    ...
    def test_actual_shared_workflow_change_requires_no_imaginary_attestation(self):
        result = DOCS.validate_path_leases(self.paths, self.packages, *self.graphs, {SHARED})
    ...
    def test_restoring_parallel_module_ownership_is_rejected(self):
        ...
        for package in packages:
            if package["id"] in {"MATRIX-1-CHANNEL-BOUNDARY", "TASKFLOW-1-EXECUTION-BOUNDARY"}:
