...
journal = ROOT / "codex-rs/hepta-control-plane/src/planner_journal.rs"
text = journal.read_text(encoding="utf-8")
replacements = {
    "snapshot.snapshot_digest,\n            snapshot.snapshot_digest,": "snapshot.snapshot_digest(),\n            snapshot.snapshot_digest(),",
    "receipt.receipt_digest,\n            receipt.receipt_digest,": "receipt.receipt_digest(),\n            receipt.receipt_digest(),",
    "entry.payload_digest == receipt.receipt_digest": "entry.payload_digest == receipt.receipt_digest()",
    "self.revoked_digests().contains(&receipt.receipt_digest)": "self.revoked_digests().contains(&receipt.receipt_digest())",
    "receipt.receipt_digest,\n        )": "receipt.receipt_digest(),\n        )",
}
for old, new in replacements.items():
    count = text.count(old)
    if count != 1:
        raise SystemExit(f"journal replacement expected once, found {count}: {old!r}")
    text = text.replace(old, new)
journal.write_text(text, encoding="utf-8")

tests = ROOT / "codex-rs/hepta-control-plane/src/planner_journal_tests.rs"
tests.write_text(r'''use std::fmt::Debug;
...
print("LANE_D_JOURNAL_API_FIX_APPLIED")
