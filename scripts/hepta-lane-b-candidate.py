#!/usr/bin/env python3
"""Fail-closed verifier for the Lane B exact candidate and module truth.

This verifier proves repository-controlled source identity, closed module/document
coverage and declared native anchors. It never converts a source or test result
into provider execution, external-effect terminality, independent acceptance,
...
def verify_candidate(manifest: dict[str, Any]) -> list[str]:
    ...
    require(not denied, "candidate changed path outside envelope: " + ", ".join(denied))
...
def verify_truth(truth: dict[str, Any], manifest: dict[str, Any]) -> tuple[int, int]:
    ...
    require([row.get("module") for row in rows] == MODULES, "truth module closed world")
    ...
    for row in rows:
        ...
        for operation in operations:
            verify_operation(module, roots, operation)
