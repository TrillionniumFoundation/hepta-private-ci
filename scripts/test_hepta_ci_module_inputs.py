...
SPEC = importlib.util.spec_from_file_location(
    "hepta_ci_module_input_subject", pathlib.Path(__file__).with_name("hepta_ci_dependencies.py")
)
...
class ModuleInputTests(unittest.TestCase):
    def setUp(self):
        ...
        for name in ("hepta-a", "hepta-b", "hepta-unrelated"):
            self.put(f"codex-rs/{name}/Cargo.toml", f'[package]\nname="{name}"\nversion="0.1.0"\n')
            self.put(f"codex-rs/{name}/src/lib.rs", "pub fn fixture() {}\n")
        self.put("codex-rs/hepta-b/Cargo.toml", '[package]\nname="hepta-b"\nversion="0.1.0"\n[dependencies]\nhepta-a={path="../hepta-a"}\n')
    ...
    def test_raw_string_path_module(self):
        self.put("codex-rs/hepta-a/src/lib.rs", '#[path = r#"../../../docs/module.md"#]\nmod external;\n')
        self.put("docs/module.md", "pub fn value() {}\n")
        self.check_changed("docs/module.md")
    ...
    def test_inline_path_ambiguity_keeps_consumer(self):
        self.put("codex-rs/hepta-a/src/lib.rs", 'mod inline { #[path = "../../../../docs/module.md"] mod external; }\n')
        self.put("docs/module.md", "pub fn value() {}\n")
        self.check_changed("docs/module.md")

    def test_conditional_path_does_not_disappear(self):
        self.put("codex-rs/hepta-a/src/lib.rs", '#[cfg_attr(unix, path = "../../../docs/unix.md")]\nmod external;\n')
    ...
    def test_outlined_child_of_external_module_is_traversed(self):
        self.put("codex-rs/hepta-a/src/lib.rs", '#[path = "../../../docs/external.rs"]\nmod external;\n')
        self.put("docs/external.rs", "mod nested;\n")
        self.put("docs/external/nested.rs", 'pub const DATA: &str = include_str!("../payload.md");\n')
        self.put("docs/payload.md", "payload\n")
        self.check_changed("docs/payload.md")
