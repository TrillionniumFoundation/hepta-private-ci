...
def receipt(module: str) -> str:
    row = json.loads(
        (ROOT / f"docs/modules/{module}/IMPLEMENTATION_MAP.json").read_text()
    )
    lines = [
        "",
        HEADING,
    ...
    for op in row.get("operations", []):
        ...
        symbol, source = (
            op.get("nativeSymbol") or "pending",
            op.get("sourcePath") or "pending",
        )
...
def verify() -> int:
    missing = [
        ...
        for p in sorted((ROOT / "docs/modules").glob("*/TECHNICAL.md"))
