...
class ConsumerExecutionTests(unittest.TestCase):
    def execute(self, failure: str):
        with tempfile.TemporaryDirectory() as directory:
            ...
            env = dict(os.environ, PATH=str(tools) + os.pathsep + os.environ["PATH"],
                       ...
                       TEST_COMMAND_LOG=str(work / "commands.log"), TEST_FAILURE=failure)
            process = subprocess.run(
                ["bash", str(ROOT / "scripts/run_platform_types_consumer_qualification.sh")],
    ...
    def test_success_requires_all_checks_and_retains_exact_source(self):
        ...
        self.assertEqual(record["sourceHead"], subprocess.check_output(
