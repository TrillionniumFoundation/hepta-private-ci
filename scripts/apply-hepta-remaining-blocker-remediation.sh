...
old = '''LANE_C_SPLIT_TEST_HARNESS_CONFLICT = (
...
'''
new = '''LANE_C_SPLIT_TEST_HARNESS_CONFLICT = (
    "qualification/module-execution-dossiers/test_implementation_contracts.py"
)
...
def merge_lane(lane: str, branch: str) -> dict[str, Any]:
'''
replace_once(old, new, "LANE_D_PRIOR_OWNER_CONFLICTS")
...
old = '''        split_test_harness_conflict = lane == "C" and conflicts == [
            LANE_C_SPLIT_TEST_HARNESS_CONFLICT
        ]
        if (
'''
new = '''        split_test_harness_conflict = lane == "C" and conflicts == [
            LANE_C_SPLIT_TEST_HARNESS_CONFLICT
        ]
        ...
        if (
'''
replace_once(old, new, "lane_d_prior_owner_conflict = (")
...
''',
    '''        "autoResolvedSplitTestHarnessOnly": split_test_harness_conflict,
        ...
        "conflicts": conflicts,
''',
    "autoResolvedLaneDPriorOwnerOnly",
)
...
if ! git diff --cached --quiet; then
  git commit --signoff -m "fix(hepta): retain exact prior-owner bundle during Lane D convergence"
  git push origin "HEAD:refs/heads/${CONTROLLER_BRANCH}"
fi

printf '%s\n' \
  "Applied exact Lane-D prior-owner conflict policy; all unrecognized conflicts remain fail-closed."
