...
class SourceRootTests(unittest.TestCase):
    def setUp(self):
        ...
        self.module = {"id": "runtime.fixture", "rootBindings": [{"path": "alias"}, {"path": "adapter"}]}
        for name in ("alias", "implementation", "adapter", "legacy"):
            (self.root / name).mkdir()
        (self.root / "legacy/callee.rs").write_text("fn run() {}\n", encoding="utf-8")
        self.binding = {
            "schema_version": 1, "module": "runtime.fixture", "declared_root": "alias",
            "implementation_root": "implementation", "binding_mode": "canonical_alias",
            "duplicate_cargo_package_created": False, "model_authority": False,
            "provider_authority": False, "source_evidence_paths": ["legacy/callee.rs"],
        }
        self.write_binding()

    def write_binding(self):
        (self.root / "alias/BINDING.json").write_text(json.dumps(self.binding), encoding="utf-8")

    def test_alias_preserves_declarations_and_exact_file_scope(self):
        before = copy.deepcopy(self.module)
        self.assertEqual(resolve_source_roots(self.root, self.module),
    ...
    def test_plain_and_missing_roots_retain_existing_behavior(self):
        module = {"id": "plain", "rootBindings": [{"path": "adapter"}, {"path": "missing"}]}
    ...
    def test_migration_uses_aliases_without_changing_evidence_or_claims(self):
        ...
        self.module.update({"owner": "owner", "deputy": "deputy", "technicalDocument": "guide"})
        source_base = {"commit": "a" * 40, "tree": "b" * 40}
        row = {
            "module": self.module["id"], "sourceBase": source_base,
            "operations": [{"operation": "run", "nativeSymbol": "fn run(",
                            "sourcePath": "legacy/callee.rs", "tests": [{"path": "test.py"}]}],
        ...
        with mock.patch.object(maps, "ROOT", self.root):
            result = maps.migrate_map(row, self.module, {self.module["id"]: "lane"}, source_base)
            repeated = maps.migrate_map(result, self.module, {self.module["id"]: "lane"}, source_base)
        self.assertEqual(row, before)
        self.assertEqual(result, repeated)
        self.assertEqual(result["resolvedRoots"], ["implementation", "legacy/callee.rs", "adapter"])
        self.assertEqual(result["declaredRoots"], ["alias", "adapter"])
        self.assertEqual(result["repositoryControlledGaps"], row["repositoryControlledGaps"])
        self.assertEqual(result["operations"][0]["tests"], row["operations"][0]["tests"])
    ...
    def test_identity_version_and_authority_mismatches_reject(self):
        ...
        for key, value in [("module", "other"), ("declared_root", "other"),
                           ("schema_version", 2), ("schema_version", True),
                           ("binding_mode", "dynamic"), ("runtimeAuthority", True), ("model_authority", True),
                           ("provider_authority", 0), ("duplicate_cargo_package_created", True)]:
    ...
    def test_paths_cannot_escape_or_broaden_to_a_crate(self):
        for value in ("../outside", "/tmp", "legacy", "legacy//callee.rs",
                      "legacy/./callee.rs", "legacy\\callee.rs", "missing.rs", "C:/outside", ""):
    ...
    def test_duplicate_keys_and_resolved_paths_reject(self):
        ...
        with self.assertRaisesRegex(ValueError, "duplicate resolved"):
            resolve_source_roots(self.root, self.module)
        (self.root / "alias/BINDING.json").write_text('{"module":"a","module":"b"}', encoding="utf-8")
        with self.assertRaisesRegex(ValueError, "duplicate binding"):
            resolve_source_roots(self.root, self.module)

    def test_symlink_targets_and_binding_reject(self):
        (self.root / "linked").symlink_to(self.root / "legacy", target_is_directory=True)
