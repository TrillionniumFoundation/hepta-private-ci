...
class WorkspacePreflightTests(unittest.TestCase):
    ...
    def test_inherited_lints_require_an_owner_definition(self):
        with (self.root / "app/Cargo.toml").open("a") as stream:
            stream.write('[lints]\nworkspace = true\n')
        self.assertTrue(any("workspace.lints is missing" in error for error in verify_workspace(self.root)[1]))
        with (self.root / "Cargo.toml").open("a") as stream:
    ...
    def test_nested_workspace_uses_its_own_inheritance(self):
        ...
        self.write("nested/helper/Cargo.toml", '[package]\nname = "helper"\nversion.workspace = true\n[dependencies]\nleaf.workspace = true\n')
        self.write("nested/leaf/Cargo.toml", '[package]\nname = "leaf"\nversion.workspace = true\n')
    ...
    def test_nested_workspace_cannot_borrow_parent_package_fields(self):
        ...
        self.write("nested/helper/Cargo.toml", '[package]\nname = "helper"\nversion.workspace = true\n')
        errors = verify_workspace(self.root)[1]
        self.assertTrue(any("workspace.package.version is missing" in error for error in errors))
    ...
    def test_explicit_missing_workspace_fails(self):
        self.write("app/Cargo.toml", '[package]\nname = "app"\nworkspace = "../absent"\nversion.workspace = true\n')
        self.assertTrue(any("package.workspace has no" in error for error in verify_workspace(self.root)[1]))
    ...
    def test_excluded_dependency_cannot_inherit_parent_workspace(self):
        ...
        self.write("Cargo.toml", '[workspace]\nmembers = ["app"]\nexclude = ["helper"]\n[workspace.package]\nversion = "1.0.0"\n')
        self.write("helper/Cargo.toml", '[package]\nname = "helper"\nversion.workspace = true\n')
        self.assertTrue(any("workspace.package.version is missing" in error for error in verify_workspace(self.root)[1]))

    def test_preflight_never_mutates_manifests(self):
        before = {path: path.read_bytes() for path in self.root.rglob("Cargo.toml")}
        self.assertEqual(verify_workspace(self.root), (1, []))
        self.assertEqual(before, {path: path.read_bytes() for path in self.root.rglob("Cargo.toml")})
