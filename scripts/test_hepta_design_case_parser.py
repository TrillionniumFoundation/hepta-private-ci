...
SPEC = importlib.util.spec_from_file_location("technical_case_parser", ROOT / "scripts/hepta-technical-closure.py")
...
class DesignCaseParserTests(unittest.TestCase):
    ...
    def test_plain_code_bold_and_bold_title_keep_the_same_identity(self):
        for line in ["- EVID-01: distinct principals", "- `EVID-01`: distinct principals",
                     "- **EVID-01:** distinct principals", "- **EVID-01**: distinct principals",
    ...
    def test_malformed_unclosed_or_unnamed_cases_are_rejected(self):
        for line in ["- **EVID-01: unclosed", "- EVID-001: wrong identity", "- evidence: no identity", ""]:
    ...
    def test_existing_source_identity_needs_no_invented_numbered_alias(self):
        ...
        self.assertEqual(self.parse(f"- `{path}`: explicit owner regression", root), ["source:" + path])
    ...
    def test_parent_escape_and_directory_alias_are_rejected(self):
        for path in ["codex-rs/../outside_tests.rs", "codex-rs/example/./src/owner_tests.rs"]:
            with self.subTest(path=path), self.assertRaisesRegex(MODULE.Invalid, "invalid named"):
    ...
    def test_committed_symlink_does_not_stand_in_for_test_source(self):
        ...
        target = subprocess.check_output(["git", "-C", str(root), "hash-object", "-w", "--stdin"],
                                         input=Path(path).name.encode()).decode().strip()
        subprocess.run(["git", "-C", str(root), "update-index", "--add", "--cacheinfo",
                        "120000", target, alias.relative_to(root).as_posix()], check=True)
        subprocess.run(["git", "-C", str(root), "-c", "user.name=Test", "-c", "user.email=test@example.invalid",
                        "-c", "commit.gpgsign=false", "commit", "-qm", "symlink"], check=True)
        with self.assertRaisesRegex(MODULE.Invalid, "committed regular file"):
    ...
    def test_current_registered_guides_parse_without_rewriting_their_descriptions(self):
        for path in (ROOT / "qualification/module-execution-dossiers/detail").glob("*.md"):
