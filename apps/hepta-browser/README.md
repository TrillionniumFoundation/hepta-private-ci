...
Current executable actions are `navigate`, `click`, `type`, `focus`, `scroll` and `wait`. The `credential`, `upload` and `download` schemas are reserved future/fail-closed capabilities and are not current Browser support; the worker returns `capability_not_connected` until their dedicated brokers and terminal observers are qualified. Credential/upload schemas carry references rather than raw secret bytes or ambient host paths. `type.text` exists only in the live action payload; the durable operation journal does **not** store `typedAction` or raw text. It stores the final payload digest plus immutable effect semantics.

The file journal validates every hydrated field, rejects unknown fields, checks canonical checksum envelopes, fsyncs dispatch intent before the effect boundary, compacts atomically before the file ceiling and retires a fully terminal profile generation after clean close. A generation with durable operation history cannot be reopened into a fresh worker; unresolved durable effects from another generation block profile advancement. Persisted recovery automatically retires the generation once all recovered operations become terminal. Live-worker reconciliation and post-Browser-process-loss reconciliation are separate driver paths. Worker-known terminal responses are drained into the durable Browser journal after the final-use boundary and are also written first to a host-private inherited-fd terminal receipt; a replacement Browser owner can recover that exact terminality after parent loss. A replacement Servo worker is never treated as evidence of a remote business outcome that was never terminally observed; those cases still require an explicit trusted business reconciler and otherwise remain indeterminate. Persisted recovery is identified only by the checksum-protected non-secret operation identity; it never requires replaying `typedAction` or `type.text`, and an injected observer must echo the exact operation, request and semantic digests before a terminal claim is accepted.
...
`LinuxBubblewrapLauncher` describes a source launch contract: empty tmpfs root, selected read-only runtime libraries/data, cleared environment, hidden ambient homes/service roots, no direct network sharing, one private writable profile and one exact worker. Browser-authorized HTTP(S) resource loads use dedicated private fd 3/4 pipes to `GrantScopedEgressBroker`; DNS/IP/redirect/origin/TLS peer selection is checked outside the sandbox while ambient direct egress remains denied. `scripts/linux-sandbox-probe.js` executes that same launcher to test host-secret invisibility, denied external IPv4 egress, absence of general shell/Python binaries and private-profile durability. Only execution on an exact host proves enforcement.

## Capacity and backpressure

The long-lived Agentd Browser service uses `PooledSubprocessBrowserDriver` with a default 16 active profiles/workers and a hard configurable ceiling of 64; every profile remains a distinct process/profile directory and single-WebView serialization domain. Profile mutations use a bounded serialization queue (64 queued operations per key by default) and fail with `BrowserBackpressureError` on overload. Separate ceilings cover origins, grants, active operations, terminal in-memory replay cache, action fields, semantic observations, frames, journal size and call deadlines.
