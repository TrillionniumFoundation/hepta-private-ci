...
function line(
  ...
  const unsigned = { schema, version, type, record: recordValue };
...
for (const kind of ["memory", "file"]) {
  ...
  test(`${kind}: same request digest does not permit changed semantic digest`, async (t) => {
    ...
    await assert.rejects(
      journal.recordDispatch(record({ semanticDigest: D3 })),
      /semantics/,
    );
    assert.equal(
      (await journal.getOperation("profile.1", 1, "operation.1"))
  ...
  test(`${kind}: conflicting terminal result rejects without changing stored result`, async (t) => {
    ...
    await journal.recordObservation(terminal());
    await assert.rejects(
      journal.recordObservation(terminal({ outcomeDigest: D2 })),
      /terminal/,
    );
    assert.deepEqual(
      await journal.getOperation("profile.1", 1, "operation.1"),
  ...
  test(`${kind}: unknown observation can still reconcile to an observed terminal result`, async (t) => {
    ...
    await journal.recordObservation(
      record({ observationReason: "provider_disconnected" }),
    );
    await journal.recordObservation(terminal());
    assert.equal(
  ...
  test(`${kind}: rejected rollback does not block an unrelated operation`, async (t) => {
    ...
    assert.equal(
      (await journal.getOperation("profile.1", 1, "operation.2")).status,
      "succeeded",
    );
  });
  ...
  test(`${kind}: independently named generations retain separate records`, async (t) => {
    ...
    assert.equal(
      (await journal.getOperation("profile.1", 2, "operation.1")).status,
      "indeterminate",
    );
  });
}
...
test("file: an unterminated final record is discarded and physically repaired", async (t) => {
  const { path, journal } = await fixture(t);
  await journal.recordDispatch(record());
  const second = line(
    "dispatch",
  ...
  const reopened = new FileBrowserOperationJournal(path);
  assert.equal(
    await reopened.getOperation("profile.1", 1, "operation.2"),
    null,
  );
...
test("file: schema-v1 records migrate to canonical v2 snapshots", async (t) => {
  const { path } = await fixture(t);
  const legacyDispatch = { ...record() };
  const legacyTerminal = { ...terminal() };
  delete legacyDispatch.terminalEvidenceDigest;
  delete legacyTerminal.terminalEvidenceDigest;
  await writeFile(
    path,
    line("dispatch", legacyDispatch, {
      schema: "hepta.browser.operation-journal.v1",
      version: 1,
    }) +
      line("observation", legacyTerminal, {
        schema: "hepta.browser.operation-journal.v1",
        version: 1,
      }),
    { mode: 0o600 },
  ...
  const reopened = new FileBrowserOperationJournal(path);
  assert.equal(
    (await reopened.getOperation("profile.1", 1, "operation.1")).status,
    "succeeded",
  );
  const migrated = await readFile(path, "utf8");
  assert.match(migrated, /hepta\.browser\.operation-journal\.v2/);
  assert.doesNotMatch(migrated, /operation-journal\.v1/);
});
