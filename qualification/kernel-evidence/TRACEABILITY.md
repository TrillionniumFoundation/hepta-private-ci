...
| `verify_chain` enforces expiry, lineage, current issuer/key/role trust, role coverage and distinct principals + signing identities | `qualification.rs`, Agentd `evidence_trust.rs` | EVID-01..04, shared-key rejection, current trust/key-rotation invalidation, correction/revocation test | same source/merge artifact | external architecture/durability review required |
| Correction/revocation authority cannot cross principal/role except explicit `security` emergency revocation | `qualification.rs`, Agentd `evidence_trust.rs` | `lineage_mutation_is_principal_scoped_with_security_revocation_exception` | `evidence-tests.json` + product trust test | security review required |
| Durable independent-decision expiry is separate from short AuthBus admission TTL | `qualification.rs` | `independent_decision_validity_outlives_short_ingress_auth_ttl` | `evidence-tests.json` | external reviewer chooses bounded receipt expiry |
...
| Product caller/writer is real Agentd composition, not a library-only seam | `hepta-agentd/src/evidence_host.rs`, `evidence_trust.rs`, control/client routing | `tests/kernel_evidence_product.rs`: valid path plus wrong tree/role, replay, expiry, stale/revoked key | `agentd-product-test.json` | external product/physical review still required for activation |
| Current trust is checked immediately before physical evidence append and positive verification | `evidence_trust.rs`, `evidence_host.rs`, `qualification.rs` | product test rotates/revokes keys and requires stale positive claims/writes to fail | `agentd-product-test.json` | security review required |
...
| Backup/restore never treats a valid stale SQLite image as fresh evidence | `recovery_frontier.rs`, Agentd `evidence_frontier.rs`, `RECOVERY_FRONTIER_V1.md` | real Agentd test signs a newer frontier then restores a valid older image and requires `recovery_required` | source/product execution receipt pending on exact head; external backend qualification still required | external durability/operator receipt required |
| Complete-database replacement is detectable against an independent rollback domain | immutable `evidence_recovery_identity` + signed migration/qualification/replay snapshot | exact signed image starts; mismatched old image fails before readiness | source/product execution receipt pending on exact head; external CAS/latest-frontier service pending | external durability/operator receipt required |
...
- signed recovery-frontier snapshot/consumer and old-image rejection: source implemented/product-tested in the candidate;
- external durable monotonic CAS/latest-frontier backend and backup publication acknowledgement: not activated;
