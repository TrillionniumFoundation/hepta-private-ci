...
| EVID-03 corrupt payload / lineage after reopen | `verify_qualification_evidence_rows`, exact-candidate stored-link integrity, FK lineage and immutable triggers | `corrupted_qualification_payload_fails_closed_after_reopen`; `cross_candidate_predecessor_corruption_fails_closed_after_reopen` | Lane A exact-source/native receipt | independent reviewer consumes recovery evidence | implemented |
