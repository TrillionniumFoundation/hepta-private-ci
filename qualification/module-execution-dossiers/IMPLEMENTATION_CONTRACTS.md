...
The source observations in `NATIVE_BINDINGS.json` are deliberately bounded: one registered primary source path per canonical module, with historical observations rather than forty proven deployments. Current verification generates exact HEAD/tree and per-file hashes after checking module ownership and required source identifiers; historical observation hashes do not block committed implementation changes. Exhaustive operation and consumer mappings must still be produced by implementation packages. No source path, function name, digest or generated profile establishes a production call, physical effect or accepted capability.
