...
`hepta-cognitive-store/src/lib.rs` contains the in-memory semantic stores and the canonical durable façade; it does not add a second database. The physical owner remains SQLx `hepta-memory::CognitiveStore` and `cognitive_1.sqlite3`. `lane_c_snapshot`, proof-bound `lane_c_snapshot_page`, and `DurableCognitiveSnapshot::read` expose authorized coherent cuts while preserving the same owner. [Lane C SQLite integration](../../codex-rs/hepta-memory/LANE_C_SQLITE.md) owns exact ID/frontier mapping, correction/deletion behavior, paging bounds and test instructions. Descriptor-bound writable `open_with_recovery` is source-implemented: it fences ordinary handles, copies retained database/WAL/journal descriptors into a private generation, verifies an independently retained exact current-cut witness plus production authority/fence, checks SQLite integrity, checkpoints, and atomically publishes the recovered generation. Ordinary reopen still has no currentness proof and must not be described as that stronger recovery path.
