...
`hepta-cognitive-store/src/lib.rs` contains an in-memory `CognitiveStore` backed by `BTreeMap`, with a maximum of 16,384 live record IDs. It is a semantic oracle, not a durable backend. The implemented durable integration reuses the existing SQLx `hepta-memory::CognitiveStore` and its `cognitive_1.sqlite3` file. `CognitiveStore::lane_c_snapshot`, `LaneCAuthoritativeSnapshotProvider` and `cognitive.read::read_authoritative` expose a request-bound authorized coherent cut without introducing a second writer, database or synchronization service. Agentd is the named product caller and reacquires/revalidates the owner cut plus authoritative receipt/vector immediately before context consumption. [Lane C SQLite integration](../../codex-rs/hepta-memory/LANE_C_SQLITE.md) owns the exact native ID/frontier mapping, bounds, correction/deletion behavior, reopen witness and test instructions. Its descriptor-safe `open_with_recovery` admission still has unresolved VFS/currentness prerequisites; ordinary reopen must not be described as that stronger recovery path.
