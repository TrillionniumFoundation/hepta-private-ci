...
`matrix_ingress_projection` keys homeserver+room+event ID and retains source digest, sync position, sender evidence, redaction/correction and scope. `matrix_dispatch_ledger` is durable SQLite state keyed by canonical `stable_txn_id` and immutable operation/logical-outbox/room/generation/payload identity. Per-attempt final-use authority claims are append-only and bind the exact operation, subject, destination/session, revocation frontier and payload. `Succeeded` is a qualified terminal state and is forbidden unless a matching durable authority claim exists. Legacy/no-claim remote echoes become terminal `ObservedUnqualified`, which prevents replay but cannot satisfy qualification. Transport, homeserver-send and redaction observations are append-only records with separate digests. Ingress dedupe, egress terminal observation and sync-watermark advancement commit under the owner transaction so a crash cannot advance the cursor past unrecorded terminal evidence.

## 4. Deterministic algorithm and scheduling

Validate enrolled room and session; dedupe by server event identity; apply source correction/redaction; publish bounded ingress. For sending, persist the dispatch identity before transport, preserve the same transaction identity across retries, obtain and consume a fresh final-use grant for each physical attempt, persist its exact authority claim before polling the lazy network future, record HTTP/SDK success only as `accepted`, and settle qualified `succeeded` only when `/sync` observes the matching homeserver event with `unsigned.transaction_id` **and** the durable final-use proof exists. A remote echo without that proof becomes `ObservedUnqualified`. Retry exhaustion parks the unresolved transaction instead of converting uncertainty into failure. Server persistence and downstream user reading remain different claims. Reconnect resumes a durable watermark without replaying revoked content.
...
- MATRIX-05: a homeserver echo without a matching durable current-attempt final-use claim becomes `ObservedUnqualified`, never qualified `Succeeded`.
- MATRIX-06: nonce-consumed/pre-claim and claim-durable/pre-network-poll crash cuts cross no network boundary; restart preserves the stable transaction and requires a fresh grant.
- MATRIX-07: terminal homeserver observation and sync checkpoint are atomic; a later mutation failure rolls both back and reopen does not resurrect either.
- MATRIX-08: ACK-loss retry reuses the same stable transaction, obtains a fresh grant for each physical attempt, and converges to one homeserver event.
- MATRIX-09: stale/rollback revocation heads, wrong-signer grants, broker death, and revocation committed before adapter entry fail closed.

These are required product test designs. Repository tests establish the owner-local invariants; exact-candidate workflow receipts and the main-only real Synapse/E2EE receipt remain distinct evidence gates.
...
- **Source tests:** [codex-rs/hepta-matrix-sdk/tests/durable_transport.rs](../../../codex-rs/hepta-matrix-sdk/tests/durable_transport.rs) includes the nonce-consumed/pre-claim and claim-durable/pre-network crash cuts plus the existing ACK-loss retry; [codex-rs/hepta-matrix-store/tests/sync_mutation_v2.rs](../../../codex-rs/hepta-matrix-store/tests/sync_mutation_v2.rs) distinguishes qualified vs unqualified terminality and verifies terminal/cursor rollback; [codex-rs/hepta-matrixd/src/runtime/tests.rs](../../../codex-rs/hepta-matrixd/src/runtime/tests.rs) covers ingress/App Server runtime behavior. Test identities are not execution receipts for this documentation revision.
