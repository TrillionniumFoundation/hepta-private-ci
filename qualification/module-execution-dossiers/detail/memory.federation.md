...
No authoritative remote facts, remote writer or peer-consent store are owned by this module. Peer enrollment and permissions remain owned by fleet/authority and memory-owner capability state. Product Agentd stores only bounded owner-layout enrollment candidates; each physical federated recall resolves the owner-controlled active cognitive-database generation and rediscovers currently active capabilities. A retained predecessor database remains owner history and cannot be selected by filename convention.
...
Product `CognitiveRuntime::AvailableFederatedV2` applies this boundary per currently enrolled owner. Capability discovery is polled as a bounded completion stream: healthy outcomes that complete inside the discovery horizon are retained, while failed or unresolved owners consume explicit failed-discovery slots. Admitted peer attempts are then polled concurrently under the remainder of the total request horizon, while deterministic sorting/deduplication before admission and after collection prevents completion order from changing output order. The query generation vector binds both capability generation/revision and the selected active owner-database generation. Preflight, post-I/O and final-use paths reject a reader when the owner publishes a different active generation. Aggregate coverage preserves requested/completed/failed peers, peer truncation, owner-candidate omission, item truncation and typed discovery/deadline-authority/integrity/transport failure counts. A scope/transport failure is not relabeled as a successful empty result. A clean discovery with no active grant consumes no request slot. An active grant for a different consumer-workspace digest is filtered before enrollment and never triggers transport. An owner store whose enrollment state cannot be observed contributes a bounded failed slot. Post-I/O revoked/stale terminal attempts also contribute failed aggregate coverage because they produced no admissible evidence.
...
The <=16-peer / <=512-final-ID values are enforced architecture bounds. The current in-process product path uses one three-second total horizon with a two-second incremental discovery horizon, retaining completed healthy owners instead of resetting the whole request when another owner stalls. This is still not target-host latency/backpressure evidence and is not evidence that a future network transport can meet the same budget; cross-host activation still requires measured fan-out, overload and cancellation behavior on the selected host.

The current local qualification records exact-candidate runs for the 16-source ceiling, 128-candidate composition bound, mixed healthy/bad owner coverage and recovery-generation cutover. Wall-clock percentiles and peak RSS are development-host observations only. Authenticated cross-host qualification remains inapplicable until a registered wire protocol and a second real authenticated host exist; a second process on the same host is not accepted as equivalent evidence.
...
- FED-18: recovery publication switches discovery to the active owner database and immediately fences a predecessor reader after revoke;
- FED-19: correction and forget mutations on a recovered owner make predecessor readers unusable and expose only the current corrected evidence;
- FED-20: a stalled or corrupt owner produces explicit discovery failure coverage without discarding healthy owner results completed inside the discovery horizon.
...
Agentd product composition uses `CognitiveRuntime::AvailableFederatedV2`. The host passes the consumer Agent identity and bounded owner-layout candidates. The physical in-process owner read is adapted to the canonical V2 transport; discovery resolves the validated active database pointer, and preflight/post-I/O authority rediscoveries bind both current durable capability state and the same owner database generation. Each reader checks the active pointer before and after capability reads, so recovered-owner revoke/correct/forget operations fence predecessor readers without requiring predecessor-file deletion. The memory extension performs another capability/generation/memory revalidation at physical model-request assembly, bounded by the product read timeout and fail-closed on timeout/unavailability. Repository-wide dispatch semantics define this as a final-use source-currentness fence before transport entry, not retroactive cancellation authority over a provider attempt that has already been admitted.
...
- **Owner authority source:** existing `CognitiveStore` federation capability grant/revoke records, the owner-controlled active database pointer and `FederatedMemoryReader` read-only owner access; the canonical module does not become a writer of those facts.
- **Recovery and discovery tests:** [codex-rs/hepta-memory/src/cognitive_federation_tests.rs](../../../codex-rs/hepta-memory/src/cognitive_federation_tests.rs) covers active-generation revoke/correct/forget and old-reader fencing; [codex-rs/hepta-memory/src/cognitive_runtime_tests.rs](../../../codex-rs/hepta-memory/src/cognitive_runtime_tests.rs) covers stalled/bad-owner isolation and explicit typed coverage.
- **Canonical source tests:** [codex-rs/hepta-memory-federation/src/v2_tests.rs](../../../codex-rs/hepta-memory-federation/src/v2_tests.rs). These remain test identities until current candidate execution receipts pass.
