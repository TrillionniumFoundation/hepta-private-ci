...
- **Remaining work:** The SQLite owner now consumes the shared `hepta-kg` durable projection core for integrity and persisted output semantics. Remaining closure is to bind the typed `KnowledgeGenerationV2` full/incremental generation path to the open-ended product relation vocabulary without semantic narrowing, then qualify the cross-path oracle (full/incremental -> SQLite -> restart -> query -> correction/tombstone) and real query consumers. Do not introduce a second fact store.
