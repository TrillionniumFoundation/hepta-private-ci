...
- **Prompt-factor real consumer:** [codex-rs/hepta-prompt-optimizer/src/graph.rs](../../../codex-rs/hepta-prompt-optimizer/src/graph.rs) requires every candidate factor to exist in the complete V2 generation, queries the exact generation for complements/substitutes/conflicts, enforces `PromptConflicts` as hard co-selection constraints and binds the relation-query digest into a graph-bound portfolio receipt. The optimizer remains read-only and authority-free.
