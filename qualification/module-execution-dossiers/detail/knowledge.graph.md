...
- **Implemented entrypoints:** `build_complete_generation` in [codex-rs/hepta-kg/src/generation.rs](../../../codex-rs/hepta-kg/src/generation.rs); `apply_incremental_delta` in [codex-rs/hepta-kg/src/generation.rs](../../../codex-rs/hepta-kg/src/generation.rs); `publish_generation` and `query_relations` in [codex-rs/hepta-kg/src/generation.rs](../../../codex-rs/hepta-kg/src/generation.rs). Complete/incremental generation, predecessor-bound publication and bounded relation query kernels are implemented.
- **SQLite composition candidate:** [codex-rs/hepta-memory/src/cognitive_kg_store/v2.rs](../../../codex-rs/hepta-memory/src/cognitive_kg_store/v2.rs) adapts the existing immutable cognitive KG rows to `KnowledgeGenerationV2`. [codex-rs/hepta-memory/src/cognitive_kg_store.rs](../../../codex-rs/hepta-memory/src/cognitive_kg_store.rs) validates the exact predecessor and canonical V2 candidate before publishing the immutable SQLite generation inside the existing product mutation transaction. New generation receipts bind the V2 generation digest; reopen verification continues to accept historical pre-V2 projection digests while recomputing the V2 digest for new generations.
- **Product read candidate:** [codex-rs/hepta-memory/src/cognitive_retrieval.rs](../../../codex-rs/hepta-memory/src/cognitive_retrieval.rs) loads the selected durable generation through the V2 adapter and routes GraphOneHop relation selection through `query_relations`; SQLite remains the persistence/index owner rather than a second relation-policy implementation.
- **State and recovery:** `KnowledgeGenerationV2` remains a rebuildable projection with source/revision/validity support, generation vector and canonical digest. Durable rows preserve occurrence identity, source lineage and validity; publication checks an exact predecessor; projection values remain non-authoritative derived state.
- **Source tests:** [codex-rs/hepta-kg/src/generation_tests.rs](../../../codex-rs/hepta-kg/src/generation_tests.rs), [codex-rs/hepta-memory/src/cognitive_kg_store_tests.rs](../../../codex-rs/hepta-memory/src/cognitive_kg_store_tests.rs). The SQLite lifecycle oracle covers full-build materialization, incremental/full equivalence, persisted V2 digest, reopen, product GraphOneHop query, correction and committed tombstone propagation. These are test identities, not pass receipts for this documentation revision.
- **Implementation and operating references:** [codex-rs/hepta-memory/LANE_C_SQLITE.md](../../../codex-rs/hepta-memory/LANE_C_SQLITE.md).
- **Remaining work:** obtain exact-head and synthetic-merge Rust test/lint evidence for this composition candidate, bind the registered cross-module read/port contracts to named product-call evidence, and retain independent semantic review, operator acceptance, activation and release as separate gates. Until those receipts exist, `productionImplementation=false`, product composition is not promoted, and no release claim is made.
