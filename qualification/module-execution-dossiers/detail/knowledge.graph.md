...
- **State and recovery:** KnowledgeGenerationV2 is a rebuildable native projection with source/revision/validity support, generation vector and canonical digest. `codex-rs/hepta-kg/src/durable.rs` now also owns the durable projection identity/digest/integrity semantics consumed by the existing SQLite owner, preserving the historical receipt digest domains for reopen compatibility. Publication checks an exact predecessor; projection values are not a durable source writer.
...
- **Remaining work:** The SQLite owner now consumes the shared `hepta-kg` durable core, losslessly maps open product relations through `CustomPredicate`, constructs both complete and derived-incremental `KnowledgeGenerationV2` paths before commit, requires them to agree, publishes against the exact predecessor, and persists the V2 publication receipt in the same SQLite transaction. Reopen rebuilds and validates the current V2 generation, and GraphOneHop gates SQLite-visible edges through `query_relations`. Remaining repository closure is prompt-factor adapter composition plus exact-head/synthetic-merge qualification; independent acceptance, activation and release remain external gates. Do not introduce a second fact store.
