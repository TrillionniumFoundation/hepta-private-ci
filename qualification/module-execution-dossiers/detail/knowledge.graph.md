...
World-model consumers must distinguish symbolic evidence relations from learned dynamics. KG reads compose with cognitive snapshots and cannot construct mutation authority. Agentd is the named product host that selects the scoped cognitive mutation feature; ordinary Codex stays default-off. Rollback selects a rebuildable compatible projection after current deletion filtering. Test-only crash rendezvous exercise process death before semantic receipt and after semantic receipt but before current-pointer CAS; reopen must recover the exact predecessor.
...
- **Oracle/product tests:** [cognitive_kg_oracle_tests.rs](../../../codex-rs/hepta-memory/src/cognitive_kg_oracle_tests.rs) compares full and incremental V2 generation with SQLite materialization, reopen, correction/tombstone and query visibility, including physical output, generation and publication digests. [cognitive_store_tests.rs](../../../codex-rs/hepta-memory/src/cognitive_store_tests.rs) adds the ignored child-kill crash-window matrix. [cognitive_kg_benchmark_tests.rs](../../../codex-rs/hepta-memory/src/cognitive_kg_benchmark_tests.rs) emits the PERF-LIBRARY pilot receipt. [cognitive_product_e2e.rs](../../../codex-rs/hepta-agentd/tests/cognitive_product_e2e.rs) exercises the real Agentd/App Server path and checks persisted receipts across restart/correction/forget plus fail-closed startup when the writer store is unavailable.
- **Current claim boundary:** the cognitive **read** product caller is composed and this candidate makes scoped cognitive mutation the default Agentd product profile while ordinary Codex remains default-off. The separate `qualification-cognitive-write` feature only adds the qualification turn-witness seam. The candidate writer remains pending current exact-head and deterministic synthetic-merge proof. `apply_incremental_delta` remains an oracle/reference equivalence path rather than the selected runtime writer algorithm. The prompt-factor graph projection remains uncomposed in this branch and is intentionally handled as a separate prompt-stack integration. Module-wide `productionImplementation` and `productExecutionProved` remain false; target-host qualification and independent acceptance remain separate gates.
