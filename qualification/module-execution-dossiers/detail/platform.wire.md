...
- **Source composition:** `hepta-runtime::HeptaRuntime::status_wire_v2` encodes the existing read-only status payload, and `hepta-native-gateway` returns it only when the existing runtime route receives `Accept: application/x-hepta-wire; version=2`; the default JSON representation is unchanged and unknown wire media versions return 406. `hepta-context-compiler` and `hepta-codex-adapter` additionally compose strict V2 schema/producer admission into the registered product ports without serializing or minting authority.
- **Verification sources:** V1/V2 boundary and frozen-vector tests, negotiation downgrade tests, schema tests, stream tests, deterministic property tests, a cargo-fuzz target, a raw-binary Rust↔Python HPTN+V2 session test, gateway content-negotiation tests, and registered-port wrong-producer rejection tests. These are source/test identities until exact-candidate workflow receipts are current.
