...
Status: bounded calibrated decision, complete-set enforcement, canonical V3 profile, independent policy/model/scorer identity, scoring commitments, authenticated RNG provenance and split profile/runtime qualification are source implemented; remaining production composition and independent acceptance are listed in section 8. Common requirements: `../EXECUTION_SEMANTICS.md` and `../TECHNICAL.md`. Canonical ownership and package predecessors are unchanged.
...
`codex-rs/hepta-intuition/examples/fast_gate.rs` supplies the release-mode kernel qualification gate at 1/16/64/128 candidates. `codex-rs/hepta-intelligence/examples/intuition_authenticated_fast_gate.rs` separately gates the authenticated end-to-end path including trust verification, profile admission, scoring commitment and assignment provenance. Both report p50/p95/p99 and throughput; the kernel gate also reports allocation behavior. The checked ceilings are conservative CI regression bounds rather than activation SLOs; production target-host SLOs remain an external evidence gate.
...
- INT-07: frozen model bytes and frozen calibration/OOD data produce measured metrics, independent policy/model/scorer identities, a score-output commitment, signed Generator/Evaluator/Observer evidence and only then an authenticated policy decision.
- INT-08: CounterBased assignment must match an authenticated RNG owner, stream, `counter == sequence`, and exact draw; caller-selected draw substitution fails closed.
- INT-09: kernel and authenticated end-to-end p50/p95/p99/throughput gates execute independently.
...
- **State and recovery:** V2/V3 fail closed when `omitted_count_bound != 0`. V3 binds policy semantics, model/scorer lineage, objective class, generation/window, frozen calibration/OOD datasets, accepted artifact digests, thresholds and risk routing into `CanonicalPolicyProfileV1`. `policy_digest`, model artifact identity and scorer-contract identity are intentionally independent. `ScoringCommitmentV1` binds the exact model, feature snapshot/schema, candidate set and score outputs. `AssignmentCommitmentV1` binds deterministic assignment or RNG owner/stream/sequence counter/draw. The consumer verifies independent Generator/Evaluator/Observer Ed25519 evidence against the existing trust snapshot before V3 is invoked.
...
- **Fast qualification:** [codex-rs/hepta-intuition/examples/fast_gate.rs](../../../codex-rs/hepta-intuition/examples/fast_gate.rs), [codex-rs/hepta-intelligence/examples/intuition_authenticated_fast_gate.rs](../../../codex-rs/hepta-intelligence/examples/intuition_authenticated_fast_gate.rs) and `.github/workflows/hepta-intuition-qualification.yml` gate separate kernel and authenticated end-to-end regressions on exact source and deterministic synthetic merge.
...
- **Remaining work / non-claims:** compose a named production caller/native V3 host port, qualify the production-selected learned model and actual RNG owner on the target host, retain target-host SLO evidence, and obtain real consumer execution, operator acceptance, canary, promotion and release evidence. None of those states is inferred from green source CI.
