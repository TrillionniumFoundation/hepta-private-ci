...
Status: native semantic store plus existing SQLite owner snapshot integration are implemented; this candidate also source-implements a shadow-only canonical MemoryEvent co-observation receipt around the existing admitted append. Durable writer replacement, source-revision equivalence, exact execution evidence and independent acceptance remain separate gates in section 8. Common requirements: `../EXECUTION_SEMANTICS.md` and `../TECHNICAL.md`. Canonical ownership and package predecessors are unchanged.
...
- **Implemented entrypoints:** `CognitiveStore` in [codex-rs/hepta-cognitive-store/src/lib.rs](../../../codex-rs/hepta-cognitive-store/src/lib.rs); V2 `append_admitted_with_canonical_shadow` in [src/v2.rs](../../../codex-rs/hepta-cognitive-store/src/v2.rs); `CognitiveStore` in [codex-rs/hepta-memory/src/cognitive_store.rs](../../../codex-rs/hepta-memory/src/cognitive_store.rs); `lane_c_snapshot` in [codex-rs/hepta-memory/src/lane_c_snapshot.rs](../../../codex-rs/hepta-memory/src/lane_c_snapshot.rs). The shadow API performs all canonical/provenance checks before the existing authoritative append and binds the resulting write receipt without claiming payload equivalence.
