...
- **Final-use authority:** the native caller freezes the exact final `TurnStartParams`, requests an independently signed exact-binding grant through [codex-rs/hepta-infer-worker-host/src/final_use_authorizer.rs](../../../codex-rs/hepta-infer-worker-host/src/final_use_authorizer.rs), verifies current revocation state, claims a non-constructible `VerifiedUseToken`, rechecks cancellation/deadline/Agent readiness/ingress, consumes the token at final-use entry, and writes the authority/request witness durably before awaiting `turn/start`. The worker does not own the issuer signing key. Adapter receipts remain `AuthorityPosture::DENY_ALL`.
