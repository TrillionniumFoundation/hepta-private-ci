...
- **Remaining repository-controlled work:** compose `kernel.authority` final-use admission into the native caller so a non-constructible `VerifiedUseToken` (or its canonical one-entry consumption boundary) is consumed immediately before `turn/start`. The caller now hashes the exact serialized final `TurnStartParams` after thread/model/context assembly and persists that digest for reconciliation. The remaining equal `payload_digest` / `lease_payload_digest` assignment is still only a local consistency check until `kernel.authority` authenticates that exact digest.
