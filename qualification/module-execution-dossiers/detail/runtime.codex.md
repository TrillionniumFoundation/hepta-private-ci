...
- **Terminal semantics and correlation:** `TurnStatus::Completed`, `Failed` and `Interrupted` map to distinct adapter statuses. Observations bind the exact request digest and validate thread/turn identity; the request digest binds operation, Agent/App Server session, thread, the actual v2 wire method `turn/start`, payload, owner generation, protocol v2 and one absolute deadline. Request-level rejection, overload, timeout, transport loss and unknown decode outcomes preserve separate retry/reconciliation postures.
- **State and recovery:** App Server remains the real thread/turn execution owner. `DurableInferenceControl` journals the exact session/thread/request binding before `turn/start`, persists returned turn identity, holds capacity for unknown effects, and rejects blind replay. The live write-ahead call returns a non-serializable abort proof bound to that dispatch revision, allowing only the same process to release a request if deadline/cancellation changes before the network call; process loss destroys the proof, so recovery cannot downgrade an uncertain dispatch. Authority acquisition, `turn/start` acknowledgement and initial terminal observation consume one absolute runtime.codex deadline rather than resetting full budgets after each await. A reopened uncertain request uses `thread/read(includeTurns=true)` and requires both the stable `client_user_message_id` and original `UserMessage` content to reconcile the original turn; same-id/different-input and multiple exact matches are hard conflicts. Missing ephemeral App Server history remains indeterminate rather than inferred not-applied; the slot remains held/quarantined until independently evidenced terminal/no-effect resolution.
...
- **Authority boundary:** adapter receipts remain `DENY_ALL` and do not mint model/provider/tool authority. The native caller freezes the exact final `TurnStartParams`, derives the final-use binding, obtains an independently signed grant from the protected authority port, synchronizes the trusted revocation head, and claims the grant through `FinalUseAuthority`. After that external await it rechecks cancellation, the exact Agent generation/readiness, ingress identity and runtime.codex deadline; only then does it consume `VerifiedUseToken::enter`, commit the authority witness/request binding to the durable write-ahead journal, and perform the first `turn/start` await. The worker never owns the issuer private key. Delegated external-tool terminality remains with its effect owner.
...
- **Remaining qualification work:** obtain current exact-head and merge-candidate CI receipts; qualify the independently operated final-use authority endpoint, signer-key custody, socket/ACL identity including the connected peer credential, trusted time and revocation distribution on the selected target host; independently anchor final-use epoch/replay state so filesystem snapshot restore or store relocation cannot roll authority backward; execute the named caller against the authenticated target-host Agentd/App Server and real provider stream; qualify delegated external-tool terminal observation and acknowledgement-loss behavior; define and independently evidence the operational resolution/quarantine policy for indeterminate effects whose ephemeral App Server history is unavailable; complete independent acceptance, activation/canary, promotion and release gates. Repository source must not self-certify those external facts.
