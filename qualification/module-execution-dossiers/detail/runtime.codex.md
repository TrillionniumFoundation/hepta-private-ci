...
- **Implemented entrypoints:** `thread_start` in [codex-rs/app-server/src/request_processors/thread_processor.rs](../../../codex-rs/app-server/src/request_processors/thread_processor.rs); `turn_start` in [codex-rs/app-server/src/request_processors/turn_processor.rs](../../../codex-rs/app-server/src/request_processors/turn_processor.rs); `adapt` and `observe_prompt_delivery_v1` in [codex-rs/hepta-codex-adapter/src/lib.rs](../../../codex-rs/hepta-codex-adapter/src/lib.rs). Existing Codex App Server remains the sole execution spine; the adapter remains a separate authority-free boundary component.
- **Prompt terminal mapping:** `observe_prompt_delivery_v1` emits the exact registered in-process `PromptDeliveryObservationV1` field shape only after a caller supplies a terminal observation and an observed provider-request digest equal to the lease-bound request payload digest. Delivered observations reject rejection reasons; rejected observations require a bounded reason ID; non-terminal/unknown outcomes emit no V1 delivery observation and remain indeterminate.
- **State and recovery:** App Server owns real thread/turn execution and persistence. The adapter is stateless and validates supplied request/deadline/digest/terminal facts; neither `adapt` nor the new delivery mapper performs an RPC or authenticates a provider by itself.
- **Source tests:** [codex-rs/hepta-codex-adapter/src/lib_tests.rs](../../../codex-rs/hepta-codex-adapter/src/lib_tests.rs), [deadline_digest_tests.rs](../../../codex-rs/hepta-codex-adapter/src/deadline_digest_tests.rs). These are test identities until exact-candidate CI executes them.
...
- **Remaining work:** establish a named non-test App Server/Agentd caller that supplies the actual provider request digest and terminal stream outcome to the mapper; qualify the real provider stream, delegated tool terminal observer and acknowledgement-loss/restart behavior. Source mapping is not runtime activation.
