...
There are current repository composition callsites in `codex-rs/hepta-control-plane/src/planner_context.rs`, `codex-rs/hepta-control-plane/src/planner_ndu.rs` and `codex-rs/hepta-intelligence/src/vertical.rs`. They establish real request-local read-only/planning composition. The authenticated production NDU owner/caller remains separately not established; neither state implies effect authority.
...
`NduSolverIterationReceipt` and `NduSolverTerminationReceipt` are local deterministic solver evidence. Successful termination records both terminal residual and the true maximum residual observed across iterations. Exhausting the registered 64-iteration bound returns `NduError::IterationExhausted`; the bounded state at exhaustion is not exposed as a successful terminal state.
...
The deterministic preference solver uses Q32 nearest/ties-even arithmetic, eta in `[1/16,1/4]`, at most 64 iterations, bounded projection and immutable revision advancement. Parent/child conflicts use explicit `subject_id` / `parent_subject_id` relations; siblings and unrelated hierarchies may advance in the same generation, while a concrete parent and child may not. One subject/generation cannot select conflicting artifacts.
...
- `NDU-DETAIL-04`: an explicit parent/child pair cannot update in one generation; siblings/unrelated hierarchies may advance concurrently, and conflicting artifacts for one subject/generation reject.
...
- **State and recovery:** The deterministic Q32 evaluator and native f64 shadow regression are distinct profiles. The durable projection-store source candidate validates/migrates its store schema, preserves revocation reserve under retention limits, and refuses restore regression. Regression solves centered `Z Sigma = B` (rate convention `Z Q = B/dt`) using scaled Cholesky and bounded diagnostics. Projection journal recovery validates both hash-chain and transition semantics with scoped revocation. The durable store adds single-writer ownership, synchronized copy-on-mutate persistence, crash-temp cleanup, poison-on-indeterminate behavior, reopen and monotonic backup restore.
