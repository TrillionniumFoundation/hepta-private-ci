...
There are current repository composition callsites in `codex-rs/hepta-agentd/src/cognitive_context.rs`, `codex-rs/hepta-control-plane/src/planner_context.rs`, `codex-rs/hepta-control-plane/src/planner_ndu.rs` and the separate `codex-rs/hepta-intelligence/src/vertical.rs` read-only vertical. The Agentd/Control chain is a real request-local read-only product caller; these paths establish bounded read-only/planning composition, not the authenticated production NDU owner/caller, production writer activation or effect authority.
...
`NduSolverIterationReceipt` and `NduSolverTerminationReceipt` are local deterministic solver evidence. Successful termination records the terminal residual and, when iterations were emitted, the maximum residual among those emitted iteration receipts. The pre-iteration residual is not mixed into that iterative maximum; a zero-iteration no-op reports its validated initial residual as both terminal and maximum. Exhausting the registered 64-iteration bound returns `NduError::PreferenceSolverUnavailable`; the bounded state at exhaustion is not exposed as a successful terminal state.
...
`bind_solver_iteration_receipt_v1` publishes an owner-local canonical-context receipt only after binding subject, objective, body generation, event, coefficient, revision, residual, projection count and state digest. It rejects zero/out-of-range iteration numbers, non-successor revisions, negative residuals, empty state digests and subject identity/class rebinding before publication. Separately, `NduAuthenticatedOwnerV1::evaluate` creates an opaque receipt whose private fields bind the original principal, owner, host generation, fence, revocation frontier, frozen policy, objective and generation; callers cannot attach that context after an authority-free evaluation. The outputs carry `AuthorityPosture::DENY_ALL` or consume an external final-use grant without minting authority.

`NduProjectionJournalV1` is the bounded semantic/serialization layer. It provides append-only hash-chain entries, semantic idempotency, selected-projection reconstruction, exact reopen, truncation/tamper detection and revocation non-resurrection. Selection replacement checks the expected selected-content predecessor. The normal Agentd product path additionally binds the full journal head, which is necessary to reject late operations across ABA histories. Recovery replays the same semantic transition rules as live mutation: a hash-valid selection or revocation that references a projection never recorded for the same objective/subject is rejected. Revocation is scoped by `(objective_digest, subject_digest, projection_digest)`, so identical projection bytes used under another objective/subject are not accidentally revoked across scope boundaries.

`NduProjectionStoreV1` is a crash-bounded durable writer **source candidate**, not an activated production writer. It requires a host-supplied private directory, holds one advisory writer lock, rejects an oversized image from metadata before allocating or reading it, then performs a bounded `max + 1` read before semantic reopen. It writes and `sync_all`s a temporary complete image, atomically renames it, synchronizes the parent directory on the Unix qualification profile, discards stale uncommitted temp images under the writer lock, and only advances in-memory state after persistence succeeds. Ordinary history reserves one future revocation slot per live projection, so full-capacity revocation and restart recovery remain possible. Backup restore is monotonic: the current committed journal must be an exact prefix of the restored journal so an old valid backup cannot remove a later revocation.
...
- **Current composition:** the request-local chain `runtime.agentd cognitive_context -> control plan_observed_context -> evaluate_prepared_plan_with_ndu -> NDU V2 evaluator` is established in source and has focused Control regressions; `codex-rs/hepta-intelligence/src/vertical.rs` is an additional read-only composition surface. `authenticatedProductionOwner` remains not established.
...
- **Qualification workflow:** `.github/workflows/hepta-ndu-recursion.yml` qualifies PR source heads and deterministic synthetic merges, runs on relevant `main` pushes/manual dispatch, and executes focused `codex-hepta-control-plane` planner-context/planner-NDU caller regressions. The workflow definition is not itself a pass receipt.
- **Named local process boundary:** `main -> load_ndu_process_bootstrap_v1 -> AgentdNduOwnerHostV1 -> NduControl -> apply_mutation_at_head_guarded` reconstructs the deterministic local owner, enforces fresh signed feed views, one writer and post-lock generation checks, and exposes historical outcome recovery. Normal-process tests must actually pass at the claimed candidate.
- **Remaining repository-controlled work:** complete the production protected-time/off-host-frontier bootstrap beyond this explicit local profile; bind production consumers to that selected writer and final authentication profile; register and compose a production stochastic coefficient profile/consumer using the now-explicit coordinate/Q24 conversion boundary.
...
## 9. Exact-candidate execution receipts

`scripts/hepta-ndu-qualification.py` accepts one of `source`, `core`, `callers`, `product`, `lint`, `host`, a `source-head` or `synthetic-merge` lane, and an absolute output directory outside the checkout. It rejects dirty candidates, executes every command in its suite even after another fails, disables test retries, bounds default test concurrency, and retains every exit code and log hash. Host JSON identity and measured performance claims are validated independently of the process exit status. It does not turn a failing/empty/missing run into a completion claim. CI schedules these suites independently and preserves the two aggregate required gate names.

Named-host receipt schema V3 requires an exact SHA/tree and measures all 2,048 reserved revocations through a full 4,096-record journal, semantic restart, actual disk reopen/restore, 1 TiB sparse-image rejection, CPU/RSS and hot-path percentiles. Filesystem fault cuts have their own executed test log; the numerical binary no longer self-asserts that those tests ran. An actual OS process kill is distinct from an injected fsync/rename failure, and neither is a physical power-cut or off-host disaster-recovery exercise.
