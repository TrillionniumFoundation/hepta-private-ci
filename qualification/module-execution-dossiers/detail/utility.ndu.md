...
`NduProjectionJournalV1` is a bounded durability reference, not a production writer. It provides append-only hash-chain entries, semantic idempotency, selected-projection reconstruction, exact reopen, truncation/tamper detection and revocation non-resurrection. Revocations are scoped by objective + subject + payload digest. `checkpoint` binds record count and current hash-chain head for external trusted signing/anchoring. The candidate implements deterministic schema/checksum validation, atomic publication, fsync, bounded retention through the 4096-record journal ceiling, and create-new backup/restore. Production composition still requires a named host to select it plus target-filesystem crash/power-loss qualification, operational retention policy, trusted checkpoint anchoring and restore drills.
...
- **Remaining work:** The repository now contains a source-level fsync-backed durable-store candidate, stochastic evidence binding and `learning.eval` convergence-certificate admission. Remaining production work is canonical external/wire admission, named-host selection and target-filesystem crash/power-loss plus restore qualification for the durable store, trusted checkpoint anchoring, authenticated real coefficient/conversion/identification/well-posedness/convergence evidence, target-host performance qualification, operator acceptance, activation and release.
