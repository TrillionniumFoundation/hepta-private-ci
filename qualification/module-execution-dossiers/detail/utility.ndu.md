...
`NduProjectionJournalV1` is the bounded semantic/serialization layer. It provides append-only hash-chain entries, semantic idempotency, selected-projection reconstruction, exact reopen, truncation/tamper detection and revocation non-resurrection. Selection replacement is an expected-predecessor compare-and-set, so a late operation cannot overwrite newer selected state. Recovery replays the same semantic transition rules as live mutation: a hash-valid selection or revocation that references a projection never recorded for the same objective/subject is rejected. Revocation is scoped by `(objective_digest, subject_digest, projection_digest)`, so identical projection bytes used under another objective/subject are not accidentally revoked across scope boundaries.
