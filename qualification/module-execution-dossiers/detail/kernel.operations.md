...
- **Implemented entrypoints:** the deterministic reference models remain `OperationLedger` in [src/ledger.rs](../../../codex-rs/hepta-operations/src/ledger.rs) and `Outbox` in [src/outbox.rs](../../../codex-rs/hepta-operations/src/outbox.rs). The current candidate additionally implements the kernel-owned `DurableOperationStore` in [src/durable.rs](../../../codex-rs/hepta-operations/src/durable.rs), backed by [migration 0001](../../../codex-rs/hepta-operations/migrations/0001_durable_operations.sql).
- **State and recovery:** durable mutations use `BEGIN IMMEDIATE` and atomically update the current projection plus an immutable event row. Reopen verifies SQLite integrity/foreign keys, the exact migration ledger, complete revision histories, projection-to-event binding and immutable-event triggers. The durable chain preserves pending/authorized/dispatched/indeterminate/terminal separation.
- **Outbox:** every durable outbox row references an existing operation. Claims are owner/generation/expiry fenced with bounded lease lifetime; live foreign claims reject and expired claims may be taken over. Queue acknowledgement is not operation terminal success.
- **Source tests:** [src/durable_tests.rs](../../../codex-rs/hepta-operations/src/durable_tests.rs) exercises crash/reopen reconciliation, two-handle semantic conflict, claim takeover, acknowledgement-not-terminal and tampered-projection rejection. The original [ledger tests](../../../codex-rs/hepta-operations/src/ledger_tests.rs) and [outbox tests](../../../codex-rs/hepta-operations/src/outbox_tests.rs) remain reference-oracle coverage. These are source identities until exact-candidate workflow receipts are green.
- **Implementation references:** [DURABLE_STORE_V1.md](../../../docs/lane-a-foundation/kernel.operations/DURABLE_STORE_V1.md) and [CURRENT_IMPLEMENTATION.md](../../../docs/lane-a-foundation/kernel.operations/CURRENT_IMPLEMENTATION.md).
- **Remaining work:** compose the durable owner into an authenticated named product caller, evaluate current durable RBAC/policy, consume `kernel.authority` final-use authority at the actual owner/effect boundary, bind destination-owned deduplication and independently observed terminal reconciliation, and complete crash/disk/multi-process plus target-host qualification. Production implementation, product execution, activation and release remain false.
