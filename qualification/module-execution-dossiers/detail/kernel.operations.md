...
- **Product composition:** `AgentdProductionOperationRuntimeConfig` is consumed by the real Agentd `runtime::run`, attaches the production host to `AgentdState`, and spawns an immediate-then-periodic observer-only reconciler. Default process-environment startup remains inactive unless an external authority/verifier/final-use/grant-provider/target bundle is explicitly supplied.
