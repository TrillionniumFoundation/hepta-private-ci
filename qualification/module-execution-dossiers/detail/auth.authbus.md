...
- **Transaction boundary:** `authorize_and_reserve_authbus` evaluates the exact current non-revoked policy revision and reserves integer quota inside one `BEGIN IMMEDIATE` transaction. A caller must invoke `validate_authbus_reservation_for_effect` immediately before crossing an effect boundary. Settlement binds non-empty terminal evidence and observed cost; queue acknowledgement is not effect terminality.
- **Recovery / rollback candidate:** migration 0012 and `authbus_recovery.rs` bind the SQLite lineage to an independently retained restore checkpoint. Replay epoch retirement requires that checkpoint, no active outbox rows, and leaves a durable tombstone so pruning cannot reopen replay.
...
- **Native tests:** `authbus_control_tests.rs` executes BUS-01 through BUS-04 against real SQLite, including independent-handle contention and terminal-state races. `authbus_recovery_tests.rs` verifies checkpoint mismatch and replay retirement tombstones.
- **Composition limit:** the registered product composition remains the narrow Agentd signed-text ingress. No provider/secret/network/filesystem adapter yet consumes the policy/quota/reservation surface, so production implementation, activation, acceptance and release remain false until exact-candidate CI and external gates close.
