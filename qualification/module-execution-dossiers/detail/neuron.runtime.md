...
- **Guarded canonical product:** `AgentdIntelligenceProductRunnerV1` consumes an opaque `AgentdNeuronInvocationV1` sharing a long-lived durable owner. `tick_guarded` checks live host admission before inference, durable preparation and publication; signed frontier and cancellation checks compose with a mandatory selected-artifact guard. The actual committed state reaches the existing intuition consumer. The qualification feature backend is a test double, not selected production Laya execution. See `codex-rs/hepta-agentd/NEURON_RUNTIME.md`.
- **Process recovery:** `runtime_process_tests.rs` exercises bootstrap/prepare/journal/witness/completion child-process exits, exact output recovery and single model invocation. Power-loss durability and independent efficacy remain separate evidence.
