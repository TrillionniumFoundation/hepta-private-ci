...
- **Exact bytes and tokenizer:** the public candidate token receipt is measured over actual bytes via `ExactContextTokenizerV2`. Serialization receives actual selected bytes, verifies content digests, constructs the actual final payload via `ContextSerializerV2`, requires one byte placement per selected item, verifies every payload slice byte-for-byte against the selected content, binds the placement digest, and retokenizes those final bytes before accepting the provider-facing budget.
