...
Status: durable factor/realization ownership, kernel final-use authenticated admission (with reviewer-signature compatibility), immutable lifecycle lineage, exact context-profile lookup and payload-backed source composition are implemented; runtime host activation, terminal model-delivery evidence and independent acceptance remain open as listed in section 8. Common requirements: `../EXECUTION_SEMANTICS.md` and `../TECHNICAL.md`. Canonical ownership and package predecessors are unchanged.
...
- **Implemented entrypoints:** `PromptRegistry` in [codex-rs/hepta-prompt-registry/src/lib.rs](../../../codex-rs/hepta-prompt-registry/src/lib.rs); `AdmissionAuthority::verify` and `FinalUseAdmissionAuthority::verify` in [codex-rs/hepta-prompt-registry/src/admission.rs](../../../codex-rs/hepta-prompt-registry/src/admission.rs); `DurablePromptRegistry` in [codex-rs/hepta-prompt-registry/src/durable.rs](../../../codex-rs/hepta-prompt-registry/src/durable.rs); `register_realization_payload_v2` and `dereference_realization_v2` in [codex-rs/hepta-prompt-registry/src/delivery.rs](../../../codex-rs/hepta-prompt-registry/src/delivery.rs); `read_compatible_v2` in [codex-rs/hepta-prompt-registry/src/v2.rs](../../../codex-rs/hepta-prompt-registry/src/v2.rs); canonical V1 codecs in [codex-rs/hepta-prompt-registry/src/protocol.rs](../../../codex-rs/hepta-prompt-registry/src/protocol.rs).
- **Authenticated admission:** the compatibility reviewer-signature verifier binds signer, reviewer, factor ID/content, reviewed scope, evidence and a short validity interval, but it is not an authoritative durable-write entrypoint. The public durable admission path is `DurablePromptRegistry::admit_factor_final_use`, which consumes `kernel.authority::FinalUseAuthority`: its binding covers reviewer, exact factor identity/content, reviewed scope and evidence, while the authority owner enforces signer trust, authority epoch, revocation head, expiry and a persistent single-use nonce. The registry mutation and fsync publication execute inside the final-use revocation fence, and the lifecycle journal records grant/reviewer/scope/evidence lineage. Legacy and compatibility direct-mutation helpers are test-only.
- **State and recovery:** `PromptRegistry` remains the deterministic core. `DurablePromptRegistry` is the source-level authoritative writer, holds one writer lock, verifies state-file ownership/mode/link count, persists a schema-versioned owner image by fsync + atomic rename + directory fsync, reopens with replayed lifecycle/relationship/event/registry-digest validation, and includes a deterministic v1 compatibility migration. A failure after rename but before directory-fsync acknowledgement returns `IndeterminateDurability`, poisons the current writer/read surface and requires reopen before further authoritative use. Revocation and realization disablement survive reopen.
- **Realization delivery:** V2 bindings include context profile and exact model/tokenizer/template/tool-schema/locale/role identity. Public durable publication uses `register_realization_payload_final_use_v2`: a kernel FinalUse grant binds actor, admitted factor identity/content/current lifecycle, exact realization binding, scope, predecessor supersession and the actual payload digest, and fsync publication executes inside the final-use revocation fence. One active realization per exact factor/profile/role is enforced unless the new revision explicitly supersedes the active predecessor. Dereference revalidates snapshot, lifecycle, expiry, exact profile and payload digest.
- **Canonical contract projection:** `PromptFactorV1` and `PromptRealizationV1` are projected from authoritative factor/binding state rather than maintained as parallel facts. Legacy v1 migration keeps missing semantic/model metadata explicitly unresolved and canonical projection fails closed with `MissingAuthoritativeLineage` rather than publishing invented identities.
- **Selection and canonicalization:** required-factor IDs are canonicalized before digesting; required factors receive one compatible realization before the remaining result budget is filled, preventing truncation starvation. The composed delivery path validates `PromptExerciseDecisionV1` against the current authenticated registry source and exact selected binding digests before dereference; selected realizations are mandatory to `context.compiler`, so the compiler cannot replace them with another compatible realization.
- **Source composition:** [codex-rs/hepta-intelligence/src/prompt_delivery.rs](../../../codex-rs/hepta-intelligence/src/prompt_delivery.rs) is a named source-level consumer. It dereferences the exact stored bytes, binds the admission-event digest and tokenizer receipt, passes those candidates into `context.compiler::compile_v2`, deterministically serializes the selected deliveries, records a `ContextSerializationReceiptV2`, and builds a `ContextAttachmentV2`. This proves byte-to-attachment source composition, not running-product or terminal provider delivery.
...
- **Remaining work:** bind the durable owner and source-composition bridge into an activated named runtime host; submit the bound `ContextAttachmentV2` through the real Codex/provider spine and obtain a terminal observation whose observed payload digest equals the attachment payload digest; run target-host/fault/load qualification and independent semantic/operator acceptance. No source-only change can self-certify those gates.
