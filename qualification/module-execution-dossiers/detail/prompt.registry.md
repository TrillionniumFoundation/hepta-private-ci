...
- **Authenticated admission:** the compatibility reviewer-signature path binds signer, reviewer, factor ID/content, reviewed scope, evidence and a short validity interval. The production-intended `FinalUseAdmissionAuthority` additionally consumes `kernel.authority::FinalUseAuthority`: the final-use binding covers reviewer, exact factor identity/content, reviewed scope and evidence, while the authority owner enforces signer trust, authority epoch, revocation head, expiry and a persistent single-use nonce. Both paths yield opaque `VerifiedAdmission` values and the mutation path journals grant/reviewer/scope/evidence lineage. Legacy unauthenticated mutation helpers are test-only.
