...
`issue_work_envelope(source_receipt, package, owner, path_lease, budget) -> ParallelLaneEnvelopeV1`; `plan_engineering_work(dag, worker_skills, worker_capacity, source_conflicts, review_capacity, ci_capacity, expected_value, architecture_debt, rollback_cost) -> EngineeringOrchestrationPlan`; `schedule_ready_packages(dags, capacities, conflicts) -> AssignmentProposal`; `generate_candidate(iteration_envelope, grammar) -> CandidateSet`; `request_independent_review(candidate, evidence) -> ReviewRequest`. Exact source inventory and declared roots are inputs, not permission to alter arbitrary files.
...
Topologically identify development-ready packages; exclude path conflicts; match required skills and bounded worker capacity; reserve review and CI capacity; rank by stable priority plus expected value, architecture-debt reduction and rollback cost; reserve shared-type ownership for a contract integrator; emit bounded assignments, integration order and a merge-queue proposal; generate no-change plus permitted mutations in credential-free sandboxes; run mandatory tests; compare against frozen independent oracles; request review. Base drift invalidates affected envelopes. Generated tests undergo mutation testing; the candidate cannot modify tests/evidence that judge itself.
...
- **Implemented entrypoints:** `issue_work_envelope`, `schedule_ready_packages`, `verify_integration_evidence`, `request_independent_review` and `record_integration_decision` through [tools/hepta-engineering-control/control_engineering_v2/facade.py](../../../tools/hepta-engineering-control/control_engineering_v2/facade.py); richer `plan_engineering_work` through [tools/hepta-engineering-control/control_engineering_v2/orchestration.py](../../../tools/hepta-engineering-control/control_engineering_v2/orchestration.py); repository product composition through `build_product_receipt` in [tools/hepta-engineering-control/control_engineering_v2/product_gate.py](../../../tools/hepta-engineering-control/control_engineering_v2/product_gate.py); `DebianSandboxAdapter` remains in [tools/hepta-engineering-control/control_engineering_v2/assimilation.py](../../../tools/hepta-engineering-control/control_engineering_v2/assimilation.py). The historical `hepta_engineering_control.py` surface is compatibility-only and is not a canonical native mapping.
...
- **Repository product caller:** `.github/workflows/hepta-consolidated-source.yml:engineering-product-gate` invokes only the v2 public composition after strong sandbox and repository qualification prerequisites, exercises the rich planner and durable SQLite assignment owner, and retains an exact Git-bound product-execution receipt. It grants no merge, deployment, release or runtime authority.\n- **Remaining work:** Obtain successful exact-head/synthetic-merge execution receipts for that caller, then bind independent reviewer/key custody and authorized target-host handoff. Scheduling or merge-queue eligibility does not itself merge, deploy, release or complete autonomous development. The fixture adapter does not discover or control live Debian services. Its supplied consent/identity still requires a trusted owner; independently witnessed target parity, live service observation and authorized production deployment remain outside this read-only fixture implementation.
