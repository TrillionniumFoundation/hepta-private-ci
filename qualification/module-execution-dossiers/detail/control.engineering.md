...
- **Implemented entrypoints:** `EngineeringController` in [tools/hepta-engineering-control/control_engineering_v2/controller.py](../../../tools/hepta-engineering-control/control_engineering_v2/controller.py); `schedule_ready_packages` in [tools/hepta-engineering-control/control_engineering_v2/facade.py](../../../tools/hepta-engineering-control/control_engineering_v2/facade.py); `claim_assignment` in [tools/hepta-engineering-control/control_engineering_v2/assignment.py](../../../tools/hepta-engineering-control/control_engineering_v2/assignment.py); `verify_integration_evidence` in [tools/hepta-engineering-control/control_engineering_v2/facade.py](../../../tools/hepta-engineering-control/control_engineering_v2/facade.py); `DebianSandboxAdapter` in [tools/hepta-engineering-control/control_engineering_v2/assimilation.py](../../../tools/hepta-engineering-control/control_engineering_v2/assimilation.py).
- **State and recovery:** The v2 scheduler consumes immutable package/dependency/path-lease inputs and returns bounded assignment generations. Schema v6 additionally binds package paths/capabilities/retry ceilings, signed worker identities and durable fenced claims with explicit heartbeat, completion, failure and requeue. Worker lease renewal is capped by the signed identity expiry. EngineeringController derives predecessor completion from durable completed claims and durably binds one repository/writer credential chain. Signed AuditAnchorReceipt verification supports an externally retained anti-rollback witness without treating the local database as HA. Integration eligibility now binds exact base commit/tree, source commit/tree, ordered synthetic-merge parents and expected merge tree under a hermetic Git environment; supplied booleans or receipts are not GitHub execution or release authority. DebianSandboxAdapter provides consent-expiry-checked query_version/query_health/read_status over a disposable Debian rootfs. Each read reopens the pinned root inode, walks every child through directory descriptors with O_NOFOLLOW, and rechecks directory/file identity before and after reading. Regular single-link files must stay on the root device; version/unit/status reads are bounded to 16 KiB/64 KiB/2 MiB. A separate disposable counter-service target now exercises actual unprivileged child readiness, SQLite commit/restart and lost-acknowledgement reconciliation; it grants no live-service authority. The read-only Debian adapter identity and receipt state are process-local; unit-file presence is metadata, not running-service health, and no shell/service-manager/network calls occur.
- **Source tests:** canonical v2 coverage is [test_control_engineering_v2.py](../../../tools/hepta-engineering-control/test_control_engineering_v2.py), [test_assignment_lifecycle.py](../../../tools/hepta-engineering-control/test_assignment_lifecycle.py), [test_evidence_public_key.py](../../../tools/hepta-engineering-control/test_evidence_public_key.py), [test_candidate_sandbox_hardening.py](../../../tools/hepta-engineering-control/test_candidate_sandbox_hardening.py), [test_lane_g_seal.py](../../../tools/hepta-engineering-control/test_lane_g_seal.py), and [test_assimilation_sandbox.py](../../../tools/hepta-engineering-control/test_assimilation_sandbox.py). Legacy `test_hepta_engineering_control.py` / `test_integration_identity.py` remain compatibility regressions only. Named fixture regressions `test_parent_symlink_swap_during_open_never_reads_outside_root` and `test_parent_directory_replacement_during_read_discards_observation` cover parent-path escape and drift rejection. These are test identities, not execution receipts for this documentation revision.
...
- **Remaining work:** Provision selected-host repository/CI/reviewer/worker/audit-witness public keys and external signing custody; retain signed audit anchors outside the engineering database; and retain fresh exact source/merge qualification observations plus independent decisions. Replicated failover, production canary/merge authority and deployment remain external. Source-composed scheduling/worker control still does not itself merge, deploy, release or complete autonomous development. The fixture adapter does not discover or control live Debian services. Its supplied consent/identity still requires a trusted owner; independently witnessed target parity, live service observation and authorized production deployment remain outside this read-only fixture implementation.
