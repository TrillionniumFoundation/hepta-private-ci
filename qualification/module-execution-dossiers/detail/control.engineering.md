...
- **Repository product caller:** `.github/workflows/hepta-consolidated-source.yml:engineering-product-gate` is the live repository/CI caller. It runs only after strong Linux/Bubblewrap sandbox qualification and the repository qualification matrix, exercises the SQLite v5 scheduler, binds exact source/synthetic-merge identities and retains a machine-readable receipt. The caller has no runtime, merge, activation, promotion or release authority.
- **Remaining work:** Bind an independently controlled reviewer identity and key custody to the emitted candidate evidence, complete authorized target-host handoff, and execute/observe an approved production deployment. Scheduling/integration eligibility does not itself merge, deploy, release or complete autonomous development. The fixture adapter does not discover or control live Debian services. Its supplied consent/identity still requires a trusted owner; independently witnessed target parity, live service observation and authorized deployment remain external gates.
