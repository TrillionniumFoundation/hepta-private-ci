...
- **Capacity and retry:** the canonical facade and CLI route every strong candidate or candidate bundle through one host-wide `HostSandboxLimiter`, enforcing at most eight cross-process sandboxes on one host. `execute_with_infrastructure_retries` permits at most two retries and only for enumerated infrastructure failures; semantic failures never retry unchanged. Portable fixture execution stays single-attempt and cannot produce `sandbox_tested`. `evaluate_mutation_probes` rejects generated-test evidence when any declared mutant survives.
- **Distributed boundary:** SQLite remains a single-host owner store. Multi-host worker writes require a fresh signed `DistributedWriteGrant` with leader epoch, fencing token, source identity and exact path scope. The highest observed frontier is durably stored per worker and stale or conflicting replay fails after restart. The external coordinator still owns consensus/leader selection; this package does not pretend SQLite is an HA consensus service.
- **Audit and key custody:** the local hash chain exports an audit head together with a deterministic digest of all authoritative owner tables. An external audit-anchor identity signs that pair and current-state verification rejects a cryptographically valid receipt whose state digest no longer matches. Production key custody binds subject signing identity, algorithm, public-key digest and hardware-attestation digest in a fresh non-exportable hardware-backed `KeyCustodyReceipt`; `CustodiedSignatureProvider` is the production port and `HmacTrustStore` remains reference/test-only.
