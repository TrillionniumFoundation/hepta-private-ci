...
Own only the canonical runtime-health observation surface, bounded lifecycle control state and a crash-recovery projection of run identity/revision/phase. The recovery file stores immutable snapshot references, cancellation metadata and the bounded cancellation-ack deadline, not authoritative objective, memory, prompt, artifact, Codex turn or provider bytes. It is bounded to 4 MiB. Each mutation is applied to a candidate coordinator, synced and atomically replaced, and only then published in memory; a failed durable write therefore cannot create an unrecoverable in-memory success. A newer generation cancels retained pre-dispatch work; dispatched/cancelling work without terminal owner observation restores as `indeterminate` and is never redispatched.
