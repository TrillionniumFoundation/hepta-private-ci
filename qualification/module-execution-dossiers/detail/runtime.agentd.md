...
- **Implemented entrypoints:** `compose_runtime` in [codex-rs/hepta-agentd/src/lane_b_runtime.rs](../../../codex-rs/hepta-agentd/src/lane_b_runtime.rs); `AgentdProductionWriterHost` in [codex-rs/hepta-agentd/src/production_writer_host.rs](../../../codex-rs/hepta-agentd/src/production_writer_host.rs); `read` in [codex-rs/hepta-agentd/src/cognitive_context.rs](../../../codex-rs/hepta-agentd/src/cognitive_context.rs); and the named `AgentdPromptPipelineOwner` in [codex-rs/hepta-agentd/src/prompt_runtime.rs](../../../codex-rs/hepta-agentd/src/prompt_runtime.rs). The prompt facade owns no alternate optimizer/model loop: it opens the durable prompt registry and runtime reconciliation state, derives the authenticated candidate source from that registry, validates canonical optimizer exercise through `compile_prompt_registry_v2`, and stages exact realization bytes into the existing App Server `PromptRuntimeHost`.
- **State and recovery:** lane_b_runtime keeps a bounded in-memory run/revision map. Durable cognitive facts stay in hepta-memory SQLite. Prompt factor/realization facts remain owned by prompt.registry, while Agentd owns a bounded durable runtime projection for staged attachments, exact pre-send provider dispatch claims and terminal reconciliation. That projection writes fsync + atomic rename + directory fsync, poisons on post-rename acknowledgement uncertainty, reopens semantic state, preserves missing/indeterminate terminal uncertainty and blocks blind retry until a monotonic terminal reconciliation. AgentdProductionWriterHost still requires an externally verified lease and explicit target attachment and is not enabled automatically by startup.
- **Source tests:** [codex-rs/hepta-agentd/src/lane_b_runtime_tests.rs](../../../codex-rs/hepta-agentd/src/lane_b_runtime_tests.rs), [codex-rs/hepta-agentd/src/cognitive_context_tests.rs](../../../codex-rs/hepta-agentd/src/cognitive_context_tests.rs), and [codex-rs/hepta-agentd/src/prompt_runtime_tests.rs](../../../codex-rs/hepta-agentd/src/prompt_runtime_tests.rs). Prompt tests cover named durable registry → canonical optimizer → context compiler → App Server host composition, crash/reopen without terminal, acknowledgement loss, indeterminate reconciliation and retry history. These are test identities, not execution receipts for this documentation revision.
- **Implementation and operating references:** [docs/readiness/LANE_B_NATIVE_HOST.md](../../../docs/readiness/LANE_B_NATIVE_HOST.md), [codex-rs/hepta-agentd/src/prompt_runtime.rs](../../../codex-rs/hepta-agentd/src/prompt_runtime.rs), [codex-rs/hepta-agentd/AUTHBUS_TEXT.md](../../../codex-rs/hepta-agentd/AUTHBUS_TEXT.md).
- **Remaining work:** Execute the named prompt pipeline through a real non-test turn ingress, prove deployed socket/generation identity and independently observed real-provider terminal evidence, and measure target-host backpressure/restart/load behavior. Source composition is not product execution or activation.
