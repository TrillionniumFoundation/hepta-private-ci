...
- **State and recovery:** lane_b_runtime keeps a bounded in-memory run/revision map. Durable cognitive facts stay in hepta-memory SQLite. `AgentdProductionWriterBootstrap` can be injected explicitly into `AgentdConfig`; `runtime.rs` then composes `AgentdProductionWriterHost` against the already-open CognitiveStore using the externally verified lease/verifier, enrolled compaction trust and authoritative snapshot provider. The default CLI supplies no bootstrap, mints no authority/trust, and remains read-only. `AgentdState` exposes the host only while Running, App Server ready and unfenced.
- **Source tests:** [codex-rs/hepta-agentd/src/lane_b_runtime_tests.rs](../../../codex-rs/hepta-agentd/src/lane_b_runtime_tests.rs), [codex-rs/hepta-agentd/src/cognitive_context_tests.rs](../../../codex-rs/hepta-agentd/src/cognitive_context_tests.rs), and [codex-rs/hepta-agentd/src/runtime_tests.rs](../../../codex-rs/hepta-agentd/src/runtime_tests.rs) for explicit production-writer bootstrap/readiness gating. These are test identities, not execution receipts for this documentation revision.
