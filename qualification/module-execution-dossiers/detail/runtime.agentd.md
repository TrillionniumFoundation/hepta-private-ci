...
- **Implemented lifecycle entrypoints:** the `AgentRunCoordinator` implementation remains in [codex-rs/hepta-agentd/src/lane_b_runtime.rs](../../../codex-rs/hepta-agentd/src/lane_b_runtime.rs), but it is now owned by live `AgentdState` and routed by [codex-rs/hepta-agentd/src/state_control.rs](../../../codex-rs/hepta-agentd/src/state_control.rs). The shared [Agentd control protocol](../../../codex-rs/hepta-agent-protocol/src/lib.rs) and `AgentdClient` expose typed start, attach, dispatch-boundary, cancel, terminal-observation, status and removal calls. This closes the former library-only/wire-API source gap.
...
- **Other implemented entrypoints:** `AgentdProductionWriterHost` in [codex-rs/hepta-agentd/src/production_writer_host.rs](../../../codex-rs/hepta-agentd/src/production_writer_host.rs); `read` in [codex-rs/hepta-agentd/src/cognitive_context.rs](../../../codex-rs/hepta-agentd/src/cognitive_context.rs). The production writer still requires an externally verified lease and explicit target attachment and is not enabled automatically by startup.
