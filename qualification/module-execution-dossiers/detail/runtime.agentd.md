...
- **Implemented entrypoints:** `compose_runtime` in [codex-rs/hepta-agentd/src/lane_b_runtime.rs](../../../codex-rs/hepta-agentd/src/lane_b_runtime.rs); `AgentdIntelligenceCaller::admit` / `mark_dispatched` in [codex-rs/hepta-agentd/src/intelligence_facade.rs](../../../codex-rs/hepta-agentd/src/intelligence_facade.rs); `AgentdProductionWriterHost` in [codex-rs/hepta-agentd/src/production_writer_host.rs](../../../codex-rs/hepta-agentd/src/production_writer_host.rs); `read` in [codex-rs/hepta-agentd/src/cognitive_context.rs](../../../codex-rs/hepta-agentd/src/cognitive_context.rs). Native Agentd host, run-binding component, intelligence-envelope consumer and explicit production-writer attachment are implemented as source surfaces.
...
- **Remaining work:** Exercise the new intelligence-envelope consumer from an admitted non-test daemon request, prove deployed socket/generation identity and the full Codex caller path, and measure backpressure/restart on the target host. The source consumer alone does not change production or activation claims.
