...
Own only the canonical runtime-health observation surface, bounded lifecycle control state and a crash-recovery projection of run identity/revision/phase. The recovery file stores immutable snapshot references and cancellation metadata, not authoritative objective, memory, prompt, artifact, Codex turn or provider bytes. It is bounded to 4 MiB and atomically replaced after synced mutation. A newer generation cancels retained pre-dispatch work; dispatched/cancelling work without terminal owner observation restores as `indeterminate` and is never redispatched.

## 4. Deterministic algorithm and scheduling

Bootstrap auth and revocation readers, stores/read ports, execution adapters and intelligence composition in explicit order. Freeze request/objective/body/artifact/authority/deadline identity before admission; every context attachment must repeat that complete tuple. The generation monitor enforces elapsed deadlines. Local drain rejects new admission, cancels only work that provably has not crossed dispatch, waits for execution-owner terminal observations, and persists remaining externally uncertain work as `indeterminate` before shutdown. A recorded post-dispatch `Cancelling` phase is intent only until the Codex adapter sends the actual interrupt and observes matching terminal acknowledgement.

## 5. Capacity and performance profile

Pilot <= 256 active runs and <= 1024 retained lifecycle records; the lifecycle recovery projection is <= 4 MiB. The control server has 32 normal concurrent connection slots plus four bounded overload-response slots. Each attachment remains bounded by the control-frame/context profile. Per-run deadlines are enforced by the daemon monitor; cancellation acknowledgement deadlines become meaningful only after the remaining physical interrupt binding is composed. Track queue age, overload responses, drain duration and dependency/readiness latency.
...
- **Daemon composition:** [codex-rs/hepta-agentd/src/state.rs](../../../codex-rs/hepta-agentd/src/state.rs) owns the coordinator and bounded recovery projection; [codex-rs/hepta-agentd/src/state_control.rs](../../../codex-rs/hepta-agentd/src/state_control.rs) routes the typed wire methods. The daemon advertises `run.lifecycle` v1 and `control.typed_backpressure` v1.
...
- **Deadline and cancellation:** the generation monitor sweeps elapsed deadlines. Pre-dispatch expiry is terminal local cancellation. Post-dispatch expiry records cancellation intent but does not claim a physical interrupt.
...
- **Remaining repository work:** bind the normal non-test Codex turn path to lifecycle start/attach/dispatch/terminal observation, and bind `RunCancel`/deadline cancellation intent to the actual App Server interrupt plus matching terminal acknowledgement. Until those callsites exist, `productCallerState` remains `not_composed`.
