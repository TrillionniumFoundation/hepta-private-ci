...
Status: native Agentd host, daemon-owned run lifecycle control, durable run-start projection, explicit indeterminate recovery seam and explicit production-writer attachment implemented; current-trust revalidation before durable projection, physical runtime.codex execution/terminal observation, target-host restart/backpressure evidence and independent acceptance remain open as listed in section 8. Common requirements: `../EXECUTION_SEMANTICS.md` and `../TECHNICAL.md`. Canonical ownership and package predecessors are unchanged.
...
- **State and recovery:** Agentd owns one bounded in-memory run/revision coordinator per process. Its active-run ceiling is bound to the Fleet `ResourceBudget.max_concurrent_turns`; closed records remain retained until an exact-revision `RunReleaseClosed` acknowledgement releases them. Admission, complete-tuple attachment, dispatch boundary, reasoned cancellation, continuous deadline transitions, the 3-second post-dispatch cancellation-ack timeout, terminal observation, exact-revision closed-record release and explicit recovery are generation-fenced through the daemon control path. On shutdown, admission closes before task teardown and the control socket remains available during the bounded drain/reconciliation window. An external durable owner may reconstruct an exact prior operation only as `Indeterminate`; Agentd does not persist or fabricate the external effect result and recovery cannot redispatch. Durable cognitive facts stay in hepta-memory SQLite; `AgentdProductionWriterHost` still requires an externally verified lease and explicit target attachment and is not enabled automatically by startup.
- **Control saturation:** the bounded UDS server returns a small typed overload frame with retry guidance instead of silently dropping a saturated connection. `run.lifecycle/1.1` also exposes the pending cancellation-ack deadline in receipts so clients can distinguish active cancellation from indeterminate recovery. This is backpressure signaling, not target-host capacity qualification.
...
- **Durable run-start projection:** `AgentRunCoordinator::start_revalidated_run_start` accepts only a product-revalidated `RunStartRecordV1` and projects the canonical durable tuple into the sole daemon coordinator: `admitted_source_digest` is the runtime request identity; runtime body, artifact set, authority epoch, generation, fence and deadline are exact durable fields. `ExplicitAbstain` never enters runtime admission. The bridge deliberately does not authenticate a raw retained journal record; current AuthBus/trust revalidation is a mandatory caller precondition.
- **Remaining repository work:** compose the current-trust/AuthBus revalidation immediately before `start_revalidated_run_start`; forward-port the canonical intelligence V3 producer into ordinary product admission; then bind the existing runtime.codex App Server turn start/interrupt and terminal observer without creating a second execution spine. Post-dispatch restart recovery remains `Indeterminate` and non-redispatchable.
