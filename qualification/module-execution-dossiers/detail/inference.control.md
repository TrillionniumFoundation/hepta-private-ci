...
- **Implemented entrypoints:** `reserve_native`, `dispatch_native`, `cancel_native`, `settle_native` and `redact_native_output_history` in [codex-rs/hepta-infer-core/src/native_control.rs](../../../codex-rs/hepta-infer-core/src/native_control.rs), consumed by the native App Server host. `NativeLocalSlotAdmission` deliberately names the shipped reservation subset; it is not the target economic/resource `InferenceReservation`.
- **Final-use authority:** immediately before `turn/start`, the host builds an exact `FinalUseBinding` over request/Agent generation/model/provider, request payload digest and resolved context digest. `FinalUseAuthority::claim` verifies signature/expiry/epoch/revocation/nonce and `with_verified_use` rechecks the live frontier while the durable dispatch intent is synced. The journal stores a non-authoritative `NativeFinalUseWitness`; runtime `ObservedReady` health no longer substitutes for effect authority.
