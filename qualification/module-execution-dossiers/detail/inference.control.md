...
- **Remaining repository work:** compose a named non-CLI product caller that obtains current policy/grant inputs from their owners. The scheduler/control/worker source remains inactive until that product composition is explicit; a source-complete CLI or planner cannot self-activate production.
