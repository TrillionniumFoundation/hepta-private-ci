...
- **State and recovery:** DurableInferenceControl owns one synced journal with a stable sidecar writer fence held only for replay and individual mutations. Concurrent handles refresh under that short fence, so provider/model execution does not serialize the whole budget domain. A reservation is a local in-flight slot; unknown execution retains capacity, unknown token usage remains None, and replay never dispatches a replacement turn. Active-journal compaction preserves current legacy/native state as checkpoints while storing the complete predecessor byte stream in a content-addressed sibling archive; indeterminate records retain their slots across compaction/reopen.
...
- **Remaining work:** Integrate economic quota/hardware capacity authorities and authenticated post-crash provider reconciliation. Active-journal capacity recovery is implemented through content-addressed archive + checkpoint compaction; archive retention/transfer lifecycle and long-horizon target-host storage measurement remain operational qualification work.
