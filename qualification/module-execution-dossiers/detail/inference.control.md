...
- **Implemented entrypoints:** `reserve_native`, `dispatch_native`, `cancel_native`, lower-level `settle_native` and product `settle_native_receipt_only` in [codex-rs/hepta-infer-core/src/native_control.rs](../../../codex-rs/hepta-infer-core/src/native_control.rs); deterministic feasible multi-worker `schedule` in [codex-rs/hepta-inferd/src/lib.rs](../../../codex-rs/hepta-inferd/src/lib.rs); `DurableInferenceControl::compact_native_journal` in [codex-rs/hepta-infer-core/src/durable_control.rs](../../../codex-rs/hepta-infer-core/src/durable_control.rs) folds native history into exact snapshots while retaining a bounded private content-addressed predecessor archive set. Archive-list/read/remove failure poisons the writer and fails closed; successful pruning is directory-synced. Product provider observations are persisted receipt-only (`output_sha256` without model text), redaction is monotonic, and compaction migrates retained pre-upgrade native output into the same digest-only canonical snapshot. Only bounded predecessor archives/external backups can retain historical pre-upgrade text.
...
- **Scheduling and physical dispatch authority:** `hepta-inferd::schedule` canonicalizes at most 256 candidate workers, rejects duplicate/invalid candidates, filters by exact model/token/concurrency feasibility, and deterministically ranks by explicit preference, remaining token capacity and stable worker ID. The complete eligible snapshot and selected generation are digest-bound; its resulting plan remains `DENY_ALL`. [codex-rs/hepta-infer-worker-host/src/native_policy.rs](../../../codex-rs/hepta-infer-worker-host/src/native_policy.rs) constructs final-use binding from the exact frozen request/provider/model/context/serialized `TurnStartParams` and current authority epoch. The one-shot `VerifiedUseToken` is consumed under the revocation mutex; under that same fence the owner syncs the durable dispatch witness and synchronously admits the exact request into the bounded App Server transport queue. That queue admission is the local effect linearization point. Live terminal success uses only the returned revalidation-only `VerifiedUseReceipt`, so terminal revalidation cannot authorize a second effect.
- **State and recovery:** `DurableInferenceControl` remains the single locked/synced writer. Reopened possibly-admitted runs never issue another turn. The worker uses App Server `thread/queue/reconcile` in `ReconcileOnly` mode with the exact journaled client-message and payload identity: `Persisted` recovers the original turn, `Missing`/`Cancelled` records exact `NoAdmission`, and ambiguous/unavailable reconciliation remains indeterminate. A recovered original turn may then be inspected for matching terminal facts. Because the post-admission `VerifiedUseReceipt` is intentionally non-serializable, restart can establish terminal effect truth and release safe holds but cannot recreate `VerifiedAtTerminal` or retroactively report authorized success.
- **Source choke point:** [scripts/hepta-inference-control-boundary.py](../../../scripts/hepta-inference-control-boundary.py) verifies the single Hepta production Rust `TurnStart` construction site and source ordering `claim_turn -> revocation fence -> dispatch_native -> synchronous try_request queue admission`; it rejects the old unfenced direct `request_typed<TurnStartResponse>` path. [hepta-lane-b-truth.yml](../../../.github/workflows/hepta-lane-b-truth.yml) is triggered by inference owner/worker, `kernel.authority` final-use and App Server transport-admission primitive changes and runs their libraries plus the boundary guard.
...
- **External evidence still required:** activate the named worker under the deployed Supervisor/Agentd composition; real provider billing reconciliation, measured physical device-capacity authority, deployed signer/revocation behavior, target crash/cancellation/reconciliation/retention qualification (including predecessor archive/backup secure deletion) and independent acceptance. Source budget units and resource advertisements do not by themselves prove provider charges, VRAM/device availability or production activation.
