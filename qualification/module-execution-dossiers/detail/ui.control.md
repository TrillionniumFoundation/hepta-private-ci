...
- **Browser source shell:** `ControlPlaneApp` in `src/browser-app.js`, with
  ...
  rendering.
- **State/recovery:** authenticated session, current/prior coherent snapshot, maximum 1024 unresolved identities, optional bounded durable identity mirror with no request payload, immutable in-flight provenance, a dispatch-before-reconcile fence, exponential reconciliation backoff and explicit `recoveryRequired` after the automatic ceiling or implausible restored clock state. Snapshot/module protocol data is consumed only from own data properties. Transport methods are snapshotted once from data-method descriptors (including class prototypes) and accessor-shaped methods are rejected without invoking getters.
- **Browser/deployment source boundary:** `SameOriginHttpTransport`, authenticated bootstrap, principal/endpoint/protocol-bound durable pending domain plus a separate live manifest/basePath binding, accessible native confirmation, content-hashed/SRI static artifact and generated security-header policy. POST bodies and streamed responses are bounded before network/whole-body materialization. The host immediately blocks mutations and pauses automatic reconciliation offline, then re-establishes observation sessions after online/session-expiry/BFCache resume without replaying mutations; hot live-binding drift fails closed instead of switching clients over the same durable store; candidate browser writer leases are released when startup or persistence-domain installation fails and are committed only after the replacement runtime is installed.
