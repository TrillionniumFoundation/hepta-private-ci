...
Status: parameter V2 remains compatibility-stable; governed V3 candidate generation now enforces an authority-free typed parameter mutation policy, authenticated parameter/topology product adapters are composed through Agentd source host callsites, and both parameter and governed topology registries use external-anchor/fence recovery. Typed writer-handoff validation and the bounded structural-canary controller are implemented in source. Topology application, target-host execution evidence, activation and independent/operator acceptance remain external gates. Common requirements: `../EXECUTION_SEMANTICS.md` and `../TECHNICAL.md`. Canonical ownership and package predecessors are unchanged.
...
Topology V2 starts with no-change and one candidate per typed operation. Add/remove/replace/split/merge/rewire/retire changes carry migration, rollback, writer-handoff and evidence digests. `admit_governed_topology_v1` requires one exact typed writer-handoff plan per update, distinct source/destination owners, an advancing writer fence and matching migration/rollback bindings. Authenticated Generator/Observer/Evaluator roles are verified before durable append. `StructuralCanaryControllerV1` observes bounded canary evidence but has no apply path. Application remains outside this module.
...
The product-workspace parameter and topology adapters live in `codex-rs/hepta-intelligence`. `codex-rs/hepta-agentd` now contains the source host callsites: they recompute current `ArtifactRegistry` and durable learning-ledger frontiers and own independent anchor/fence seams before returning success. These source callsites are not evidence that a deployed target host has executed or accepted the path. Rollback of selected model/topology state remains an authorized transition outside this proposal engine.
...
- **Remaining work:** obtain final exact-head/synthetic-merge qualification, independent semantic/security review, target-host execution evidence, operator recovery evidence and a real bounded structural canary. Topology application remains intentionally outside the proposal engine and requires a separately accepted activation/writer-handoff path. The parameter path still does not train or install weights.
