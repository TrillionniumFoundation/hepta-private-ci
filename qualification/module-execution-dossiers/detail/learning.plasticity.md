...
Parameter V3 starts with no-change and evaluates every declared positive scale over explicit parameter signals. A `MutationGrammarManifestV1` is mandatory: each signal must match the exact artifact/window-bound parameter allowlist and layer mapping, stay within grammar bounds and be marked `LearnableParameter`; authority, evaluator, deletion, runtime-topology and credential surfaces fail closed. Each signal maps eligibility, modulator and learning rate to one parameter, uses checked Q32 arithmetic, clamps to explicit bounds, rejects unknown norm layers and duplicate parameter identities, applies the canonical per-layer/global trust regions, removes duplicate semantic candidates, and derives update candidate IDs from the selected artifact, exact window and canonical content. The source therefore proves generator-relative completeness for the declared bounded V3 profile rather than accepting an arbitrary caller candidate list as complete.
