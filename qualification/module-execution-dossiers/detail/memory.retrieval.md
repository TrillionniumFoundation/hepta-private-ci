...
The channel vocabulary is lexical, vector, entity, graph, temporal, causal, procedural and contradiction support. A positive-weight policy channel must have exactly one corresponding generator-owner batch. Missing, unexpected, or explicitly `Unavailable` required owner batches fail closed. The current durable SQLite owner supplies lexical, entity, generic graph, temporal and three typed KG channels: causal (`hepta.relation.causes.v1`), procedural (`hepta.relation.procedure-step.v1`) and contradiction support (`hepta.relation.contradicts.v1`). Generic graph retrieval excludes those reserved typed edges. Vector remains disabled until a real current encoder/vector-index owner supplies an authenticated batch.
...
The SQLite owner currently observes seven channels with at most 32 rows per channel before deduplication, for at most 224 raw owner events before union. A channel reports `Exhausted` or `LimitReached`; generator-relative incompleteness is preserved rather than rewritten as complete recall.
...
- **Owner composition:** [codex-rs/hepta-memory/src/cognitive_retrieval_adapter.rs](../../../codex-rs/hepta-memory/src/cognitive_retrieval_adapter.rs) converts one SQLite owner observation into canonical generator batches without accepting caller-authored source scores. The SQLite projection now owns exact typed causal/procedural/contradiction-support relation channels separately from generic graph evidence; Vector is intentionally absent pending a real encoder/index owner. [codex-rs/hepta-agentd/src/cognitive_context.rs](../../../codex-rs/hepta-agentd/src/cognitive_context.rs) is the explicit product-host candidate and ranks before legacy top-four truncation.
...
- **Qualification path:** `.github/workflows/hepta-consolidated-source.yml` runs exact source and synthetic/base-merge package tests, formatting and strict all-target Clippy for the relevant packages. Lane E/converged-learning gates additionally exercise the durable causal owner. `.github/workflows/hepta-memory-retrieval-qualification-host.yml` captures the three release-mode capacity probes plus exact host/source and raw GNU `time` output as qualification-host evidence; GitHub-hosted measurements do not satisfy the separately required approved target-host receipt.
- **Remaining repository-controlled work:** obtain green exact-head and synthetic-merge execution receipts for this candidate, obtain a green qualification-host probe artifact, and fix any surfaced regressions. Keep the implementation map at `productionImplementation=false`, `productExecutionProved=false`, `activation=false` and `release=false` until those claims have their required evidence.
- **Remaining external/owner work:** supply and qualify an actual current encoder/vector-index generator before enabling Vector; independently observe downstream learned-ranker propensities/outcomes; run named target-host p50/p95/p99, throughput, CPU/RSS/allocation and large-store measurements; complete independent semantic acceptance, canary, promotion and release.

The additive `adapt_generation_bound_recall_to_canonical_shadow_v1` adapter remains a shadow-only compatibility bridge. It requires exact legacy cue/union/generation and record ID/revision/digest bindings to canonical events; it grants no writer, model-call, selection or release authority.
