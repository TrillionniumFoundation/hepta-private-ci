...
Current product precedence is narrower and explicit: `hepta-memory::CognitiveStore::observe_memory_retrieval` is the only physical candidate generator; `memory.retrieval::rank_owner_candidates` validates, binds and deterministically orders its bounded aggregate RRF observation; the optional `PinnedCognitiveRanker` may only permute those admitted results; the selected attachment set is source/citation/KG revalidated again before context publication. The native App Server worker then re-reads the same cognitive-context query immediately before `turn/start` and requires the ordered selected item identities/revisions/content/hash values and read/abstain decision to be unchanged. This is a fail-closed pre-dispatch currentness check, not an atomic lease spanning two processes. The SQLite GraphOneHop path now preserves canonical persisted KG relation semantics (`temporal_*`, `causes`/`enables`, `procedure_step`, `contradicts`) as owner-observed evidence channels bound into `OwnerRankReceiptV1`; those tags do not fabricate normalized per-channel scores, calibrated OOD values or HNMF activations. Generation-bound recall therefore remains a separate native capability until production vector/calibration and engram/synapse projections exist. Generation-bound recall first forms the score-eligible, result-bounded delivery set; OOD/contradiction/channel-coverage abstention is evaluated only over that set. Below-floor and lower-ranked tail entries remain bound by the canonical union digest but cannot be returned or poison an unrelated deliverable set.
...
- RET-03: revoked/stale source after ranking cannot be attached to a model request. The Agentd test `post_ranking_withdrawal_fails_closed_before_context_delivery` exercises the ranking-to-context-publication race seam, and the worker test `predispatch_context_check_binds_exact_selected_memories` binds the final selected attachment immediately before `turn/start`.
- RET-04: no-intervention, lexical-only, no-recurrence and no-inhibition baselines measure independent utility and resource cost. The source test `lexical_only_ablation_has_distinct_policy_union_and_packet_identity` establishes distinct receipt identity for lexical-only versus multi-channel execution; no-recurrence/no-inhibition HNMF baselines and longitudinal utility/resource outcomes remain shadow/qualification evidence.
...
- **State and recovery:** Ranking is stateless. The existing `hepta-memory` SQLite store remains the physical content/index owner and generates a bounded observation in one read transaction. `OwnerRankReceiptV1` binds the full admitted observation-relative candidate set, aggregate owner score, owner observation/support digest and canonical evidence-channel tags derived from the actual owner query path. Agentd resolves content and revalidates the exact selected bindings before publication.
- **Source tests:** [codex-rs/hepta-memory-retrieval/src/owner_rank_tests.rs](../../../codex-rs/hepta-memory-retrieval/src/owner_rank_tests.rs), [codex-rs/hepta-memory-retrieval/src/v2_tests.rs](../../../codex-rs/hepta-memory-retrieval/src/v2_tests.rs), [codex-rs/hepta-memory-retrieval/src/generation_bound_tests.rs](../../../codex-rs/hepta-memory-retrieval/src/generation_bound_tests.rs), [codex-rs/hepta-memory/src/cognitive_retrieval_observation_tests.rs](../../../codex-rs/hepta-memory/src/cognitive_retrieval_observation_tests.rs), [codex-rs/hepta-agentd/src/cognitive_context_tests.rs](../../../codex-rs/hepta-agentd/src/cognitive_context_tests.rs), [codex-rs/hepta-agentd/src/cognitive_context_budget_tests.rs](../../../codex-rs/hepta-agentd/src/cognitive_context_budget_tests.rs), [codex-rs/hepta-infer-worker-host/src/native_app_server_tests.rs](../../../codex-rs/hepta-infer-worker-host/src/native_app_server_tests.rs). These are test identities, not execution receipts for this documentation revision.
...
- **Remaining work:** The real SQLite owner/generator and ranking-to-delivery revalidation are product-composed for the existing MemoryFts/EntityFts/GraphOneHop/Recency RRF path, and canonical persisted KG temporal/causal/procedural/contradiction relations now survive the owner observation boundary as bound evidence metadata. A production vector generator, calibrated per-channel scores/OOD, engram/synapse projection and four-step HNMF settling are still not product implementations. RET-04 no-recurrence/no-inhibition and longitudinal outcome evidence, target-host latency/resource measurements and independent acceptance remain open.
