...
- **Physical provider seam and learning handoff:** [codex-api/src/encoded_body_observer.rs](../../../codex-rs/codex-api/src/encoded_body_observer.rs), [codex-api/src/endpoint/responses.rs](../../../codex-rs/codex-api/src/endpoint/responses.rs) and [core/src/client.rs](../../../codex-rs/core/src/client.rs) expose the exact canonical encoded Responses JSON immediately before the HTTP transport boundary and explicit terminal Completed/Rejected/Indeterminate/Abandoned outcomes. [hepta-codex-adapter/src/lib.rs](../../../codex-rs/hepta-codex-adapter/src/lib.rs) validates exact submitted-byte/delivery semantics, while [hepta-learning-ledger/src/ledger.rs](../../../codex-rs/hepta-learning-ledger/src/ledger.rs) admits prompt-delivery evidence into the durable owner chain and rejects policy self-observation. The physical seam defaults to disabled and therefore does not fabricate product composition, provider execution, outcome or credit.
...
- **Remaining repository-controlled work:** carry an owner-authenticated prompt-delivery binding through the real Agentd/App Server `turn/start` admission path and install the existing Core `EncodedRequestBodyObserver` on the exact `ModelClientSession`; ordinary `responsesapi_client_metadata` is not authority. Cover the Agentd → App Server → Core → Responses HTTP callsite with exact-byte and terminal-state product tests, including retry/crash reconciliation. Until then `production_implementation=false`, `productExecutionProved=false` and the implementation-map `productCallerState=source_composed_pre_dispatch` remain truthful.
