...
- **State and recovery:** open_state_dir owns locked private Unix state; claim verifies Ed25519, syncs nonce consumption, snapshots the exact durable authority epoch/revocation frontier and binds that frontier into the token witness before dispatch. with_verified_use and VerifiedUseToken::enter recheck live time/epoch/revocation and require the claim-time frontier to remain exact-current before effect entry. Any frontier advance invalidates the outstanding token; uncertain persistence fences the handle.
