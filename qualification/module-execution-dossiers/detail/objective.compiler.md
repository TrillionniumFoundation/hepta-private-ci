...
The compiler owns no durable store. Its pure output binds request, principal, source, schema, selected profile, hard constraints, legal actions, success and terminal predicates, evidence requirements, resource/risk policy and native semantic digest. `encode_objective_function_v1` separately materializes the registered canonical JSON contract and protocol-wire digest. The Agentd product-source path publishes both identities through the destination-owned durable run-start v2 journal together with ingress authentication and `RunStartSnapshotV1`. The journal syncs each framed append before in-memory publication, uses exact run identity for idempotent replay, rejects native or protocol semantic reuse as conflict and supports bounded recovery; legacy v1 records are read-compatible but fail closed at Agentd final use. The deployed host and its external anti-rollback/backup policy remain activation concerns.
