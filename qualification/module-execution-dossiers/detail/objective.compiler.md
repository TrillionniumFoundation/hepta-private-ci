...
- **Repository-controlled blockers before a stronger claim:** align the canonical readiness protocol registry with enforced V1 source/aggregate bounds (or version the source protocol); define a source grammar for enum-set/action-implication payloads before claiming those rich domains; implement exact canonical `ObjectiveFunctionV1` projection; bind a named authenticated production caller/owner store; atomically persist/reconcile the objective plus run snapshot; replace manual profile byte estimation if 256 KiB is protocol-hard; obtain exact-head and synthetic-merge evidence.
