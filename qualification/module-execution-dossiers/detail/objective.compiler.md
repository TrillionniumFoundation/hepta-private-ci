...
Latency claims require a named host, compiler, build profile, input class and exact source. The product-composition workflow records a separate 32-sample authenticated-admission + compile + durable-fsync p50/p95/p99 receipt on its named Ubuntu runner. This is qualification telemetry only and does not satisfy a selected production-host SLA. A normal successful compile measurement cannot be reused as a conflict-extraction measurement.
...
- **State and recovery:** Stateless outputs bind the immutable source/principal/profile/schema/unit/time/intent tuple; unknown mappings fail closed. `prepare_intelligence_run_v1` now supplies the bounded caller candidate and atomically appends `RunStartPublicationV1` to the owner `learning.ledger`; anchored reopen replays the same objective/snapshot frame. The native compiler still has no durable objective database.
