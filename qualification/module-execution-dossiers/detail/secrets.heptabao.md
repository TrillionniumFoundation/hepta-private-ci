...
Status: authorized exact-version KV v2 HTTPS read plus bounded source-level dynamic secret-lease issuance, renewal, revocation, known-lease lookup and a private durable SQLite lease registry are implemented. Provider support for reconciling an issuance whose response is lost, exact-pin service qualification, product composition and independent acceptance remain in section 8. Common requirements: `../EXECUTION_SEMANTICS.md` and `../TECHNICAL.md`. Canonical ownership and package predecessors are unchanged.
