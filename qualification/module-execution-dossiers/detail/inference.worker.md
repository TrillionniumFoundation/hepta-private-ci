...
- **Repository-controlled source closure:** `hepta-local-inference-runtime-host` is the named long-lived local product root around `LocalWorkerHost`, `InferenceWorker<LocalProcessDriver>` and `FinalUseResourceGrantVerifier`. The host shares live `FinalUseAuthority`, accepts concurrent monotonic revocation updates, revalidates during runs, cancels/fences on authority loss and unloads resident models. The local runtime cancel protocol requires an exact terminal acknowledgement before classifying cancellation; acknowledgement loss/deadline kills and waits for the child and remains indeterminate. `DurableInferenceControl::reconcile_native_usage` is the trusted exact-turn settlement seam for independently authenticated usage evidence and is exact/idempotent/durable. No repository-controlled source boundary gap is currently claimed; deployed issuers/feeds, real billing/device evidence and target-host acceptance remain external.
