...
Status: actual App Server driver with quota/resource/final-use-bound durable admission, no-replay read-only crash reconciliation and a separate local-model driver interface implemented; remaining physical local-model capabilities and independent acceptance are listed in section 8. Common requirements: `../EXECUTION_SEMANTICS.md` and `../TECHNICAL.md`. Canonical ownership and package predecessors are unchanged.
...
- **Implemented entrypoints:** `run` in [codex-rs/hepta-infer-worker-host/src/native_run_control.rs](../../../codex-rs/hepta-infer-worker-host/src/native_run_control.rs); policy/final-use binding in [codex-rs/hepta-infer-worker-host/src/native_policy.rs](../../../codex-rs/hepta-infer-worker-host/src/native_policy.rs); App Server execution/reconciliation in [codex-rs/hepta-infer-worker-host/src/native_app_server.rs](../../../codex-rs/hepta-infer-worker-host/src/native_app_server.rs); `load_model` in [codex-rs/hepta-infer-worker-host/src/model_worker.rs](../../../codex-rs/hepta-infer-worker-host/src/model_worker.rs).
- **State and recovery:** the native run reserves through inference.control, consumes an exact signed final-use grant, persists dispatch before `turn/start`, and commits matching observations. App Server threads are persisted specifically for crash recovery. Reopened uncertain runs first perform read-only `thread/read` and may settle an exact `Completed`/`Failed` turn; they never call `turn/start` or provider replay. The non-serializable final-use token is not reconstructed after a crash, so reconciled terminality cannot become retroactively authorized success.
- **Source tests:** [native_policy_tests.rs](../../../codex-rs/hepta-infer-worker-host/src/native_policy_tests.rs), [native_run_control_tests.rs](../../../codex-rs/hepta-infer-worker-host/src/native_run_control_tests.rs), [native_app_server_tests.rs](../../../codex-rs/hepta-infer-worker-host/src/native_app_server_tests.rs), [model_worker_tests.rs](../../../codex-rs/hepta-infer-worker-host/src/model_worker_tests.rs). These are test identities, not execution receipts for this documentation revision.
...
- **Remaining work:** implement/prove the local weights/device/memory driver before claiming isolated local inference. Real provider crash/reconciliation behavior, physical device capacity and independent deployment remain external qualification gates.
