...
The current Rust realization deliberately classifies three public read surfaces. Legacy `read` is a V1 caller-supplied projection retained for source/shadow compatibility; `read_v2` is the typed lower-level projection over caller-supplied immutable snapshot bytes. Neither proves host-current source authority. The current `hepta-intelligence::run_read_only_vertical` consumer is explicitly not product-composed and remains in the legacy compatibility class. Product callers use `read_authoritative`, which acquires the snapshot from an `AuthoritativeCognitiveSnapshotProvider`, binds scope/purpose, authority epoch, owner frontiers, host generation/profile identities, lease and receipt digests, and returns an `AuthoritativeReadGuardV1`. That guard must reacquire the current provider immediately before downstream use and rejects provider, vector or snapshot drift.
...
- READ-05: neither legacy `read` nor `read_v2` caller-supplied projection can be mistaken for the product authoritative guarantee; product code calls `read_authoritative`.
...
The production read adapter is `codex-rs/hepta-agentd/src/cognitive_context.rs`. It composes the existing SQLite owner with `AuthoritativeCognitiveSnapshotProvider`; it does not add a writer. Legacy `read` and `read_v2` remain available for shadow/owner/test compatibility but are documented as lower-level primitives and are not the Agentd product entry path.
...
- **Implemented entrypoints:** legacy `read` in [codex-rs/hepta-cognitive-read/src/lib.rs](../../../codex-rs/hepta-cognitive-read/src/lib.rs); `read_authoritative` in [codex-rs/hepta-cognitive-read/src/authoritative.rs](../../../codex-rs/hepta-cognitive-read/src/authoritative.rs); `read_v2` in [codex-rs/hepta-cognitive-read/src/v2.rs](../../../codex-rs/hepta-cognitive-read/src/v2.rs); `DurableCognitiveSnapshot` in [codex-rs/hepta-memory/src/lane_c_snapshot.rs](../../../codex-rs/hepta-memory/src/lane_c_snapshot.rs); `AgentdAuthoritativeSnapshotProvider` in [codex-rs/hepta-agentd/src/cognitive_context.rs](../../../codex-rs/hepta-agentd/src/cognitive_context.rs).
...
- **State and recovery:** legacy `read` and `read_v2` remain deterministic lower-level caller-supplied projections. `AuthoritativeReadGuardV1` owns the acquisition request, authoritative snapshot receipt and bound result so final-use revalidation is part of the product contract. `DurableCognitiveSnapshot` is an owned immutable single-transaction cut, not a moving adapter view. Native bytes are not an admitted ModulePort/wire protocol.
