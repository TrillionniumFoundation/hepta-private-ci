...
Host supplies a current host-owned trust provider and authorized handles; `ProductionLedgerWriter` reloads the current trust snapshot for every signed mutation, rejects trust revision/epoch rollback or same-revision drift, and then uses `LearningEvidenceVerifierV1` to authenticate the role and exact payload before validating the semantic receipt and current anchor. The native `RootedLearningEvidenceTrustProviderV1` pins an out-of-band Ed25519 root public key and accepts signer/controller distribution only through complete root-signed manifests whose generation/predecessor and authority epoch advance monotonically; the journal prepares, appends and syncs one canonical frame before publishing core state. A separately bound `DurableAnchorWitness` can retain acknowledged anchors outside the journal file. A retry after uncertain I/O must reconcile the original identity, predecessor and digest. Failed anchored recovery cannot retry unanchored. Unknown outcome stays pending or censored, never zero reward. Physical erasure and model unlearning are distinct from logical exclusion.
...
- **Implemented entrypoints:** `DurableLedger` / `SegmentedLedger`; `LearningEvidenceVerifierV1`; `LearningEvidenceTrustProviderV1` / `RootedLearningEvidenceTrustProviderV1`; `ProductionLedgerWriter`; `DurableAnchorWitness`; `LedgerIndexCheckpointV1` / `LedgerRecoveryWorkV1`; and canonical protocol adapters. Anchored file ledger, per-mutation current-trust admission with anti-rollback, durable correction/credit/unlearning events, ledger-derived dataset freeze, independent witness storage, semantic protocol validation and verifiable index/recovery-work receipts are source-implemented.
...
- **Source tests:** durable/recovery, signed-evidence, convergence, production-writer/current-trust, witness, canonical-protocol semantic validation and long-history checkpoint/recovery-work suites under `codex-rs/hepta-learning-ledger/src/*_tests.rs`. The capacity fixture covers the full 8,192-record V1 single-segment profile. These are test identities, not execution receipts for this documentation revision or target-host latency measurements.
...
- **Remaining work:** Bind the current-trust provider, `ProductionLedgerWriter` and `DurableAnchorWitness` to one named production process with an exclusive physical writer and durable directories; operate live pinned-root private-key custody/manifest publication and independent observer processes; qualify filesystem/power-loss behavior and target-host latency/storage growth; and obtain independent semantic acceptance. Logical revocation/unlearning lineage leaves audit bytes and does not establish physical erasure, independent scientific truth or long-term efficacy.
