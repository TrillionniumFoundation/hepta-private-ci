...
8. Bind a persisted Core turn ID, observe its terminal status through a bounded recent-turn scan, and if the known turn has aged beyond that window use one authoritative `thread/read(includeTurns=true)` full-history lookup; then reconcile the durable TaskFlow step/run and terminalize the automation occurrence.
...
- **Stable queue recovery and terminal observer:** `codex-rs/hepta-agentd/src/automation.rs` uses `thread/queue/reconcile(AllowIfAbsent)` for first admission. `src/automation_recovery.rs` uses `ReconcileOnly` after lost acknowledgement, scans the recent persisted-turn window, and on window exhaustion performs an exact full-history `thread/read(includeTurns=true)` lookup before publishing terminality. A transport/read failure remains a recovery error rather than being converted into false terminal absence.
