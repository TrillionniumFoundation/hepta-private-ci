...
- **External effect seam:** `src/authorized_effect.rs` computes a closed-world owner-local typed intent digest inside automation and consumes the durable kernel `FinalUseAuthority`; `src/effect_dispatch_ledger.rs` plus migration `0013` persist pre-contact grant lineage, first provider observation and append-only terminal reconciliation after `indeterminate`. Restart scanning never authorizes redispatch. This owner-local intent is not the registered producer-owned `OperationIntentV1`: current `main` still exposes only the `kernel.operations` in-memory reference model, so product activation remains blocked on a real `kernel.operations` producer/adapter.
...
The repository-controlled source boundary is closed for the Agentd -> App Server durable causal spine, Calendar V2, owner-local external-effect durability, at-most-once dispatch evidence and indeterminate restart reconciliation. It is **not** closed for the registered cross-owner external-effect contract: `OperationIntentV1` is produced by `kernel.operations`, whose current canonical implementation is still an in-memory reference model without a production producer/caller. Automation must consume that producer-owned contract rather than renaming or treating `AuthorizedEffectIntent` as a substitute. A concrete downstream domain-effect provider/terminal observer remains independently owned as well. Authentic/current IANA tzdb provenance, selected-host DST/multi-scheduler behavior and performance remain qualification evidence, not source claims.
