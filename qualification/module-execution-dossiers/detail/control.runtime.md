...
- **Named host source:** [codex-rs/hepta-supervisor/src/global_control.rs](../../../codex-rs/hepta-supervisor/src/global_control.rs) composes durable `HeptaEvidenceStore` AuthBus replay for all required global owners including `runtime.fleet`, exact signed-fleet-to-live-allocation cross-checking, the real global planner/NDU path, `PlannerJournalStoreV1`, and the independent final-use authority fence. `GlobalControlHostPolicyV1` freezes fleet principal/floors, required owners, NDU policy and freshness/lifetime policy at construction; every required owner must have pinned trust and planning time comes from a host-owned process-monotonic epoch. Final use is fenced to a receipt issued by the current host generation that remains the durable selected decision, so journal revocation or restart requires replanning before dispatch.
