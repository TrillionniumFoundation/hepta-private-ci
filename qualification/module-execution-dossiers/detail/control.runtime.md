...
Status: core planner, canonical resource binding, semantic journal replay, local-Unix durable owner store, authenticated global source composition and independent-authority binding implemented; narrow Agentd context composition exists, while a named global product host and independent acceptance remain separate. Common requirements: `../EXECUTION_SEMANTICS.md`, `../TECHNICAL.md` and `docs/readiness/CONTROL_RUNTIME_EXECUTION.md`.
...
4. Canonicalize candidates, owners, payload digests, prebound effect-identity digests and resource axes.
...
- `RCP-08`: grant requests bind final payloads plus the plan-sealed effect identity and remain deny-all.
...
- **Implemented entrypoints:** planner core `collect_snapshot`, `prepare_plan`, `canonical_resource_profile_digest`, `canonical_effect_binding_digest_v1`, `finalize_plan` and `request_execution_grants` in [planner.rs](../../../codex-rs/hepta-control-plane/src/planner.rs); real NDU owner composition `evaluate_prepared_plan_with_ndu` in [planner_ndu.rs](../../../codex-rs/hepta-control-plane/src/planner_ndu.rs); narrow product caller adapter `plan_observed_context` in [planner_context.rs](../../../codex-rs/hepta-control-plane/src/planner_context.rs); authenticated/fleet global composition in [global_plane.rs](../../../codex-rs/hepta-control-plane/src/global_plane.rs); independent final-use binding in [authority_bridge.rs](../../../codex-rs/hepta-control-plane/src/authority_bridge.rs); durable journal owner store in [planner_store.rs](../../../codex-rs/hepta-control-plane/src/planner_store.rs); plus `OrganHostV1`, compiled-body admission and synthetic embodiment references.
- **State and recovery:** `GlobalStateSnapshotV1` binds owner readiness/frontiers and expiry; missing/stale owners block planning. `PlannerJournalV1` now validates semantic selection/revocation replay as well as the hash chain. `PlannerJournalStoreV1` adds an owner-only local-Unix durable profile with lock, atomic replacement, file/directory fsync, V1→V2 migration, append-only history and current-revocation restore floors. A post-rename directory-sync failure is `Indeterminate`: the writer is poisoned and the named host blocks final use until drop/reopen reconciles the durable state. OrganHostV1 runs trusted compiled-in read-only handlers and is not a sandbox or effect executor. Failed owner-callback restoration now quarantines predecessor dispatch and retains rollback errors; this is not a durable writer migration service. Native handoff matches host-owned protocol/profile/version/schema before graph construction. SyntheticCartIoV1 owns only an in-memory deterministic Q24 plant; read_sensor checks age/calibration and dispatch checks actuator identity, observation binding and DENY_ALL before a simulator step. Recreating the adapter resets this simulated plant; it is not physical-device recovery.
- **Source tests:** planner, NDU, context, journal, durable-store, global-plane and authority-bridge tests live under `codex-rs/hepta-control-plane/src/*_tests.rs`, alongside organ runtime/wire and embodiment fixtures. `global_plane_tests.rs` exercises a real `runtime.fleet::LeaseLedger` allocation plus signed owner facts through the real NDU implementation, while the named supervisor-host regression requires the durably admitted `runtime.fleet` owner revision/frontier to match that exact live allocation; `authority_bridge_tests.rs` exercises `FinalUseAuthority` signature/binding/nonce enforcement, rejects structured effect-identity drift against the digest sealed before planning, and verifies the final `with_verified_use` revalidation around dispatch. Test identities are not execution receipts until the current exact PR head is green.
...
- **Named host source:** [codex-rs/hepta-supervisor/src/global_control.rs](../../../codex-rs/hepta-supervisor/src/global_control.rs) composes durable `HeptaEvidenceStore` AuthBus replay for all required global owners including `runtime.fleet`, exact signed-fleet-to-live-allocation cross-checking, the real global planner/NDU path, `PlannerJournalStoreV1`, and the independent final-use authority fence. `GlobalControlHostPolicyV1` freezes fleet principal/floors, required owners, NDU policy, revocation frontier and freshness/lifetime policy at construction; every required owner must have pinned trust. After durable AuthBus admission, producer-local timing metadata is projected into the host-owned process-monotonic planner epoch, so an independent producer cannot control freshness arithmetic. The admitted fleet support digest preserves the durable AuthBus binding and then adds the exact live allocation-grant digest; final-use revalidation requires both domains unchanged. Final use is fenced to a receipt issued by the current host generation that remains the durable selected decision, so journal revocation or restart requires replanning before dispatch.
- **Remaining work:** qualify the canonical current-main supervisor candidate and register a selected deployment ingress/profile. The existing supervisord network protocol remains unchanged, so source composition is not daemon activation. Record named-host load/latency and fault-injection evidence and complete current exact-head plus synthetic-merge qualification. `GrantRequestSetV1` remains DENY_ALL; only independently signed `kernel.authority` final-use grants can become effect authority. HIL/device adapters, physical terminal reconciliation, independent acceptance, activation and release remain separate.
