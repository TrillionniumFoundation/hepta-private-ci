...
Status: source candidate global planner and owner-local decision journal implemented; the narrow Agentd cognitive-context caller is product-composed, while authenticated multi-owner global composition, production durability and independent acceptance remain separate. Common requirements: `../EXECUTION_SEMANTICS.md`, `../TECHNICAL.md` and `docs/readiness/CONTROL_RUNTIME_EXECUTION.md`.
...
evaluate_prepared_plan_with_ndu(snapshot, prepared, input, now) -> EvaluatedPlanV1
...
PlannerDurableStoreV1::{open, persist}
```
...
Every prepared input, evaluation binding, plan receipt and grant-request set is `AuthorityPosture::DENY_ALL`. Effectful candidates must pre-bind `ExecutionGrantBindingV1 { subject_id, destination_id, scope_digest }`; those fields and each final payload are sealed into `GrantRequestV1::request_digest`. A grant request is still not a capability: `kernel.authority` must independently verify the signed final-use grant.
...
`PlannerJournalV1` provides a bounded append-only reference for snapshot/decision/selection/revocation records. It validates sequence, predecessor hashes, semantic identities and entry hashes on reopen. Reopen also replays decision-before-selection/revocation constraints and rejects a selection after revocation instead of accepting any self-consistent hash chain. `PlannerDurableStoreV1` persists immutable journal generations under a single-writer lock, syncs each file and containing directory, requires contiguous generation numbers and monotonic journal-prefix extension, and fails recovery on a corrupt successor. This prevents rollback while the current store directory survives; a full-directory restore from an older backup still requires an independent anti-rollback witness. A revoked decision cannot be reselected or resurrected through restart.
...
- **Implemented entrypoints:** `collect_snapshot`, `canonical_resource_profile_digest`, `prepare_plan`, `finalize_plan` and `request_execution_grants` in [codex-rs/hepta-control-plane/src/planner.rs](../../../codex-rs/hepta-control-plane/src/planner.rs); `evaluate_prepared_plan_with_ndu` in [codex-rs/hepta-control-plane/src/planner_ndu.rs](../../../codex-rs/hepta-control-plane/src/planner_ndu.rs); `plan_observed_context` in [codex-rs/hepta-control-plane/src/planner_context.rs](../../../codex-rs/hepta-control-plane/src/planner_context.rs), called by [codex-rs/hepta-agentd/src/cognitive_context.rs](../../../codex-rs/hepta-agentd/src/cognitive_context.rs); `OrganHostV1` in [codex-rs/hepta-control-plane/src/organ_runtime.rs](../../../codex-rs/hepta-control-plane/src/organ_runtime.rs); `admit_compiled_body_graph_v2` in [codex-rs/hepta-control-plane/src/organ_wire.rs](../../../codex-rs/hepta-control-plane/src/organ_wire.rs); `SyntheticCartIoV1` in [codex-rs/hepta-control-plane/src/embodiment/io.rs](../../../codex-rs/hepta-control-plane/src/embodiment/io.rs).
- **State and recovery:** GlobalStateSnapshotV1 binds owner readiness/frontiers and expiry; missing/stale owners block planning. PlannerJournalV1 provides bounded hash-chain plus semantic replay. PlannerDurableStoreV1 adds immutable fsync-bound generation files, single-writer fencing, monotonic-prefix recovery and fail-closed corrupt-generation handling; independent whole-directory anti-rollback and operator restore acceptance remain separate. OrganHostV1 runs trusted compiled-in read-only handlers and is not a sandbox or effect executor. Failed owner-callback restoration now quarantines predecessor dispatch and retains rollback errors; this is not a durable writer migration service. Native handoff matches host-owned protocol/profile/version/schema before graph construction. SyntheticCartIoV1 owns only an in-memory deterministic Q24 plant; read_sensor checks age/calibration and dispatch checks actuator identity, observation binding and DENY_ALL before a simulator step. Recreating the adapter resets this simulated plant; it is not physical-device recovery.
...
- **Remaining work:** The Agentd cognitive-context adapter is a real but request-local, read-only product caller; it does not establish the multi-owner global planner. The grant-request contract now seals subject, destination, scope and payload before finalization, the owner-local durable generation store exists, and `AgentdControlRuntimeAuthorityHost` maps the sealed request to `kernel.authority::FinalUseBinding` without caller-supplied substitution. The remaining global composition is blocked on owner ports, most concretely `runtime.fleet`: its registered `DomainRead::fleet_allocation_grantV1` is not materialized as an authenticated authoritative read, and the existing local calculator explicitly denies `AuthenticatedFleetView` and `CanonicalAllocationGrant`. After that predecessor closes, connect the remaining owner adapters and named global caller to the durable store and final-use host. Production still requires an independent anti-rollback witness for whole-directory restore, backup/restore and recovery ceremony qualification, named-host measurements, real simulator/HIL/device adapters, physical terminal reconciliation and independent acceptance.
