...
- **Remaining work:** The Agentd cognitive-context adapter is a real but request-local, read-only product caller; it does not establish the multi-owner global planner. The grant-request contract now seals subject, destination, scope and payload before finalization, and the owner-local durable generation store now exists. Remaining repository composition is to connect authenticated multi-owner owner-summary adapters, the durable store, and the sealed request to a named global product caller using `kernel.authority::FinalUseAuthority`. Production still requires an independent anti-rollback witness for whole-directory restore, backup/restore and recovery ceremony qualification, named-host measurements, real simulator/HIL/device adapters, physical terminal reconciliation and independent acceptance.
