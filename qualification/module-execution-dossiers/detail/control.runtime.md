...
- **Remaining work:** compose the durable AuthBus replay path and global multi-owner planner through a named production host and, where cross-process use is required, admit a canonical external wire protocol. Bind `PlannerJournalStoreV1` to that host's lifecycle/retention policy, record named-host load/latency and fault-injection evidence, and complete current exact-head plus synthetic-merge qualification. `GrantRequestSetV1` remains DENY_ALL; only independently signed `kernel.authority` final-use grants can become effect authority. HIL/device adapters, physical terminal reconciliation, independent acceptance, activation and release remain separate.
