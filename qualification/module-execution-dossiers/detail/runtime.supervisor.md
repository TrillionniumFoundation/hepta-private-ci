...
- **State and recovery:** Supervisor keeps managed-process phases in memory and uses FleetRegistry lifecycle/release facts for recovery. Generation and release predecessor checks govern drain/restart/upgrade/rollback; process liveness alone is not full readiness. Main-Agent fault restart now uses configured bounded exponential backoff, a configured recovery window and an attempt budget capped at three. Operator drain/stop/kill and generation fencing disable automatic restart; Stop/Kill also cancel a queued retry while no child is running.
- **Launch acquisition cleanup:** if spawn succeeds but durable process-lease publication fails, the child remains quarantined under the live supervisor handle and kill is retried until exit is observed; the supervisor does not publish a false Failed state while that child is still live. This closes the in-process handle-loss case for SUP-03, but daemon-crash/host containment during that unpersisted cleanup window still requires target-host qualification.
- **Source tests:** [codex-rs/hepta-supervisor/src/supervisor_tests.rs](../../../codex-rs/hepta-supervisor/src/supervisor_tests.rs), including `upgrade_preflight_allows_matrix_only_release_change`, `lease_publication_failure_keeps_child_tracked_until_exit` and `unexpected_exit_restarts_with_bounded_backoff_and_attempt_budget`, `operator_stop_and_kill_cancel_scheduled_automatic_restart`; plus [codex-rs/hepta-supervisor/src/unix_tests.rs](../../../codex-rs/hepta-supervisor/src/unix_tests.rs). These are test identities, not execution receipts for this documentation revision.
...
- **Remaining work:** Qualify the actual deployed executable, target-host watchdog/drain behavior, daemon-crash containment during unpersisted launch cleanup and independently accepted release transition; source tests do not prove the target host process lifecycle.
