...
- **Implemented entrypoints:** `start`, `tick`, `drain`, `upgrade`, `rollback`, production-grant admission, mutation-status query and independently signed recovery resolution in [codex-rs/hepta-supervisor/src/supervisor.rs](../../../codex-rs/hepta-supervisor/src/supervisor.rs); the product daemon surface is in [codex-rs/hepta-supervisor/src/daemon.rs](../../../codex-rs/hepta-supervisor/src/daemon.rs). Native process lifecycle and release transition supervision are implemented.
- **Production release authority:** When the daemon is configured with an external grant verifier and pinned revocation frontier, unsigned upgrade/rollback RPCs fail closed. Signed grants bind exact source/target release manifest plus agentd/matrixd program digests, compatibility evidence, H7 artifact, current control/lifecycle/authority fences and the pinned revocation frontier. [release_selection.rs](../../../codex-rs/hepta-supervisor/src/release_selection.rs) persists the independently selected transition as a first-class record.
- **Drain/readiness:** The Unix driver uses an Agentd `Drain` RPC and requires an exact Agent/spawn/current-draining-generation acknowledgement. Agentd closes admission and reports separate critical-store, revocation-baseline and required-port readiness gates. Drain acknowledgement is not user-task terminality; uncertain in-flight work keeps its owning runtime recovery semantics.
- **State and recovery:** Supervisor keeps managed-process phases in memory, persists the process lease, release-selection/signed-intent state and bounded restart budget, and uses FleetRegistry lifecycle/release facts for recovery. Unexpected exits use bounded exponential backoff and an attempt window. Lease-publication failure keeps the spawned child tracked on a hard-kill quarantine path until observed exit. Rollback re-resolves current allowance/catalog provenance and, for signed transitions, rechecks the host-pinned revocation frontier. Ambiguous signed mutations become `recovery_required` and require an independently signed decision bound to the observed release bytes before becoming terminal.
- **Source tests:** [codex-rs/hepta-supervisor/src/supervisor_tests.rs](../../../codex-rs/hepta-supervisor/src/supervisor_tests.rs), [codex-rs/hepta-supervisor/src/unix_tests.rs](../../../codex-rs/hepta-supervisor/src/unix_tests.rs), [codex-rs/hepta-supervisor/src/release_selection.rs](../../../codex-rs/hepta-supervisor/src/release_selection.rs), [codex-rs/hepta-supervisor/src/signed_authority.rs](../../../codex-rs/hepta-supervisor/src/signed_authority.rs), plus Agentd readiness/drain tests in [codex-rs/hepta-agentd/src/runtime_tests.rs](../../../codex-rs/hepta-agentd/src/runtime_tests.rs). These are test identities, not execution receipts for this documentation revision.
...
- **Remaining work / external gates:** Qualify the exact deployed executable and host identity; demonstrate target-host startup/watchdog/drain/restart/recovery timings and crash/fault behavior; deploy/verify the external revocation/compatibility-frontier distribution that supplies the pinned daemon value; and obtain independent operational acceptance of signed release transition and recovery. Source tests do not self-issue deployment, selection, acceptance, promotion or release evidence.
