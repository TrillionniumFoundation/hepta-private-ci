...
Validate configuration, ownership and artifacts; establish a new process fence; start dependencies in initialization order; wait for readiness, not merely liveness; admit runs only after all critical gates. Shutdown stops admission, drains owned work, reconciles unknown effects and releases only resources actually acquired. Automatic fault recovery uses bounded exponential backoff and a fixed attempt budget; a flapping essential organ stops after budget exhaustion. A stable healthy recovery window replenishes that budget. Explicit operator restart remains a separately requested one-shot transition and does not silently widen the fault-recovery budget.
...
- **State and recovery:** Supervisor keeps managed-process phases in memory and uses FleetRegistry lifecycle/release facts for recovery. Generation and release predecessor checks govern drain/restart/upgrade/rollback; process liveness alone is not full readiness. Unexpected Agent exits and startup-health failures enter a configured bounded fault-restart policy (default 250 ms minimum backoff, 30 s maximum backoff, three attempts and a 60 s stable-health recovery window); budget exhaustion transitions the inactive lifecycle to `Stopped`. Stop/kill cancels both pending and in-flight fault recovery before a replacement launch can be scheduled. A child spawned before lease publication fails remains tracked through its exact live process handle until terminal observation, including when the first cleanup kill fails.
- **Release transition detail:** Daemon preflight and the native transition use the same release-equivalence rule: identical release identity, or identical Agent and Matrix commands, is unchanged. A Matrix-only bundle change is therefore admitted instead of being rejected as `TargetReleaseUnchanged`.
