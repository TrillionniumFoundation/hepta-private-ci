...
- **Implemented entrypoints:** lifecycle `start`, `tick`, `drain`, `stop`, `kill`, `restart`, `upgrade` and `rollback`; externally authorized `apply_production_grant`; signed recovery status/terminalization through `production_mutation_state` and `resolve_production_recovery`. The daemon rejects ordinary unsigned Upgrade/Rollback whenever an external production grant verifier is configured.
- **State and recovery:** exact process leases, bounded restart budget, unified release-transition journal and signed intents are persisted below the Agent run root. The release journal binds source/target immutable release bytes, the per-Agent allow/revoke admission frontier, release/lifecycle generations, rollback predecessor and optional signed authority. Unsigned crash recovery terminalizes only from exact release-state CAS evidence (`committed`, `rolled_back`, or `aborted`); ambiguous combinations remain recovery-required rather than being overwritten. Signed recovery additionally requires its independent recovery decision, except that a matching already-terminal release transaction can idempotently close the narrow transaction→intent crash cut.
- **Drain/readiness:** Unix drain uses the typed Agentd Drain RPC, not SIGTERM. Agentd closes request admission, waits for handlers already inside the App Server RPC gate, waits for assistant turns, and refuses graceful drained acknowledgement while durable Automation/TaskFlow work remains unclassified. Readiness binds exact process identity plus mandatory store, revocation-baseline, required-port and admission gates; process liveness alone is insufficient.
- **Rollback/revocation:** explicit and automatic rollback re-resolve the predecessor at final use. A release revoked while supervisord is down cannot be silently adopted: after exact process identity is proven, a revoked current child is fenced/killed and lifecycle moves to Failed; a revoked old predecessor does not prevent adoption of a still-valid current child.
- **Source tests:** [codex-rs/hepta-supervisor/src/supervisor_tests.rs](../../../codex-rs/hepta-supervisor/src/supervisor_tests.rs), [codex-rs/hepta-supervisor/src/unix_tests.rs](../../../codex-rs/hepta-supervisor/src/unix_tests.rs), and Agentd/App Server product integration tests. These are test identities, not execution receipts for this documentation revision.
