...
`build_sensor_core(design) -> OperatorSensorCoreManifestV1`; `fit_transition_model(dataset) -> TabularWorldModelV1`; `build_targets(dataset, objective) -> BellmanOperatorArtifact`; `fit_tabular_operator(dataset, sensor_core, profile) -> TabularOperatorArtifactV1`; `encode_tabular_payload_v1` / `LoadedTabularOperatorV1::from_pinned_payload`; `encode_world_model_payload_v1` / `LoadedTabularWorldModelV1::from_pinned_payload`; `admit_operator_regularity(assessment) -> OperatorRegularityAdmissionV1`; qualification-only operator acceptance uses `prepare`, `verify_and_seal`, and `verify_receipt`. Qualification training first constructs `VerifiedOperatorDatasetV2` from a self-verifying `DatasetSnapshotReceiptV3`, then uses `build_targets_bound_v2`, `fit_tabular_operator_bound_v2` or `fit_transition_model_bound_v2`; applicability and regularity claims use `admit_signed_operator_applicability_v2` / `admit_signed_operator_regularity_v2` with the host-owned `LearningEvidenceVerifierV1`. Raw `predict_tabular_operator` and `predict_transition` remain deprecated compatibility surfaces, not the authenticated load boundary.
...
NDU consumes bounded continuation values but does not replace the world model. C1 may use the simpler baseline allowed by its registered package; it must not silently bypass existing DAG predecessors. Rollback loads a complete compatible operator/sensor tuple under current revocations. Agentd now has `AgentdOfflineOperatorHostV1`, a durable offline/shadow coordination source path that consumes the verified frozen dataset, fits and publishes a deny-all candidate, consumes signed independent evaluation, and will not reload until the artifact lifecycle contains an externally authored `Selector` transition. It then constructs `PinnedCognitiveRanker`, which revalidates registry/revocation state for each cognitive read. The host cannot create the `Selected` transition itself; deployed-daemon scheduling/activation and longitudinal efficacy remain separate.
...
- **Implemented entrypoints:** `build_targets` in [codex-rs/hepta-bellman-operator/src/lib.rs](../../../codex-rs/hepta-bellman-operator/src/lib.rs); `validate_applicability_certificate` in [codex-rs/hepta-bellman-operator/src/reference.rs](../../../codex-rs/hepta-bellman-operator/src/reference.rs); `build_sensor_core` in [codex-rs/hepta-bellman-operator/src/reference.rs](../../../codex-rs/hepta-bellman-operator/src/reference.rs); `evaluate_bellman_reference` in [codex-rs/hepta-bellman-operator/src/reference.rs](../../../codex-rs/hepta-bellman-operator/src/reference.rs); `admit_operator_regularity` in [codex-rs/hepta-bellman-operator/src/reference.rs](../../../codex-rs/hepta-bellman-operator/src/reference.rs); `fit_tabular_operator` in [codex-rs/hepta-bellman-operator/src/learned.rs](../../../codex-rs/hepta-bellman-operator/src/learned.rs); `fit_tabular_operator_strict_v2` in [codex-rs/hepta-bellman-operator/src/learned_strict.rs](../../../codex-rs/hepta-bellman-operator/src/learned_strict.rs); `encode_tabular_payload_v1` in [codex-rs/hepta-bellman-operator/src/loaded.rs](../../../codex-rs/hepta-bellman-operator/src/loaded.rs); `LoadedTabularOperatorV1::from_pinned_payload` in [codex-rs/hepta-bellman-operator/src/loaded.rs](../../../codex-rs/hepta-bellman-operator/src/loaded.rs); `fit_transition_model` in [codex-rs/hepta-bellman-operator/src/world_model.rs](../../../codex-rs/hepta-bellman-operator/src/world_model.rs); `encode_world_model_payload_v1` in [codex-rs/hepta-bellman-operator/src/loaded_world_model.rs](../../../codex-rs/hepta-bellman-operator/src/loaded_world_model.rs); `LoadedTabularWorldModelV1::from_pinned_payload` in [codex-rs/hepta-bellman-operator/src/loaded_world_model.rs](../../../codex-rs/hepta-bellman-operator/src/loaded_world_model.rs); `prepare` in [codex-rs/hepta-operator-acceptance/src/lib.rs](../../../codex-rs/hepta-operator-acceptance/src/lib.rs); `verify_and_seal` in [codex-rs/hepta-operator-acceptance/src/lib.rs](../../../codex-rs/hepta-operator-acceptance/src/lib.rs); `verify_receipt` in [codex-rs/hepta-operator-acceptance/src/lib.rs](../../../codex-rs/hepta-operator-acceptance/src/lib.rs). Replay-resistant fitting, deterministic reference, pinned tabular/world-model loading and qualification-only acceptance are source implemented; none grants production promotion authority.
- **Additive qualification boundaries:** `VerifiedOperatorDatasetV2::from_receipt`, `build_targets_bound_v2`, `fit_tabular_operator_bound_v2` and `fit_transition_model_bound_v2` bind exact V3 frozen-dataset evidence to target/training rows; `admit_signed_operator_applicability_v2` and `admit_signed_operator_regularity_v2` require Generator and Evaluator attestations over the exact same payload, reuse the ledger's host-owned Ed25519 trust verifier, and apply signed role separation across principal, credential chain, signing key and controller identity in addition to epoch, lifetime and revocation checks. Cryptographic separation authenticates provenance and control identity; it does not self-prove scientific truth or broader organizational independence.
- **State and recovery:** Pure candidate artifacts bind immutable data/profile/sensor identities. `encode_tabular_payload_v1` / `LoadedTabularOperatorV1` and the parallel world-model loader provide bounded independently pinned persisted inference. `AgentdOfflineOperatorHostV1` adds a bounded single-writer coordination journal for Prepared → Trained → Published → Evaluated → externally Selected-observed → Reloaded; each append is synchronized before in-memory publication, reopen verifies the chain, and semantic identity reuse conflicts. Dataset, artifact, evaluation and selection truth remain with their existing owners; the host cannot self-select. `train` remains a compatibility alias for target construction.
- **Source tests:** [codex-rs/hepta-bellman-operator/src/reference_tests.rs](../../../codex-rs/hepta-bellman-operator/src/reference_tests.rs), [codex-rs/hepta-bellman-operator/src/learned_tests.rs](../../../codex-rs/hepta-bellman-operator/src/learned_tests.rs), [codex-rs/hepta-bellman-operator/src/loaded_tests.rs](../../../codex-rs/hepta-bellman-operator/src/loaded_tests.rs), [codex-rs/hepta-bellman-operator/src/world_model_tests.rs](../../../codex-rs/hepta-bellman-operator/src/world_model_tests.rs), [codex-rs/hepta-bellman-operator/src/loaded_world_model_tests.rs](../../../codex-rs/hepta-bellman-operator/src/loaded_world_model_tests.rs), plus `hepta-operator-acceptance` ceremony/trust tests. These are test identities, not execution receipts for this documentation revision.
