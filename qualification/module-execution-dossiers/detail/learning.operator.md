...
Public target/reference and fitting surfaces are `build_targets`, structural `validate_applicability_certificate` / `admit_operator_regularity`, authenticated `admit_authenticated_applicability_v1` / `admit_authenticated_regularity_v1`, `build_sensor_core`, `evaluate_bellman_reference`, replay-safe `fit_tabular_operator`, strict `fit_tabular_operator_strict_v2`, authenticated-dataset `fit_tabular_operator_verified_v1`, replay-safe `fit_transition_model` and authenticated-dataset `fit_transition_model_verified_v1`. `verify_operator_dataset_v1` is the cross-owner dataset admission boundary.
...
Evidence identity is the replay boundary. Cross-owner training additionally consumes a self-verifying `DatasetSnapshotReceiptV3` authenticated by a host-owned Observer key; `VerifiedOperatorDatasetV1` is privately constructed and verified fitters require the exact sample evidence-digest set to equal the frozen source-record set. `build_targets` rejects repeated support digests even under different sample identities. Public tabular and world-model fitting reject repeated evidence digests before counts, means or transition probabilities are computed. The public tabular fit requires at least two registered actions, matching the deterministic reference contract.
...
- OP-05: authenticated V3 dataset admission binds the exact training rows; detached dataset digests or altered row sets fail closed.
- OP-06: applicability and regularity require evaluator-signed exact structural digests plus generator/evaluator role separation before they can be described as independently evaluated.
...
NDU consumes bounded continuation values but does not replace the world model. C1 may use the simpler baseline allowed by its registered package; it must not silently bypass existing DAG predecessors. Rollback loads a complete compatible operator/sensor tuple under current revocations through the artifact owner. Agentd already provides one explicit read consumer: `PinnedCognitiveRanker` loads the selected pinned tabular operator and revalidates the current artifact view on every cognitive read. It cannot train, select, add context or dispatch a turn, so the default learning loop remains uncomposed.
...
- **Authenticated data/evaluator admission:** `verify_operator_dataset_v1` authenticates a V3 dataset receipt with `LearningEvidenceVerifierV1`; verified fitters bind the exact row set. `admit_authenticated_applicability_v1` and `admit_authenticated_regularity_v1` require evaluator signatures over the exact structural digest and enforce generator/evaluator independence.
