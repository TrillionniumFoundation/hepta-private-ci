...
Primary root: `codex-rs/hepta-bellman-operator`. The canonical Cargo binding also assigns `codex-rs/hepta-operator-acceptance` to this module as a qualification-only acceptance package; it is not a production writer or promotion authority.
...
`build_sensor_core(design) -> OperatorSensorCoreManifestV1`; `fit_transition_model(dataset) -> TabularWorldModelV1`; `build_targets(dataset, objective) -> BellmanOperatorArtifact`; `fit_tabular_operator(dataset, sensor_core, profile) -> TabularOperatorArtifactV1`; `admit_operator_regularity(assessment) -> OperatorRegularityAdmissionV1`. Persisted prediction uses `encode_tabular_payload_v1` plus `LoadedTabularOperatorV1::from_pinned_payload(...).predict(...)` for the learned operator and `encode_world_model_payload_v1` plus `LoadedTabularWorldModelV1::from_pinned_payload(...).predict(...)` for the world model.

The raw `predict_tabular_operator`, `predict_tabular_operator_indexed_v2` and `predict_transition` functions remain deprecated compatibility surfaces. They validate public in-memory structure but do not establish independent artifact provenance; composed callers must use the pinned loaded abstractions. The original `train` function remains a compatibility alias for `build_targets`; it is explicitly a target builder, not a neural trainer. Transition/dynamics estimation and continuation-value estimation have separate artifacts and evidence.
...
The tabular learner requires a complete canonical sensor-by-action grid and a minimum sample count for every cell. It stores per-cell mean, minimum, maximum, sample count and evidence digest. Missing or underfilled cells fail. Sample identity and underlying evidence identity are independently unique: relabelling the same evidence under a new sample ID is rejected. The world model applies the same evidence-identity rule before counts or transition frequencies are accumulated. The legacy target builder retains its existing support-digest semantics for compatibility. Predictions outside the fitted grid fail rather than extrapolate.
