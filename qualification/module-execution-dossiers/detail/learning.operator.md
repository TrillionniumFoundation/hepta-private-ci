...
- OP-07: qualification fitting consumes a verified V3 dataset receipt and requires the exact frozen source-record evidence set; a detached digest, omitted row or foreign row rejects.
...
NDU consumes bounded continuation values but does not replace the world model. C1 may use the simpler baseline allowed by its registered package; it must not silently bypass existing DAG predecessors. Rollback loads a complete compatible operator/sensor tuple under current revocations. Agentd already has one explicit read-only consumer, `PinnedCognitiveRanker`, which loads a selected pinned tabular operator and revalidates registry/revocation state for each cognitive read; this is real consumer composition, but it is not the default automatic train → evaluate → independently select → new-process reload loop.
...
- **Additive qualification boundaries:** `VerifiedOperatorDatasetV2::from_receipt`, `fit_tabular_operator_bound_v2` and `fit_transition_model_bound_v2` bind exact V3 frozen-dataset evidence to training rows; `admit_signed_operator_applicability_v2` and `admit_signed_operator_regularity_v2` require Generator and Evaluator attestations over the exact same payload, reuse the ledger's host-owned Ed25519 trust verifier, and apply signed role separation across principal, credential chain, signing key and controller identity in addition to epoch, lifetime and revocation checks. Cryptographic separation authenticates provenance and control identity; it does not self-prove scientific truth or broader organizational independence.
