...
- **Additive qualification boundaries:** `VerifiedOperatorDatasetV2::from_receipt`, `fit_tabular_operator_bound_v2` and `fit_transition_model_bound_v2` bind exact V3 frozen-dataset evidence to training rows; `admit_signed_operator_applicability_v2` and `admit_signed_operator_regularity_v2` reuse the ledger's host-owned Ed25519 trust verifier and reject role, principal, credential, payload, epoch, lifetime or revocation drift. A valid signature authenticates provenance, not scientific truth or organizational independence.
