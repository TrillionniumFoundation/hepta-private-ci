...
The tabular learner requires a complete canonical sensor-by-action grid and a minimum sample count for every cell. It stores per-cell mean, minimum, maximum, sample count and evidence digest. Duplicate underlying evidence is rejected even when relabelled with a different sample ID. Missing or underfilled cells fail. Raw prediction requires an independent pin over the complete canonical public payload plus artifact lineage; persisted candidates use the once-validated loaded surface. Predictions outside the fitted grid fail rather than extrapolate.
...
The action-conditioned world-model baseline groups immutable observed samples by state/action, rejects duplicate evidence across relabelled sample IDs, publishes exact Q32 branch distributions that sum to one and rejects unsupported pairs. Raw prediction requires a complete canonical payload/model/dataset pin; `LoadedTabularWorldModelV1` provides validated repeated lookup. Every prediction is marked synthetic. A model prediction cannot become an independent factual outcome.
