...
- **Native produced contracts and product caller:** [contracts_v1.rs](../../../codex-rs/hepta-intelligence/src/contracts_v1.rs) implements bounded `LegalActionCandidateSetV1` and authority-free `IntelligenceHostEnvelopeV1`. [codex-rs/hepta-agentd/src/intelligence_host.rs](../../../codex-rs/hepta-agentd/src/intelligence_host.rs) validates the typed handoff; `AgentRunCoordinator::run_native_intelligence_v3` in [lane_b_runtime.rs](../../../codex-rs/hepta-agentd/src/lane_b_runtime.rs) is the registered Agentd source callsite and performs the real run-state attach when invoked; the current daemon composition in `runtime.rs`/`state_control.rs` does not yet own or invoke this coordinator. Immediately before that mutation it must obtain a current admitted snapshot through `CurrentCapabilitySnapshotProviderV3` and match it exactly to the frozen `CapabilitySnapshotV2`; stale revocation frontier or implementation generation fails before Agentd handoff and before learning append. Provider authentication remains a host/owner trust obligation, so this source callsite is not itself product-execution evidence.
...
- **Remaining repository closure:** wire the coordinator into the real Agentd daemon/App Server product path under the `runtime.agentd` owner package; bind the mandatory `CurrentCapabilitySnapshotProviderV3` to authenticated current owner/trust sources, execute the named `AgentRunCoordinator::run_native_intelligence_v3` callsite with those facts, observe actual Codex delivery/terminal outcomes, exercise Outcome/Credit reconciliation, enforce owner-side interruption for blocking I/O, and pass exact-head plus deterministic synthetic-merge qualification. Independent acceptance, target-host canary, promotion and release remain external gates.
