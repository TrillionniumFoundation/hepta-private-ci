...
Status: read-only vertical, signed evaluated-shadow composition and prompt-registry byte-to-attachment source composition implemented; runtime host dispatch and independent acceptance remain open in section 8. Common requirements: `../EXECUTION_SEMANTICS.md` and `../TECHNICAL.md`. Canonical ownership and package predecessors are unchanged.
...
- **Implemented entrypoints:** `run_read_only_vertical` in [codex-rs/hepta-intelligence/src/vertical.rs](../../../codex-rs/hepta-intelligence/src/vertical.rs); `run_evaluated_shadow_v1` in [codex-rs/hepta-intelligence/src/evaluated_shadow.rs](../../../codex-rs/hepta-intelligence/src/evaluated_shadow.rs); `compile_prompt_registry_v2` in [codex-rs/hepta-intelligence/src/prompt_delivery.rs](../../../codex-rs/hepta-intelligence/src/prompt_delivery.rs). Read-only vertical, signed evaluated-shadow composition and source-level prompt-registry payload composition are implemented.
- **State and recovery:** The vertical derives cross-stage objective/read/context/NDU bindings in one call. Evaluated shadow additionally verifies signed evidence and appends a decision through the existing DurableLedger. The prompt-registry bridge owns no prompt facts: it reads a frozen durable registry snapshot through the registered read-only module port, dereferences exact payload bytes with revocation/model/context-profile checks, binds admission lineage and tokenizer receipts, passes trusted candidates to `context.compiler`, then records serialization and attachment receipts for the exact selected byte envelope.
- **Source tests:** [codex-rs/hepta-intelligence/src/vertical_tests.rs](../../../codex-rs/hepta-intelligence/src/vertical_tests.rs), [codex-rs/hepta-intelligence/src/evaluated_shadow_tests.rs](../../../codex-rs/hepta-intelligence/src/evaluated_shadow_tests.rs), [codex-rs/hepta-intelligence/src/prompt_delivery_tests.rs](../../../codex-rs/hepta-intelligence/src/prompt_delivery_tests.rs). These are test identities, not execution receipts for this documentation revision.
- **Implementation and operating references:** [codex-rs/hepta-intelligence/EVALUATED_SHADOW.md](../../../codex-rs/hepta-intelligence/EVALUATED_SHADOW.md), [docs/modules/prompt.registry/TECHNICAL.md](../../../docs/modules/prompt.registry/TECHNICAL.md).
- **Remaining work:** Bind the source-level composition into the named runtime host, submit the already-bound `ContextAttachmentV2` through actual model dispatch, retain a terminal observation whose observed payload digest equals the attachment payload digest, and supply trusted current host ports/evaluation/calibration evidence. The bridge does not create another model loop, production writer or effect authority.
