...
`build_legal_candidates_v1(...) -> LegalActionCandidateSetV1`; `run_composition_v3(request, ports) -> LaneFCompositionReceiptV3`; `run_composition_v3_with_control(request, ports, control) -> LaneFCompositionReceiptV3`; product-side `AgentRunCoordinator::run_native_intelligence_v3(...) -> IntelligenceRunReceiptV3`; and compatibility `run_read_only_vertical_outcome_v1(...) -> ReadOnlyVerticalOutcomeV1` are the current source surfaces. The older design-operation names remain vocabulary, not separate facades. Facts and execution remain with their registered owners.
...
- **Native produced contracts and product caller:** [contracts_v1.rs](../../../codex-rs/hepta-intelligence/src/contracts_v1.rs) implements bounded `LegalActionCandidateSetV1` and authority-free `IntelligenceHostEnvelopeV1`. [codex-rs/hepta-agentd/src/intelligence_host.rs](../../../codex-rs/hepta-agentd/src/intelligence_host.rs) validates the typed handoff. `AgentdState` owns one `AgentRunCoordinator` in [state.rs](../../../codex-rs/hepta-agentd/src/state.rs), and [state_intelligence.rs](../../../codex-rs/hepta-agentd/src/state_intelligence.rs) exposes the daemon-owned `AgentdState::run_native_intelligence_v3` source seam, delegating to `AgentRunCoordinator::run_native_intelligence_v3_with_control` in [lane_b_runtime.rs](../../../codex-rs/hepta-agentd/src/lane_b_runtime.rs). Immediately before handoff the path obtains a current admitted snapshot through `CurrentCapabilitySnapshotProviderV3` and matches it exactly to the frozen `CapabilitySnapshotV2`; stale revocation frontier or implementation generation fails before Agentd handoff and before learning append. [intelligence_snapshot.rs](../../../codex-rs/hepta-agentd/src/intelligence_snapshot.rs) implements `AuthenticatedCapabilitySnapshotProviderV3` using an owner-controlled trust file and exact owner Ed25519/current-sequence attestations. Ordinary product bootstrap still must enroll those real owner-issued attestations, and ordinary App Server turn admission still does not invoke the V3 seam, so this source composition is not itself product-execution evidence.
...
- **Remaining repository closure:** invoke the daemon-owned V3 seam from ordinary Agentd/App Server turn admission; enroll `AuthenticatedCapabilitySnapshotProviderV3` with the real owner-issued attestations/current trust registry; observe actual Codex delivery/terminal outcomes; exercise Outcome/Credit reconciliation across crash/restart boundaries; enforce owner-side interruption for blocking I/O; and pass exact-head plus deterministic synthetic-merge qualification. Independent acceptance, target-host canary, promotion and release remain external gates.
