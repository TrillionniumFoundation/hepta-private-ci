...
- **Implemented entrypoints:** `run_read_only_vertical` in [codex-rs/hepta-intelligence/src/vertical.rs](../../../codex-rs/hepta-intelligence/src/vertical.rs); `run_evaluated_shadow_v1` in [codex-rs/hepta-intelligence/src/evaluated_shadow.rs](../../../codex-rs/hepta-intelligence/src/evaluated_shadow.rs). Read-only vertical is available normally; signed evaluated-shadow composition is source-implemented but deliberately available only under the explicit `evaluated-shadow-qualification` feature so default/product builds cannot use the weak legacy Decision write port.
