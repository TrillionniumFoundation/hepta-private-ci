...
`estimate_ope(plan, rows) -> OpeEstimate`; `estimate_cluster_intervals(plan, rows, assignments) -> ClusterOpeEstimate`; `estimate_sequential(plan, trajectories) -> SequentialEstimate`; `fit_temporal_fold(plan, training, targets) -> TemporalFoldReceipt`; `evaluate_temporal_holdout(plan, training, targets, observations, assignments) -> TemporalEvaluationReceipt`; `freeze_cross_fold_plan_v2(plan, metric_roles) -> CrossFoldPlanReceiptV1`; `FencedFinalHoldoutOwnerV1::consume(&frozen_plan_receipt) -> FinalHoldoutJournalReceiptV1` for contended production ownership; `decide_with_signed_evidence_v2(...) -> SignedEvaluationDecisionV1`; and `decide_with_signed_longitudinal_evidence_v3(...)` for `SystemLongitudinal`. Direct evaluators are trusted-only compatibility surfaces behind `trusted-inprocess-eval`.
...
The in-memory `FinalHoldoutRegistry` consumes that typed receipt. It permits an exact idempotent retry, rejects semantic mutation under the same plan identity, and prevents a second plan from reusing either the final-holdout digest or final-holdout window. Its sealed use receipt binds the complete frozen-plan semantics and registry state. `DurableFinalHoldoutJournalV1` is the single-host/cooperative-owner adapter. Contended production deployments use `FencedFinalHoldoutOwnerV1` with a host-provided linearizable `FinalHoldoutCasStoreV1`; monotonic fencing makes stale owners fail CAS and indeterminate commits require recovery.
...
- **Remaining work:** Provide a host implementation of the fenced CAS store plus production scheduler/fence issuer where multi-writer ownership is required; provide live authenticated outcomes and real future-window evidence; estimator fixtures cannot establish longitudinal efficacy. The repository already contains the signed `run_evaluated_shadow_v1` consumer path, but that shadow composition is not runtime activation or longitudinal efficacy evidence.
