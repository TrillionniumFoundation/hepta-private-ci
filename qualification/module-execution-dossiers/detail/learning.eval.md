...
The in-memory `FinalHoldoutRegistry` remains the semantic primitive. `DurableFinalHoldoutJournalV1` persists it for a cooperating single-host owner. Multi-host composition uses `FencedFinalHoldoutOwnerV1` with a host-supplied linearizable compare-and-swap fence: exact plan reservation precedes journal mutation, anchor commit follows journal sync, and `reconcile_pending` closes either crash cut without permitting a second plan to consume the same holdout. The external fence remains responsible for authenticated ownership, independent durability and anti-rollback.
...
- **Implemented entrypoints:** `evaluate_temporal_holdout`; production plan/admission via `freeze_cross_fold_plan_v2`, `decide_with_signed_evidence_v2` and `decide_with_signed_longitudinal_evidence_v3`; multi-host durable ownership via `FencedFinalHoldoutOwnerV1::{consume,reconcile_pending}`. The current product-facing shadow composition is `codex_hepta_intelligence::run_evaluated_shadow_v1`, which verifies signed V2 evaluation evidence and exact candidate bytes before invoking host ports.
- **State and recovery:** Temporal evaluation binds a frozen plan and exact joined held-out cohort. The durable journal provides synced/anchored byte recovery; the fenced owner adds an externally linearizable CAS reservation/commit protocol and exact pending-reservation reconciliation. Host trust provisioning, fence authentication/anti-rollback, production scheduling, live outcomes and real future-calendar evidence remain external. Signed `SystemLongitudinal` admission requires V3 observed-time evidence, not window IDs alone.
...
- **Remaining work:** Bind a real product scheduler and authenticated/anti-rollback implementation of the external holdout fence; provide live authenticated outcomes, independent current anchors and real future-window evidence. Repository fixtures and signed CI evidence cannot establish longitudinal efficacy, independent acceptance, selection, promotion or release.
