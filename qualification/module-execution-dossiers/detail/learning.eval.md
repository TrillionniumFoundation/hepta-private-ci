...
Concrete source mappings are recorded in `../../../codex-rs/hepta-intelligence-eval/NATIVE_MAPPING.md` and `../../../docs/lane-e/LANE_E_IMPLEMENTATION_MATRIX.json`. Production admission is normatively defined by `../../../codex-rs/hepta-intelligence-eval/PRODUCTION_CONTRACT.md`. Preserve existing estimators and compatibility where explicitly gated; do not create another authority or execution spine.

## 2. Public operations and contract details

`estimate_ope(plan, rows) -> OpeEstimate`; `estimate_cluster_intervals(plan, rows, assignments) -> ClusterOpeEstimate`; `estimate_sequential(plan, trajectories) -> SequentialEstimate`; `fit_temporal_fold(plan, training, targets) -> TemporalFoldReceipt`; `evaluate_temporal_holdout(plan, training, targets, observations, assignments) -> TemporalEvaluationReceipt`; `freeze_cross_fold_plan_v2(plan, roles) -> CrossFoldPlanReceiptV1`; `FinalHoldoutRegistry::consume(&frozen_plan_receipt) -> HoldoutUseReceiptV1`; `DurableFinalHoldoutJournalV1::consume_fenced(authority, expected, plan)` for multi-owner durability; `decide_with_signed_evidence_v2(bundle, roles, evidence, verifier, now)` for qualification; and `decide_with_signed_longitudinal_evidence_v3(...)` for system-longitudinal claims. Unsigned direct decision entry points are crate-private and available cross-crate only through the explicit `trusted-inprocess-eval` compatibility feature.
...
The in-memory `FinalHoldoutRegistry` consumes that typed receipt. It permits an exact idempotent retry, rejects semantic mutation under the same plan identity, and prevents a second plan from reusing either the final-holdout digest or final-holdout window. Its sealed use receipt binds the complete frozen-plan semantics and registry state. `DurableFinalHoldoutJournalV1::consume_single_host_trusted` is restricted to one cooperative host. Multi-process or multi-host production ownership uses `consume_fenced` with a linearizable host-owned `HoldoutAnchorAuthorityV1`. Deterministic local encoding/capacity checks complete before reservation; `Ok(false)` means the CAS definitely did not commit, while any CAS error is outcome-unknown and poisons the handle. After a successful reservation, any local append failure is `Indeterminate` and requires reconciliation rather than permitting a stale replica to reuse the holdout.

`IndependentEvaluationBundleV1` carries generator/evaluator identity metadata plus the exact sealed frozen-plan and holdout-use receipts, objective, dataset, estimand, estimate, support, confidence, retention, unlearning, snapshot and future-window facts. Structural identity alone is not external authentication. Production ingress verifies the generator and evaluator signatures through host-owned `LearningEvidenceVerifierV1`, then validates receipt integrity and semantic equality before statistical admission. The evaluator cannot share a principal, credential chain or signing key with the generator.
...
The former single temporal holdout and conservative cluster code is no longer labelled generic cross-fitting by implication. `freeze_cross_fold_plan_v2` supplies an explicit complete analysis contract, preregistered metric roles and sealed receipt, and `FinalHoldoutRegistry::consume` derives its use binding from that receipt rather than loose caller arguments. The durable adapter now supports externally fenced multi-owner consumption; the concrete linearizable anchor backend and nuisance-model scheduler remain product bindings. Native estimator, independent observer and authentication adapters are separately identified. The evaluator emits eligibility evidence, never selection or release authority.
...
- **Implemented entrypoints:** `evaluate_temporal_holdout`, `freeze_cross_fold_plan_v2`, `decide_with_signed_evidence_v2`, `decide_with_signed_longitudinal_evidence_v3` and the fenced durable holdout owner path are implemented. Unsigned direct decisions are no longer default-public.
- **State and recovery:** Temporal evaluation binds a frozen plan and exact joined held-out cohort, isolates training labels and checks cluster lineage. FinalHoldoutRegistry remains the in-memory semantic registry. DurableFinalHoldoutJournalV1 wraps its journal with locked/synced recovery and adds `consume_fenced` for an externally retained linearizable anchor CAS; the host still owns the concrete authority backend, authentication/currentness and production scheduling. Signed SystemLongitudinal admission requires V3 observed-time evidence, not window IDs alone.
...
- **Remaining external/product work:** Bind `HoldoutAnchorAuthorityV1` to the product's authenticated linearizable anchor store and nuisance-model scheduler; provide live authenticated outcomes and real future-window evidence; estimator fixtures and repository CI cannot establish longitudinal efficacy or operator acceptance.

## 9. Native closure and remaining evidence

Repository-controlled source coverage is checked by `../../../scripts/hepta-lane-e-closure.py`; exact-head and ordered-parent synthetic-merge execution are defined in `.github/workflows/hepta-lane-e-gap-closure.yml`. The workflow compiles all targets, runs owner and cross-crate tests, executes the signed evaluated-shadow runtime consumer, repeats the signed qualification suite as an adversarial stress audit, enforces the evaluator line-coverage floor, runs strict Clippy/rustfmt, and retains commit-addressed provenance artifacts.
