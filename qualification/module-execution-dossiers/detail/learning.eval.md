...
`SignedHoldoutAnchorV1` closes the authenticated-origin edge for the independently retained recovery anchor. `authenticate_holdout_anchor_v1` verifies the exact namespace binding, sequence and head under a trusted `Observer`; requires that signed anchor to equal the exact host-owned `minimum_anchor`; and also requires a host-owned monotonic `minimum_issued_at` watermark. Neither currentness boundary can be supplied by the request itself. `recover_with_authenticated_holdout_anchor_v1` authenticates first and refuses a zero bootstrap anchor; initialization stays explicit. The product host still has to persist the latest signed witness plus the anchor/freshness currentness record outside the holdout journal before releasing confirmatory labels or acknowledging use externally.
