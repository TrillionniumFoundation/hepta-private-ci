...
- **Remaining work:** migrate `run_evaluated_shadow_v1` to consume `ProductQualificationReceiptV1` rather than invoking low-level signed admission directly; bind the evidence sink and holdout namespace to a named target host; then provide live authenticated outcomes and real future-window/retention/privacy/unlearning evidence. The repository now has a concrete cross-process locked-file CAS backend, but cross-host use still requires qualification of the shared filesystem's lock/fsync semantics.
