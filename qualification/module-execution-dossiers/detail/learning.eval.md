...
Concrete source mappings are recorded in `../../../codex-rs/hepta-intelligence-eval/NATIVE_MAPPING.md`, the normative production API boundary in `../../../codex-rs/hepta-intelligence-eval/PRODUCTION_CONTRACT.md`, and current source/qualification state in `../../../docs/lane-e/LANE_E_IMPLEMENTATION_MATRIX.json`. Preserve existing estimators; legacy evaluator compatibility is explicit/feature-gated and must not become another production ingress or authority.

## 2. Public operations and contract details

Estimator operations remain `estimate_ope`, `estimate_cluster_intervals`, `estimate_sequential`, `fit_temporal_fold` and `evaluate_temporal_holdout`. New qualification plans use `freeze_cross_fold_plan_v2`; durable local ownership uses `FencedFinalHoldoutJournalV2::consume`; external non-longitudinal admission uses `decide_with_signed_evidence_v2`; `SystemLongitudinal` admission uses `decide_with_signed_longitudinal_evidence_v3`. Direct `decide_independently*` is structural trusted/qualification-only, and the feature-gated legacy `evaluate` alias is not part of the default production API.
...
The in-memory `FinalHoldoutRegistry` consumes that typed receipt. It permits an exact idempotent retry, rejects semantic mutation under the same plan identity, and prevents a second plan from reusing either the final-holdout digest or final-holdout window. Its sealed use receipt binds the complete frozen-plan semantics and registry state. A product adapter must persist both registry and receipts under an exclusive writer and authenticated storage boundary. `FencedFinalHoldoutJournalV2` binds a nonzero host-issued writer fence and requires the current context on every consume; a multi-host product must keep the current fence and acknowledged anchor in a transactional CAS authority or dedicated holdout service.
...
- **Source tests:** [codex-rs/hepta-intelligence-eval/src/temporal_evaluation_tests.rs](../../../codex-rs/hepta-intelligence-eval/src/temporal_evaluation_tests.rs), [codex-rs/hepta-intelligence-eval/src/closure_tests.rs](../../../codex-rs/hepta-intelligence-eval/src/closure_tests.rs), [codex-rs/hepta-intelligence-eval/src/durable_holdout_tests.rs](../../../codex-rs/hepta-intelligence-eval/src/durable_holdout_tests.rs), plus signed composed-boundary tests in [codex-rs/hepta-intelligence/src/evaluated_shadow_tests.rs](../../../codex-rs/hepta-intelligence/src/evaluated_shadow_tests.rs). These are source identities; CI qualification receipts are retained separately per exact commit.
...
Repository-controlled source coverage is checked by `../../../scripts/hepta-lane-e-closure.py`; exact-head and ordered-parent synthetic-merge execution are defined in `.github/workflows/hepta-lane-e-gap-closure.yml`. The workflow compiles all targets, runs owner and cross-crate tests, the signed evaluated-shadow end-to-end path, durable holdout reopen/replay stress, strict Clippy/rustfmt, collects learning.eval coverage, and retains commit-addressed exact-head and synthetic-merge qualification receipts.
