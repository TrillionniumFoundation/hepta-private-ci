...
- **Security boundary:** Rust constructs the exact kernel `FinalUseBinding`, durably claims a single-use nonce through `FinalUseAuthority`, persists `Invoking`, and consumes the non-serializable `VerifiedUseToken` through `with_verified_use` immediately before OS entry so current epoch/revocation/time is checked again. Endpoint identity and loopback gateway bearer capability remain separately bound through a signed endpoint manifest and OS keyring. Update activation re-verifies signature/channel/target tuple, the staged package digest and the currently installed predecessor digest before replacement.
- **Platform/UI:** `eframe 0.36.2` / `egui` + AccessKit is selected. Product targets are Windows 11 x86_64, macOS 14+ arm64/x86_64 and Ubuntu 24.04 x86_64. The native window exposes runtime, operation, signed-update staging/activation and accessibility state; activation closes the GUI before handing off to the separate updater. Keyboard focus, HiDPI and English/Chinese shell copy are implemented. Local policy denial is testable before OS entry; terminal success is not fabricated where the OS has no trustworthy observation API.
...
- **Remaining repository work:** commit a reproducible native Cargo lock; observe exact-head and Windows/macOS/Linux merge-candidate CI for the final candidate; run the packaged crash/restart/updater-death qualification. The product authority ingress is the operator-selected independently issued `SignedFinalUseGrant`; the read-only gateway remains a backend observation boundary rather than an authority issuer. Windows and Unix both use kernel-owned durable final-use stores in this candidate. **Remaining external work:** production signing/notarization/distribution trust roots, installed Windows AppUserModelID notification identity, real target-host OS permission/revocation observations where applicable, installed-package restart/rollback evidence, screen-reader/accessibility acceptance, operator acceptance, promotion and release.
