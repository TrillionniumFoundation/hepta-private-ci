...
struct TestFile { path: PathBuf }
impl TestFile {
    fn new(label: &str) -> Self {
        let nonce = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_nanos();
        Self { path: std::env::temp_dir().join(format!(
            "hepta-plasticity-{label}-{}-{nonce}.journal", std::process::id()
        )) }
    }
    fn create(&self) -> std::fs::File {
        OpenOptions::new().create_new(true).read(true).write(true).open(&self.path)
...
impl Drop for TestFile { fn drop(&mut self) { let _ = std::fs::remove_file(&self.path); } }

fn id(value: &str) -> StableId { StableId::new(value.to_owned()).expect("valid id") }
fn digest(value: &str) -> Digest32 { Digest32::of_bytes(value.as_bytes()) }
fn generation(value: u64) -> Generation { Generation::new(value).expect("valid generation") }

fn governed_topology() -> (GovernedTopologyProposalV1, WriterHandoffPlanV1) {
    let handoff = build_writer_handoff_plan_v1(
        id("module:adaptive-head"), id("owner:current"), id("owner:next"), 12, 13,
        digest("source-store"), digest("migration"), digest("rollback"), digest("ack-contract"),
    ...
    let proposal = propose_topology_v2(TopologyProposalRequestV2 {
        ...
        window: ProposalWindowV2 { window_id: id("window:qualification:1"), window_digest: digest("window") },
    ...
    let governed = admit_governed_topology_v1(
        proposal, vec![handoff.clone()], digest("observer-authentication"), digest("evaluator-authentication"),
    ).expect("governed topology");
    (governed, handoff)
}

fn persist_and_reopen(fixture: &TestFile, governed: GovernedTopologyProposalV1) -> GovernedTopologyProposalV1 {
    let scope = digest("topology-registry-scope");
    let anchor = {
        let mut registry = DurableTopologyProposalRegistryV1::bootstrap_empty(fixture.create(), scope, 41, 8)
            .expect("bootstrap registry");
        let append = registry.append(Digest32::ZERO, governed.clone()).expect("append governed topology");
        assert!(!append.authority.grants_any());
        registry.current_anchor().expect("anchor read").expect("anchor exists")
    };
    let reopened = DurableTopologyProposalRegistryV1::reopen_anchored(fixture.open(), scope, 41, 8, anchor)
...
fn pls3_governed_topology_persists_then_accepts_bounded_canary() {
    ...
    let mut controller = StructuralCanaryControllerV1::new(StructuralCanaryPlanV1 {
        ...
        maximum_steps: 2, maximum_regressions: 0, minimum_successful_steps: 2,
    }).expect("canary controller");
    for sequence in 1..=2 {
        let receipt = controller.observe(StructuralCanaryObservationV1 {
            sequence, health_digest: digest(&format!("health:{sequence}")),
            evidence_digest: digest(&format!("evidence:{sequence}")),
            regression_count: 0, safety_violation: false, lineage_mismatch: false, rollback_verified: true,
...
fn pls3_safety_violation_aborts_after_durable_admission() {
    ...
    let mut controller = StructuralCanaryControllerV1::new(StructuralCanaryPlanV1 {
        ...
        writer_handoff_set_digest: governed.handoff_set_digest,
        baseline_health_digest: digest("baseline-health"),
        maximum_steps: 4, maximum_regressions: 0, minimum_successful_steps: 2,
    }).expect("canary controller");
    let aborted = controller.observe(StructuralCanaryObservationV1 {
        sequence: 1, health_digest: digest("unsafe-health"), evidence_digest: digest("unsafe-evidence"),
        regression_count: 0, safety_violation: true, lineage_mismatch: false, rollback_verified: true,
    }).expect("terminal abort receipt");
    assert_eq!(aborted.state, StructuralCanaryStateV1::Aborted);
    assert_eq!(controller.finish().expect("finish preserves abort").state, StructuralCanaryStateV1::Aborted);
}
