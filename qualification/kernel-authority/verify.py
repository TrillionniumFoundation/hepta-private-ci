...
OPERATIONS = {
    "lease_put_replace", "lease_revoke", "lease_verify_final_use",
    "prune_1", "prune_128", "prune_1024", "epoch_rollover",
    "final_use_claim", "final_use_final_verify", "revocation_head_apply",
    "restart_open",
}
FAULTS = {
...
def text(value: Any, label: str) -> str:
    need(isinstance(value, str) and value and value.strip() == value, f"{label}: non-empty string")
...
def sha(value: Any, label: str, length: int) -> str:
    ...
    need(len(value) == length and all(c in "0123456789abcdef" for c in value), f"{label}: lowercase hex")
...
def artifact_path(root: Path, relative: Any) -> Path:
    ...
    need("\\" not in relative and ":" not in relative, f"artifact path: non-canonical {relative!r}")
    parts = relative.split("/")
    need(all(part not in {"", ".", ".."} for part in parts), f"artifact path: unsafe {relative!r}")
    candidate = root
    for part in parts:
        candidate /= part
    ...
    need(resolved.relative_to(root.resolve()).as_posix() == relative, f"artifact path: alias {relative!r}")
    need(candidate.is_file(), f"artifact path: missing file {relative!r}")
    return candidate


def artifact_index(root: Path, rows: Any) -> dict[str, Path]:
    ...
    for index, row in enumerate(rows):
        ...
        need(hashlib.sha256(path.read_bytes()).hexdigest() == sha256(row["sha256"], f"{label}.sha256"),
...
def claim_specs(document: dict[str, Any], commit: str, tree: str) -> list[ReceiptSpec]:
    ...
    add = lambda path, label, kind, data: specs.append(
        ReceiptSpec(text(path, label), label, kind, canonical(data, f"{label}.expected"))
    )
    ...
    exact(clock, {"backendId", "qualificationReceipt", "maxClockUncertaintyMs", "failClosed", "rollbackIndependent"}, "trustedTime")
    clock_backend = text(clock["backendId"], "trustedTime.backendId")
    uncertainty = positive(clock["maxClockUncertaintyMs"], "trustedTime.maxClockUncertaintyMs", MAX_CLOCK_UNCERTAINTY_MS)
    must_true(clock["failClosed"], "trustedTime.failClosed")
    must_true(clock["rollbackIndependent"], "trustedTime.rollbackIndependent")
    add(clock["qualificationReceipt"], "trustedTime.qualificationReceipt", "trusted_time_qualification", {
        "authorityOwnerId": owner, "authorityStateSchema": state_schema,
        "backendId": clock_backend, "maxClockUncertaintyMs": uncertainty,
        "failClosed": True, "rollbackIndependent": True,
    })

    frontier = document["antiRollback"]
    need(isinstance(frontier, dict), "antiRollback: object")
    exact(frontier, {"backendId", "qualificationReceipt", "durableCas", "conflictExclusion", "noGenesisFallback", "restoredSnapshotRejected"}, "antiRollback")
    ...
    add(frontier["qualificationReceipt"], "antiRollback.qualificationReceipt", "anti_rollback_qualification", {
        "authorityOwnerId": owner, "authorityStateSchema": state_schema,
        "backendId": frontier_backend, "durableCas": True,
        "conflictExclusion": True, "noGenesisFallback": True,
        "restoredSnapshotRejected": True,
    })

    dist = document["revocationDistribution"]
    need(isinstance(dist, dict), "revocationDistribution: object")
    exact(dist, {"transportId", "qualificationReceipt", "enrolledNodes", "feedLifetimeMs", "convergenceSlaMs", "scenarios", "allCurrentHeadAcknowledged", "staleFeedFailClosed"}, "revocationDistribution")
    ...
    need(len(set(nodes)) == len(nodes), "revocationDistribution.enrolledNodes: duplicates")
    ...
    must_true(dist["allCurrentHeadAcknowledged"], "revocationDistribution.allCurrentHeadAcknowledged")
    must_true(dist["staleFeedFailClosed"], "revocationDistribution.staleFeedFailClosed")
    add(dist["qualificationReceipt"], "revocationDistribution.qualificationReceipt", "revocation_distribution_qualification", {
        "transportId": transport, "enrolledNodes": nodes,
        "feedLifetimeMs": lifetime, "convergenceSlaMs": sla,
    ...
    for index, scenario in enumerate(scenarios):
        ...
        exact(scenario, {"name", "receipt", "maxDeliveryMs", "maxAckMs", "deliveredNodeCount", "acknowledgedNodeCount"}, label)
        ...
        delivered = nonnegative(scenario["deliveredNodeCount"], f"{label}.deliveredNodeCount")
        acknowledged = nonnegative(scenario["acknowledgedNodeCount"], f"{label}.acknowledgedNodeCount")
        ...
        add(scenario["receipt"], f"{label}.receipt", "revocation_scenario", {
            "transportId": transport, "enrolledNodes": nodes, "name": name,
            "maxDeliveryMs": delivery, "maxAckMs": ack,
            "deliveredNodeCount": delivered, "acknowledgedNodeCount": acknowledged,
        })
    ...
    for index, role in enumerate(custody):
        ...
        exact(role, {"role", "custodyBackendId", "activeKeyIds", "qualificationReceipt", "rotationReceipt", "compromiseReceipt", "historicalAuditRetained", "applicationPrivateKeyExposure"}, label)
        ...
        need(role["applicationPrivateKeyExposure"] == "role_process_only", f"{label}.applicationPrivateKeyExposure")
        common = {"role": name, "custodyBackendId": backend, "activeKeyIds": keys}
        add(role["qualificationReceipt"], f"{label}.qualificationReceipt", "key_custody_qualification", {
            **common, "historicalAuditRetained": True,
            "applicationPrivateKeyExposure": "role_process_only",
        })
        add(role["rotationReceipt"], f"{label}.rotationReceipt", "key_rotation_rehearsal", {
            **common, "stagedOverlapRehearsed": True,
        })
        add(role["compromiseReceipt"], f"{label}.compromiseReceipt", "key_compromise_response", {
            **common, "responseRehearsed": True,
        })
    need(seen_roles == KEY_ROLES, "keyCustody: incomplete roles")

    capacity = document["capacity"]
    need(isinstance(capacity, dict), "capacity: object")
    exact(capacity, {"qualificationReceipt", "measurements", "faultResults", "reserveAlert"}, "capacity")
    ...
    for index, row in enumerate(measurements):
        ...
        need(isinstance(row, dict), f"{label}: object")
        exact(row, {"point", "operation", "receipt", "sampleCount", "p50Ms", "p95Ms", "p99Ms", "latencyBudgetMs", "bytesWritten", "fsyncP99Ms", "peakRssBytes"}, label)
        point = text(row["point"], f"{label}.point")
        operation = text(row["operation"], f"{label}.operation")
        need(point in POINTS and operation in OPERATIONS, f"{label}: unknown point/operation")
        ...
        add(row["receipt"], f"{label}.receipt", "capacity_measurement", {
            "point": point, "operation": operation, "sampleCount": samples,
            "p50Ms": p50, "p95Ms": p95, "p99Ms": p99,
            "latencyBudgetMs": budget, "bytesWritten": written,
            "fsyncP99Ms": fsync, "peakRssBytes": rss,
        })
    required_matrix = {(point, operation) for point in POINTS for operation in OPERATIONS}
    ...
    for index, row in enumerate(fault_rows):
        ...
        need(isinstance(row, dict), f"{label}: object")
        exact(row, {"case", "receipt", "outcome", "indeterminatePreserved", "stateResetAttempted"}, label)
        ...
        add(row["receipt"], f"{label}.receipt", "capacity_fault_result", {
            "case": name, "outcome": outcome,
            "indeterminatePreserved": True, "stateResetAttempted": False,
        })
    ...
    exact(reserve, {"receipt", "hardLimit", "reserveThreshold", "observedRemaining", "triggered"}, "capacity.reserveAlert")
    hard = positive(reserve["hardLimit"], "capacity.reserveAlert.hardLimit")
    threshold = positive(reserve["reserveThreshold"], "capacity.reserveAlert.reserveThreshold")
    remaining = nonnegative(reserve["observedRemaining"], "capacity.reserveAlert.observedRemaining")
    need(threshold < hard and remaining <= threshold, "capacity.reserveAlert: not demonstrated before hard limit")
    must_true(reserve["triggered"], "capacity.reserveAlert.triggered")
    add(reserve["receipt"], "capacity.reserveAlert.receipt", "capacity_reserve_alert", {
        "hardLimit": hard, "reserveThreshold": threshold,
        "observedRemaining": remaining, "triggered": True,
    })
    add(capacity["qualificationReceipt"], "capacity.qualificationReceipt", "capacity_qualification", {
        "points": sorted(POINTS), "operations": sorted(OPERATIONS),
        "measurementCount": len(measurements), "faultCases": sorted(FAULTS),
        "faultResultCount": len(fault_rows), "reserveAlertDemonstrated": True,
    })

    acceptance = document["operatorAcceptance"]
    need(isinstance(acceptance, dict), "operatorAcceptance: object")
    exact(acceptance, {"reviewerId", "receipt", "acceptedCandidateCommit", "acceptedCandidateTree", "accepted"}, "operatorAcceptance")
    reviewer = text(acceptance["reviewerId"], "operatorAcceptance.reviewerId")
    accepted_commit = git_sha(acceptance["acceptedCandidateCommit"], "operatorAcceptance.acceptedCandidateCommit")
    accepted_tree = git_sha(acceptance["acceptedCandidateTree"], "operatorAcceptance.acceptedCandidateTree")
    need(accepted_commit == commit and accepted_tree == tree, "operatorAcceptance: candidate mismatch")
    must_true(acceptance["accepted"], "operatorAcceptance.accepted")
    add(acceptance["receipt"], "operatorAcceptance.receipt", "operator_acceptance", {
        "reviewerId": reviewer, "acceptedCandidateCommit": commit,
        "acceptedCandidateTree": tree, "accepted": True,
    })
    return specs


def validate(document: dict[str, Any], root: Path, expected_commit: str | None, expected_tree: str | None) -> None:
    exact(document, {"schema", "schemaVersion", "candidate", "authority", "trustedTime", "antiRollback", "revocationDistribution", "keyCustody", "capacity", "operatorAcceptance", "artifacts"}, "bundle")
    ...
    for spec in specs:
        ...
        used.add(spec.path)
        receipt = load_json(artifacts[spec.path])
        exact(receipt, {"schema", "schemaVersion", "candidate", "kind", "producerId", "observedAtUnixMs", "synthetic", "data"}, f"{spec.label}.envelope")
        need(receipt["schema"] == RECEIPT_SCHEMA and receipt["schemaVersion"] == RECEIPT_SCHEMA_VERSION, f"{spec.label}: receipt schema")
        ...
        need(git_sha(receipt_candidate["commit"], f"{spec.label}.candidate.commit") == commit, f"{spec.label}: receipt commit mismatch")
        need(git_sha(receipt_candidate["tree"], f"{spec.label}.candidate.tree") == tree, f"{spec.label}: receipt tree mismatch")
        need(text(receipt["kind"], f"{spec.label}.kind") == spec.kind, f"{spec.label}: receipt kind")
        ...
        need(canonical(receipt["data"], f"{spec.label}.data") == spec.data, f"{spec.label}: receipt content mismatch")


def current_identity(expected_sha: str) -> tuple[str, str]:
    commit = subprocess.run(["git", "rev-parse", f"{expected_sha}^{{commit}}"], cwd=ROOT, check=True, text=True, capture_output=True).stdout.strip()
    tree = subprocess.run(["git", "rev-parse", f"{commit}^{{tree}}"], cwd=ROOT, check=True, text=True, capture_output=True).stdout.strip()
...
def fixture(root: Path) -> dict[str, Any]:
    ...
    measurements = [{
        ...
        "sampleCount": 1000, "p50Ms": 10, "p95Ms": 20, "p99Ms": 30,
        "latencyBudgetMs": 40, "bytesWritten": 4096,
        "fsyncP99Ms": 5, "peakRssBytes": 1048576,
    } for point in sorted(POINTS) for operation in sorted(OPERATIONS)]
    fault_rows = []
    for name in sorted(FAULTS):
        outcome = "rollback_rejected" if name == "restart_with_older_local_snapshot" else ("fenced" if name in UNCERTAIN_FAULTS else "reopen_succeeds")
        fault_rows.append({"case": name, "receipt": f"fault-{name}.json", "outcome": outcome, "indeterminatePreserved": True, "stateResetAttempted": False})
    document: dict[str, Any] = {
        "schema": SCHEMA, "schemaVersion": SCHEMA_VERSION,
        "candidate": {"commit": commit, "tree": tree},
        "authority": {"ownerId": "security-authority", "stateSchema": 2},
        "trustedTime": {"backendId": "fixture-clock", "qualificationReceipt": "clock.json", "maxClockUncertaintyMs": 100, "failClosed": True, "rollbackIndependent": True},
        "antiRollback": {"backendId": "fixture-frontier", "qualificationReceipt": "frontier.json", "durableCas": True, "conflictExclusion": True, "noGenesisFallback": True, "restoredSnapshotRejected": True},
        "revocationDistribution": {
            "transportId": "fixture-wire", "qualificationReceipt": "revocation.json",
            "enrolledNodes": nodes, "feedLifetimeMs": 5000, "convergenceSlaMs": 1000,
            "scenarios": [{"name": name, "receipt": f"{name}.json", "maxDeliveryMs": 100, "maxAckMs": 200, "deliveredNodeCount": 2, "acknowledgedNodeCount": 2} for name in sorted(SCENARIOS)],
            "allCurrentHeadAcknowledged": True, "staleFeedFailClosed": True,
        },
        "keyCustody": [{
        ...
        "capacity": {
            "qualificationReceipt": "capacity.json", "measurements": measurements,
            "faultResults": fault_rows,
            "reserveAlert": {"receipt": "reserve-alert.json", "hardLimit": 16384, "reserveThreshold": 1024, "observedRemaining": 1000, "triggered": True},
        },
        "operatorAcceptance": {"reviewerId": "independent-reviewer", "receipt": "operator.json", "acceptedCandidateCommit": commit, "acceptedCandidateTree": tree, "accepted": True},
        "artifacts": [],
    }
    artifacts = []
    for spec in claim_specs(document, commit, tree):
        receipt = {
            "schema": RECEIPT_SCHEMA, "schemaVersion": RECEIPT_SCHEMA_VERSION,
            "candidate": {"commit": commit, "tree": tree}, "kind": spec.kind,
            "producerId": "fixture-independent-producer",
            "observedAtUnixMs": 1900000000000, "synthetic": False,
            "data": spec.data,
        }
        path = root / spec.path
        write_json(path, receipt)
        artifacts.append({"path": spec.path, "sha256": hashlib.sha256(path.read_bytes()).hexdigest()})
    document["artifacts"] = artifacts
    return document


def mutate_receipt(document: dict[str, Any], root: Path, name: str, mutate: Callable[[dict[str, Any]], None]) -> None:
...
def self_test() -> int:
    cases: list[tuple[str, Callable[[dict[str, Any], Path], None]]] = [
        ("clock_not_fail_closed", lambda d, _: d["trustedTime"].__setitem__("failClosed", False)),
        ("rollback_not_rejected", lambda d, _: d["antiRollback"].__setitem__("restoredSnapshotRejected", False)),
        ("duplicate_node", lambda d, _: d["revocationDistribution"]["enrolledNodes"].append("node-a")),
        ("missing_scenario", lambda d, _: d["revocationDistribution"].__setitem__("scenarios", d["revocationDistribution"]["scenarios"][:-1])),
        ("key_exposure", lambda d, _: d["keyCustody"][0].__setitem__("applicationPrivateKeyExposure", "general_application")),
        ("ack_exceeds_sla", lambda d, _: d["revocationDistribution"]["scenarios"][0].__setitem__("maxAckMs", 1001)),
        ("ack_before_delivery", lambda d, _: (d["revocationDistribution"]["scenarios"][0].__setitem__("maxDeliveryMs", 300), d["revocationDistribution"]["scenarios"][0].__setitem__("maxAckMs", 200))),
        ("missing_measurement", lambda d, _: d["capacity"].__setitem__("measurements", d["capacity"]["measurements"][:-1])),
        ("p99_exceeds_budget", lambda d, _: d["capacity"]["measurements"][0].__setitem__("p99Ms", 41)),
        ("lost_indeterminate", lambda d, _: d["capacity"]["faultResults"][0].__setitem__("indeterminatePreserved", False)),
        ("reserve_not_exercised", lambda d, _: d["capacity"]["reserveAlert"].__setitem__("observedRemaining", 2000)),
        ("operator_tree_drift", lambda d, _: d["operatorAcceptance"].__setitem__("acceptedCandidateTree", "c" * 40)),
        ("synthetic_receipt", lambda d, r: mutate_receipt(d, r, "clock.json", lambda x: x.__setitem__("synthetic", True))),
        ("receipt_candidate_drift", lambda d, r: mutate_receipt(d, r, "frontier.json", lambda x: x["candidate"].__setitem__("tree", "c" * 40))),
        ("receipt_kind_drift", lambda d, r: mutate_receipt(d, r, "normal.json", lambda x: x.__setitem__("kind", "revocation_distribution_qualification"))),
        ("missing_measurement_content", lambda d, r: mutate_receipt(d, r, d["capacity"]["measurements"][0]["receipt"], lambda x: x.__setitem__("data", {}))),
        ("measurement_content_mismatch", lambda d, r: mutate_receipt(d, r, d["capacity"]["measurements"][0]["receipt"], lambda x: x["data"].__setitem__("p99Ms", 29))),
        ("receipt_reuse", lambda d, _: d["capacity"]["measurements"][1].__setitem__("receipt", d["capacity"]["measurements"][0]["receipt"])),
        ("artifact_substitution", lambda d, _: d["artifacts"][0].__setitem__("sha256", "d" * 64)),
    ]
    ...
    print(json.dumps({"status": "PASS_KERNEL_AUTHORITY_PRODUCTION_EVIDENCE_SELF_TEST", "receiptContentValidated": True, "activationGranted": False}, sort_keys=True))
    return 0
...
def verify(path: Path, expected_sha: str) -> int:
    ...
    print(json.dumps({
        ...
        "evidenceAdmitted": True, "receiptContentValidated": True,
...
def main() -> int:
    ...
    return self_test() if args.command == "self-test" else verify(args.evidence, args.expected_sha)
