...
def execute(nonce,*,config:Config,now=None,clock:Callable[[],datetime]|None=None):
    ...
    try:
        ...
        if started and not terminalized and not terminalization_attempted and all(value is not None for value in (a,state,target,ads,hs,ts)):
            terminalization_attempted=True; failure=f"post_start_{stage}_failed"; fallback_run=run or {"exit_code":-1,"stdout":b"","stderr":b"","leader_reaped":False,"cleanup_confirmed":False,"escaped_descendants_absence_proven":False,"target_contact_performed":contact_may_have_occurred}; result=_result_record(a,target,ads,hs,ts,fallback_run,failure,clock)
