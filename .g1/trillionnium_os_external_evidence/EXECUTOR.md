...
executable, module, target, configuration or output path. The executor derives:
...
role separation, authorization expiry, fixed harness and target-attestation
hashes, target custodian and environment class. It persists a distinct
`STARTED_NO_AUTOMATIC_RETRY` marker before invoking the descriptor-bound harness
in a private empty working directory with a fixed environment.
...
only `CAPTURE_COMPLETE_PENDING_INDEPENDENT_REVIEW`; it does not change a gap.
