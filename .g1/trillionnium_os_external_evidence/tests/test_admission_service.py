...
class AdmissionServiceTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        subprocess.run(
            ["/usr/bin/openssl", "genpkey", "-algorithm", "RSA", "-pkeyopt", "rsa_keygen_bits:2048", "-out", cls.private_key],
            check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
        )
        subprocess.run(
            ["/usr/bin/openssl", "pkey", "-in", cls.private_key, "-pubout", "-out", cls.public_key],
            check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
        )
    ...
    def request(self, kind: str = "installed_root_linux_process_matrix") -> dict:
        policies = {
            "installed_root_linux_process_matrix": ("L2", "owner-open-r5-l2", "TARGET-0001"),
    ...
    def policy(self) -> dict:
        return {
            ...
            "grant_public_key_sha256": hashlib.sha256(self.pub.read_bytes()).hexdigest(),
    ...
    def grant(self, request: dict, request_raw: bytes) -> dict:
        if request["evidence_kind"] == "destructive_fault_matrix":
            roles = {
                "producer": "capture-producer", "fault_operator": "fault-operator",
                "destructive_authorizer": "destructive-authorizer", "admission_issuer": "external-admission",
            }
            authorization_class = "DESTRUCTIVE_CAPTURE"
        else:
            roles = {
                "producer": "capture-producer", "target_operator": "target-operator",
                "admission_issuer": "external-admission",
            }
            authorization_class = "TARGET_CAPTURE"
        mirrored = {key: request[key] for key in (
            "repository", "source_commit", "source_tree", "promotion_pr_number", "promotion_pr_head",
            "evidence_kind", "evidence_level", "external_lane", "authorization_nonce",
            "authorization_ticket", "authorization_expires_at",
        )}
        return {
            "schema": "org.trillionnium.external-evidence-execution-grant.v1",
            "version": "1", "status": "AUTHORIZED", "grant_id": "b" * 32,
            "request_sha256": hashlib.sha256(request_raw).hexdigest(), **mirrored,
            "requester": request["requested_by"], "roles": roles,
            "issuer": "external-admission", "key_id": "grant-key-20260907",
            "issued_at": self.stamp(NOW - timedelta(minutes=1)),
            "expires_at": self.stamp(NOW + timedelta(minutes=30)),
            "authorization_class": authorization_class,
            "harness_sha256": "c" * 64, "target_attestation_sha256": "d" * 64,
    ...
    def prepare(self, request: dict | None = None, mutate_grant=None) -> None:
        ...
        self.write(self.policy_path, (json.dumps(self.policy(), sort_keys=True) + "\n").encode())
        self.write(self.request_path, request_raw)
        self.write(self.grant_path, grant_raw)
        subprocess.run(
            ["/usr/bin/openssl", "dgst", "-sha256", "-sign", self.private_key, "-out", self.signature_path, self.grant_path],
            check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
        )
    ...
    def test_admits_once_without_target_authority(self) -> None:
        self.prepare()
        result = admit(self.request_path, self.grant_path, self.signature_path, config=self.config, now=NOW)
    ...
    def test_rejects_signed_cross_splice(self) -> None:
        self.prepare(mutate_grant=lambda grant: grant.__setitem__("source_tree", "f" * 40))
        with self.assertRaisesRegex(AdmissionError, "cross-splice"):
            admit(self.request_path, self.grant_path, self.signature_path, config=self.config, now=NOW)

    def test_rejects_unseparated_external_roles(self) -> None:
        self.prepare(mutate_grant=lambda grant: grant["roles"].__setitem__("target_operator", "capture-producer"))
