...
class TrustedExecutorTest(unittest.TestCase):
    ...
    def prepare(self, *, exit_code=0, output_bytes=0, sleep_seconds=0, expiry_seconds=1800, manifest_extra=None, bundle_fifo=False, close_output=False, delayed_marker=False, escape_descendant=False, mutate_admission=None, mutate_policy=None, symlink_harness=False):
        ...
        script="#!/bin/sh\nset -eu\n"
        if close_output: script+="exec 1>&- 2>&-\n"
        if sleep_seconds: script+=f"sleep {sleep_seconds}\n"
        if delayed_marker: script+='touch "$TMPDIR/delayed-marker"\n'
    ...
    def test_closed_output_pipes_do_not_bypass_authority_expiry(self):
        self.prepare(close_output=True,sleep_seconds=.7,expiry_seconds=.2,delayed_marker=True); started=time.monotonic(); clock=lambda:NOW+timedelta(seconds=time.monotonic()-started)
    ...
    def test_bundle_walk_errors_fail_closed(self):
        bundle=self.root/"bundle-walk-error"; bundle.mkdir(mode=0o700)
        def broken_walk(top,topdown=True,onerror=None,followlinks=False):
            self.assertIsNotNone(onerror); onerror(PermissionError("blocked subtree")); return []
        with mock.patch.object(trusted_executor.os,"walk",broken_walk):
            with self.assertRaisesRegex(ExecutionError,"bundle traversal failed"): trusted_executor.inspect_bundle(bundle,os.getuid(),{"max_bundle_files":32,"max_bundle_bytes":1024},{},"target")

    def _assert_post_start_fault(self,patcher,expected_failure):
        with patcher:
            with self.assertRaisesRegex(ExecutionError,expected_failure): execute(NONCE,config=self.config,now=NOW)
        result=self.result(); self.assertEqual(result["status"],"CAPTURE_FAILED_NO_RETRY"); self.assertEqual(result["failure"],expected_failure); self.assertTrue((self.state/"started"/f"{NONCE}.json").exists())
        with self.assertRaises(ReplayError): execute(NONCE,config=self.config,now=NOW)

    def test_post_start_mkdir_fsync_popen_and_worker_spawn_failures_are_terminalized(self):
        self.prepare(); original_mkdir=trusted_executor.os.mkdir
        def fail_work_mkdir(path,mode=0o777,*,dir_fd=None):
            if Path(path)==self.state/"work"/NONCE: raise OSError("injected work mkdir failure")
            return original_mkdir(path,mode) if dir_fd is None else original_mkdir(path,mode,dir_fd=dir_fd)
        self._assert_post_start_fault(mock.patch.object(trusted_executor.os,"mkdir",fail_work_mkdir),"post_start_work_directory_failed")

        self.tearDown(); self.setUp(); self.prepare(); original_fsync=trusted_executor.fsync_dir
        def fail_work_fsync(path):
            if Path(path)==self.state/"work": raise OSError("injected work fsync failure")
            return original_fsync(path)
        self._assert_post_start_fault(mock.patch.object(trusted_executor,"fsync_dir",fail_work_fsync),"post_start_work_directory_failed")

        self.tearDown(); self.setUp(); self.prepare()
        self._assert_post_start_fault(mock.patch.object(trusted_executor.subprocess,"Popen",side_effect=OSError("injected Popen failure")),"post_start_harness_execution_failed")

        self.tearDown(); self.setUp(); self.prepare()
        self._assert_post_start_fault(mock.patch.object(trusted_executor.selectors,"DefaultSelector",side_effect=OSError("injected selector failure")),"post_start_harness_execution_failed")
        ...
        self.tearDown(); self.setUp(); self.prepare()
        self._assert_post_start_fault(mock.patch.object(multiprocessing.context.SpawnProcess,"start",side_effect=OSError("injected worker spawn failure")),"post_start_bundle_inspection_failed")

    def test_result_persistence_uses_fallback_and_never_overwrites(self):
        self.prepare(); original_raw=trusted_executor.write_raw_once
        def fail_primary(directory,name,raw):
            if Path(directory)==self.state/"results": raise OSError("injected result medium failure")
            return original_raw(directory,name,raw)
        with mock.patch.object(trusted_executor,"write_raw_once",fail_primary):
            with self.assertRaisesRegex(ExecutionError,"fallback no-retry receipt recorded"): execute(NONCE,config=self.config,now=NOW)
        self.assertFalse((self.state/"results"/f"{NONCE}.json").exists()); fallback=json.loads((self.state/"terminal_failures"/f"{NONCE}.json").read_text()); self.assertEqual(fallback["status"],"TERMINAL_RESULT_PERSISTENCE_FAILED_NO_RETRY"); self.assertEqual(fallback["intended_status"],"CAPTURE_COMPLETE_PENDING_INDEPENDENT_REVIEW")
        with self.assertRaises(ReplayError): execute(NONCE,config=self.config,now=NOW)

        self.tearDown(); self.setUp(); self.prepare(); original_raw=trusted_executor.write_raw_once; original_write=trusted_executor.write_once
        def fail_both_raw(directory,name,raw):
            if Path(directory)==self.state/"results": raise OSError("injected result medium failure")
            return original_raw(directory,name,raw)
        def fail_fallback(directory,name,value):
            if Path(directory)==self.state/"terminal_failures": raise OSError("injected fallback medium failure")
            return original_write(directory,name,value)
        with mock.patch.object(trusted_executor,"write_raw_once",fail_both_raw), mock.patch.object(trusted_executor,"write_once",fail_fallback):
            with self.assertRaisesRegex(ExecutionError,"durable STARTED_NO_AUTOMATIC_RETRY remains authoritative"): execute(NONCE,config=self.config,now=NOW)
        self.assertTrue((self.state/"started"/f"{NONCE}.json").exists()); self.assertFalse((self.state/"results"/f"{NONCE}.json").exists()); self.assertFalse((self.state/"terminal_failures"/f"{NONCE}.json").exists())
        with self.assertRaises(ReplayError): execute(NONCE,config=self.config,now=NOW)
