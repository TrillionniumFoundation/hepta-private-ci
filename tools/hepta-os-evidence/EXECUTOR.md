...
The executor now requires `/proc` to describe its own PID namespace: self PID,
the named PID entry and directory identity must agree with `getpid()`. A host
procfs visible inside a different PID namespace can alias an unrelated process;
such a deployment rejects before reading policy, consuming a nonce or sending a
signal. The check repeats during descendant scans. Mount procfs for the private
executor PID namespace during independent provisioning; the source does not
perform mounts or broaden privileges. Native execution regressions explicitly
skip when this prerequisite is absent, while namespace rejection and admission
signature tests remain executable. A skipped native test is not containment
qualification or target evidence.
