...
def canonical_repo_path(raw: str) -> str:
    """Return one platform-independent canonical repository-relative path."""
...
def canonical_paths(values: Iterable[str], *, limit: int = MAX_PATHS) -> tuple[str, ...]:
    raw = bounded_tuple(values, limit, "path_limit_exceeded")
    result = tuple(sorted({canonical_repo_path(value) for value in raw}))
    if not result:
        _error("empty_paths")
    return result
...
def paths_overlap(left: str, right: str) -> bool:
    a = canonical_repo_path(left)
    b = canonical_repo_path(right)
...
def path_is_within(path: str, roots: Sequence[str]) -> bool:
    ...
    return any(
        value == canonical_repo_path(root)
        or value.startswith(canonical_repo_path(root) + "/")
        for root in roots
    )
