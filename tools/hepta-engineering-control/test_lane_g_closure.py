...
class PersistedDecisionClosureTests(unittest.TestCase):
    def test_schema_v10_and_rejection_survive_reopen(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with EngineeringStore(database) as reopened:
                self.assertEqual(tuple(reopened.connection.execute("SELECT * FROM integration_decisions").fetchone()), first)

    def test_raw_eligibility_cannot_bypass_sealed_composition(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            with EngineeringStore(Path(temporary) / "engineering.sqlite3") as store:
                with self.assertRaisesRegex(EngineeringError, "sealed_evidence_required"):
                    store.record_integration_decision("decision-a", "e" * 64, True, (), now_ns=100)
                self.assertEqual(store.connection.execute("SELECT COUNT(*) FROM integration_decisions").fetchone()[0], 0)
