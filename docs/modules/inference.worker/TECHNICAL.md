...
The native App Server worker uses the durable `inference.control` owner for local-slot admission, dispatch identity, cancellation intent and observed settlement. Provider dispatch binds a stable client user-message identity and canonical input digest; restart recovery uses Core `thread/queue/reconcile` in `ReconcileOnly` mode to recover the exact original turn or prove that no durable admission occurred, and never submits a replacement turn. Recovered terminal turns wait under the bounded App Server RPC deadline for the exact turn's persisted token-usage replay; missing authoritative usage remains unknown rather than becoming zero. The hosted source composition root is `hepta-inference-runtime-host`; deployment remains a separate evidence gate. The local path includes `LocalProcessDriver`, which digest-pins the runtime executable, weights, tokenizer, preprocessor, quantization and device descriptor, requires an explicit runtime memory-reservation/load handshake, re-checks artifacts before the model becomes usable, and enforces both aggregate worker memory reservations and per-run response deadlines. `InferenceWorker` accepts only `VerifiedResourceGrant`; `FinalUseResourceGrantVerifier` now provides a concrete kernel-backed path that binds the complete resource grant to one exact worker identity/generation, checks current `FinalUseAuthority` epoch/revocation and consumes a durable single-use nonce. The authenticated worker subject is retained in the verification receipt and exact-matched again by `InferenceWorker::new`, preventing cross-worker capability transfer; `VerifiedResourceGrant` is move-only so the successful authority claim cannot be copied into multiple worker instances. The verified grant remains a bounded creation snapshot, but the named long-lived `hepta-local-inference-runtime-host` now keeps the shared `FinalUseAuthority` live: `LocalWorkerHost` revalidates before admission and during execution, consumes concurrent revocation updates, cancels and fences the worker generation on authority loss, and unloads resident models. Local runtime cancellation is a bounded protocol rather than a best-effort flag: a matching terminal cancel acknowledgement is required; deadline/disconnect falls back to kill/wait with an indeterminate outcome. A trusted exact-turn usage reconciliation port can durably refine previously unknown usage while preserving replay safety. Deployment of the issuer/revocation feed, authoritative billing evidence, real CPU/GPU placement, cgroup/device enforcement and target-host isolation remain external qualification gates. The [native host guide](../../readiness/LANE_B_NATIVE_HOST.md#durable-inference-journal) specifies journal limits and recovery semantics.
