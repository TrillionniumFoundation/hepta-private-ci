...
| revocation freshness / convergence | signed FinalUse feed V2 + opaque local-apply receipt + node ack bound to that exact receipt/update + convergence verifier requiring pinned feed verifier and signed distributor update | Bao source host; external fanout supplies envelopes/acks | in-flight-expiry TLS; wrong-receipt, stale/future-ack and forged-distributor convergence tests; report records distributor/node trust-key IDs | authenticated apply→ack→convergence chain implemented; transport/latency SLA external |
