...
| revocation freshness / convergence | signed FinalUse feed V2 + pre-dispatch/final-consumer Bao freshness gates + convergence verifier requiring the pinned feed verifier, signed distributor update and node-signed acks | Bao source host; external fanout supplies envelopes/acks | in-flight-expiry TLS test; stale/future-ack and forged-distributor convergence tests; report records distributor/node trust-key IDs | end-to-end authenticated missing-node proof implemented; transport/latency SLA external |
