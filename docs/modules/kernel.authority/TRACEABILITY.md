...
| revocation freshness / convergence | signed FinalUse feed V2 + pre-dispatch and final-consumer Bao freshness gates + node-signed `FinalUseRevocationAck` convergence verifier | Bao source host; external fanout supplies envelopes/acks | in-flight-expiry TLS test; stale/future-ack control tests | fail-closed partition/catch-up and cryptographic missing-node proof implemented; transport/latency SLA external |
