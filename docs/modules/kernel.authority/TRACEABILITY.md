...
| --- | --- | --- | --- | --- |
| `authority_lease` | `AuthorityLeaseRegistry` admin + `AuthorityLeaseVerifier` | none selected | exact-predecessor put/revoke retry, stale-token, prune/frontier unit tests; B4 inventory | native owner implemented; composition pending |
...
| `ModulePort::kernel.authority::runtime.fleet` | `FleetRevocationCoordinator` composes signed feed + closed enrolled-node acknowledgement verification into fleet admission/convergence state; generic lease consumption remains separate | `codex-rs/hepta-fleet/src/revocation_control.rs` is a non-test source-composed fleet control-plane caller | coordinator catch-up/quarantine/stale-feed tests; B4 classifies convergence verifier use | revocation control-plane composition implemented; wire fanout and deployed process activation external |
