...
| `VerifiedUseTokenWitnessV1` | serializable `VerifiedUseTokenWitnessV1` emitted by `deliver_final_use_with_witness`, `dispatch_final_use_with_witness` and `deliver_authority_lease_with_witness`; opaque tokens remain authorization-only | no current product caller requests serialized witness; Bao and fleet continue to use opaque authority paths | strict round-trip/unknown-field tests plus FinalUse and lease entry-binding tests; B4 inventories every witness-producing boundary | typed/canonical-JSON evidence protocol source-implemented; authorityDelta remains none; product witness consumption pending |
...
| `ModulePort::kernel.authority::browser.servo` | `BrowserServoPort` uses canonical `claim_final_use` + `dispatch_final_use`; Browser emits authority challenge then durable dispatch boundary | `hepta-agentd-browser` / `BrowserServoPort::call` | Agentd Browser tests + Browser protocol/runtime regressions + B4 closed caller inventory | named non-test product source path composed; exact-candidate execution/activation pending |
...
| trusted time | `AuthorityClock` bound into FinalUse and general lease owners | host-supplied implementation required | injected-clock tests + `qualification/kernel-authority/verify.py` production-bundle admission | interface enforced; attested production source external |
| anti-rollback | `AuthorityFrontierStore` CAS, `FinalUseFrontier`, `AuthorityLeaseFrontier` | host-supplied implementation required | restored-snapshot tests + content-addressed external receipt admission | protocol enforced; external durable backend external |
...
| key rotation | bounded epoch-window `FinalUseIssuerTrustKey` ring for grants plus `FinalUseTrustKey` rings for approval/feed; issuer trust-set digest pinned in FinalUse store V2 | host configuration | overlap/retirement tests; production bundle requires custody, staged-rotation and compromise-response receipts for all three roles | protocol implemented; HSM/KMS ceremony external |
| capacity / GC | bounded stores; lease `prune_expired_leases` preserves store-schema-V2 retired revision lineage; explicit epoch rollover is the only lineage reset | authority owner | capacity/prune/reopen tests; production bundle requires all load points and crash/fault cases from `CAPACITY_QUALIFICATION.md` | online GC implemented without same-epoch lease-id replay; target-host measurements external; FinalUse nonce history still epoch-bounded |
| no-bypass proof | `CALLERS.toml` + `KERNEL_AUTHORITY_BOUNDARIES.json` | repository scanner names Bao/Fleet/Agentd Browser canonical callers and requires zero product callers of raw claim/delivery/dispatch APIs | B4 closed-world test | repository-controlled |
| activation / release | none | none | implementation map / external acceptance | false until separate gates pass |

## Reading rule

A row is source-composed when it names a non-test product caller. It is execution-proved only when
