...
| capacity / GC | bounded stores; lease `prune_expired_leases`; explicit epoch rollover | authority owner | capacity/prune tests; production bundle requires all load points and crash/fault cases from `CAPACITY_QUALIFICATION.md` | lease online GC implemented; target-host measurements external; FinalUse nonce history still epoch-bounded |
