...
The signed Objective product ingress also admits and recovers runs through the same `AgentdState` `RunLedger`; it does not instantiate a second `AgentRunCoordinator`. Durable Objective/learning facts remain with their canonical ledger owner while Agentd retains only bounded lifecycle/reconciliation metadata.
