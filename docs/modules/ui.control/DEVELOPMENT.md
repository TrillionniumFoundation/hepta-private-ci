...
- cross-session/origin provenance cannot settle a pending operation, in-flight acknowledgement provenance remains bound across close/reconnect races, reconciliation cannot overtake the original mutation dispatch, and close cannot re-arm or start reconciliation while the client is shutting down;
