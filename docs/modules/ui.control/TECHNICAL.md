...
The repository's existing `codex-hepta-native-gateway --serve-ui` surface is not that adapter: its stable live-shell contract is deliberately loopback-only/read-only and rejects mutation POSTs. `ui.control` must not gain authority by silently widening that gateway; a mutation-capable host needs a separately admitted authenticated composition that consumes current final-use authority at the actual owner/effect boundary.
...
A transport exception after submission does not erase the local operation and does not imply backend cancellation. The operation becomes `indeterminate`. Reconnect preserves the operation ID, semantic digest and immutable origin provenance and calls the reconciliation boundary; it does not blindly re-submit the mutation. In-flight acknowledgements are checked against that captured provenance even if close/reconnect changes the current session while the request is awaiting I/O. Pending entries whose original mutation dispatch is still in flight are excluded from both direct and automatic reconciliation, preventing reconnect from racing terminal observation ahead of the request acknowledgement. Closing suppresses new reconciliation scheduling/manual batches, and browser offline handling pauses the automatic reconciliation timer so transport loss does not burn the retry ceiling. Reconciliation is dispatched in bounded batches of at most 8 concurrent read-only queries, backs off from 1 s to 60 s and stops automatic attempts after 64 failures/absences or 24 h, marking `recoveryRequired` for explicit read-only operator reconciliation. Implausible future durable-clock metadata also restores as `recoveryRequired`. Only a provenance-valid registered terminal status accompanied by `terminalObserved: true` removes pending work.
