...
The source candidate is checked by `.github/workflows/hepta-consolidated-source.yml`, including closed-world inventory, package tests, all-target compilation, strict Clippy and clean tracked state. `learning.eval` now also source-implements its owned `NduConvergenceCertificateV1`: issuance rejects self-evaluation, applies frozen residual/conservation/spectral-radius thresholds, treats missing independent support as unavailable, and emits accepted/rejected/unavailable decisions. Consumer validation rechecks the canonical certificate digest and frozen subject/objective-class/solver/initialization context before an accepted digest can feed NDU stochastic admission. Cross-owner composition is exercised in `hepta-shadow-qualification`; real conditional-identification/well-posedness data and independent acceptance remain evidence gates. This receipt is source implementation evidence only. It grants no runtime, production-writer, model-provider, external-effect, independent-acceptance, selection, promotion, merge or release authority.
