...
**Production API authority:** [`codex-rs/hepta-intelligence-eval/PRODUCTION_CONTRACT.md`](../../../codex-rs/hepta-intelligence-eval/PRODUCTION_CONTRACT.md) is the normative source for production admission. Direct `decide_independently*` entry points are trusted-only deterministic cores; external qualification requires signed V2 evidence and external longitudinal claims require signed V3 observed-time evidence. The lightweight legacy evaluator is crate-private.
