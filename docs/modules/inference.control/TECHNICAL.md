...
The native worker opens `DurableInferenceControl` with an absolute private journal, stable request ID and explicit in-flight budget. In the committed implementation, `open` locks the journal before replay and retains the locked `File` for the full owner lifetime. A second independently opened owner fails with `WriterUnavailable` until the first is dropped. Provider/model execution does not release that lock. Use one retained owner for a journal; this is not a short-transaction, multi-handle refresh protocol.

Each ordinary mutation stages only its affected record, appends and syncs before publication, and preserves unrelated history allocations. Native admission/dispatch currently uses a fixed 16 MiB remaining-space threshold. This is not a per-request proof that every admitted maximum-size terminal output can still be recorded. The committed owner has no checkpoint compaction or content-addressed archive publication. Those are implementation and crash-qualification work, not an available operator recovery procedure. Do not delete the journal, forget terminal identities, or describe an uncommitted prototype as completed capacity recovery.

A possibly dispatched record retains its local slot or indeterminate responsibility across restart; missing usage is not zero. Product-chain qualification must distinguish trusted in-process observations and exact native reconciliation from independent provider acceptance. Economic quota, hardware capacity, a complete standalone control-service lifecycle, safe reclamation and long-horizon storage qualification are separate obligations. The [current implementation detail](../../../qualification/module-execution-dossiers/detail/inference.control.md#8-current-native-implementation) records these boundaries without granting completion or release.
