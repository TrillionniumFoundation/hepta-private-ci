...
- [codex-rs/hepta-infer-worker-host/src/native_app_server_tests.rs](../../../codex-rs/hepta-infer-worker-host/src/native_app_server_tests.rs); named case: `predispatch_context_check_binds_exact_selected_memories`.
...
Documentation completion requires this guide, exact registry references and closed-world validation. Source completion requires code in the declared root and candidate tests. Composition requires a named caller. The owner-observation ranking path has the named caller `hepta-agentd::cognitive_context`, and `hepta-infer-worker-host::native_app_server` consumes that selected context with an immediate pre-dispatch currentness re-read; neither establishes the still-unimplemented HNMF channel/engram pipeline. Qualification requires current exact-candidate evidence. Acceptance, selection, promotion and release are separate externally governed states.
