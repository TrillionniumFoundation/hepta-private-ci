...
The current owner source is `NeuronRuntime` in [codex-rs/hepta-neuron/src/runtime.rs](../../../codex-rs/hepta-neuron/src/runtime.rs). One tick verifies the canonical owner input, obtains an exact inference-control feature receipt through `InferenceControlModelPort`, computes the deterministic sparse successor, durably prepares the complete operation/result, compare-and-appends the sparse journal, validates the committed checkpoint, advances the independently retained `AnchorWitnessStore`, and then durably marks the operation complete. The `HPTNOP01` sidecar freezes the complete runtime configuration and retains the exact output, so restart, first-witness failure and acknowledgement loss reconcile without model reexecution. Missing result history fails closed and is never initialized during recovery. [codex-rs/hepta-agentd/src/neuron_runtime.rs](../../../codex-rs/hepta-agentd/src/neuron_runtime.rs) provides the compiled Agentd-owned long-lived source boundary and [codex-rs/hepta-intelligence/src/neuron_runtime.rs](../../../codex-rs/hepta-intelligence/src/neuron_runtime.rs) remains a typed caller. The Agentd daemon startup/run-lifecycle owner is still composed on the separate `runtime.agentd` convergence line; source presence here is not daemon activation or product-execution evidence.
