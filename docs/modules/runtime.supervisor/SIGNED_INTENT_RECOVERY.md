...
`resolve` requires the operator to repeat the durable grant digest, the old epoch's control-revision successor (exactly `intent.expected_control_revision + 1`), the current FleetRegistry lifecycle generation, the current release-state generation, and the authority epoch. The current release must exactly match the target for `commit` or the source for `abort`. The lifecycle must be `failed` or `stopped`, and both process leases must already be cleared.
