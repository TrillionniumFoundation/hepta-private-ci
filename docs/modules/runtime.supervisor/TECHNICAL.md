...
The current supervisor serializes one Agent mutation at the owner lock, generation-fences every managed process, and persists three crash-relevant control records below the Agent run root: the exact process lease, the bounded automatic-restart budget, and the independently authorized release-selection/signed-intent pair. The release-selection record is the authoritative production-transition journal: it is fsynced before the execution intent and binds transition, authority epoch, control predecessor/successor, lifecycle generation, exact source/target release provenance, compatibility receipt and revocation frontier. The intent is a recoverable execution witness reconstructed from that journal when a crash occurs between the two writes. Terminal journal repair additionally requires the Fleet durable current release and immutable release bytes to match; journal status alone never manufactures a committed transition. In production-authority mode, unsigned upgrade/rollback RPCs are rejected. Agent drain uses an exact Agentd `Drain` RPC acknowledgement; if supervisord crashes after the durable lifecycle becomes `Draining` but before the RPC, recovery replays the idempotent typed Drain request. The acknowledgement means new Agentd admission is closed, not that every user task succeeded. Unknown in-flight work retains its owner recovery semantics and is never relabelled terminal by the supervisor.
...
Unexpected Agent exits now use a durable bounded restart window with exponential backoff and a fixed attempt ceiling. A process whose lease publication fails remains tracked and hard-kill quarantined until exit is observed; a failed first cleanup signal cannot discard the only process handle. Signed transition recovery distinguishes provable crash cuts: a `Prepared` selection with the original Running lifecycle generation is pre-dispatch and closes as `rolled_back`; a terminal selection whose Fleet current release and immutable bytes match repairs a missing/stale terminal intent; every crossed or contradictory boundary becomes `recovery_required` and quarantines only the affected Agent. The read-only production-mutation status exposes the durable intent digest required by the independently signed recovery decision, while the pre-existing schema-v2 `MutationAccepted` response shape remains unchanged. Recovery must bind observed release bytes, current lifecycle/control fences, authority epoch, the current pinned revocation frontier and the current host-pinned compatibility receipt before the durable selection/intent can become `committed` or `rolled_back`. Fleet release admission also has a durable per-Agent revocation primitive: revoking a release removes and fsyncs its allowance, so every later start/upgrade/explicit rollback/automatic rollback re-resolution rejects that release even when its immutable catalog bytes still exist.
...
A daemon started with the complete production verifier/revocation/compatibility tuple is the production release-transition mode: ordinary `Upgrade` and `Rollback` requests are rejected and only signed variants may enter release transition. The revocation frontier is host-pinned and monotone within one daemon incarnation. The compatibility receipt SHA-256 is separately host-pinned and immutable for that incarnation; a signed grant cannot establish its own compatibility currentness. The native CLI therefore requires `--revocation-frontier N --compatibility-receipt-sha256 SHA256` together with the grant/H7 verifier tuples. Supplying the authoritative latest revocation frontier and compatibility receipt remains the responsibility of the external authority/configuration distributor (or a restart/reconfiguration ceremony). Repository source does not claim that external distribution is deployed.
...
- [codex-rs/hepta-supervisor/src/supervisor_tests.rs](../../../codex-rs/hepta-supervisor/src/supervisor_tests.rs) — lifecycle fencing, typed-drain recovery replay, bounded restart/recovery, release-selection/intent crash cuts, terminal durable-release proof, revocation-frontier monotonicity, matrix-only release changes, launch/lease cleanup quarantine, upgrade/rollback and independently signed recovery ceremony.
...
- [codex-rs/hepta-supervisor/src/signed_authority.rs](../../../codex-rs/hepta-supervisor/src/signed_authority.rs) — exact release-byte/frontier/CAS bindings, stale-compatibility rejection and recovery-decision signature checks.
...
| `load_next` | `pub fn upgrade(` | `codex-rs/hepta-supervisor/src/supervisor.rs` | `codex-rs/hepta-supervisor/src/supervisor_tests.rs`, `release_selection.rs`, `signed_authority.rs` |
...
| `signed_upgrade` | `pub fn apply_production_grant(` | `codex-rs/hepta-supervisor/src/supervisor.rs` | `codex-rs/hepta-supervisor/src/signed_authority.rs`, `release_selection.rs` |
| `signed_rollback` | `pub fn apply_production_grant(` | `codex-rs/hepta-supervisor/src/supervisor.rs` | `codex-rs/hepta-supervisor/src/signed_authority.rs`, `release_selection.rs` |
| `reconcile_signed_intent` | `pub fn resolve_production_recovery(` | `codex-rs/hepta-supervisor/src/supervisor.rs` | `codex-rs/hepta-supervisor/src/signed_authority.rs`, `release_selection.rs` |
