...
Agent drain uses an exact Agentd `Drain` RPC acknowledgement. Agentd closes new App Server admission first, tracks the exact bounded set of running assistant turns, and reports drained only when that set reaches zero under the same spawn/draining generation. A supervisord crash after durable Draining replays the idempotent typed request. Drain acknowledgement is lifecycle evidence, not user-task success; unknown external effects remain with their owning operation/effect reconciler.
