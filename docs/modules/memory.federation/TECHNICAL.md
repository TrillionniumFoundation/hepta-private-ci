...
The named product composition is [`CognitiveRuntime::AvailableFederatedV2`](../../../codex-rs/hepta-memory/src/cognitive_runtime.rs), composed by Agentd and consumed by the Memory extension. Product attachment registration is guarded by `has_product_federation()` and uses the V2-only `retrieve_product_federated` / `revalidate_product_federated` APIs. The local owner adapter resolves the owner-controlled active-database pointer instead of opening the legacy filename by convention, binds that database generation into the V2 generation vector, and rejects any reader whose opened generation is no longer current. `CognitiveRuntime::AvailableFederated`/`FederatedRecallSet` remain explicit compatibility surfaces and cannot satisfy those product APIs. This is still a source/composition fact, not proof of exact-current-head execution, independent acceptance, activation or release. Read the [current native implementation](../../../qualification/module-execution-dossiers/detail/memory.federation.md#8-current-native-implementation) alongside the [module-specific implementation design](../../../qualification/module-execution-dossiers/detail/memory.federation.md) for the exact claim boundary and remaining external gates.
...
- product adapter/aggregator in `hepta-memory::CognitiveRuntime::AvailableFederatedV2`: resolves each candidate owner through the owner-controlled active generation, retains already completed healthy discovery results when another owner is slow or unavailable, starts admitted peer attempts concurrently under one shared request horizon, invokes the canonical V2 boundary once per peer and deterministically aggregates typed coverage;
- owner-generation fence in `FederatedMemoryReader`: checks the active database path before and after capability or evidence reads so recovery publication retires predecessor readers without deleting historical files;
- physical-send revalidator in the Memory extension: rechecks capability, owner generation and exact memory binding immediately before model-input delivery.
...
`memory.federation` owns no database, migration, remote fact, enrollment registry, credential store or durable retry state. The current product adapter opens existing owner cognitive state through the established read-only federation reader and never creates a second memory database. The path is selected by the memory owner's validated `.cognitive-active-v1` generation pointer; a retained predecessor database is historical evidence, not a selectable current owner. Discovery and every subsequent reader revalidation compare the opened path with the current pointer, so recovered-owner revoke, correction or forget operations cannot be bypassed by an old path or an old reader. Durable capability grant/revoke history remains owned by the cognitive store/authority boundary.
...
The engine holds no global lock across I/O and owns no transaction. Product `CognitiveRuntime::AvailableFederatedV2` keeps only bounded owner-layout candidates plus the consumer identity, rediscovers current read-only grants for each physical retrieval, and caps total source slots at the existing federation bound. Discovery is concurrently polled through a bounded completion stream: results that finish before the discovery horizon are retained, while unfinished owners consume explicit failed-discovery slots instead of discarding healthy outcomes. Admitted peer attempts are then polled concurrently with `join_all` under the remainder of the shared request horizon. Deterministic sorting/deduplication occurs before admission and again after results return, so completion order cannot change aggregate candidate order. The current local profile reserves one second of the three-second total product budget for discovery; individual attempts share the request's global horizon rather than each receiving a fresh unbounded timeout.

The local product query binds the active owner-database generation filename together with capability identity, capability generation and revision. Preflight and post-I/O authority rediscovery accept `Current` only when both capability state and owner database generation remain unchanged. Every `FederatedMemoryReader` also compares its opened database with the owner's validated active pointer before and after capability checks. A recovery pointer publication therefore fences an existing reader even if the predecessor file remains intact and internally valid.
...
- an unobservable or discovery-deadline owner contributes a bounded failed discovery slot, while already completed healthy owner discoveries remain eligible and a successfully observed owner with no active matching grant is simply not enrolled;
- owner recovery publication retires predecessor readers immediately: an old database path, old reader, old grant, corrected predecessor revision or forgotten predecessor revision cannot satisfy current discovery or final-use revalidation;
...
The canonical V2 result bound is `MAX_FEDERATED_RESULTS_V2 = 512`. The product caller preserves the existing `MAX_FEDERATION_SOURCES_PER_AGENT = 16` admitted-peer bound; owner-layout enrollment candidates are additionally bounded at 128 before discovery. The current in-process product read uses one two-second bounded federation budget, with a two-second discovery horizon and the remaining shared horizon for concurrent physical attempts, so adding peers does not multiply an unbounded per-peer wall-clock allowance. Discovery completion is incremental: one slow owner becomes failed coverage without erasing healthy results completed inside the budget. Coverage explicitly reports admitted `requested/completed/failed`, peer truncation after discovery, owner candidates omitted before discovery, result truncation, and bounded typed failure classes. Result aggregation is deterministic and final candidate selection is capped again by the existing memory retrieval result bound.
...
### Local in-process qualification profile

The local qualification profile is intentionally narrower than authenticated cross-host federation. It exercises the exact candidate with 128 bounded owner-layout candidates, the 16-source admission ceiling, mixed healthy/corrupt/stalled discovery, recovery-generation cutover, and the 512-item canonical peer bound. Qualification records wall-clock distributions and peak resident memory for the selected development host, plus exact source and command identities. The record must identify concurrent host load and may not be promoted into a deployment SLO. A true cross-host profile remains blocked until a second authenticated host, registered wire schema, peer credential binding, remote cut witness and two-host fault matrix exist.
...
- [codex-rs/hepta-memory/src/cognitive_federation.rs](../../../codex-rs/hepta-memory/src/cognitive_federation.rs).
- [codex-rs/hepta-memory/src/cognitive_store.rs](../../../codex-rs/hepta-memory/src/cognitive_store.rs).
- [codex-rs/hepta-memory/src/cognitive_store_recovery.rs](../../../codex-rs/hepta-memory/src/cognitive_store_recovery.rs).
...
- [codex-rs/hepta-memory/src/cognitive_federation_tests.rs](../../../codex-rs/hepta-memory/src/cognitive_federation_tests.rs), covering current active-owner generation selection, recovered-generation revoke/correct/forget behavior and predecessor-reader fencing.
- [codex-rs/hepta-memory/src/cognitive_runtime_tests.rs](../../../codex-rs/hepta-memory/src/cognitive_runtime_tests.rs), covering product composition, explicit typed discovery failure coverage, retention of healthy results beside a bad or stalled owner, owner-candidate omission, peer truncation and wrong-workspace non-enrollment.
...
- [codex-rs/core/tests/suite/model_provider_policy_http_ephemeral.rs](../../../codex-rs/core/tests/suite/model_provider_policy_http_ephemeral.rs), whose provider-path regression proves a rejecting final-use guard runs after policy admission and before physical HTTP dispatch.
