...
The registered native source spans [https_consumer.rs](../../../codex-rs/hepta-bao-adapter/src/https_consumer.rs), [lease_client.rs](../../../codex-rs/hepta-bao-adapter/src/lease_client.rs), and [lease_registry.rs](../../../codex-rs/hepta-bao-adapter/src/lease_registry.rs). Current entrypoints include `consume_kv_v2`, `request_secret_lease`, `renew_secret_lease`, `revoke_secret_lease`, `reconcile_issue_observation`, `reconcile_renew_observation`, and `reconcile_revoke_observation`. The latter is an authenticated-host reconciliation seam because the currently pinned HeptaBao runtime does not expose its operation-outcome ledger through a direct HTTP endpoint. Read the [current native implementation](../../../qualification/module-execution-dossiers/detail/secrets.heptabao.md#8-current-native-implementation-and-remaining-gates) for exact source/non-claim boundaries.
...
The posture is least authority, bounded input, typed contracts, digest binding and independent evidence. Credentials never enter general logs, learning datasets, prompt factors, durable lease metadata or ordinary cross-module receipts. A trusted synchronous consumer is a privileged host capability: `consumer_id` does not authenticate an arbitrary caller-provided closure. Ordinary KV and dynamic issuance receipts do not export secret-dependent response/value digests. The expected KV digest remains inside the independently signed request and is checked before delivery. Any narrower owner-only diagnostic that intentionally retains a secret-dependent digest treats it as sensitive metadata with explicit retention/keying policy. Zeroization applies to application-owned buffers and is not a claim that TLS/HTTP/parser/allocator/kernel layers never held transient plaintext. Authority is operation-bound, final-payload-bound, short-lived and revocation-aware.

Negative tests cover denied capabilities, cross-owner writes, stale or revoked grants, replay with payload drift, unknown fields, oversize input, scope escape, untrusted instruction escalation and secret/provider leakage. Security review is mandatory for new effect boundaries, persistence, network, model invocation or authority semantics.

## 10. Performance, capacity and hot-path policy

The [module-specific implementation design](../../../qualification/module-execution-dossiers/detail/secrets.heptabao.md) specifies the implemented algorithm and current bounds. Provider responses remain capped at 1 MiB; dynamic issuance accepts at most 64 bounded string fields and a bounded lease duration. Final-use nonce claims use a fixed-record append-only fsynced journal rather than rewriting the complete JSON set, so the former 16,384 claims-per-epoch limit is removed. `authority.json` stays compact and does not serialize the unbounded replay set; `claims.log` is the complete replay truth and may be rewritten only during checkpoint/compaction. The separate revocation-ID set remains bounded. Replay memory/journal bytes still grow with a long-lived epoch, so target-host capacity, restart scale, disk-full behavior and an owner-controlled epoch-rotation policy remain qualification obligations.
...
- [codex-rs/hepta-bao-adapter/src/lease_registry_tests.rs](../../../codex-rs/hepta-bao-adapter/src/lease_registry_tests.rs); covers changed-semantic operation conflicts, durable Unknown recovery, provider lease-identity collision, quarantine-to-revoke recovery, unsafe state paths and rejected-renew rollback.
- [codex-rs/hepta-contracts/src/final_use_tests.rs](../../../codex-rs/hepta-contracts/src/final_use_tests.rs); covers durable replay, revocation/epoch fencing, process locking and compact replay-journal checkpointing.
...
In `codex-rs`, run `just test -p codex-hepta-bao-adapter -p codex-hepta-contracts`. The command is a test invocation, not a stored result. The Lane-A workflow executes the exact PR head and deterministic synthetic merge, then uploads SHA-named machine-readable source/native receipts under `.hepta-evidence/lane-a-*.json`. Inspect those exact-candidate artifacts for passes, failures and skips. The [module-specific implementation design](../../../qualification/module-execution-dossiers/detail/secrets.heptabao.md) separately labels target acceptance designs.
