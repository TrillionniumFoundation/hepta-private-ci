...
| named Agentd/App Server composition | source implemented through `AgentdPromptPipelineOwner` and the existing Codex prompt/provider extension |
| durable provider-attempt/terminal owner | source implemented; pre-send dispatch claim, crash/reopen and indeterminate reconciliation are persisted |
| deployed runtime owner / product activation | not established by source presence or tests |
| terminal real-model payload observation | not proved on a real provider/target host |
...
Migrations are deterministic and checksum-bound. The current source implementation stores a schema-versioned owner snapshot under a single-writer lock, verifies file ownership/mode/link count, writes `registry.next`, fsyncs it, atomically renames it to `registry.json`, and fsyncs the state directory before publishing the new in-process image. Reopen replays lifecycle lineage, validates materialized lifecycle state, record/binding/payload/supersession relationships, active-profile uniqueness, revocation frontier and the whole-registry digest. If rename succeeds but directory fsync fails, the result is `IndeterminateDurability`; the in-process owner is poisoned and rejects authoritative reads or writes until reopen/reconciliation. The v1 compatibility migration preserves lifecycle/revocation state and emits explicit imported-lineage events; migration never resets a revoked factor to an admitted state, and canonical V1 projection fails closed when original semantic/model lineage was absent. Rollback across a schema boundary must restore state compatible with the binary.

Projection domains rebuild from declared sources and publish complete generations atomically. Projections never become sources of truth. Retention and deletion preserve lineage and prevent resurrection through indexes, caches, artifacts or backup restore.

## 7. Runtime, concurrency and transaction model

The [current native implementation](../../../qualification/module-execution-dossiers/detail/prompt.registry.md#8-current-native-implementation) keeps `PromptRegistry` as a deterministic in-memory domain core and makes `DurablePromptRegistry` the authoritative source-level writer. A mutation is applied to a cloned core, durably committed, and only then published to the live process image, so storage failure cannot expose an uncommitted state. The state directory admits one writer through a lock file. The named Agentd composition now opens the durable registry under the agent home and a separate durable prompt-runtime owner under the agent run root. `AgentdPromptPipelineOwner` derives the optimizer candidate adapter from that registry, validates the canonical exercise path through `compile_prompt_registry_v2`, and stages the exact compiled bytes into the same `PromptRuntimeHost` installed in App Server. Immediately before a physical provider send, runtime.codex records the exact provider-request binding through a durable dispatch callback; a crash or acknowledgement loss after that claim but before terminal observation blocks blind retry until reconciliation. These are source-composition facts, not deployed activation or real-provider evidence.
...
- [codex-rs/hepta-agentd/src/prompt_runtime.rs](../../../codex-rs/hepta-agentd/src/prompt_runtime.rs) — named Agentd composition owner plus durable staged/dispatch/terminal state, reopen validation and monotonic indeterminate reconciliation.
- [codex-rs/hepta-codex-adapter/src/runtime_prompt.rs](../../../codex-rs/hepta-codex-adapter/src/runtime_prompt.rs) — existing Codex prompt/provider extension; records the exact physical provider-request digest before send and maps only observed terminal classes into delivery evidence.
...
- [codex-rs/hepta-prompt-registry/src/durable.rs](../../../codex-rs/hepta-prompt-registry/src/durable.rs); unit cases cover schema migration including an older revocation frontier followed by a later non-revocation mutation, unresolved legacy canonical-projection rejection, restart/non-resurrection, concurrent-writer exclusion, storage-full predecessor preservation, truncated/corrupt reopen rejection, failed-migration predecessor preservation, exact backup/restore, post-rename indeterminate-durability poisoning/reopen, final-use nonce/revocation fencing and a 128-factor pilot capacity fixture.
...
- [codex-rs/hepta-agentd/src/prompt_runtime_tests.rs](../../../codex-rs/hepta-agentd/src/prompt_runtime_tests.rs); source-composition cases cover named durable registry → canonical optimizer receipts → context compiler → App Server host staging, dispatch-without-terminal crash/reopen, acknowledgement-loss poisoning/reopen, indeterminate-to-terminal reconciliation, and NotDispatched retry history followed by final delivery.
...
Documentation completion requires this guide, exact registry references and closed-world validation. Source completion requires code in the declared root and candidate tests. Composition requires a named caller. The current source tree contains both `hepta-intelligence::compile_prompt_registry_v2` and a named Agentd/App Server composition through `AgentdPromptPipelineOwner`; the latter opens the durable registry and runtime owners, stages exact exercised bytes into the real Codex prompt/provider extension, and durably claims provider dispatch before the effect boundary. Source composition still does not establish a deployed product invocation, independently observed real-provider delivery, target-host qualification or acceptance. Qualification requires current exact-candidate evidence. Acceptance, selection, promotion and release are separate externally governed states.
...
The source candidate is checked by `.github/workflows/hepta-consolidated-source.yml`, including closed-world inventory, package tests, all-target compilation, strict Clippy and clean tracked state. `IMPLEMENTATION_MAP.sourceBase` is module-local provenance for the last reviewed mapped source commit/tree; exact candidate HEAD and deterministic synthetic-merge identities are supplied by CI. For strict prompt.registry verification, every mapped source/test/delegated evidence path must remain byte-unchanged from that provenance baseline to the candidate HEAD. The source now contains the durable owner, authenticated admission, payload-backed realization delivery, canonical contract codecs and a named source-level composition caller; these facts advance source implementation only. They grant no running-host, model-provider, external-effect, independent-acceptance, selection, promotion, merge or release authority.
