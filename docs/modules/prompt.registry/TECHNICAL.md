...
The registered primary source is [codex-rs/hepta-prompt-registry/src/lib.rs](../../../codex-rs/hepta-prompt-registry/src/lib.rs); observed identifiers include `PromptRegistry`, `PromptFactor`, `PromptRealization`, `RegistryReceipt`, `register_factor`, `admit_factor`. This is a source navigation binding, not proof that every target operation or production consumer exists. Read the [current native implementation](../../../qualification/module-execution-dossiers/detail/prompt.registry.md#8-current-native-implementation) alongside the [module-specific implementation design](../../../qualification/module-execution-dossiers/detail/prompt.registry.md) for the implemented subset and remaining product work.
