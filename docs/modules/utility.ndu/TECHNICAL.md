...
Projection domains rebuild from declared sources and publish complete generations atomically. `NduProjectionStoreV1` is the current durable-writer source candidate: it is single-writer, synchronizes a complete temporary image before rename, synchronizes the parent directory on the Unix qualification profile, poisons an indeterminate handle, permits only monotonic backup restore, wraps the journal in a versioned/checksummed V1 store image, deterministically migrates the predecessor raw-journal image, and enforces a retention policy that reserves revocation capacity and forbids destructive compaction of revocation history. It is not a selected or activated production writer. Projections never become sources of truth. The NDU store retains scoped revocation audit digests to prevent resurrection. It does not own the external dataset/model-parameter payloads named by those digests; physical deletion of those payloads remains with their authoritative source/artifact owners and must be proved by their deletion/unlearning evidence. Backup restore cannot remove a later NDU revocation.
