...
The registered primary source is [codex-rs/hepta-ndu/src/lib.rs](../../../codex-rs/hepta-ndu/src/lib.rs); observed identifiers include `evaluate_candidates`, `solve_preference_target`, `validate_staged_updates`, `evaluate_recursive_utility`, `mul_q32_ties_even`, `admit_covariance_profile`. This is a source navigation binding, not proof that every target operation or production consumer exists. Read the [current native implementation](../../../qualification/module-execution-dossiers/detail/utility.ndu.md#8-current-native-implementation) alongside the [module-specific implementation design](../../../qualification/module-execution-dossiers/detail/utility.ndu.md) for the implemented subset and remaining product work.
