...
Embed the deterministic evaluator under a frozen objective and versioned policy. Agentd's canonical intelligence owner port and the Agentd/Control planning chain are named request-local read-only callers; neither is the authenticated production NDU owner/caller or global adaptive reconfiguration. `NduAuthenticatedOwnerV1` creates evaluation evidence inside the frozen owner context, so private receipt fields bind the original principal, owner, host generation, fence, revocation frontier, policy, objective and generation rather than accepting context attached after evaluation. Projection selection uses an exact expected-predecessor compare-and-set that is also bound into the final-use payload.

`NduProjectionJournalV1` remains the semantic journal, while `NduProjectionStoreV1` is a crash-bounded durable-writer source candidate with exclusive writer locking, complete-image temp write + file sync, atomic rename, Unix parent-directory sync, indeterminate-handle fencing and monotonic backup restore. Store open rejects an oversized image from metadata before allocating or reading the image, and ordinary history reserves one future revocation slot for every live projection. Full-envelope revocation and restart recovery are regression-tested. Neither source existence nor local qualification substitutes for governed production writer selection, host enrollment, retention/off-host backup policy, monitoring or target-host acceptance.
...
### 11.1 Named deterministic process owner

The executable bootstrap is `codex-hepta-agentd --ndu-bootstrap-descriptor <absolute-path> --ndu-bootstrap-descriptor-digest <sha256>`, after normal Fleet process identity has been loaded. The descriptor schema is `hepta.agentd.ndu-bootstrap.v1`; its supported trust profile is explicitly `local-deterministic`. It contains `agent_id`, private canonical `store_root` and `authority_directory`, pinned `authority_signer`/`authority_key`, independently pinned `revocation_distributor`/`revocation_key`, a `revocation_update_path`, and a bounded policy. Descriptor bytes are capped at 64 KiB and compared with the launcher's pinned digest. The policy names every utility direction, utility/uncertainty aggregation, risk/resource ceiling and aggregation, tolerance, required organ and optional scalarization weight. Unknown fields and unregistered enumerations are rejected. See `ndu_process_bootstrap.rs` and the complete construction in `tests/ndu_process_e2e.rs`; do not copy test private keys into a deployment.

The registered owner is attached through `AgentdConfig::with_ndu_owner_host` and `AgentdState::attach_ndu_owner`. `NduControlRequestV1` is carried on the existing bounded, private control socket. `Context` acknowledges the currently installed authenticated revocation view; `Prepare` returns an unsigned final-use binding at an exact journal head; `Apply` requires an externally signed matching grant; `Selection` reads the current projection; `Outcome` returns a historical committed record for response-loss reconciliation. Preparing does not authorize a write. The client never retries an unknown mutation automatically.

The sole writer lock is acquired before the live generation/readiness guard is rechecked. `apply_mutation_at_head_guarded` rechecks that same guard again inside final-use authority at actual mutation entry, after any authority-lock wait. The mutation uses an active final-use effect fence through durable return. At physical entry the host reauthenticates the signed feed and rejects a changed or expired view without writing; authority updates that encounter an entered effect return `DispatchInProgress` and must retry after it returns. Each control operation verifies a fresh independently signed revocation update; an expired, forged, regressed or missing feed fails closed. Installation of that view and final-use admission use the existing authority owner. Revocation linearizes when the authority installs the verified view, not merely when an external publisher starts writing a file. The publisher replaces the feed atomically and uses `Context` to observe installation.

Product mutation binds both the selected-content predecessor and the full journal head. The latter changes even in A-to-B-to-A selection histories and prevents old requests from acquiring a new interpretation. A stale head is rejected before consuming a grant. An already committed result can be queried after restart without treating its old grant or receipt as current authorization. The first owner binding uses a synced temporary file, atomic rename and parent-directory sync; only an empty store may reconcile an interrupted unpublished binding. `owner-binding.v1` freezes the stable principal/scope and policy before accepting mutations; a nonempty legacy store without this binding needs explicit reviewed migration, not automatic adoption. Host generation/fence is rebuilt from the verified Fleet lifecycle while the stable owner/policy binding remains unchanged.

This normal process path is a local deterministic baseline, not a claim of independent protected time or off-host anti-rollback. `FinalUseAuthority::open_state_dir` retains its compatibility trust limits. Production must supply and qualify the protected clock/external frontier profile, governed enrollment, independent evidence and release conditions before activation. Stochastic coefficients, Cell/Circuit changes and external learning gains are not enabled by this endpoint.
...
The source candidate is checked by `.github/workflows/hepta-consolidated-source.yml`, including closed-world inventory, package tests, all-target compilation, strict Clippy and clean tracked state. A passing exact-candidate receipt is source implementation evidence only; a workflow definition is not such a receipt. It grants no runtime, production-writer, model-provider, external-effect, independent-acceptance, selection, promotion, merge or release authority.
