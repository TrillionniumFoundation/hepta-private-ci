...
Embed the deterministic evaluator under a frozen objective, immutable axis-semantics digest and versioned policy. The actual request-local context planner is described in the native host guide; it invokes the owner evaluator but does not activate global adaptive reconfiguration. Stochastic numeric output crosses toward a production-facing consumer only through the owner-local evidence admission that binds coefficient, normalization/runtime/conditioning, Q24 conversion, conditional-identification, well-posedness, independent convergence, rollback, objective-class, expiry and covariance-profile identities. The owner crate also exposes `NduProjectionStoreV1`, a Unix durable-store candidate that holds a kernel exclusive writer lock, validates a schema/checksum envelope on reopen, atomically replaces the bounded journal, fsyncs the file and directory, rolls back in-memory state on persistence failure, and supports verified create-new backup/restore. It is not a production-writer claim until a named host selects it and the target filesystem, restore process and trusted checkpoint anchor are qualified.
