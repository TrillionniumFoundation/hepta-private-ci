...
Store open verifies migration checksums and requires each canonicalized `sqlite_schema` object to match the complete table/trigger/index definition shipped by migration `0011`, rather than accepting a few identifying substrings. It also verifies checkpoint/proof digests and signed witness replay, predecessor continuity, owner identity, and content-addressed payload bytes. Keyword-preserving schema tampering, contract/data corruption or witness substitution fails closed. Rollback never restores deleted payload bytes or expired authority.
...
Read use is also a product boundary. `select_current_compaction_checkpoint` reacquires a fresh authoritative snapshot from the attached provider and requires the durable checkpoint to match the current memory, fact, tombstone, source-ledger, KG, prompt-registry, retrieval, encoder, authority, model, tokenizer, template, tool-schema, memory-snapshot and compatibility cut. It must also resolve the exact non-revoked payload bytes. Before returning bytes, Agentd runs `revalidate_compaction_trust`: the proof's signed tokenizer implementation/attestation/key identity and evaluator id/implementation/attestation/key identity must exactly match the currently enrolled `AgentdCompactionTrustV1`. A cryptographically self-consistent historical proof therefore cannot remain current merely because its old public key is persisted. The same deployment-trust re-admission applies to rollback candidates. There is no legacy read path that treats durable existence alone as currentness.

## 8. Failure semantics, recovery and rollback

Crash-before-commit leaves both the prior checkpoint and prior payload selection unchanged. The owner path has explicit transaction fault cuts after payload insertion and after checkpoint insertion but before commit; each cut rolls the transaction back, and restart/reopen must observe neither a new checkpoint nor a leaked payload. A separate revocation fault cut after tombstone insertion but before commit must reopen with the payload still live. Conversely, if commit succeeded but the caller lost the response, restart plus exact lease/content replay returns `Unchanged` and does not create a second generation. Reopen reconstructs every persisted checkpoint/proof contract, replays the raw evaluator signature witness, and verifies the durable predecessor chain before returning a head. Content-addressed payload corruption is detected by recomputing its digest.
...
Tokenizer accounting is not caller self-report. `CompactionPolicyV2` binds tokenizer semantic identity and implementation digest; `TrustedTokenizerV1` is host-enrolled; every `TokenizationReceiptV1` is Ed25519-signed over subject digest, tokenizer identity/implementation and exact byte/token counts. Candidate identity additionally binds the trusted tokenizer attestation and verification-key digest. Policy tokenizer, snapshot tokenizer and trust-root tokenizer must be identical.

Independent semantic qualification is also cryptographically authenticated. `TrustedCompactionEvaluatorV1` is host-enrolled; `CompactionQualificationV2` signs the exact candidate digest **plus the tokenizer implementation/attestation/key identity**, evaluator implementation/attestation, evaluation artifact, retained-query/reconstruction/contradiction evidence and all pass/fail outcomes. `prove_compaction` first requires those signed tokenizer fields to equal the tokenizer trust that produced the candidate, then verifies the evaluator Ed25519 signature and derives the proof's signature/verification-receipt digests. Persisted witness material can replay that signature but cannot mint current deployment enrollment; Agentd must re-admit it at final use. A non-zero digest or caller boolean is not proof.
...
Negative tests cover resurrection, stale/missing protected support, forged tokenization receipts, tokenizer substitution, payload-byte drift, evaluator-signature tampering, recomputed tokenizer-trust metadata tampering, deployment trust-root rotation at current use, stale current cuts, revoke/GC ordering, transaction fault cuts, lease/fence drift, concurrent generation conflicts and persisted/schema corruption. Public verification keys/signatures may be retained as authority-free durable witnesses; private signing material and credentials never enter general logs, learning artifacts or checkpoint metadata.
...
- `hepta-memory/src/qualified_compact_store_tests.rs`: idempotent publication, committed-response-loss replay after restart, content-addressed payload reopen/resolution, generation/predecessor CAS, concurrent same-generation winner, exact-current-cut selection, revocation-before-GC, rollback-candidate re-admission, real owner-path fault cuts after payload/checkpoint/revocation writes, raw proof-signature replay, recomputed tokenizer-trust tamper rejection, bounded checkpoint-capacity stop, and exact-migration schema/data corruption fail-closed behavior;
- `hepta-agentd/src/production_writer_host_tests.rs`: full authoritative snapshot -> signed tokenizer -> build -> signed evaluator -> prove -> authorized durable publication -> current re-admission -> restart payload-resolution path, plus missing-host-trust/missing-provider rejection and fail-closed evaluator/tokenizer key rotation at current use;
- `hepta-cognitive-types/src/lane_c_tests.rs`: canonical checkpoint/proof contract digest invariants.

Tracked `IMPLEMENTATION_MAP.sourceBase` is either the candidate itself or an exact owner-source observation. For the observation form, every path listed in `observedSourcePaths` must remain byte-identical from `observedAtHead` through candidate HEAD; documentation/projection files may advance afterward so the map can record that immutable observation without self-reference. **Exact-head evidence** is different: the CI execution receipt records the actual candidate commit/tree being compiled and tested. **Synthetic-merge evidence** records the deterministic merge commit/tree. These concepts must not be conflated.
