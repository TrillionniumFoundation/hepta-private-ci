...
The synchronous seam remains available. The async seam uses `FinalUseAuthority::with_verified_use_async_with_witness` plus `ProviderEffectTaskFlowDriver`: automation hashes the exact caller-supplied wire bytes before consuming the grant, requires that digest to equal the durable TaskFlow payload digest, and derives a provider logical-effect key from the final-use-bound destination plus TaskFlow run/step. The local step attempt is deliberately excluded from that provider key, so a new local attempt after provider-proven absence cannot silently create a new external effect identity; a changed payload under the same logical effect becomes a provider key/payload conflict. The canonical non-authorizing authority witness is committed with the immutable dispatch-attempt row before provider contact. Restart lookup re-derives the provider intent from the durable `AuthorizedEffectPending` record rather than accepting a caller-supplied key.

A named source host now exists in `runtime.agentd`: `AgentdAutomationEffectHost` loads a rotating issuer trust ring, independently signed revocation feed, owner-locked external clock/frontier store and attested-provider contract bytes, then drives this exact TaskFlow seam through `HttpProviderEffectAdapter`. `CALLERS.toml` and the independent B4 inventory name those concrete callers. This closes repository source composition, not deployment activation: the selected target still needs independently qualified time/storage, provider attestation/terminal observation, HSM/KMS custody and operator acceptance.
...
Migrations are additive from v3 through v20. Migration v12 adds Calendar V2 history; v13 adds terminal reconciliation after an initial indeterminate external-effect observation; v14 freezes the schedule revision on claimed legacy runs so an in-flight claim cannot float to a later schedule revision; v15 adds append-only reconciliation evidence for legacy dispatch-unknown rows whose historical schedule revision was never frozen; v16 persists the opaque App Server `next_cursor`; v20 adds the immutable canonical FinalUse witness table and no-update/no-delete triggers. Such legacy ambiguity can open a new claim only after an exact provider-side proven-absent receipt, and the retired occurrence/client identity is never reused. A binary that does not understand schema v20 must not replace the current owner against an upgraded store.
...
For external effects, automation computes `AuthorizedEffectIntent` itself over run/step/attempt/operation/subject/destination/payload/final-use-scope/policy-generation/dependency-state/compensation identity. `FinalUseAuthority::claim` durably consumes the signed grant nonce. The synchronous path uses `with_verified_use`; the async path uses an active-effect fence entered after live revalidation and before the provider future is created. No mutex guard is held across `await`. A concurrent trusted revocation update returns `DispatchInProgress`, marks revocation pending and prevents every new claim/entry with `RevocationPending`; the exact monotonic update commits after the bounded provider future completes or is cancelled. The immutable dispatch attempt records the grant ID, authority epoch, nonce digest and canonical authority witness before provider contact. Driver errors are allowed only before provider contact; ambiguous contact or a non-terminal provider acknowledgement returns `Indeterminate` and is later closed only by append-only provider reconciliation.
...
The existing Agentd -> App Server automation activity and Calendar V2 control remain source-composed. Agentd now also has a registered TaskFlow external-effect host: host schema V2 requires independently provisioned issuer/distributor trust, a signed revocation feed, an external authority trust root and exact provider attestation, then uses the canonical HTTP adapter/coordinator. This establishes one real product source caller and durable witness/recovery chain. It does not activate arbitrary external effects or prove the selected target's time, storage, credentials, provider terminal observer or network behavior; those remain target-host qualification and acceptance gates.
...
For this source candidate, the Agentd/Codex causal chain, Calendar V2 owner/control surface, durable TaskFlow step/outcome chain, producer-owned `kernel.operations::OperationIntentV1` composition and FinalUse provider-effect path are present and bounded. The registered Agentd caller binds a concrete issuer/feed/trust-store/provider configuration and persists the canonical authority witness before provider contact. Product completion still requires selected-host execution evidence, qualified clock/frontier and provider observation, authentic/current timezone provenance, HSM/KMS custody, deployment qualification, independent acceptance and activation evidence. Promotion/release remain separate externally governed states.
...
| `external_effect` | `src/authorized_effect.rs`, `src/effect_dispatch_ledger.rs`; Agentd `automation_effect_host.rs` / `authority_trust_host.rs`; `hepta-contracts::FinalUseAuthority` | owner-computed canonical intent; async final-use fence+witness; exact wire-payload digest; provider-stable logical key; immutable attempt/witness/observation/reconciliation; named source host composed, target activation external |
