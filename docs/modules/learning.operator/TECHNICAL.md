...
The registered primary source is [codex-rs/hepta-bellman-operator/src/lib.rs](../../../codex-rs/hepta-bellman-operator/src/lib.rs); observed identifiers include `Transition`, `DatasetSnapshot`, `TrainingRequest`, `BellmanOperatorArtifact`, `train`. This is a source navigation binding, not proof that every target operation or production consumer exists. Read the [current native implementation](../../../qualification/module-execution-dossiers/detail/learning.operator.md#8-current-native-implementation) alongside the [module-specific implementation design](../../../qualification/module-execution-dossiers/detail/learning.operator.md) for the implemented subset and remaining product work.
