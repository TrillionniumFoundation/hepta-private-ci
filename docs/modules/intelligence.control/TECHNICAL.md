...
Owner rejection, unauthenticated/unavailable currentness, key/generation/authority/revocation drift, stage timeout and total timeout all fail before dispatch publication. Abstain and slow-path are explicit terminal advisory outcomes and never fabricate context/evaluation/dispatch receipts. Product Decision/Outcome persistence is not exposed by the runner until it is composed through authenticated `LedgerWriter`; the historical raw-V1 pending replay object exists only in qualification test support and is not a production recovery claim.
