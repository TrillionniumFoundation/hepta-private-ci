...
The registered source root is `codex-rs/hepta-intelligence`. The canonical V3 convergence surfaces are `build_legal_candidates_v1`, `run_composition_v3`, `run_composition_v3_with_control`, and `NativeV3OwnerPorts`; the named runtime product callsite is `AgentRunCoordinator::run_native_intelligence_v3` in `codex-rs/hepta-agentd/src/lane_b_runtime.rs`. V3 binds objective admission, the native legal set, policy-bound NDU, signed independent evaluation, optional sparse Neuron and prompt portfolio stages, calibrated Intuition V2, context compilation, typed Agentd handoff and the durable learning Decision into one predecessor chain and one frozen `CapabilitySnapshotV2`. Every external port input carries the selected capability ID, implementation digest and generation from that snapshot and the returned receipt must echo the exact binding. `run_read_only_vertical` and `run_evaluated_shadow_v1` remain compatibility/qualification surfaces rather than competing facades. This source mapping is not product-execution or deployment evidence. Read the [current native implementation](../../../qualification/module-execution-dossiers/detail/intelligence.control.md#8-current-native-implementation) alongside the [module-specific implementation design](../../../qualification/module-execution-dossiers/detail/intelligence.control.md) for the remaining qualification boundary.
