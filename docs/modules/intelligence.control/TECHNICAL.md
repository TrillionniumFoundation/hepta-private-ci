...
The registered source root is `codex-rs/hepta-intelligence`. The canonical V3 convergence surfaces are `build_legal_candidates_v1`, `run_composition_v3`, `run_composition_v3_with_control`, and `NativeV3OwnerPorts`. Agentd daemon state now owns the canonical run coordinator in `codex-rs/hepta-agentd/src/state.rs` and exposes the named source callsite `AgentdState::run_native_intelligence_v3` through `state_intelligence.rs`, which delegates to `AgentRunCoordinator::run_native_intelligence_v3_with_control`. Ordinary App Server turn admission does not yet invoke this seam, so physical product execution remains pending. V3 binds objective admission, the native legal set, policy-bound NDU, signed independent evaluation, optional sparse Neuron and prompt portfolio stages, calibrated Intuition V2, context compilation, typed Agentd handoff and the durable learning Decision into one predecessor chain and one frozen `CapabilitySnapshotV2`. Every external port input carries the selected capability ID, implementation digest and generation from that snapshot and the returned receipt must echo the exact binding. `run_read_only_vertical`, `run_read_only_vertical_outcome_v1` and `run_evaluated_shadow_v1` remain compatibility/qualification surfaces rather than competing facades. The outcome wrapper preserves explicit objective abstention as a non-error result without changing the legacy ABI. This source mapping is not product-execution or deployment evidence. Read the [current native implementation](../../../qualification/module-execution-dossiers/detail/intelligence.control.md#8-current-native-implementation) alongside the [module-specific implementation design](../../../qualification/module-execution-dossiers/detail/intelligence.control.md) for the remaining qualification boundary.
...
The V3 composition library now has concrete current-owner adapters for objective admission, policy-bound NDU, signed independent evaluation, optional sparse Neuron/prompt optimization, calibrated Intuition V2, context compilation and sealed learning-ledger Decision append. Agentd performs the typed host attach through the existing `AgentRunCoordinator`; immediately before that handoff the registered Agentd V3 source path invokes `CurrentCapabilitySnapshotProviderV3` and requires exact equality with the frozen `CapabilitySnapshotV2`, covering authority epoch, revocation frontier and every implementation generation. Agentd now supplies `AuthenticatedCapabilitySnapshotProviderV3`, which reloads an owner-controlled trust registry and verifies exact owner Ed25519 attestations/current sequences plus authority epoch, body generation, configuration and revocation frontier. Ordinary product bootstrap still has to enroll the real owner-issued attestations and current trust file; intelligence.control cannot self-mint that trust and still does not create a second model/tool/effect loop. Synchronous blocking owner calls cannot be preempted by the coordinator, so each adapter receives an absolute stage deadline and the selected host must enforce cancellation at its own blocking I/O boundary. Unavailable/abstain/slow-path outcomes remain explicit and are never fabricated as success.
...
- [codex-rs/hepta-intelligence/src/capability_snapshot_tests.rs](../../../codex-rs/hepta-intelligence/src/capability_snapshot_tests.rs); covers final-use revocation/generation drift rejection.
- [codex-rs/hepta-intelligence/src/native_ports_v3_tests.rs](../../../codex-rs/hepta-intelligence/src/native_ports_v3_tests.rs); traverses admitted/current owner APIs and a real file-backed learning ledger without FakePorts.
- [codex-rs/hepta-agentd/src/lane_b_runtime_tests.rs](../../../codex-rs/hepta-agentd/src/lane_b_runtime_tests.rs); covers the AgentRunCoordinator handoff and runtime lifecycle integration.
