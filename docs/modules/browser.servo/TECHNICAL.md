...
The current repository-owned implementation includes `browser.js`, `action.js`, `bridge.js`, `runtime.js`, `runtime-host.js`, `runtime-contract.js`, `runtime-boundary.js`, `journal.js`, `worker-protocol.js`, `worker-driver.js`, `egress-broker.js`, the Agentd parent protocol/service, and `servo-worker/`. Cross-owner composition is attached to the long-lived Agentd daemon through `browser_servo.rs`, `runtime.rs` and `state_control.rs`; `hepta-agentd-browser` remains a one-shot diagnostic/qualification surface, not the product lifecycle owner.
...
The current source is decomposed into bounded authority-free proposal/projection ingress; typed-action normalizer and payload digest binder; provenance-preserving proposal/effect bridge; per-profile bounded single-writer queue; live final-use authority handshake; strict durable pre-dispatch journal; private parent protocol and long-lived Agentd Browser owner; private Browser/Servo worker protocol; bounded subprocess worker pool; host-side grant-scoped egress broker; current-pin one-Servo/one-WebView-per-profile worker; Linux namespace launch contract and real sandbox probe; crash-surviving worker terminal receipt; and reconciliation/terminal receipt reporter.
...
Current executable browser actions are closed-world: `navigate`, `click`, `type`, `focus`, `scroll`, `wait`. The bounded `credential`, `upload` and `download` schemas are reserved future capabilities and fail closed as `capability_not_connected`; they are not current module support. Navigation binds normalized URL, `policyDigest` and `expectedRevision`; the bridge uses the proposal `navigationId` as the operation identity. Thus policy revision and proposal identity are included in the final request digest rather than being dropped at adapter handoff.
...
Before worker admission, validation, authority denial, invalid protocol, persistence failure or stale page/action-surface rejection cannot claim a crossed effect. A worker-confirmed pre-dispatch rejection is persisted as terminal failed. After the worker emits `dispatch_boundary`, driver timeout, channel loss, worker crash or unknown response remains `indeterminate` until reconciliation. In-process reconciliation uses the live worker. After worker admission, the Browser owner performs terminal drain outside the final-use fence and persists any worker-known terminal response. The worker also fsyncs a bound terminal receipt through an inherited host-private fd before returning terminality, so Browser-parent loss does not erase terminality already known to the worker. After Browser-process loss, `reconcile_persisted_operation` first validates that worker receipt against the durable profile/generation/operation/page/payload/grant/epoch identity. A generic remote-business oracle is still impossible to fabricate: if the old worker never terminally observed the business effect, a separately trusted persisted reconciler is required and otherwise the result remains indeterminate. The crash observer receives only the journal's non-secret durable identity, not the original `typedAction` or `type.text`, and its observation must bind the exact operation ID, request digest and semantic digest before Browser accepts terminality.
...
The sandboxed worker has no ambient/direct network route. Authorized HTTP(S) WebResource loads instead cross inherited private pipes to `GrantScopedEgressBroker`, which binds the operation/profile/effect grant/authority epoch, validates allowed origin and redirect target, resolves a bounded DNS set, pins the selected IP for connect, rejects private/special IPs unless explicitly granted, retains normal HTTPS certificate verification/SNI and records the connected peer/TLS fingerprint. The initial broker is intentionally bodyless GET/HEAD only.

## 10. Performance, capacity and hot-path policy

Current hard source bounds include <=16 active profile/worker processes per production Browser service by default (configurable 1..64); <=128 origins/profile; <=1024 effect grants/profile; a generic owner ceiling of <=1024 nonterminal operations/profile with the current one-WebView subprocess driver restricted to 1 outstanding effect; <=256 terminal operations retained in host memory; <=64 queued mutations per serialization key; <=1 MiB host observation request; <=256 KiB semantic observation returned by the real Servo worker; <=1 MiB private worker frame; <=64 MiB file journal with automatic compaction beginning at 48 MiB; bounded action fields; explicit Browser driver/authority deadlines; and Linux worker defaults of 8 GiB RLIMIT_AS, 300 seconds RLIMIT_CPU, 4096 RLIMIT_NOFILE and 256 RLIMIT_NPROC. Agentd binds the exact `prlimit` path/digest and these numeric ceilings into the Browser child environment.

Each current worker is one WebView/profile generation; the production service uses a bounded worker pool rather than sharing one worker across profiles. The dossier's <=16 concurrent-tab pilot target is not a current claim and requires a later measured scheduler/profile.
...
Repository-side Browser/Servo worker source, private protocols, real final-use handoff, long-lived Agentd owner, pooled worker composition, grant-scoped egress source and crash-surviving worker terminal recovery are present. Remaining sequence is evidence/platform oriented: produce/review the exact `Cargo.lock`, execute the reproducible worker build/SBOM gate, execute real Linux sandbox/worker tests on the exact candidate, bind target measurements, add platform equivalents where targeted, then obtain independent activation/acceptance/promotion/release decisions.

Credential/upload/download are reserved future capabilities, not current support, and stay fail-closed until their authority brokers and terminal observers are implemented and independently qualified.

## 14. Activation, compatibility and retirement

The long-lived Agentd daemon now owns one reusable private Browser caller and revocation monitor when Browser runtime configuration is present. The one-shot CLI remains diagnostic only. Neither path invents a trusted verifying key, authority epoch, revocation frontier or worker selection. Activation still requires a trusted live authority/revocation owner, qualified exact worker artifact, target sandbox identity, durable journal/profile root configuration, resource limits and successful cross-owner/source checks.
