...
- `codex-rs/hepta-agentd/src/runtime.rs` / `state_control.rs` — long-lived Agentd-owned Browser caller attached once to daemon state and reused across `open_profile -> observe_page -> navigate_or_act -> reconcile -> close_profile`;
- `codex-rs/hepta-agentd/src/bin/hepta-agentd-browser.rs` — bounded one-shot diagnostic/qualification caller only; it is not the product lifecycle owner.

There is no Browser discovery listener. The long-lived Agentd daemon composes the private Browser child only when a trusted owner supplies the authority/revocation and artifact configuration required for activation; otherwise Browser remains fail-closed.
...
| long-lived Agentd Browser owner | `hepta-agentd/src/runtime.rs`, `state_control.rs` | implemented in source; one private caller is attached to daemon state |
| one-shot Browser CLI | `hepta-agentd-browser` | diagnostic/qualification surface only; not the product lifecycle owner |
...
| grant-scoped egress broker | `src/egress-broker.js` + worker resource interception | implemented in source; direct worker networking remains denied |
| crash-surviving worker terminal receipt | inherited fd 5 + `worker-driver.js` recovery | implemented for worker-known terminal outcomes; genuinely unobservable business effects remain indeterminate |
...
| real Servo grant-scoped E2E | `scripts/real-worker-e2e.js` | implemented source gate: open/navigate/observe/type/click/form navigation/redirect denial/close |
...
Current executable worker action kinds are:
...
The schemas for `credential { selector, credentialRef }`, `upload { selector, fileRef, fileDigest, maxBytes }` and `download { url, maxBytes }` are **reserved future capabilities**, not current module support. They carry bounded opaque references where applicable, but the current worker deliberately returns `capability_not_connected`. They must not appear in a current capability/support table until their broker plus terminal observer is independently qualified.
...
The file journal performs exact field validation on every hydrated record, rejects unknown fields, validates checksum envelopes and semantic identity, fsyncs before dispatch, uses private non-symlink files and parent directories, compacts atomically before the file ceiling and retires a fully terminal profile generation only after fsyncing a separate private generation high-water, so deleting bulky terminal records cannot resurrect the same profile generation after restart. A profile generation with durable operation history cannot be reopened into a fresh worker; unresolved durable effects from another generation block profile advancement. Persisted recovery observes the old identity without redispatch, and automatically retires the generation once all recovered operations are terminal. Live-worker reconciliation and post-process-loss reconciliation are deliberately separate driver methods. After the worker admission boundary, the Browser owner immediately drains a worker terminal observation outside the final-use fence and persists it when available. In addition, the selected worker writes each worker-known terminal result to a host-private inherited-fd receipt before returning that terminal protocol response; a replacement Browser process validates profile/generation/operation/page/payload/grant/epoch binding before accepting the receipt. This closes the Browser-parent-crash window for terminality already known by the worker. A new Servo worker is still never treated as evidence of a remote business effect that the old worker never terminally observed: if no bound worker receipt exists, an explicit trusted business reconciler is required and genuinely unobservable effects remain `indeterminate`. Crash reconciliation never reconstructs or replays secret `typedAction` bytes.
...
The launcher starts from an empty tmpfs root and does not bind host `/` or `/usr` wholesale. The Servo process retains no direct network namespace access. HTTP(S) WebResource loads cross dedicated inherited pipes into the host-side `GrantScopedEgressBroker`, which rebinds every request to the admitted operation/profile/effect grant, validates origin, resolves and pins one DNS answer, rejects private/special addresses unless explicitly named by the profile grant, checks redirect targets before follow, preserves HTTPS SNI/certificate verification, verifies the connected peer IP and records DNS/IP/TLS evidence in the egress receipt. The current broker deliberately permits only bodyless GET/HEAD resource requests.
...
Profile mutations use a bounded single-writer queue. The production Agentd service composes `PooledSubprocessBrowserDriver`: default maximum 16 active profile/worker processes, configurable only within the hard 1..64 ceiling. Each profile still owns one isolated worker/WebView and one outstanding effect at a time; the pool never shares a profile directory or worker process across principals. No more than 64 operations may be queued for one serialization key; overload fails with `BrowserBackpressureError` rather than allowing unbounded promise growth.
