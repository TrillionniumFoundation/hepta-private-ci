...
| Authorization | yes | yes, candidate | partial: Bao read candidate |
| Quota | yes | yes, candidate | partial: Bao read candidate |
| Reservation / settlement | yes | yes, candidate | partial: Bao read candidate |

The candidate control implementation is split deliberately across owners: `codex-rs/hepta-authbus/src/control.rs` owns pure policy/quota/reservation contract types; `codex-rs/hepta-evidence/src/authbus_control.rs` and migrations 0011/0012 own durable fixed-window quota transactions, immutable reservation bindings, the Active -> EffectStarted final-use fence, settlement/reconciliation, restore-checkpoint binding and replay-epoch retirement; `codex-rs/hepta-agentd` remains the narrow signed-text host and now requires a separate external restore-checkpoint witness whenever AuthBus trust is enabled; `codex-rs/hepta-bao-adapter` has a candidate effect wrapper that binds the complete FinalUseBinding before committing EffectStarted and entering HTTPS. Source implementation is not production acceptance: the candidate must pass exact-head, synthetic-merge, native product and independent semantic/security qualification before any production claim.
