...
The realized file/symlink footprint must equal the paths declared by the admitted single mutation or atomic mutation set (including both source and target for a rename), or the empty footprint declared by `no_change`. New directory entries are admitted only when they are newly created strict ancestors of those declared objects; arbitrary directory creation or directory-mode drift fails closed. Every changed path must remain inside the envelope roots and outside protected roots. Candidate-owned changes may not touch test/spec files, fixture/golden/snapshot trees, source files containing inline test or evaluator definitions, or evaluator-control manifests such as Cargo/Python/Node test discovery and runner configuration. Dependency or build-manifest changes therefore remain ordinary owner-reviewed work rather than self-evaluated candidate mutations. File count and a conservative before-plus-after byte budget are enforced. After checks, the entire workspace manifest must be identical to its pre-check manifest; any extra write fails closed.
