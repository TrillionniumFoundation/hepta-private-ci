...
| Frozen datasets | `DatasetSnapshotV1`, then `DatasetSnapshotV2` | `DatasetSnapshotReceiptV3` plus host-trusted `DatasetOwner` evidence | V2/V3 structural receipts remain readable by compatible legacy callers | Product training verifies the complete V3 digest preimage, then verifies a domain-separated Ed25519 `DatasetOwner` attestation whose trusted principal exactly equals the embedded producer | Rollback may expose structural V2/V3 only to callers explicitly registered for that compatibility level; a detached/self-authored receipt cannot satisfy authenticated operator training |
| Operator target builder | `train(TrainingRequest)` / `build_targets` | receipt-bound `build_targets_bound_v2` | Legacy names retain deterministic target-builder behavior and reject replayed support evidence | New qualification verifies `DatasetSnapshotReceiptV3`, exact snapshot/objective/ledger-head/source-record binding, then emits the V3 dataset identity | Legacy caller-supplied snapshot/head/support fields are compatibility inputs and cannot substitute for receipt-bound qualification; no form is a neural trainer |
| Learned operator | V1 complete-grid tabular fit/predict | `VerifiedOperatorDatasetV2` receipt-bound fit plus independently pinned loaded payload | V1 artifacts remain inspectable | V1/V2 fitting reject relabelled duplicate evidence; new qualification verifies `DatasetSnapshotReceiptV3`, exact row evidence and uses independently pinned `LoadedTabularOperatorV1` for inference | Raw V1/V2 predictors or caller-supplied detached dataset digests are compatibility surfaces and may not substitute for receipt-bound fit plus pinned load admission |
| World model | V1 action-conditioned tabular model | receipt-bound `fit_transition_model_bound_v2` plus pinned `HEPTWM01` / `LoadedTabularWorldModelV1` | Exact supported pairs remain readable | New qualification takes dataset identity from `VerifiedOperatorDatasetV2`, requires the exact frozen evidence set, then uses independently pinned loaded inference | Raw `fit_transition_model(dataset_digest, rows)` / `predict_transition` are compatibility-only; no version may rebind foreign rows or convert prediction into independent factual outcome |
...
| Evidence identity | caller-constructed `AuthenticatedPrincipalV1` | `LearningEvidenceVerifierV1` with `Generator`, `Observer`, `Evaluator`, and `DatasetOwner` roles | Legacy validation remains structural, not cryptographic | External operator evaluation and dataset-owner provenance use host-trusted Ed25519 keys, exact role/identity/credential checks and domain-separated signed payloads | Replacing the verifier epoch or revoking the signer invalidates previous trust bindings; a valid signature authenticates provenance, not scientific validity |
