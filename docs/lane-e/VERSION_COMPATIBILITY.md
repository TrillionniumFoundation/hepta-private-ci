...
| Learned operator | V1 complete-grid tabular fit/predict | strict V2 fit plus independently pinned loaded payload | V1 artifacts remain inspectable | V1 and V2 fitting both reject relabelled duplicate evidence; new qualification uses independently pinned `LoadedTabularOperatorV1` for inference | Raw V1/V2 predictors are deprecated compatibility surfaces and may not substitute for pinned load admission |
| World model | V1 action-conditioned tabular model | pinned `HEPTWM01` payload plus `LoadedTabularWorldModelV1` | Exact supported pairs remain readable | Fitting rejects relabelled duplicate evidence; new qualification uses independently pinned loaded inference | Raw `predict_transition` is compatibility-only; no version may convert prediction into independent factual outcome |
