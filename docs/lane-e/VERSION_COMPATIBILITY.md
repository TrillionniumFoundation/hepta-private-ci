...
| Evidence identity | caller-constructed `AuthenticatedPrincipalV1` | `LearningEvidenceVerifierV1` plus signed evaluator admission for applicability/regularity | Legacy validation remains structural, not cryptographic | External operator evaluation uses host-trusted Ed25519 keys, evaluator role/identity/credential checks and exact signed assessment payloads | Replacing the verifier epoch or revoking the signer invalidates previous trust bindings; a valid signature authenticates provenance, not scientific validity |
