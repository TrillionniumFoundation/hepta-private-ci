...
| Learned operator | V1 complete-grid tabular fit/predict | strict V2 plus dataset-bound V3 fit | V1 artifacts remain inspectable; V1 fitting now also rejects duplicate evidence | New qualification uses dataset-bound V3 when frozen-dataset row membership is available and strict V2 otherwise | A compatibility API may not bypass evidence uniqueness or dataset binding required by the selected profile |
| World model | V1 action-conditioned tabular model | dataset-bound additive fit | Exact supported pairs only; duplicate underlying evidence rejects | New cross-owner integrations bind sample evidence to the verified frozen-dataset evidence set; predictions remain synthetic and deny-all | No version may convert prediction into independent factual outcome or detached caller-supplied rows |
