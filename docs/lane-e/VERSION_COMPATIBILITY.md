...
| Evidence identity | caller-constructed `AuthenticatedPrincipalV1` | `LearningEvidenceVerifierV1` and private `VerifiedLearningEvidenceV1` | Legacy/direct `decide_independently*` validation remains structural, not cryptographic | External production admission uses host-trusted Ed25519 keys and signed V2/V3 evaluation entry points | Replacing the verifier epoch invalidates previous trust bindings; reverify after revocation |
...
| Final holdout | in-memory `FinalHoldoutRegistry`, then `DurableFinalHoldoutJournalV1` | `FencedFinalHoldoutJournalV2` with host-issued `HoldoutOwnerContextV2` | V1 remains valid for trusted single-host/cooperating-owner recovery | New host integration supplies a fresh current writer fence on every consume; multi-host deployments keep fence+anchor in transactional CAS/dedicated authority | Restarting empty, presenting a stale fence, or treating a local file lock as a distributed lease is invalid |
...
for the exact host boundary, claim limitations and capacity behavior. The
normative evaluator API status matrix is
