...
| Learned operator | V1 complete-grid tabular fit/predict | strict V2 plus `DatasetSnapshotReceiptV3`-bound fit | V1 artifacts remain inspectable | New qualification uses receipt-bound strict V2 evidence uniqueness and canonical indexed/loaded prediction | A strict V2 artifact may use V1 prediction only for compatibility testing, not to bypass strict admission |
| World model | V1 action-conditioned tabular model with detached dataset digest | evidence-unique core plus `DatasetSnapshotReceiptV3`-bound fit | Exact supported pairs remain readable | New qualification derives dataset identity from the verified receipt and rejects relabelled duplicate evidence | No version may convert prediction into independent factual outcome or pair arbitrary rows with a detached dataset digest |
