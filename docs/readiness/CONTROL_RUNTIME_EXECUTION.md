...
5. `authority_bridge.rs` maps each deny-all `GrantRequestV1` to an exact `FinalUseBinding`. `claim_final_use_for_grant_request_v1` accepts only an independently signed grant and delegates signature, revocation and single-use nonce enforcement to `FinalUseAuthority`.

The global composition is currently a source-complete integration surface with native cross-module tests. It is not yet a named global product host or a newly admitted external wire protocol. The existing named product caller remains the narrow Agentd measured-context path. Exact-head PR qualification, named-host performance/fault evidence, independent acceptance, activation and release remain separate.
