...
`plan_global_and_record_v1` composes that same transaction with `PlannerJournalStoreV1`. Before a selected plan can be returned to an effect caller, it reopens the committed journal, records the exact snapshot and decision digests, records the selection under a caller-supplied idempotent operation identity, and advances the durable frontier. Equal retries reuse the same semantic records; reusing the operation identity for a different decision fails closed.
