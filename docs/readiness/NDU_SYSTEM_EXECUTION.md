...
An organ contributes only facts it owns. `utility.ndu` aggregates supported contributions. `control.runtime` consumes a digest-bound projection of the resulting evaluation; it does not reimplement NDU. `learning.ledger` records the complete candidate/contribution set under its own writer rules. The repository already contains request-local read-only/planning callers through `plan_observed_context` and `evaluate_prepared_plan_with_ndu`; these are real product callsites but are not the authenticated production NDU owner/caller.
...
`eta` is in `[1/16,1/4]`. The preference target solver emits immutable revisions and at most 64 local iteration receipts. If the registered residual tolerance is not reached inside that bound, the solver returns `IterationExhausted`; the last bounded numerical state is not exposed as a successful terminal state.

Parent/child staging is determined from explicit `subject_id` / `parent_subject_id` relations. A concrete parent and child cannot both select new artifacts in one generation; siblings and unrelated hierarchies may advance concurrently. Reusing one subject/generation for a different artifact conflicts. A global subject-class ban is not the intended invariant.
