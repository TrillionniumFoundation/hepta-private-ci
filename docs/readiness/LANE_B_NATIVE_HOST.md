...
1. Obtain one immutable owner transaction cut through `CognitiveStore::lane_c_snapshot`.
2. Bind the exact read-specific owner frontiers, scope/purpose, consumer profile and host lifecycle epoch into a request-bound `LaneCAuthoritativeSnapshotProvider`, then execute `read_authoritative`; raw `read_v2` is only the internal projection primitive.
...
4. Reacquire/revalidate the owner cut and runtime generation before response publication, then retain the exact authoritative guard in Agentd under the read binding digest.
5. The native model worker syncs durable dispatch intent, calls `CognitiveContextFinalize` immediately before provider `TurnStart`, and only proceeds when Agentd consumes the one-shot guard and freshly revalidates host epoch, owner frontiers/vector, original lease/receipts and optional ranker witness. Failed finalization records a local pre-`TurnStart` stop and sends no provider turn.
