...
7. After App Server connection/thread creation and immediately before durable dispatch intent / physical `TurnStart`, the worker sends the unchanged context snapshot back to Agentd. Agentd reacquires the current owner snapshot and verifies every selected ID/revision/content digest. Any stale binding fails before model dispatch.
