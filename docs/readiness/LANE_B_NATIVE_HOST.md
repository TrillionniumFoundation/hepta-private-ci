...
| Matrix transport | Existing `hepta-matrixd`, `MatrixDurableStore` and SDK sender | durable dispatch state is owned by `MatrixDurableStore`; `send_observer` is only a compatibility facade and no duplicate sender is started |
