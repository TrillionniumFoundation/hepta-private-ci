...
| Matrix transport | Existing `hepta-matrixd`, `MatrixDurableStore` and SDK sender | `send_observer` is a façade over the canonical durable dispatch ledger; HTTP acceptance is non-terminal and no duplicate sender/state owner is started |
