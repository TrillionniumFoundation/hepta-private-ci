...
1. Capture the current Agentd lifecycle generation as the host authority epoch and acquire `CognitiveStore::authoritative_lane_c_snapshot_provider`. The SQLite owner, not the caller, fills the memory/source/tombstone/knowledge-fact/KG frontiers.
2. Execute `read_authoritative` with the bounded `ReadRequestV2`. The lower-level `read_v2` remains an implementation primitive and is no longer the product delivery boundary.
3. Rank with the existing SQLite retrieval provider and accept only exact record ID, revision and content digest matches admitted by the authoritative cut. When a pinned learned ranker is present, its selected model payload digest is included in the host vector and its registry witness is revalidated independently.
4. Build and plan the context, then call `revalidate_authoritative_lane_c_snapshot` immediately before final use. This rechecks request/snapshot/read receipts, lease/deadline, the exact owner cut and the full bound generation vector.
5. Refresh Agentd lifecycle state and reject publication if the lifecycle generation differs from the captured authority epoch.

The query is 1–2048 bytes and the requested result limit is 1–4. The Lane C projection admits at most 1024 records and 1 MiB of canonical encoding. Intersecting that bounded record prefix with search candidates can omit relevant records outside the prefix; `omitted_records` reports the read truncation. The complete context payload is bounded to 24 KiB of JSON encoding, including escaping and its envelope. Oversized items are omitted, not silently truncated. This is verified memory retrieval, not evidence of learned model weights or complete recall.

The Agentd context path currently consumes neither compact-checkpoint nor prompt-registry records. Their broad Lane-C vector slots are explicit fixed not-consumed sentinels; they are not claims that those owners were observed. If either becomes an input to context construction, the product host must bind and revalidate the real current owner receipt/generation before delivery.
