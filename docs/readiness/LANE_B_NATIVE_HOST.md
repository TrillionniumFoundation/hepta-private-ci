...
| Fleet lifecycle | Existing supervisor-owned `FleetRegistry` / supervisord | `FleetAllocationStore` durably owns placement grants and fences around the deterministic `lease_ledger`; physical/authenticated capacity observation and deployed consumer enforcement remain external integration evidence |
...
- Supply real authenticated capacity/pressure observations and deployed consumer enforcement receipts. Durable resource leases, deterministic placement, final authority binding, restart recovery and usage reconciliation now live under the existing Fleet owner; caller-provided capacity is still not hardware discovery.
