...
| Inference reservation, authority and settlement | The native worker calls `DurableInferenceControl` and consumes `FinalUseAuthority` | One journal and lock own local slot admission/dispatch/settlement; an independently signed single-use grant fences the actual turn dispatch; economic quota/device authority remains external |
...
Build from the repository, then perform final-use issuance as a two-owner flow:

```sh
cargo build --manifest-path codex-rs/Cargo.toml -p codex-hepta-infer-worker-host --bin hepta-infer-worker

# 1. No provider contact: materialize the exact binding that a separate
#    kernel.authority issuer must sign.
hepta-infer-worker --profile native-app-server \
  --agentd-socket /absolute/owner/agentd.sock --agent-id UUID --generation 1 \
  --model MODEL --model-provider PROVIDER \
  --operation-id operation-001 --request-id run-001 --maximum-in-flight 1 \
  --quota-reservation-sha256 64_LOWER_HEX \
  --resource-snapshot-sha256 64_LOWER_HEX \
  --authority-binding-only --context-query 'optional memory query' \
  < prompt.txt > final-use-binding.json

# 2. After the independent authority owner signs that exact binding and
#    supplies the pinned verifier/revocation configuration:
hepta-infer-worker --profile native-app-server \
  --agentd-socket /absolute/owner/agentd.sock --agent-id UUID --generation 1 \
  --model MODEL --model-provider PROVIDER \
  --operation-id operation-001 --request-id run-001 --maximum-in-flight 1 \
  --quota-reservation-sha256 64_LOWER_HEX \
  --resource-snapshot-sha256 64_LOWER_HEX \
  --authority-config /absolute/private/final-use-authority.json \
  --journal /absolute/private/native-runs.journal \
  --context-query 'optional memory query' < prompt.txt
```

The binding-only phase reads the generation-fenced Agent context needed for the exact payload but does not connect to the provider. The authority configuration pins signer identity/key, durable nonce/revocation state, authority epoch/revision and one `SignedFinalUseGrant`; the worker verifies and consumes that grant but cannot mint it. If context, payload, model/provider, operation, quota/resource digests, generation or scope drift between issuance and execution, claim fails closed.

The model must be configured and authenticated in the existing owning App Server. The worker does not install credentials, select hardware, mint quota or grant itself tools. The native module and its dependencies compile by default in both Cargo and Bazel. The CLI requires explicit `--profile native-app-server` selection and rejects missing or unsupported profiles before reading the prompt or contacting the provider. The prompt limit is 32 KiB, observed output limit is 1 MiB, and bounded events cap at 256. `--timeout-ms` defaults to 120000 and applies to turn observation; connection/RPC calls have separate five-second bounds. Cancellation, deadline, event loss, disconnect and fencing trigger an actual interrupt request. Interrupt acknowledgement alone is insufficient: an unobserved outcome remains `indeterminate`. The CLI prints observed JSON and exits nonzero on provider failure, interruption, indeterminacy, or lost/unverified owner authority. It never automatically replays an uncertain turn/start.
...
Execution requires an absolute `--journal`, stable `--operation-id`/`--request-id`, exact `--model-provider`, explicit `--maximum-in-flight` from 1 to 256, quota/resource evidence digests and an absolute `--authority-config`. The first native admission pins the local slot limit for the journal. This is a local concurrent-run budget, not a token cap, payment authorization, hardware discovery or a global limit across unrelated journals. The bound quota/resource digests are semantic inputs to the signed final-use scope; they are not treated as self-authenticating authority. The file lock serializes writers; a host can hold uncertain runs while admitting another only within its pinned budget. New journals are mode 0600 on Unix and native admission rejects a group/world-accessible journal. New native observations persist output SHA-256 receipts rather than raw model text; legacy journals can contain raw output and remain private/retention-sensitive. The App Server thread remains ephemeral, so process-loss reconciliation cannot assume retained provider history.

Before any provider contact, admission binds the request/operation identity to the exact Agent/generation/model, prompt/query/socket/timeout digest and the externally supplied quota/resource evidence digests. Before `turn/start`, the worker verifies the actual thread provider equals the expected provider, claims the independently signed `FinalUseBinding`, and syncs dispatch intent including the thread/provider/context and authority-binding digest. The actual `turn/start` RPC executes while `FinalUseAuthority` holds the live revocation fence; a successful revocation update therefore cannot race between final verification and the bounded dispatch result. The stable request ID also travels as `client_user_message_id`; correctness does not assume this alone provides provider exactly-once execution. The actual returned turn ID is then synced. After dispatch may have happened, duplicates and restarts never submit a new turn. A reopened record without a terminal observation becomes indeterminate and retains its slot. A completed duplicate returns the stored observation without contacting Agentd or the provider, even if that generation is no longer live; it is a historical result, not a new authorization.

Provider terminality, `owner_authority` and `final_use_authority` are independent. Authorized success requires matching provider completion, final owner readiness and a verified single-use final-use grant. During execution, not-ready/fenced health, identity/generation errors, transport errors and health timeouts permanently mark that attempt `Lost`; this fact is journaled before interrupt grace. Late provider `Completed` and real token counts remain observable and can release the local run slot, but never clear lost authority or make the CLI succeed. After provider unsubscribe/shutdown, an observed terminal without prior authority loss undergoes one final exact-owner health check before return and settlement; a previously lost owner remains denied without another RPC. This covers `select!` choosing completion ahead of a health tick. `ObservedReady` still means ready at that last owner-health check; it is not itself an authority grant. Provider-effect revocation freshness is instead enforced by the independent final-use fence around `turn/start`. Historical journal observations missing owner or final-use fields load as `Unverified` and cannot become successful through replay or a later token-only update.

Pre-dispatch cancellation or connection failure records a local stop and releases its slot without fabricating provider terminality or zero tokens. After dispatch, cancellation/timeout/fencing records interrupt intent and sends the real interrupt RPC; only a matching terminal notification releases the local slot. `observed_output_tokens` remains null if no matching usage event arrived, including on terminal failure or interruption. Observed u64 counts are retained even above a synthetic token budget; no economic settlement is inferred. The typed trusted-host settlement port can refine unknown execution/usage with later matching observations, but automatic provider recovery after restart is still absent. New worker settlement hashes the observed model text, clears it before journal append and retains only the digest receipt; the first live call can still return the in-memory text. A replayed terminal receipt cannot recover that text and the CLI exits nonzero rather than re-executing the request. Deleting the journal, changing the request ID or treating a missing observation as zero is not a recovery procedure.
...
- Connect authoritative economic quota and device-capacity producers/verifiers, complete the deterministic eligible-worker scheduler contract, and implement authenticated provider reconciliation after process loss. Native execution now binds operation/quota/resource evidence digests into the independently signed final-use scope, but those digests do not mint or validate the underlying economic/device authority. Local-slot reservations and observed usage settlement are wired; hosted execution does not prove local model artifacts, memory/device grants or process isolation.
