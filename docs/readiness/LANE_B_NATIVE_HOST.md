...
| Hosted model execution | `hepta-infer-worker --profile native-app-server` | Calls the owning Agent's existing App Server provider and reconciles durable private thread/turn history after restart |
| Local model execution | `hepta-infer-worker --profile local-process` on Unix | Composes request/lease/reservation preflight, kernel-signed resource admission, exact artifact/device/isolation proof and a private Unix runtime adapter; target-host qualification remains external |
...
The model must be configured and authenticated in the existing owning App Server. The worker does not install credentials, select hardware or grant itself tools. The native module and its dependencies compile by default in both Cargo and Bazel. The CLI requires explicit `--profile native-app-server` selection and rejects missing or unsupported profiles before reading the prompt or contacting the provider. The prompt limit is 32 KiB, observed output limit is 1 MiB, and bounded events cap at 256. `--timeout-ms` defaults to 120000 and applies to turn observation; connection/RPC calls have separate five-second bounds. Cancellation, deadline, event loss, disconnect and fencing trigger an actual interrupt request. Interrupt acknowledgement alone is insufficient: an unobserved outcome remains `indeterminate`. The CLI prints observed JSON and exits nonzero on provider failure, interruption, indeterminacy, or lost/unverified owner authority. It never blindly replays an uncertain turn/start. On reopen it resumes the exact durable thread and reconciles the stable `client_user_message_id` plus canonical input digest. A persisted turn is read/recovered in place; only a provider-proved `Missing` result with the unchanged frozen context digest permits same-thread redispatch.

### Local-process product profile

On Unix, the same binary also exposes `--profile local-process`. This path does not mint an inference lease or resource capability. It receives the existing request/lease/reservation envelope, validates it through the established inference boundary, consumes a kernel `FinalUseAuthority` signed single-use resource grant bound to the worker/capacity/semantic digest, hashes exact model artifacts plus device and isolation receipts, then invokes a direct private Unix runtime socket. The runtime must report the same model/runtime/device/isolation digests and an observed memory value within the grant before the handle is admitted. Inference sends the actual prompt bytes, bounds output, records real consumed tokens and hashes returned output locally. Cleanup remains permitted after authority expiry/revocation so stale authority cannot pin local device memory.

This source binding is not itself proof that a selected deployment actually enabled cgroups/namespaces/seccomp/GPU ACLs. Those controls are a target-host qualification gate and must be independently bound to the exact isolation receipt before an OS-sandbox claim is made.
...
The CLI requires an absolute `--journal`, stable `--request-id` and explicit `--maximum-in-flight` from 1 to 256. The first native admission pins that limit for the journal. This is a local concurrent-run budget, not a token cap, payment authorization, hardware discovery or a global limit across unrelated journals. The file lock serializes writers; a host can hold uncertain runs while admitting another only within its pinned budget. New journals are mode 0600 on Unix and native admission rejects a group/world-accessible journal. The journal contains observed model text; use the owning Agent's private storage and retention policy. The App Server thread is private and durable. Its stable `client_user_message_id` plus canonical input digest are the provider-owned recovery identity used by `thread/queue/reconcile`; guarded `turn/recover` resumes the same persisted logical turn when recovery is enabled.

Before any provider contact, admission binds the request ID to the exact Agent/generation/model and a digest of prompt, optional memory query, socket and timeout. Before `turn/start`, a synced intent adds the durable thread/provider and exact serialized context digest. The stable request ID travels as `client_user_message_id`; on restart the worker resumes that thread and calls `thread/queue/reconcile` with the canonical input digest. `Persisted` binds/reads the same turn, `InProgress` may recover the same turn, and `Missing` permits retry only on that same thread when the frozen context digest is unchanged and cancellation is absent. `Queued`, `Cancelled`, transport loss or contradictory history never authorize a fresh turn. A completed duplicate with known usage returns the stored observation as historical fact, not new authorization.
...
- Economic quota remains external to this local-slot worker. Repository-controlled provider reconciliation and local model/runtime/device/isolation binding are implemented; the remaining inference work is target-host evidence with identified real model artifacts, hardware resource behavior and independently verified deployed OS/device isolation.
