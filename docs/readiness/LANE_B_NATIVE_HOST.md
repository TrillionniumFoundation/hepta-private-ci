...
6. In the native App Server worker, immediately before `turn/start`, re-read the same cognitive-context query and require the ordered selected memory IDs/revisions/content/hash values and `read_allowed` decision to remain identical. Selection or policy drift rejects before provider dispatch; unrelated whole-cut digest drift is not treated as attachment drift.
