...
5. The native worker retains that exact receipt while it verifies owner readiness, App Server home/model/thread and current generation. Immediately before journaling dispatch and sending model `TurnStart`, it calls `AgentdClient::finalize_cognitive_context`.
6. Agentd reacquires the canonical Agent-private Lane C cut, exact-compares `snapshot_digest`, reruns the same bounded V2 read, exact-compares `read_digest`, and revalidates the cut again. Any observed correction, delete/tombstone, validity change, citation/frontier change, rollback or owner conflict fails closed and the stale context is not attached to a model turn.
...
- Add and execute a full transport-level deterministic cognitive race fixture that drives `CognitiveContext -> owner correction/tombstone -> CognitiveContextFinalize -> no TurnStart` through the Agentd control socket and native App Server boundary. The owner-level race regression exists, and the production worker invokes the gate, but exact-candidate transport qualification remains separate evidence.
