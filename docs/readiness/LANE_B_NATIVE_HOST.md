...
| Inference reservation and settlement | The native worker calls `DurableInferenceControl` | One journal and lock own request/token/concurrency/economic-budget holds, quota/resource bindings, final-use dispatch identity, observed settlement and bounded native compaction; real billing/device truth remains external |
...
hepta-infer-worker --profile native-app-server --agentd-socket /absolute/owner/agentd.sock --agent-id UUID --generation 1 --model MODEL --journal /absolute/private/native-runs.journal --request-id run-001 --maximum-in-flight 1 --maximum-output-tokens 4096 --maximum-budget-units 100 --quota-reservation /absolute/private/quota.json --resource-advertisement /absolute/private/resource.json --final-use-state-dir /absolute/private/final-use --final-use-signer-id SIGNER --final-use-verifying-key /absolute/private/signer.hex --final-use-revocations /absolute/private/revocations.json --final-use-grant /absolute/private/grant.json --context-query 'optional memory query' < prompt.txt
```
...
The CLI requires an absolute `--journal`, stable `--request-id`, explicit `--maximum-in-flight` from 1 to 256, per-request `--maximum-output-tokens` and `--maximum-budget-units`, plus quota/resource evidence and final-use authority files. The first native admission still pins the local concurrent-run limit for the journal. Separately, cross-bound `QuotaReservation`/`ResourceAdvertisement` evidence limits request count, output-token holds, active concurrency and owner-defined economic budget units; these are included in the signed final-use binding and are not provider billing receipts or hardware discovery. The file lock serializes writers; uncertain execution retains its local/token/economic holds. New journals and compaction archives are mode 0600 on Unix and native admission rejects a group/world-accessible journal. The journal and retained predecessor archives contain observed model text; keep them in the owning Agent's private storage. The App Server thread is persisted specifically so crash-unknown execution can be read back by exact ID; this is recovery state, not an instruction to keep provider history indefinitely.

Before any provider contact, admission binds the request ID to the exact Agent/generation/model, quota/resource/economic policy and a digest of prompt, optional memory query, socket, timeout and request holds. Immediately before physical `turn/start` admission, the worker freezes the exact request/provider/model/context plus serialized `TurnStartParams`, resolves and claims an independently signed final-use grant, then enters the kernel revocation mutex. Under that single fence it first syncs the thread/provider/context/final-use dispatch witness and then synchronously `try_send`s the exact request into the bounded single-owner App Server transport queue. Successful queue admission is the local effect linearization point: after it, transport loss is accepted-or-unknown and must reconcile rather than replay. If the bounded queue explicitly rejects before admission, Core records `NoAdmission` and releases the conservative holds without fabricating provider terminality or usage. The stable request ID also travels as `client_user_message_id`; correctness does not assume this alone provides provider exactly-once execution. The actual returned turn ID is then synced. After queue admission may have happened, duplicates and restarts never submit a new turn. A reopened record without a terminal observation becomes indeterminate and retains its holds. A completed duplicate returns the stored observation without contacting Agentd or the provider, even if that generation is no longer live; it is a historical result, not a new authorization.

Provider terminality, `owner_authority` and `final_use_authority` are independent. During execution, not-ready/fenced health, identity/generation errors, transport errors and health timeouts permanently mark that attempt `Lost`; this fact is journaled before interrupt grace. Late provider `Completed` and real token counts remain observable and can release the local run slot, but never clear lost authority or make the CLI succeed. After provider unsubscribe/shutdown, an observed terminal without prior authority loss undergoes one final exact-owner health check before return and settlement; a previously lost owner remains denied without another RPC. This covers `select!` choosing completion ahead of a health tick. `ObservedReady` means ready at that last owner-health check and is not provider-dispatch authority. The one-shot `VerifiedUseToken` is consumed only at effect admission; that operation returns a non-serializable, revalidation-only `VerifiedUseReceipt` that cannot authorize a second provider effect. Successful live completion additionally requires `FinalUseAuthority::revalidate_used` on that receipt for the exact binding. Revocation/epoch drift therefore denies success even when the provider completed, without executing the effect again. Process-loss reconciliation has only the durable witness, not the in-memory receipt, so it can recover effect truth but cannot fabricate `VerifiedAtTerminal`. Historical journal observations missing either authority field load as `Unverified` and cannot become successful through replay or a later usage-only update.

Pre-admission cancellation or connection failure records a local stop and releases its holds without fabricating provider terminality or zero tokens. A local transport-queue rejection after the durable dispatch witness is separately reconciled as exact `NoAdmission`; it is the only post-intent path that may release holds without provider terminality because the request never crossed the local effect-admission point. After queue admission, cancellation/timeout/fencing records interrupt intent and sends the real interrupt RPC; only matching terminal evidence or exact no-admission proof releases the applicable holds. `observed_output_tokens` remains null if no matching usage event arrived, including on terminal failure or interruption. Observed u64 counts refine token holds even above the requested bound; no economic settlement is inferred. On restart, the worker resumes the exact persisted thread and calls `thread/queue/reconcile` in `ReconcileOnly` mode with the journaled `client_user_message_id` and canonical payload digest. `Persisted` recovers the original turn and `thread/read` can recover its terminal state; `Missing`/`Cancelled` durably proves `NoAdmission`; queued/ambiguous/unavailable outcomes remain non-replayable. Because the final-use terminal receipt is intentionally non-serializable, reconciliation can establish terminal effect truth and release local concurrency but cannot reconstruct authorized success. Deleting the journal, changing the request ID or treating a missing observation as zero is not a recovery procedure.

The journal has a 64 MiB total byte budget, an 8 MiB encoded-line budget and at most 16384 records across legacy/native types. Before native history reaches the reserved high-water space, the single writer atomically folds native events into exact snapshot records, preserves legacy lines byte-for-byte, writes and syncs a private content-addressed predecessor archive, locks and syncs the replacement inode before rename, then keeps a bounded archive count. Archive enumeration, metadata lookup or deletion failure now fails closed and poisons the writer; successful pruning is followed by directory fsync, so the daemon never silently accepts new inference while sensitive predecessor archives grow beyond the configured count. Replay continues to reject oversize, malformed or incomplete histories without truncation. Automated source retention is count-bounded rather than time-based; deployment policy still decides backup lifetime, secure deletion and external backup retention for provider text.

## Remaining implementation work

- Compose a named non-CLI product caller that obtains current quota/resource records and signed final-use grants from their owners. The native worker source now enforces request/token/concurrency/economic-budget holds, exact final-use dispatch authority, read-only post-crash terminal reconciliation and bounded journal compaction, but source fixtures do not prove real billing, physical device capacity, local model artifacts or production activation.
