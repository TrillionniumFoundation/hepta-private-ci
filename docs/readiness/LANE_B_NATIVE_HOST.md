...
| Local model execution | `codex_hepta_infer_worker_host::local_process_driver` plus selected host runtime | The worker verifies the exact model/runtime/device artifact tuple, invokes one resident runtime process and binds PID/handle/memory acknowledgement; target-host CPU/GPU/sandbox qualification remains external |
...
- Compose the named production caller and authority adapter around the canonical inference.control writer. The local process driver now verifies model/runtime/device artifacts and runtime memory acknowledgement, and hosted execution now reconciles terminal provider history after process loss without replay. Economic quota, authoritative recovered token-usage/billing reconciliation, real device-capacity enforcement and deployment-specific OS/GPU isolation still require their owning authorities/evidence.
...
Local model source support now includes `LocalProcessModelDriver`: it verifies absolute regular non-symlink weights/tokenizer/preprocessor/quantization/license/SBOM/runtime/device files by SHA-256 before spawn, clears inherited environment, requires the runtime to echo the exact artifact tuple and OS PID with a non-zero bounded memory reservation, carries the actual bounded inference payload, and hashes raw output in the worker. `model_worker::ResourceGrant` is deliberately a trusted process-local capability, not a self-authenticating wire token; an IPC/network caller must verify the external `kernel.authority` credential before constructing it. These checks do not prove cgroup, namespace, seccomp/Landlock, GPU ACL/MIG, network or filesystem sandboxing; target-host evidence is mandatory. The consolidated operational gates are in [the inference.worker production readiness runbook](../modules/inference.worker/PRODUCTION_READINESS.md).
...
## Candidate qualification for current inference.worker changes

The exact-revision workflow [`.github/workflows/inference-worker-qualification.yml`](../../.github/workflows/inference-worker-qualification.yml) is the source qualification path for the current worker/core candidate. It checks out the exact source SHA, runs focused inference-control/worker tests and strict worker Clippy, and retains command-record digests plus source/tree SHA in a receipt. Do not copy historical pass counts below onto a new source revision. Real CPU/GPU/device/OOM/reset/crash/leak/sandbox qualification, independent acceptance, activation and release are separate gates and are explicitly not granted by that workflow.
