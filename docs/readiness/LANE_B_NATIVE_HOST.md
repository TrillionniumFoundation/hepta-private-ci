...
hepta-infer-worker --profile native-app-server --agentd-socket /absolute/owner/agentd.sock --agent-id UUID --generation 1 --model MODEL --model-provider PROVIDER --journal /absolute/private/native-runs.journal --request-id run-001 --maximum-in-flight 1 --final-use-config /absolute/private/final-use.json --context-query 'optional memory query' < prompt.txt
```

The model must be configured and authenticated in the existing owning App Server. The worker does not install credentials, select hardware, mint final-use authority or grant itself tools. The native module and its dependencies compile by default in both Cargo and Bazel. The CLI requires explicit `--profile native-app-server`, exact `--model-provider` and an absolute `--final-use-config`; missing authority input rejects before a new provider turn can be dispatched and releases the still-local slot. The config contains the trusted signer identity/public key, private authority state directory, current epoch/revocation frontier and one independently signed `SignedFinalUseGrant`; see `codex-rs/hepta-contracts/FINAL_USE.md`. The prompt limit is 32 KiB, observed output limit is 1 MiB, and bounded events cap at 256. `--timeout-ms` defaults to 120000 and applies to turn observation; connection/RPC calls have separate five-second bounds. Cancellation, deadline, event loss, disconnect and fencing trigger an actual interrupt request. Interrupt acknowledgement alone is insufficient: an unobserved outcome remains `indeterminate`. The CLI prints observed JSON and exits nonzero on provider failure, interruption, indeterminacy, lost/unverified owner authority or absent final-use admission. It never automatically replays an uncertain turn/start.
...
`DurableInferenceControl` acquires an exclusive file lock before replay, validates a candidate transition before append, syncs the journal before publishing the in-memory state and fences its writer after any ambiguous write/sync failure. Reopening a corrupt or partial record fails rather than treating an unknown operation as safe to replay. The native profile consumes additive `reserve_native`, `dispatch_native_authorized`, `native_started`, `cancel_native` and `settle_native` ports in this same owner and file. The older `dispatch_native` remains an unverified/reference transition and rejects any supplied final-use witness; it cannot establish authorized success. Legacy journal records remain replay-compatible. New native records are versioned `native-v1`. An older binary cannot replay them and must not replace the new owner while such records exist.

The CLI requires an absolute `--journal`, stable `--request-id` and explicit `--maximum-in-flight` from 1 to 256. The first `NativeLocalSlotAdmission` pins that limit for the journal. This is a local concurrent-run budget, not the target `InferenceReservation`: it is not a token cap, payment authorization, hardware discovery or a global limit across unrelated journals. The file lock serializes writers; a host can hold uncertain runs while admitting another only within its pinned budget. New journals are mode 0600 on Unix and native admission rejects a group/world-accessible journal. New settlement events persist only output digest/status/usage/authority metadata; raw provider text is returned to the live caller but is not written to new durable observations. On first open by the updated native worker, `redact_native_output_history` rewrites older native observation lines under a locked, fully-synced atomic replacement and writes an `OutputHistoryRedacted` marker. Private storage is still mandatory because request/model metadata remain sensitive and filesystem snapshots/backups can retain pre-migration blocks outside this owner's control. The App Server thread remains ephemeral, so process-loss reconciliation cannot assume retained provider history.

Before any provider contact, local-slot admission binds the request ID to the exact Agent/generation/model and a digest of prompt, optional memory query, socket and timeout. Immediately before `turn/start`, the worker and durable owner share one canonical `native_final_use_binding` over request identity, Agent generation, exact model/provider, request payload digest and the resolved owner-context digest. `FinalUseAuthority::claim` verifies signature, expiry, authority epoch, current revocation frontier and one-time nonce. `with_verified_use_witness` rechecks the live frontier and exposes a non-serializable, non-constructible `VerifiedUseWitness` only inside that fence. `dispatch_native_authorized` independently recomputes the expected binding, rejects mismatches or any caller-supplied durable witness, derives the public `NativeFinalUseWitness` itself, then appends and syncs the dispatch intent. Only after that serialized admission returns does the worker issue `turn/start`. The journal witness is audit evidence under the journal owner's integrity boundary; it is not the opaque token and cannot be installed through the ordinary dispatch API. The stable request ID also travels as `client_user_message_id`; correctness does not assume this alone provides provider exactly-once execution. The actual returned turn ID is then synced. After dispatch may have happened, duplicates and restarts never submit a new turn. A reopened record without a terminal observation becomes indeterminate and retains its slot. A completed duplicate returns the stored digest/status/usage observation without contacting Agentd or the provider; raw historical output is intentionally unavailable after durable redaction.

Provider terminality, runtime owner health and final-use effect authority are three separate facts. During execution, not-ready/fenced health, identity/generation errors, transport errors and health timeouts permanently mark that attempt `Lost`; this fact is journaled before interrupt grace. Late provider `Completed` and real token counts remain observable and can release the local run slot, but never clear lost owner state. `ObservedReady` still means only ready at the last health check; it is not treated as the provider-effect authority. A successful result additionally requires the durable dispatch witness created only by `dispatch_native_authorized` from the sealed live kernel witness. Revocation that wins before `with_verified_use_witness` admission denies dispatch; revocation after that serialized admission is non-retroactive for the already-entered effect and owner fencing/cancellation remains the runtime stop path. Historical journal observations missing authority fields load fail-closed and cannot become successful through replay or a later token-only update.
...
The journal has a 64 MiB total byte budget, an 8 MiB encoded-line budget and at most 16384 records across legacy/native types. New admission/dispatch also requires 16 MiB of remaining journal space for the next bounded observation and metadata. Append checks bounds before writing; replay bounds actual reads and checks capacity incrementally. Oversize, malformed or incomplete histories fail without truncation. The output-redaction rewrite removes retained native model text and can reduce bytes, but it deliberately preserves event history/revisions and is not a general archival/record-compaction scheme. At capacity the owner still refuses new durable writes; bounded archival/compaction for long-lived history remains implementation work.

## Remaining implementation work

- Connect economic quota and device-capacity authorities, implement the registered `OperationIntentV1` composition, and add authenticated provider reconciliation after process loss. Native local-slot reservations, kernel final-use admission and observed usage settlement are wired; hosted execution does not prove economic quota, local model artifacts, memory/device grants or process isolation.
