...
| Matrix transport | Existing `hepta-matrixd`, `MatrixDurableStore` and SDK sender | `MatrixDurableStore` owns the dispatch ledger; `send_observer` is compatibility exports only and no duplicate sender exists |
...
- Keep Matrix send state and terminal observations under `MatrixDurableStore` with one stable transaction identity. Transport acceptance is non-terminal; only atomic homeserver sync reconciliation can settle qualified success, and missing current-attempt authority evidence remains `ObservedUnqualified`.
