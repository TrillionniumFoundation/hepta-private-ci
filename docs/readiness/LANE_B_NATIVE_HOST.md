...
Before any provider contact, admission binds the request ID to the exact Agent/generation/model, quota/resource/economic policy and a digest of prompt, optional memory query, socket, timeout and request holds. Immediately before `turn/start`, the worker builds a final-use binding over the exact request/provider/model/context and serialized turn payload, consumes an independently signed grant through `FinalUseAuthority`, and syncs that grant witness together with the actual thread/provider/context dispatch intent. The stable request ID also travels as `client_user_message_id`; correctness does not assume this alone provides provider exactly-once execution. The actual returned turn ID is then synced. After dispatch may have happened, duplicates and restarts never submit a new turn. A reopened record without a terminal observation becomes indeterminate and retains its slot. A completed duplicate returns the stored observation without contacting Agentd or the provider, even if that generation is no longer live; it is a historical result, not a new authorization.

Provider terminality, `owner_authority` and `final_use_authority` are independent. During execution, not-ready/fenced health, identity/generation errors, transport errors and health timeouts permanently mark that attempt `Lost`; this fact is journaled before interrupt grace. Late provider `Completed` and real token counts remain observable and can release the local run slot, but never clear lost authority or make the CLI succeed. After provider unsubscribe/shutdown, an observed terminal without prior authority loss undergoes one final exact-owner health check before return and settlement; a previously lost owner remains denied without another RPC. This covers `select!` choosing completion ahead of a health tick. `ObservedReady` means ready at that last owner-health check and is not provider-dispatch authority. Successful completion additionally requires terminal-time `FinalUseAuthority::with_verified_use` for the exact previously claimed binding. Revocation/epoch drift therefore denies success even when the provider completed. Historical journal observations missing either authority field load as `Unverified` and cannot become successful through replay or a later usage-only update.

Pre-dispatch cancellation or connection failure records a local stop and releases its slot without fabricating provider terminality or zero tokens. After dispatch, cancellation/timeout/fencing records interrupt intent and sends the real interrupt RPC; only a matching terminal notification releases the local slot. `observed_output_tokens` remains null if no matching usage event arrived, including on terminal failure or interruption. Observed u64 counts are retained even above a synthetic token budget; no economic settlement is inferred. The typed trusted-host settlement port can refine unknown execution/usage with later matching observations. On restart, the worker performs read-only `thread/read` against the exact persisted thread and accepts only a matching `Completed`/`Failed` turn; it never invokes `turn/start` or a replay/recover operation. Because the final-use verification token is intentionally non-serializable, reconciliation can establish terminal effect truth and release local concurrency but cannot reconstruct authorized success. Deleting the journal, changing the request ID or treating a missing observation as zero is not a recovery procedure.

The journal has a 64 MiB total byte budget, an 8 MiB encoded-line budget and at most 16384 records across legacy/native types. Before native history reaches the reserved high-water space, the single writer atomically folds native events into exact snapshot records, preserves legacy lines byte-for-byte, writes and syncs a private content-addressed predecessor archive, locks and syncs the replacement inode before rename, then keeps a bounded archive count. Replay continues to reject oversize, malformed or incomplete histories without truncation. Automated source retention is count-bounded rather than time-based; deployment policy still decides backup lifetime and secure deletion for provider text.
