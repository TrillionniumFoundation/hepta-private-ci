...
Read the stable module guide, Section 16 readiness overlay, existing detail and matching implementation profile together. Proposed operation names must map to actual native symbols and consumers within an existing work package. The named acceptance cases are test designs, not pass receipts; `hepta-technical-closure.py verify` derives their current count from the designs. `NATIVE_BINDINGS.json` registers one primary source observation for each module; `implementation_contracts.py verify-repository` derives current coverage and Git identity. Neither observation counts nor those structural checks establish complete native API mappings, executed product tests or production callers. The module-specific current implementation sections identify actual interfaces, state, test sources, operating instructions and remaining work.
