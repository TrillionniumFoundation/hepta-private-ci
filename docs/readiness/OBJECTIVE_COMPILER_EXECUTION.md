...
The p95/p99 targets apply only to a named path, fixture and host. A normal-path latency measurement cannot be reused as a conflict-extraction measurement. `.github/workflows/hepta-objective-product-composition.yml` records host/compiler identity and two independent release-profile receipts on its named GitHub Ubuntu host: a 32-sample `authenticated-admission+compile+durable-fsync` p50/p95/p99 measurement, a separate 32-sample `authenticated-admission+compile+durable-fsync+agentd-runtime-admission` product-host measurement, and a 16-sample 256-atom inclusion-minimal-conflict measurement that asserts exactly 257 oracle calls per sample. These are qualification telemetry, not a production-target SLA. No network or synchronous central RPC is permitted on the deterministic compiler path.
