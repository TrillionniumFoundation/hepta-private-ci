...
Existing exported deterministic primitives in `codex-rs/hepta-ndu/src/lib.rs` include `evaluate_candidates`, `solve_preference_target`, `validate_staged_updates`, `evaluate_recursive_utility` and `mul_q32_ties_even`. The source also contains the bounded shadow covariance/backward-regression kernel plus admitted original/whitened Z-coordinate and signed-Q24 conversion. These numerical building blocks are now followed by source-level coefficient-lineage binding through learning.artifacts, learning.eval-owned conditional-identification/well-posedness/convergence decisions, and an intelligence.control DENY_ALL composition adapter. Those gates still do not authenticate real observations by themselves, establish a complete FBSDE solution, activate a production consumer or prove efficacy.
...
Documentation completion means this file, its exact blob identity in `ALGORITHM_SPECS.json`, paper scopes and required CI agree. Source-level stochastic admission additionally requires current artifact-withdrawal revalidation, coefficient-lineage binding, accepted independent identification/well-posedness/convergence decisions and exact-head/synthetic-merge tests; none substitutes for authenticated real evidence. Source completion additionally requires native code, stores, callers and tests. Dynamic efficacy additionally requires supported real future outcomes and distinct independent decisions. This document does not advance `D0_SPECIFIED_ONLY`, authorize a production writer or set all capability gaps closed.
