...
Normalization, units, feature order, clipping locations, scales and conditional-moment conventions belong to immutable artifacts bound into `RunStartSnapshotV1`. Native deterministic utility profiles bind the applicable axis registry through `axis_semantics_digest`; the utility-profile digest changes when that manifest identity changes. `COST_k` is not the covariance matrix. Convert microseconds to the declared solver time unit with checked arithmetic; never silently treat microseconds as seconds. Reject invalid dimensions, non-finite numbers, unknown units and missing profiles. Pre-clipping violations and projection counts remain observable.
...
`NduCoefficientManifestV1` binds artifact, subject/objective class, dimensions, fixed-point scales, bounds, normalization, runtime, predecessor, expiry and rollback. The native covariance admission profile now additionally binds nonzero coefficient-manifest, conditioning-profile, coordinate-system and numeric-conversion-profile digests alongside units and numerical thresholds. These are identity bindings, not proof that the referenced artifacts are approved or statistically identified. The integration package must register and authenticate those versioned references before admitting a stochastic implementation. An incompatible existing consumer returns unavailable; it cannot silently assume identity covariance.
...
Preference/utility rows append by revision. The native Unix projection store can make an owner-local journal image crash-consistent using one OS writer lock, file fsync, same-directory rename and parent-directory fsync; it does not supply retention/backup policy or product activation by itself. A selected projection pointer changes atomically only after immutable data and required evidence exist. Corrections append revoked ancestry and rebuilt successors. Lineage is source -> approved feature generation -> dataset -> training code -> coefficient -> evaluation -> selected snapshot -> runtime observation. No raw secret, unrestricted prompt or consumable authority token is stored in this chain.
